  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[kunghap_business] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
      ...
      if (chapter === 1 || chapter === 2) {
        ...
        if (expectedEl && otherEl && expectedEl !== otherEl && sectionText && !sectionText.includes(expectedEl) && sectionText.includes(otherEl)) {
          console.error(`[kunghap_business] ${chapter}장 본인/상대 뒤바뀜 의심 (시도${i+1}): 기대=${expectedEl}, 감지=${otherEl}`);
          continue;
        }
      }
...
async function generateConcernAdvice(id: string) {
  ...
  try {
    ...
    const cs = prev.compatScore as { score?: number; label?: string; desc?: string } | undefined;
    if (cs?.score !== undefined) lines.push(`사업 궁합 점수: ${cs.score}점 (${cs.label ?? ""})`);
    const bf = prev.bizFutureFlow as { desc?: string } | undefined;
  ...
  const user = `다음은 두 사람의 비즈니스궁합 만세력이오.\n\n[${name1}님 만세력]\n${manseryeokText}\n\n[${partnerName1}님 만세력]\n${partnerManseryeokText}${daeunSeunBlock ? "\n\n" + daeunSeunBlock : ""}${prevChapterContext}\n\n[고민에 대한 명리학적 조언 작성]\n고민: "${concern}"\n\n위 리포트 분석 결과(사업 궁합 점수·미래 흐름·대운·세운)를 종합하여, 고민에 직접 답하는 현실적이고 맞춤화된 조언을 작성하오. ⚠️ 점수(예: '72점' 등 숫자+점)는 절대 언급하지 마오 — 어떤 수치도 인용하지 말고, 정성적으로만 표현하오.\n\n아래 JSON 형식으로만 답하오:\n{\n  "concernAdvice": {\n    "paragraphs": [\n      "이 고민을 두 사람의 명식(일간·오행·십성·합충) 구조와 연결한 풀이 — 이 고민이 생긴 사주적 이유를 설명하오. 점수는 절대 언급하지 말고 정성적으로만 표현하오 (4~5문장, 200자 이상)",\n      "현재(${currentYear}년) 시점 기준으로 두 사람의 대운·세운 흐름이 이 고민에 어떤 영향을 주는지 분석하오. 리포트에서 도출된 시기나 흐름이 이미 지난 경우 현재 이후 관점으로 자연스럽게 전환하여 실질적 조언을 작성하오 (4~5문장, 200자 이상)",\n      "이 고민을 풀어가기 위한 조언과 마음가짐을, 리포트에서 분석된 사업 미래 흐름을 근거로 구체적으로 작성하오 (3~4문장, 150자 이상)"\n    ]\n  }\n}\n\n홍연 말투(~이오/~하오/~겠소).`;
...
export async function GET(request: NextRequest) {
  ...
  const totalChapters = Object.keys(BUSINESS_KUNGHAP_CHAPTER_SECTIONS).map(Number);
  const doneCount = totalChapters.filter(n => isBusinessKunghapChapterReady(content, n)).length;
