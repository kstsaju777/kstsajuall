...
//   → 명식 + 1장 풀이 생성·저장 → {resultId, view, partnerView, content}
// POST {id, chapter}  → 그 장만 생성 → {sections}
// GET  ?id=<resultId> → 저장된 결과 조회
...
// ─── 챕터 프롬프트 빌드 ──────────────────────────────────────────────────────
function buildJaehweChapterPrompt(chapter: number, input: {
...
[의뢰인 고민]: ${concern || "없음"}`;
  ...
  const chapterPrompts: Record<number, { user: string; keys: string[] }> = {
    1: {
      keys: ["myGist", "partnerGist", "breakupMeaning"],
      user: `제1장: 두 사람의 타고난 기질과 이별의 의미

다음 세 섹션을 분석해주세요:
1. "myGist": ${name}(${gLabel})의 일간과 사주 구성으로 본 타고난 기질·성향·연애 스타일
2. "partnerGist": ${partnerName}(${pgLabel})의 일간과 사주 구성으로 본 타고난 기질·성향·연애 스타일
3. "breakupMeaning": 두 사람이 만나고 헤어질 수밖에 없었던 사주적 의미와 인연의 깊이`,
    },
    2: {
      keys: ["breakupCause", "conflictPattern", "emotionFlow"],
      user: `제2장: 이별의 사주적 원인

다음 세 섹션을 분석해주세요:
1. "breakupCause": 두 사람의 사주에서 이별이 발생한 근본적인 사주적 원인 (오행 충돌, 형충파해, 운의 흐름 등)
2. "conflictPattern": 두 사람 사이에서 반복되는 갈등 패턴과 그 이면의 심리
3. "emotionFlow": 헤어진 후 각자의 감정 흐름과 현재 내면 상태`,
    },
    3: {
      keys: ["partnerView", "partnerFeelings", "contactTiming"],
      user: `제3장: 상대방의 마음 — 지금 나를 어떻게 생각하는가

다음 세 섹션을 분석해주세요:
1. "partnerView": ${partnerName}의 사주 기준으로 ${name}이 어떤 의미인지, 현재 ${name}을 어떻게 바라보는지
2. "partnerFeelings": ${partnerName}의 현재 감정 상태와 재회에 대한 내면의 욕구
3. "contactTiming": 지금 연락하면 어떤 반응이 예상되는지, 어떤 방식과 타이밍이 효과적인지`,
    },
    4: {
      keys: ["reunionTiming", "luckyPeriod", "warningPeriod"],
      user: `제4장: 재회 시기와 운의 흐름

다음 세 섹션을 분석해주세요:
1. "reunionTiming": 두 사람의 대운·세운 흐름으로 본 재회 가능성이 높은 시기 (구체적 연도·월 포함)
2. "luckyPeriod": 재회에 가장 유리한 운의 기간과 그 이유
3. "warningPeriod": 절대 연락하거나 만나면 안 되는 최악의 시기와 이유`,
    },
    5: {
      keys: ["obstacles", "badActions", "correctApproach"],
      user: `제5장: 재회를 방해하는 요인과 올바른 접근법

다음 세 섹션을 분석해주세요:
1. "obstacles": ${name}의 사주에서 재회를 방해하는 신살·오행·대운 요인
2. "badActions": 재회를 원한다면 반드시 피해야 할 행동과 말투 (구체적으로)
3. "correctApproach": 재회 가능성을 높이는 올바른 접근법과 자기 변화 방향`,
    },
    6: {
      keys: ["letter"],
      user: `마무리: 홍연의 편지
...
재회를 꿈꾸는 ${name}에게 진심 어린 위로와 용기를 전해주세요.
단순한 위로가 아닌, 사주를 통해 본 두 사람의 인연의 의미를 담아주세요.
...
// ─── 한 챕터 생성 ────────────────────────────────────────────────────────────
async function genChapterContent(chapter: number, input: Parameters<typeof buildJaehweChapterPrompt>[1]) {
  const { system, user } = buildJaehweChapterPrompt(chapter, input);
  for (let i = 0; i < 2; i++) {
    try {
...
async function createReport(body: unknown) {
  ...
  const orderCode = `jw-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  // product lookup - jaehwe slug 없으면 첫번째 product 사용 (graceful fallback)
  const { data: product } = await service.from("products").select("id").eq("slug", "jaehwe-compat").maybeSingle();
  const { data: fallbackProduct } = product ? { data: null } : await service.from("products").select("id").limit(1).maybeSingle();
  const productId = product?.id ?? fallbackProduct?.id ?? null;
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isJaehweChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of JAEHWE_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
  ...
  try {
    const sections = await genChapterContent(chapter, {
      ...
      mySajuText: stored.mySajuText ?? "",
      partnerName: stored.partnerName ?? "",
      partnerGender: stored.partnerGender ?? "",
      partnerSajuText: stored.partnerSajuText ?? "",
