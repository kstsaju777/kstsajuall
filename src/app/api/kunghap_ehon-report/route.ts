...
async function generateConcernAdvice(id: string) {
  ...
  try {
    ...
    const ds = prev.divorceScore as { score?: number; label?: string; desc?: string } | undefined;
    if (ds?.score !== undefined) lines.push(`이혼 위험도 점수: ${ds.score}점 (${ds.label ?? ""})`);
    const rs = prev.reconcileScore as { score?: number; label?: string } | undefined;
  ...
  const system = `당신은 홍연당의 사주 풀이 AI이오. 두 사람의 이혼궁합 고민에 대한 명리학적 조언을 JSON으로만 답하오. 절대 JSON 외 텍스트를 출력하지 마오.\n⚠️ 문법 규칙: 권고·조언 문장에서 동사의 현재형 수식절은 반드시 '-는 것이' 형태를 사용하오. '-은 것이'(완료형 수식) 형태를 미래·현재 권고 조언에 사용하는 것 절대 금지. (예: '관계를 다듬는 것이 좋겠소' ○ / '관계를 다듬은 것이 좋겠소' ✗)`;
  const user = `다음은 두 사람의 이혼궁합 만세력이오.\n\n[${name1}님 만세력]\n${manseryeokText}\n\n[${partnerName1}님 만세력]\n${partnerManseryeokText}${daeunSeunBlock ? "\n\n" + daeunSeunBlock : ""}${prevChapterContext}\n\n[고민에 대한 명리학적 조언 작성]\n고민: "${concern}"\n\n위 리포트 분석 결과(이혼 위험도·재결합 가능성·대운·세운)를 종합하여, 고민에 직접 답하는 현실적이고 맞춤화된 조언을 작성하오. ⚠️ 점수(예: '72점' 등 숫자+점)는 절대 언급하지 마오 — 어떤 수치도 인용하지 말고, 정성적으로만 표현하오.\n\n아래 JSON 형식으로만 답하오:\n{\n  "concernAdvice": {\n    "paragraphs": [\n      "이 고민을 두 사람의 명식(일간·오행·십성·합충) 구조와 연결한 풀이 — 이 상황이 생긴 사주적 이유를 설명하오. 점수는 절대 언급하지 말고 정성적으로만 표현하오 (4~5문장, 200자 이상)",\n      "현재(${currentYear}년) 시점 기준으로 두 사람의 대운·세운 흐름이 이 고민에 어떤 영향을 주는지 분석하오. 리포트에서 도출된 시기가 이미 지난 경우 현재 이후 관점으로 자연스럽게 전환하여 실질적 조언을 작성하오 (4~5문장, 200자 이상)",\n      "이 고민을 풀어가기 위한 조언과 마음가짐을, 리포트에서 분석된 내용을 근거로 구체적으로 작성하오 (3~4문장, 150자 이상)"\n    ]\n  }\n}\n\n홍연 말투(~이오/~하오/~겠소).`;
...
export async function GET(request: NextRequest) {
  ...
  const totalChapters = Object.keys(EHON_KUNGHAP_CHAPTER_SECTIONS).map(Number);
  const doneCount = totalChapters.filter(n => isEhonKunghapChapterReady(content, n)).length;
