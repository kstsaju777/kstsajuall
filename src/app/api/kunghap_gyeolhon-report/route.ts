  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[kunghap_gyeolhon] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
      ...
      // (주의: 이 파일의 input.ilganFull/partnerIlganFull은 일주 간지(예: "庚子")로
      // 용도가 다르므로, 여기서는 "庚 (경금)" 형식인 input.ilgan/partnerIlgan을 사용한다.)
      const extractIlganHangul = (full?: string) => full?.match(/\(([가-힣]+)\)/)?.[1] ?? "";
      if (chapter === 1 || chapter === 2) {
        const myEl = extractIlganHangul(input.ilgan);
        const ptEl = extractIlganHangul(input.partnerIlgan);
        ...
        if (expectedEl && otherEl && expectedEl !== otherEl && sectionText && !sectionText.includes(expectedEl) && sectionText.includes(otherEl)) {
          console.error(`[kunghap_gyeolhon] ${chapter}장 본인/상대 뒤바뀜 의심 (시도${i+1}): 기대=${expectedEl}, 감지=${otherEl}`);
          continue;
        }
      }
...
async function generateConcernAdvice(id: string) {
  ...
  if (!concern && data.order_id) {
    ...
    concern = arr.length >= 2 ? arr[0] : arr[0] ?? "";
  }
  ...
  try {
    ...
    const ms = prev.marriageScore as { score?: number; label?: string; desc?: string } | undefined;
    if (ms?.score !== undefined) lines.push(`결혼 궁합 점수: ${ms.score}점 (${ms.label ?? ""})`);
    const bt = prev.bestTiming as { period?: string; desc?: string } | undefined;
    if (bt?.period) lines.push(`결혼하기 좋은 시기: ${bt.period}${bt.desc ? " — " + bt.desc.slice(0, 80) + "…" : ""}`);
    const cp = prev.crisisPoints as Array<{ title?: string }> | undefined;
    if (Array.isArray(cp) && cp.length > 0) lines.push(`주요 위기 요소: ${cp.map(c=>c.title).filter(Boolean).join(", ")}`);
  ...
  const system = `당신은 홍연당의 사주 풀이 AI이오. 두 사람의 결혼궁합 고민에 대한 명리학적 조언을 JSON으로만 답하오. 절대 JSON 외 텍스트를 출력하지 마오.\n⚠️ 문법 규칙: 권고·조언 문장에서 동사의 현재형 수식절은 반드시 '-는 것이' 형태를 사용하오. '-은 것이'(완료형 수식) 형태를 미래·현재 권고 조언에 사용하는 것 절대 금지. (예: '관계를 다듬는 것이 좋겠소' ○ / '관계를 다듬은 것이 좋겠소' ✗, '대화를 나누는 것이 중요하오' ○ / '대화를 나누은 것이 중요하오' ✗)`;
  const user = `다음은 두 사람의 결혼궁합 만세력이오.\n\n[${name1}님 만세력]\n${manseryeokText}\n\n[${partnerName1}님 만세력]\n${partnerManseryeokText}${daeunSeunBlock ? "\n\n" + daeunSeunBlock : ""}${prevChapterContext}\n\n[고민에 대한 명리학적 조언 작성]\n고민: "${concern}"\n\n위 리포트 분석 결과(궁합 점수·위기 요소·결혼 시기·대운·세운)를 종합하여, 고민에 직접 답하는 현실적이고 맞춤화된 조언을 작성하오. ⚠️ 점수(예: '72점', '58점' 등 숫자+점)는 절대 언급하지 마오 — 궁합 점수·결혼 궁합 점수를 포함해 어떤 수치도 인용하지 말고, '궁합이 좋은 편', '아직 노력이 필요한 단계'처럼 정성적으로만 표현하오.\n\n아래 JSON 형식으로만 답하오:\n{\n  "concernAdvice": {\n    "paragraphs": [\n      "이 고민을 두 사람의 명식(일간·오행·십성·합충) 구조와 연결한 풀이 — 왜 이 고민이 생겼는지 두 사람의 사주 구조로 설명하오. 점수는 절대 언급하지 말고 정성적으로만 표현하오 (4~5문장, 200자 이상)",\n      "현재(${currentYear}년) 시점 기준으로 두 사람의 대운·세운 흐름이 이 고민에 어떤 영향을 주는지 분석하오. 리포트에서 도출된 결혼 적합 시기가 이미 지난 경우 '이미 그 시기가 시작되었으니 지금도 해당 흐름 안에 있소' 형태로 자연스럽게 현재 이후 관점으로 전환하여 실질적 조언을 작성하오 (4~5문장, 200자 이상)",\n      "이 고민을 풀어가기 위해 두 사람이 지금 당장 실천할 수 있는 조언과 마음가짐을, 리포트에서 분석된 위기 요소 해소 방향을 포함하여 작성하오 (3~4문장, 150자 이상)"\n    ]\n  }\n}\n\n홍연 말투(~이오/~하오/~겠소).`;
