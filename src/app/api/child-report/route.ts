...
async function createReport(body: unknown) {
  ...
  const { data: product } = await service.from("products").select("id").eq("slug", "child-saju").maybeSingle();
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isChildChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of CHILD_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
