...
async function createReport(body: unknown) {
  ...
  const { data: product } = await service.from("products").select("id").eq("slug", "kunghap_gyeolhon").maybeSingle();
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isGyeolhonChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of GYEOLHON_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
