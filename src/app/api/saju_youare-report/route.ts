...
async function genChapterContent(chapter: number, input: { name: string; gender: "male" | "female"; manseryeokText: string; pillars?: { pos: string; gan: string; ganEl: string; ji: string; jiEl: string; sipTop: string; sipBot: string; sinsal?: string }[]; birthYear?: number; seun?: { label: string; gz: string }[]; ilganChar?: string; yongsinEl?: string; heusinEl?: string; gisinEl?: string }) {
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[saju_youare] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
...
async function generateConcernAdvice(id: string) {
  ...
  try {
    ...
    else if (parentingFlow?.desc) lines.push(`성장 흐름: ${parentingFlow.desc.slice(0, 80)}…`);
    const remedy = prev.remedy as { desc?: string; items?: Array<{ title?: string }> } | undefined;
    if (remedy?.desc) lines.push(`양육 보완법: ${remedy.desc.slice(0, 80)}…`);
...
async function saveContent(id: string, content: Record<string, unknown>, skipAlimtalk = false) {
  ...
  const totalChapters = Object.keys(YOUARE_CHAPTER_SECTIONS).map(Number);
  const allDone = totalChapters.every(n => isYouareChapterReady(merged, n));
  ...
  if (!skipAlimtalk && !alreadySent && allDone && (!needsImage || imageReady) && data?.order_id) {
    ...
    if (claimed && claimed.length > 0) {
      ...
      if (si?.phone) {
      const reportUrl = `https://www.hongyeondang.com/saju/saju_youare/report-preview?id=${id}`;
        ...
        (async () => {
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              ...
              console.error(`[saju_youare] 이미지 생성 실패 (시도${attempt + 1}):`, e);
            }
          }
        })();
...
async function generateChapter(body: unknown) {
  ...
  try {
    ...
    // ch6: remedy 용신/희신/기신 십성→오행 변환
    if (chapter === 6) {
      const rd = (obj as Record<string, unknown>).remedy as Record<string, unknown> | undefined;
      if (rd) {
        ...
        const allText = [rd.intro, ...((rd.paragraphs as string[]) ?? [])].filter(Boolean).join(" ");
        rd.yongsinEl = resolveEl(rd.yongsinEl, "용신", allText);
        rd.heusinEl  = resolveEl(rd.heusinEl,  "희신", allText);
        rd.gisinEl   = resolveEl(rd.gisinEl,   "기신", allText);

        // 확정된 오행을 myeongsik에 저장 (재생성 시 일관성 유지)
        if (rd.yongsinEl) {
          const updatedMyeongsik = { ...stored, yongsinEl: rd.yongsinEl, heusinEl: rd.heusinEl ?? "", gisinEl: rd.gisinEl ?? "" };
...
export async function GET(request: NextRequest) {
  ...
  const totalChapters = Object.keys(YOUARE_CHAPTER_SECTIONS).map(Number);
  const doneCount = totalChapters.filter(n => isYouareChapterReady(content, n)).length;
