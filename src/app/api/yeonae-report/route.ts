...
async function createReport(body: unknown) {
  ...
  const { data: product } = await service.from("products").select("id").eq("slug", "yeonae-compat").maybeSingle();
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isYeonaeChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of YEONAE_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
