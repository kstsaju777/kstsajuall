...
const PRODUCT_NAME = "반려궁합";
const PRODUCT_PRICE = 29900;
const REPORT_PATH = "saju/kunghap_banryeo/report-preview";
  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[kunghap_banryeo] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
      // 1장(나의 원국)·2장(반려동물 원국)은 두 존재의 만세력이 나란히 주입되어 LLM이
      // 드물게 보호자/반려동물을 뒤바꿔 쓴다(예: 1장에 반려동물의 일간으로 풀이). 실제
      ...
      if (chapter === 1 || chapter === 2) {
        ...
        const sectionKey = chapter === 1 ? "myWonguk" : "petWonguk";
        ...
        if (expectedEl && otherEl && expectedEl !== otherEl && sectionText && !sectionText.includes(expectedEl) && sectionText.includes(otherEl)) {
          console.error(`[kunghap_banryeo] ${chapter}장 보호자/반려동물 뒤바뀜 의심 (시도${i+1}): 기대=${expectedEl}, 감지=${otherEl}`);
          continue;
        }
      }
      if (isBanryeoKunghapChapterReady(obj, chapter)) return { obj, ...meta };
      console.error(`[kunghap_banryeo] ${chapter}장 ready 실패 (시도${i+1}):`, JSON.stringify(obj).slice(0, 500));
    } catch (e) {
      console.error(`[kunghap_banryeo] ${chapter}장 예외 (시도${i+1}):`, e);
    }
  }
...
async function generateConcernAdvice(id: string) {
  ...
  const daeunSeunBlock = [daeunNote, seunLine, yongsinNote].filter(Boolean).join("\n\n");

  let prevChapterContext = "";
  try {
    const prev = JSON.parse(data.interpretation_md || "{}") as Record<string, unknown>;
    const lines: string[] = [];
    const cs = prev.chemistryScore as { score?: number; label?: string } | undefined;
    if (cs?.score !== undefined) lines.push(`반려동물 궁합 점수: ${cs.score}점 (${cs.label ?? ""})`);
    const pt = prev.petTemper as { desc?: string } | undefined;
...
async function saveContent(id: string, content: Record<string, unknown>, skipAlimtalk = false) {
  ...
  const allDone = totalChapters.every(n => isBanryeoKunghapChapterReady(merged, n));
  ...
  if (!skipAlimtalk && allDone && (!needsImage || imageReady) && data?.order_id) {
    ...
    if (si?.phone) {
      const reportUrl = `https://www.hongyeondang.com/saju/kunghap_banryeo/report-preview?id=${id}`;
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isBanryeoKunghapChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of BANRYEO_KUNGHAP_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
  ...
  try {
    ...
    if (chapter === 1) {
      ...
      if (my) {
        ...
        const pillars = (stored?.view?.pillars ?? []) as Array<{ pos: string; ganEl: string }>;
        const ilganEl = pillars.find(p => p.pos === "일주")?.ganEl ?? "";
