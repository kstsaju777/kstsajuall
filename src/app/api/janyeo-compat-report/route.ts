...
async function createReport(body: unknown) {
  ...
  const { data: product } = await service.from("products").select("id").eq("slug", "kunghap_janyeo").maybeSingle();
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isJanyeoCompatChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of JANYEO_COMPAT_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
