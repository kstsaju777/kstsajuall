  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[kunghap_janyeo] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
      // 1장(나의 원국)·2장(자녀 원국)은 두 사람의 만세력이 나란히 주입되어 LLM이
      // 드물게 본인/자녀를 뒤바꿔 쓴다(예: 1장에 자녀의 일간으로 풀이). 실제
      ...
      if (chapter === 1 || chapter === 2) {
        ...
        const sectionKey = chapter === 1 ? "myWonguk" : "childWonguk";
        ...
        if (expectedEl && otherEl && expectedEl !== otherEl && sectionText && !sectionText.includes(expectedEl) && sectionText.includes(otherEl)) {
          console.error(`[kunghap_janyeo] ${chapter}장 본인/자녀 뒤바뀜 의심 (시도${i+1}): 기대=${expectedEl}, 감지=${otherEl}`);
          continue;
        }
      }
...
async function generateConcernAdvice(id: string) {
  ...
  try {
    ...
    const bs = prev.bondScore as { score?: number; label?: string } | undefined;
    if (bs?.score !== undefined) lines.push(`유대감 점수: ${bs.score}점 (${bs.label ?? ""})`);
    const ss = prev.studyStyle as { desc?: string } | undefined;
    if (ss?.desc) lines.push(`자녀 학습 성향: ${ss.desc.slice(0, 100)}…`);
    const tal = prev.talent as { desc?: string } | undefined;
    if (tal?.desc) lines.push(`자녀 재능: ${tal.desc.slice(0, 100)}…`);
  ...
  const partnerHonorific = stored?.partnerGender === "female" ? "양" : "군";
  const partnerWithHonorific = `${partnerName1}${partnerHonorific}`;
  const system = `당신은 홍연당의 사주 풀이 AI이오. 부모와 자녀의 자녀궁합 고민에 대한 명리학적 조언을 JSON으로만 답하오. 절대 JSON 외 텍스트를 출력하지 마오.\n⚠️ 문법 규칙: 권고·조언 문장에서 동사의 현재형 수식절은 반드시 '-는 것이' 형태를 사용하오. '-은 것이'(완료형 수식) 형태를 미래·현재 권고 조언에 사용하는 것 절대 금지. (예: '관계를 다듬는 것이 좋겠소' ○ / '관계를 다듬은 것이 좋겠소' ✗)\n⚠️ 자녀를 지칭할 때 반드시 "${partnerWithHonorific}"으로만 부르시오. "${partnerName1}님"처럼 "님" 호칭을 붙이면 절대 안 되오 — 자녀에게는 "님"을 쓰지 않고 "${partnerHonorific}"만 사용하오.`;
  const user = `다음은 부모님과 자녀의 만세력이오.\n\n[${name1}님(부모) 만세력]\n${manseryeokText}\n\n[${partnerWithHonorific}(자녀) 만세력]\n${partnerManseryeokText}${daeunSeunBlock ? "\n\n" + daeunSeunBlock : ""}${prevChapterContext}\n\n[고민에 대한 명리학적 조언 작성]\n고민: "${concern}"\n\n위 리포트 분석 결과(자녀 학습 성향·재능·대운·세운)를 종합하여, 고민에 직접 답하는 현실적이고 맞춤화된 조언을 작성하오. ⚠️ 점수(예: '72점' 등 숫자+점)는 절대 언급하지 마오 — 어떤 수치도 인용하지 말고, 정성적으로만 표현하오.\n\n아래 JSON 형식으로만 답하오:\n{\n  "concernAdvice": {\n    "paragraphs": [\n      "이 고민을 부모와 자녀의 명식(일간·오행·십성·합충) 구조와 연결한 풀이 — 이 고민이 생긴 사주적 이유를 설명하오. 점수는 절대 언급하지 말고 성향은 정성적으로만 표현하오 (4~5문장, 200자 이상)",\n      "현재(${currentYear}년) 시점 기준으로 부모와 자녀의 대운·세운 흐름이 이 고민에 어떤 영향을 주는지 분석하오. 리포트에서 도출된 시기가 이미 지난 경우 현재 이후 관점으로 자연스럽게 전환하여 실질적 조언을 작성하오 (4~5문장, 200자 이상)",\n      "이 고민을 풀어가기 위한 조언과 마음가짐을, 리포트에서 분석된 자녀 성향과 부모 역할 방향을 근거로 구체적으로 작성하오 (3~4문장, 150자 이상)"\n    ]\n  }\n}\n\n홍연 말투(~이오/~하오/~겠소).`;
...
async function generateChapter(body: unknown) {
  ...
  try {
    ...
    // 용신/희신/기신 — 사주 API가 억부법으로 계산한 확정값(view.gyeokguk) 최우선, 없으면 LLM 판단 값으로 폴백
    const apiGyeokguk = stored?.view?.gyeokguk as { name?: string; yongsinEl?: string; heusinEl?: string; gisinEl?: string } | undefined;
    const partnerApiGyeokguk = stored?.partnerView?.gyeokguk as { name?: string; yongsinEl?: string; heusinEl?: string; gisinEl?: string } | undefined;
    ...
    // ch2: childYongsin 저장 (partnerYongsinEl 키로 저장)
    if (chapter === 2) {
      const child = (obj as Record<string, unknown>).childYongsin as Record<string, unknown> | undefined;
      if (child) {
        ...
        const allText = [child.intro, child.desc, child.yongsinReason, child.heusinReason, child.gisinReason].filter(Boolean).join(" ");
        child.yongsinEl = resolveEl(child.yongsinEl, "용신", allText);
        child.heusinEl  = resolveEl(child.heusinEl,  "희신", allText);
        child.gisinEl   = resolveEl(child.gisinEl,   "기신", allText);
        if (child.yongsinEl) {
          await service.from("saju_results").update({ myeongsik: { ...stored, partnerYongsinEl: child.yongsinEl, partnerHeusinEl: child.heusinEl ?? "", partnerGisinEl: child.gisinEl ?? "" } as never }).eq("id", id);
        }
      }
    }
