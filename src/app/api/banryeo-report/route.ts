...
async function createReport(body: unknown) {
  ...
  const { data: product } = await service.from("products").select("id").eq("slug", "kunghap_banryeo").maybeSingle();
...
async function generateChapter(body: unknown) {
  ...
  if (!force && isBanryeoChapterReady(content, chapter)) {
    const sections: Record<string, unknown> = {};
    for (const k of BANRYEO_CHAPTER_SECTIONS[chapter] ?? []) sections[k] = content[k];
