  ...
  for (let i = 0; i < 3; i++) {
    try {
      ...
      if (hasCorruptedText(obj)) {
        console.error(`[saju_health] ${chapter}장 응답 손상(깨진 문자) 감지 (시도${i+1}):`, JSON.stringify(obj).slice(0, 300));
        continue;
      }
      if (isHealthChapterReady(obj, chapter)) return { obj, ...meta };
      console.error(`[saju_health] ${chapter}장 isHealthChapterReady 실패 (시도${i+1}):`, JSON.stringify(obj).slice(0, 500));
    } catch (e) {
...
async function saveContent(id: string, content: Record<string, unknown>, skipAlimtalk = false) {
  ...
  const allDone = totalChapters.every(n => isHealthChapterReady(merged, n));
  ...
  if (!skipAlimtalk && allDone && (!needsImage || imageReady) && data?.order_id) {
    ...
    if (si?.phone) {
      const reportUrl = `https://www.hongyeondang.com/saju/saju_health/report-preview?id=${id}`;
...
async function generateChapter(body: unknown) {
  ...
  try {
    ...
    // 용신/희신/기신/격국 — 사주 API가 억부법으로 계산한 확정값(view.gyeokguk) 최우선, 없으면 1장 LLM 판단 값으로 폴백
    ...
    const gyeokguk: string | undefined = apiGyeokguk?.name || (stored?.gyeokguk as string | undefined) || undefined;
