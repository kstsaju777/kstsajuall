...
export default function HomePage() {
  return (
    <div className="pb-10 px-4 pt-4 flex flex-col gap-4">
      {CARDS.map((card, idx) => {
        const comingSoon = idx > 0;
        return (
          <Link key={card.name} href={comingSoon ? "#" : card.href} onClick={comingSoon ? (e) => e.preventDefault() : undefined} className="block w-full rounded-2xl overflow-hidden relative" style={{ aspectRatio: "4/3", pointerEvents: comingSoon ? "none" : "auto" }}>
            {card.type === "video" ? (
              <video src={card.image} className="w-full h-full object-cover" autoPlay muted loop playsInline style={comingSoon ? { filter: "blur(8px) brightness(0.15)", transform: "scale(1.05)" } : {}} />
            ) : (
              <img src={card.image} alt={card.name} className="w-full h-full object-cover" style={comingSoon ? { filter: "blur(8px) brightness(0.15)", transform: "scale(1.05)" } : {}} />
            )}
            {comingSoon ? (
              /* 서비스 준비중 오버레이 */
