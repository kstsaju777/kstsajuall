  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #5bbfeaaa,0 0 20px 4px #a8dff555} 50%{box-shadow:0 0 16px 4px #a8dff5cc,0 0 40px 10px #5bbfea88} }
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(91,191,234,0.15)", border: "1px solid rgba(91,191,234,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#a8dff5" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#a8dff5" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있어요.<br />오래 걸려도 정상이니 그대로 기다려 주세요.<br />새로고침하면 처음부터 다시 생성되어 시간이 더 걸립니다.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#3a6878" }}>
...
function ErrorScreen({ message }: { message: string }) {
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center px-8 text-center" style={{ background: "#000608" }}>
...
export default function SajuJanyeoCheckoutSuccessPage() {
  return (
    <Suspense>
      <SuccessInner />
    </Suspense>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const caRes = await fetch("/api/saju_janyeo-report", {
          method: "POST", headers: { "Content-Type": "application/json" },
