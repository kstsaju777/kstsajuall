...
const WHITE    = "#ffffff";
const RED      = "#5bbfea";
const RED_SOFT = "#3aaad8";
const RED_PALE = "#f0f9ff";
const ROSE     = "#3aaad8";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
...
const FAQS = [
  ...
  { q: "어떤 항목을 분석하나요?", a: "타고난 기질, 재능과 적성, 학습 방식, 성장 시기, 인간관계 등 총 8장에 걸쳐 상세히 풀이하오." },
];
          ...
          <div key={i} className="rounded-2xl overflow-hidden"
            ...
            <button
              ...
              <span className="flex-shrink-0 text-[18px] font-light transition-transform duration-200"
                style={{ color: "#1a6a8a", transform: openIdx === i ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
            </button>
            {openIdx === i && (
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>자녀사주 결제 안내</h3>
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(91,191,234,0.16)", border: `1px solid ${ACCENT}55`, color: "#a8dff5" }}>
          ...
          <div className="mb-5">
            <div className="w-full text-left rounded-2xl px-4 py-3.5"
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>자녀사주</span>
        ...
        <div className="fixed z-[60] px-6"
          ...
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)}
              ...
              style={{ background: "linear-gradient(135deg, #5bbfea, #3aaad8)" }}>혜택 받고 계속하기</button>
...
const TIME_COLORS: Record<string, string> = {
  "방금": "#5bbfea", "방금 전": "#5bbfea",
  "1분 전": "#3aaad8", "2분 전": "#3aaad8",
  "3분 전": "#1a7fa8", "5분 전": "#6c757d", "7분 전": "#6c757d",
};
function randomName() {
      ...
      <div style={{ position: "fixed", bottom: 100, right: "max(8px, calc(50vw - 232px))", zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, pointerEvents: "none" }}>
        {toasts.map((t) => (
          <div key={t.id} style={{ animation: "toastInRight 0.35s ease", display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(0,0,0,0.7)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 9999, padding: "6px 14px", fontSize: 12, color: "#fff", whiteSpace: "nowrap", backdropFilter: "blur(6px)" }}>
            <span style={{ backgroundColor: TIME_COLORS[t.time] ?? "#3aaad8", color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 9999, padding: "2px 7px" }}>{t.time}</span>
    ...
    <div className="fixed bottom-0 flex items-center gap-3 px-4 z-50"
      ...
      <style>{`
        @keyframes janyeoBtnNeon {
          0%   { background: #5bbfea; box-shadow: 0 0 12px 3px rgba(91,191,234,0.7); }
          33%  { background: #3aaad8; box-shadow: 0 0 12px 3px rgba(58,170,216,0.7); }
          66%  { background: #a8dff5; box-shadow: 0 0 12px 3px rgba(168,223,245,0.7); }
          100% { background: #5bbfea; box-shadow: 0 0 12px 3px rgba(91,191,234,0.7); }
        }
        @keyframes janyeoBtnBeat {
      ...
      <button onClick={onPay}
        ...
        style={{ color: "#001a24", animation: "janyeoBtnNeon 3s ease-in-out infinite, janyeoBtnBeat 2s ease-in-out infinite" }}>
        자녀사주 확인하기
      </button>
    </div>
  );
...
function CreatingScreen({ doneCount, currentChapter }: { doneCount: number; currentChapter: number }) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      style={{ background: "radial-gradient(ellipse at 50% 40%, #000a10 0%, #000608 100%)" }}>
      <style>{`
        @keyframes shimmer { 0%{transform:translateX(-100%)} 100%{transform:translateX(400%)} }
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #5bbfeaaa,0 0 20px 4px #3aaad855} 50%{box-shadow:0 0 16px 4px #a8dff5cc,0 0 40px 10px #5bbfea88} }
      ...
      <div className="relative w-20 h-20 mb-6">
        <div className="absolute inset-0 rounded-full" style={{ background: "radial-gradient(circle, #5bbfea22 0%, transparent 70%)" }} />
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            top: "50%", left: "50%", marginTop: "-2px", marginLeft: "-2px",
            background: i % 2 === 0 ? "#5bbfea" : "#a8dff5",
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#3aaad8" : "#5bbfea"}`,
      ...
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#a8dff5", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#7ac8e8" }}>
          <span>{doneCount} / {TOTAL} 장 완성</span>
          <span>{pct}%</span>
        </div>
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#001018" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #3aaad8, #5bbfea, #a8dff5)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <p className="text-[11px] text-center leading-relaxed mt-4" style={{ color: "#3a6878" }}>
...
const PRODUCT_INFO = { name: "자녀사주", price: 19900 };
    ...
    <div className="w-full h-full" style={{ backgroundColor: WHITE }}>
      <div className="w-full h-full overflow-y-auto" style={{ scrollbarWidth: "none", paddingBottom: 80 }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_janyeo/s1.jpg" alt="" className="w-full block" />
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "26%", left: "69%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${suffix}의 보호자님,`}</p>
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "54.8%", left: "42%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            ...
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${suffix}의 사주팔자가`}</p>
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "95.5%", left: "60%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            ...
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${suffix}의 사주팔자요.`}</p>
          </div>
        </div>

        {/* ② 명식 */}
        <MyeongsikSection saju={saju} name={`${name}${suffix}`} date={date} calendar={calendar} gender={gender} />
        ...
        <div className="relative">
          ...
          <img src="/media/checkout/saju_janyeo/s2.jpg" alt="" className="w-full block" />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "12.9%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix} 사주가 과연`}</p>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "18.9%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix} 사주로 그린`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>사주화를 보여주며 시작되오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "27.8%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix}의 사주에`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>어떤 능력과 재능이 있는지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "34.4%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix}에게 필요한 기운과`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>피해야할 기운이 어떤건지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "47.2%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix}의 학습특성과`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>공부법까지 알려드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "56.6%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>시기별로 어떻게</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>양육해야 할지도 알려드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "67.4%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>친구들과의 관계는</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>시기별로 어떨지도 짚어드리겠소.</p>
          </div>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "91.6%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>아이를 키우는 데</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>필요한 조언도 짚어드리겠소.</p>
          </div>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_janyeo/s3.jpg" alt="" className="w-full block" />
          <div className="absolute bottom-0 left-0 right-0 h-16 pointer-events-none" style={{ background: `linear-gradient(to bottom, transparent, ${WHITE})` }} />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "2.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${suffix}의 모든것에 대해`}</p>
          ...
          <div className="fixed bottom-0 z-[60] rounded-t-3xl overflow-hidden"
            ...
            <div className="toss-widget-wrap">
              <TossWidget
                ...
                successUrl={`${typeof window !== 'undefined' ? window.location.origin : 'https://www.hongyeondang.com'}/saju/saju_janyeo/checkout/success`}
...
export default function SajuJanyeoCheckoutPage() {
  return (
    <Suspense fallback={
