...
const PRODUCT = { name: "자녀사주", original: 49800, discount: 50, price: 24900 };
  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      <div className="relative overflow-hidden" style={{ height: 240 }}>
        <img src="/media/cards/saju_janyeo/janyeo-0.jpg" alt=""
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4" style={{ backgroundColor: WHITE, boxShadow: "0 -4px 20px rgba(0,0,0,0.08)" }}>
      ...
      <button onClick={onPay} className="w-full py-4 rounded-2xl font-bold text-[16px] text-white flex items-center justify-center gap-2 active:scale-95 transition-all"
        ...
        <span>🔓</span>
        <span>{name}{gender === "남아" ? "군" : gender === "여아" ? "양" : "님"} 자녀사주 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch("/api/saju_janyeo-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, date, time, calendar, gender, email }) });
      const d = res.ok ? await res.json() : null;
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/saju_janyeo/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/saju_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/saju_janyeo/report?id=${d.resultId}` : `/saju/saju_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/saju_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
  ...
  if (creating) return (
    <div className="fixed inset-0 flex flex-col items-center justify-center gap-6 z-50" style={{ background: "#fdf8f4" }}>
      ...
      <div className="text-center">
        <p className="text-[16px] font-bold" style={{ color: "#3a1a0a", fontFamily: "'Noto Serif KR'" }}>결과지를 완성하고 있소…</p>
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        <ImageTextBlock label="자녀사주 · 정밀 리포트" headline={`${name}${gender === "남아" ? "군" : gender === "여아" ? "양" : "님"}의\n운명을\n살펴봤어요`} accent="살펴봤어요" />
