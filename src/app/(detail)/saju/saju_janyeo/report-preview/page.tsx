...
const TOTAL_CH = Object.keys(CHILD_CHAPTER_SECTIONS).length;
...
const CH_SUBS: Record<number, string> = {
  0: "",
  1: "아이가 이런 성격인 이유, 사주에 담겨 있어요",
  2: "아이의 사주 속에 숨겨진 특별한 재능을 찾습니다",
  3: "어떤 방식으로 배우고, 어떤 길을 걸어갈지 봅니다",
  4: "아이에게 힘든 시기와 좋은 시기를 미리 알아봐요",
  5: "아이를 더 건강하게 키우는 사주별 육아 팁",
  6: "홍연이 부모님께 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/saju_janyeo/janyeo-0.jpg",
  1: "/media/cards/saju_janyeo/janyeo-0.jpg",
  2: "/media/cards/saju_janyeo/janyeo-0.jpg",
  3: "/media/cards/saju_janyeo/janyeo-0.jpg",
  4: "/media/cards/saju_janyeo/janyeo-0.jpg",
  5: "/media/cards/saju_janyeo/janyeo-0.jpg",
  6: "/media/cards/saju_janyeo/janyeo-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isChildChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/saju_janyeo/janyeo-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: PINKD }}>✦ 자녀사주 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, onRetry, onNext }: {
  ...
  const sectionKeys = CHILD_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isChildChapterReady(content, ch) : false;
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: PINKD }}>✦ 자녀사주 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/saju_janyeo/janyeo-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>자녀사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/child-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date) {
      fetch("/api/child-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          setResultId(rid);
          if (rid) {
            router.replace(`/saju/saju_janyeo/report-preview?id=${rid}&name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(CHILD_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isChildChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/saju_janyeo/report-preview?${base}&ch=${n}`);
  };
