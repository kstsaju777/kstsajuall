...
//
// 미리보기: http://localhost:3000/saju/saju_janyeo/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/saju_janyeo-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/saju_janyeo/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/saju_janyeo/report?${id ? `id=${id}&` : ""}ch=${n}`);
