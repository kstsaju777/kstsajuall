...
const IMG_BG    = "/media/cards/saju_janyeo/janyeo-0.jpg";
  ...
  return (
    <div className="flex items-center gap-3 px-5 pb-8 pt-4" style={{ backgroundColor: CARD_BG }}>
      ...
      <button onClick={onNext} disabled={nextDisabled} className="flex-1 py-3.5 rounded-2xl text-[16px] font-bold transition-all"
        style={{ backgroundColor: PINK, color: "#000", opacity: nextDisabled ? 0.35 : 1, letterSpacing: "-0.3px" }}>{nextLabel}</button>
    </div>
  );
}
          ...
          {["남아", "여아"].map((g) => {
            ...
            return (
              <button key={g} onClick={() => { setGender(g); setTimeout(() => onNext(g), 350); }}
                ...
                style={{
                  backgroundColor: active ? "rgba(91,191,234,0.18)" : "rgba(255,255,255,0.04)",
...
function StepBirthDate({ onPrev, onNext, gender, initial, initialCalendar }: {
  ...
  const greeting = "자녀 태어난 날짜";

  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>{greeting}</p>
        ...
        <div className="flex gap-2 mb-5">
          {["양력", "음력"].map((c) => (
            <button key={c} onClick={() => setCalendar(c)}
              ...
              style={{
                backgroundColor: calendar === c ? "rgba(91,191,234,0.15)" : "rgba(255,255,255,0.04)",
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold flex items-center justify-between"
              ...
              style={{
                backgroundColor: (btime && btime !== "시간 모름") ? "rgba(91,191,234,0.15)" : "rgba(255,255,255,0.04)",
                ...
                <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                  ...
                  {BIRTH_TIMES.map((t) => (
                    <button key={t} className="w-full text-left px-4 py-2.5 text-[14px] transition-colors"
                      ...
                      style={{ backgroundColor: btime === t ? "rgba(91,191,234,0.15)" : "transparent", color: btime === t ? "#fff" : "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          <button className="px-4 py-3 rounded-xl text-[14px] font-bold"
            ...
            style={{
              backgroundColor: btime === "시간 모름" ? "rgba(91,191,234,0.15)" : "rgba(255,255,255,0.04)",
...
// ─── Step 4: 이름 ─────────────────────────────────────────────────────────────
function StepName({ onPrev, onNext, gender, initial }: {
  onPrev: () => void; onNext: (v: string) => void; gender?: string; initial?: string;
}) {
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-4" style={{ color: TEXT_CLR }}>
          <span className="font-bold">{name ?? "아이"}{gender === "남아" ? "군" : gender === "여아" ? "양" : "님"}의 </span>
                ...
                <div className="absolute bottom-full left-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                  ...
                  {PHONE_PREFIXES.map((p) => (
                    <div key={p} onClick={() => { setPhonePrefix(p); setPhoneOpen(false); }}
                      className="px-4 py-3 text-[14px] cursor-pointer"
                      style={{ backgroundColor: phonePrefix === p ? "rgba(91,191,234,0.2)" : "transparent", color: phonePrefix === p ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {p}
                    </div>
                  ))}
                </div>
...
function StepLoading({ name, date, time, calendar, gender, email, concern, phone }: {
  ...
  const goNext = () => {
    ...
    router.push(`/saju/saju_janyeo/checkout?${params.toString()}`);
  };
...
export default function JanyeoFormPage() {
