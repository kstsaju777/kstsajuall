...
//
// 미리보기: http://localhost:3000/saju/saju_yeonae/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/saju_yeonae-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/saju_yeonae/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/saju_yeonae/report?${id ? `id=${id}&` : ""}ch=${n}`);
    ...
    <div ref={rootRef} style={{ backgroundColor: CREAM, minHeight: "100%" }}>
      ...
      {ch === "1" && (
        <>
          ...
          <div className="relative overflow-hidden" style={{ height: 360 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/saju_yeonae/saju_yeonae_1/saju_yeonae_1_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 20%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>

          <Quote>{`${name}님의 사주를\n펼치는 순간이오.\n\n사주팔자는 태어난 연·월·일·시,\n네 기둥으로 이루어지오.\n각 기둥에는 천간과 지지, 두 글자씩\n총 여덟 글자가 담기오.\n\n이 여덟 글자 안에\n${name}님의 기질과 운의 흐름이\n모두 담겨 있소.\n\n이게 바로 ${name}님의 사주팔자요.`}</Quote>
