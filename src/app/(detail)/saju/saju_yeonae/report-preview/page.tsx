...
//
// 미리보기: http://localhost:3000/saju/saju_yeonae/report-preview
//
...
import { isYeonaeSajuChapterReady, YEONAE_SAJU_CHAPTER_SECTIONS } from "@/lib/saju/yeonae-saju-report-content";
...
const SAJU_ORDER = ["정통명리 종합사주", "영재발굴 자녀사주", "우리아가 유아사주", "오래살자 건강사주", "대박이야 재물사주"];
...
function ReviewBox() {
  ...
  return (
    <div className="rounded-2xl p-5 mx-6 mb-8" style={{ background: WHITE, border: "1.5px solid #e05080", boxShadow: "0 0 6px #e0508080, 0 0 16px #e0508040, 0 0 32px #e0508020" }}>
...
// 장번호 → 표시 제목 (연애사주 9장 구조)
const CHAPTER_TITLES: Record<string, string> = {
  "0": "인트로 · 사주팔자란 무엇인가",
  "1": "제1장 · 나는 어떤 그릇으로 태어났나",
  "2": "제2장 · 나는 어떻게 사랑하는가",
  "3": "제3장 · 어떤 사람이 나와 잘 맞는가",
...
const TOC_A: TocEntry[] = [
  ...
  { disp: "제3장",  chip: "이상형", title: "어떤 사람이 나와 잘 맞는가",       no: "3" },
        ...
        <div className="px-5 py-6">
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[15px]" style={{ color: MUTE }}>[연애사주]</span>
            </div>
            <button
              onClick={onClose}
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/saju_yeonae-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/saju_yeonae/report-preview?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const genChapter = (n: number, force = false) => {
    ...
    fetch("/api/saju_yeonae-report", {
      ...
      .then((d) => {
        ...
        if (!sec || !isYeonaeSajuChapterReady(sec, toApiChapter(n))) return;
        setReport((p) => {
          if (!p) return p;
  ...
  const next = (n: string) => router.push(`/saju/saju_yeonae/report-preview?${id ? `id=${id}&` : ""}ch=${n}`);
...
<>
      ...
      {ch === "0" && (
        <>
          ...
          <div className="relative overflow-hidden" style={{ height: 460 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/saju_yeonae/saju_yeonae_intro/saju_yeonae_intro_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 20%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>
      ...
      {ch === "2" && (
        <>
          ...
          <div className="relative overflow-hidden" style={{ height: 520 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/saju_yeonae/saju_yeonae_2/saju_yeonae_2_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 55%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>
      ...
      {ch === "4" && (
        <>
          <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 4 장 · 인연시기</p>
            <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>
              내 인연은 언제 나타나는가
            </h1>
          </div>
