...
const TOTAL_CH = Object.keys(YEONAE_SAJU_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "연애운사주 들어가며", 1: "나의 타고난 기질과 연애 성향", 2: "연애운과 인연의 흐름", 3: "이상적인 이성 유형", 4: "연애 패턴과 감정 스타일", 5: "연애를 방해하는 요인", 6: "홍연의 편지",
};

const CH_SUBS: Record<number, string> = {
  0: "", 1: "나의 사주에 담긴 연애 기운을 살펴봅니다", 2: "인연이 찾아오는 시기와 흐름입니다", 3: "나와 잘 맞는 이성 유형을 안내합니다", 4: "나만의 연애 패턴을 솔직하게 살펴봅니다", 5: "연애를 가로막는 것들, 미리 알고 피하세요", 6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  1: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  2: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  3: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  4: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  5: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
  6: "/media/cards/saju_yeonae/saju_yeonae-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isYeonaeSajuChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/saju_yeonae/saju_yeonae-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: PINKD }}>✦ 연애운사주 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, onRetry, onNext }: {
  ...
  const sectionKeys = YEONAE_SAJU_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isYeonaeSajuChapterReady(content, ch) : false;
...
function LoadingDots({ title }: { title: string }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <p className="text-[12px]" style={{ color: MUTE }}>홍연이 그대의 연애운을 들여다보고 있어요</p>
    </div>
  );
}
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: PINKD }}>✦ 연애운사주 완료 ✦</p>
...
function GeneratingWait() {
  ...
  return (
    <div className="flex flex-col items-center justify-center px-6 text-center" style={{ minHeight: "70vh" }}>
      ...
      <p className="text-[15px] font-bold" style={{ color: INK }}>연애운을 풀이하고 있어요{d}</p>
      <p className="text-[13px] mt-1" style={{ color: MUTE }}>홍연이 그대의 사랑을 살피는 중이에요</p>
    </div>
  );
}
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/saju_yeonae/saju_yeonae-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        <div className="rounded-full animate-spin" style={{ width: 44, height: 44, border: "3px solid rgba(255,255,255,0.15)", borderTopColor: "#e9bdae" }} />
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>연애운사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/yeonae-saju-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date) {
      fetch("/api/yeonae-saju-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          setResultId(rid);
          if (rid) {
            router.replace(`/saju/saju_yeonae/report-preview?id=${rid}&name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(YEONAE_SAJU_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isYeonaeSajuChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/saju_yeonae/report-preview?${base}&ch=${n}`);
  };
