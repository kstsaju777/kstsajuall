...
const PRODUCT = { name: "연애운사주", original: 59800, discount: 50, price: 29900 };
  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      <div className="relative overflow-hidden" style={{ height: 240 }}>
        <img src="/media/cards/saju_yeonae/saju_yeonae-0.jpg" alt=""
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>연애운사주 결제 안내</h3>
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(233,189,174,0.16)", border: `1px solid ${PINK_D}55`, color: "#e9bdae" }}>
            총 <span style={{ color: PINK_D }}>{saved.toLocaleString()}원</span> 할인받았어요!
          </div>
          <div className="w-full text-left rounded-2xl px-4 py-3.5 mb-5" style={{ backgroundColor: DCARD, border: `1.5px solid ${PINK_D}`, boxShadow: `0 0 0 3px ${PINK_D}22` }}>
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>연애운사주</span>
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4" style={{ backgroundColor: WHITE, boxShadow: "0 -4px 20px rgba(0,0,0,0.08)" }}>
      ...
      <button onClick={onPay} className="w-full py-4 rounded-2xl font-bold text-[16px] text-white flex items-center justify-center gap-2 active:scale-95 transition-all"
        ...
        <span>🔓</span>
        <span>{name}{gender === "남아" ? "군" : gender === "여아" ? "양" : "님"} 연애운사주 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    setCurrentChapter(1);
    try {
      const res = await fetch("/api/saju_yeonae-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, date, time, calendar, gender, email }) });
      if (!res.ok || !res.body) { router.push(`/saju/saju_yeonae/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); return; }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.done && data.resultId) { router.push(`/saju/saju_yeonae/report?id=${data.resultId}`); return; }
            if (data.error) { router.push(`/saju/saju_yeonae/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); return; }
          } catch { /* JSON 파싱 실패 무시 */ }
        }
      }
    } catch { router.push(`/saju/saju_yeonae/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        <ImageTextBlock label="연애운사주 · 정밀 리포트" headline={`${name}${gender === "남아" ? "군" : gender === "여아" ? "양" : "님"}의\n운명을\n살펴봤어요`} accent="살펴봤어요" />
