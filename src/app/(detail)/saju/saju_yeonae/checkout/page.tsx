...
const WHITE    = "#ffffff";
const RED      = "#e9bdae";
const RED_SOFT = "#f0cdc0";
const RED_PALE = "#fdf5f2";
const ROSE     = "#f0cdc0";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
          ...
          <div key={i} className="rounded-2xl overflow-hidden"
            ...
            <button
              ...
              <span className="flex-shrink-0 text-[18px] font-light transition-transform duration-200"
                style={{ color: "#b07060", transform: openIdx === i ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
            </button>
            {openIdx === i && (
...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCT = { name: "연애사주", original: 39800, discount: 50, price: 19900 };
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>연애사주 결제 안내</h3>
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(233,189,174,0.16)", border: `1px solid ${ACCENT}55`, color: "#f0cdc0" }}>
          ...
          <div className="mb-5">
            <div className="w-full text-left rounded-2xl px-4 py-3.5"
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>연애사주</span>
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 나의 연애 이야기</p>
                </div>
          ...
          <button onClick={onConfirm}
            ...
            style={{ background: "linear-gradient(135deg, #e9bdae, #d4a090)", boxShadow: "0 6px 20px rgba(233,189,174,0.4)" }}>
            결제하기
          </button>
        ...
        <div className="fixed z-[60] px-6"
          ...
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)}
              ...
              style={{ background: "linear-gradient(135deg, #e9bdae, #d4a090)" }}>혜택 받고 계속하기</button>
...
const TIME_COLORS: Record<string, string> = {
  "방금": "#e9bdae", "방금 전": "#e9bdae",
  "1분 전": "#d4a090", "2분 전": "#d4a090",
      ...
      <div style={{ position: "fixed", bottom: 100, right: "max(8px, calc(50vw - 232px))", zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, pointerEvents: "none" }}>
        {toasts.map((t) => (
          <div key={t.id} style={{ animation: "toastInRight 0.35s ease", display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(0,0,0,0.7)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 9999, padding: "6px 14px", fontSize: 12, color: "#fff", whiteSpace: "nowrap", backdropFilter: "blur(6px)" }}>
            <span style={{ backgroundColor: TIME_COLORS[t.time] ?? "#d4a090", color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 9999, padding: "2px 7px" }}>{t.time}</span>
...
function StickyPayCTA({ onPay, name }: { onPay: () => void; name: string }) {
  ...
  return (
    <div className="fixed bottom-0 flex items-center gap-3 px-4 z-50"
      ...
      {/* 오른쪽: 버튼 */}
      <style>{`
        @keyframes yeonaeSajuBtnNeon {
          0%   { background: #e9bdae; box-shadow: 0 0 12px 3px rgba(233,189,174,0.7); }
          33%  { background: #d4a090; box-shadow: 0 0 12px 3px rgba(212,160,144,0.7); }
          66%  { background: #f0cdc0; box-shadow: 0 0 12px 3px rgba(240,205,192,0.7); }
          100% { background: #e9bdae; box-shadow: 0 0 12px 3px rgba(233,189,174,0.7); }
        }
        @keyframes yeonaeSajuBtnBeat {
      ...
      <button onClick={onPay}
        ...
        style={{ animation: "yeonaeSajuBtnNeon 3s ease-in-out infinite, yeonaeSajuBtnBeat 2s ease-in-out infinite" }}>
        연애사주 확인하기
      </button>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #e9bdaeaa,0 0 20px 4px #d4a09055} 50%{box-shadow:0 0 16px 4px #f0cdc0cc,0 0 40px 10px #e9bdae88} }
      ...
      <div className="relative w-20 h-20 mb-6">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span style={{ fontSize: 28, filter: "drop-shadow(0 0 8px #e9bdae)" }}>✦</span>
        </div>
      </div>
      <p className="text-[18px] font-bold mb-1" style={{ color: "#fff5ee", fontFamily: "'Noto Serif KR', serif", textShadow: "0 0 20px #e9bdae88" }}>
        결과지를 완성하고 있소…
      </p>
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#f0cdc0", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#c9a090" }}>
...
const PRODUCT_INFO = { name: "연애사주", price: 19900 };
    ...
    <div className="w-full h-full" style={{ backgroundColor: WHITE }}>
      <div className="w-full h-full overflow-y-auto" style={{ scrollbarWidth: "none", paddingBottom: 80 }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_yeonae/s1.jpg" alt="" className="w-full block" />
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "54.8%", left: "42%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>연애운 풀이에 앞서,</p>
        ...
        <div className="relative">
          ...
          <img src="/media/checkout/saju_yeonae/s2.jpg" alt="" className="w-full block" />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "13.2%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "19.35%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님 사주로 볼 때`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>연애성향이 어떤지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "28.4%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>어떤 유형의 이성과</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>잘 맞는지도 짚어드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "35.2%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님과 인연이 될`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>사주와 생년월일 랭킹까지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "48.3%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님이`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>피해야 할 유형까지 짚어드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "57.55%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>언제 인연이 오는지</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>그래프로 한눈에 보여드리겠소.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "69.6%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>언제가 가장</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>인연이 활짝 피는 시기인지도.</p>
          </div>
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "82.2%", left: "34%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "91.4%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님의 연애운을 높혀줄`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>사주 개운법과 비방책까지</p>
          </div>
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "4.6%", left: "65%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_yeonae/s3.jpg" alt="" className="w-full block" />
          <div className="absolute bottom-0 left-0 right-0 h-16 pointer-events-none" style={{ background: `linear-gradient(to bottom, transparent, ${WHITE})` }} />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "2.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님의 연애운에 대해`}</p>
          ...
          <div className="fixed bottom-0 z-[60] rounded-t-3xl overflow-hidden"
            ...
            <div className="toss-widget-wrap">
              <TossWidget
                ...
                successUrl={`${typeof window !== 'undefined' ? window.location.origin : 'https://www.hongyeondang.com'}/saju/saju_yeonae/checkout/success`}
...
export default function SajuYeonaeCheckoutPage() {
  return (
    <Suspense fallback={
