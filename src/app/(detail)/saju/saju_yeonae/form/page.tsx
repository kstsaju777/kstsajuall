...
const IMG_BG     = "/media/cards/saju_yeonae/saju_yeonae-0.jpg";
  ...
  return (
    <div className="flex items-center gap-3 px-5 pb-8 pt-4" style={{ backgroundColor: CARD_BG }}>
      ...
      <button onClick={onNext} disabled={nextDisabled} className="flex-1 py-3.5 rounded-2xl text-[16px] font-bold transition-all"
        style={{ backgroundColor: PINK, color: "#3a1a0a", opacity: nextDisabled ? 0.35 : 1, letterSpacing: "-0.3px" }}>{nextLabel}</button>
    </div>
  );
}
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>나의 성별</p>
          ...
          {["남성", "여성"].map((g) => {
            ...
            return (
              <button key={g} onClick={() => { setGender(g); setTimeout(() => onNext(g), 350); }}
                ...
                style={{
                  backgroundColor: active ? "rgba(233,189,174,0.15)" : "rgba(255,255,255,0.04)",
...
function StepBirthDate({ onPrev, onNext, initial, initialCalendar }: {
  ...
  return (
    <>
      ...
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>그대의 생년월일</p>
        ...
        <div className="flex gap-2 mb-5">
          {["양력", "음력"].map((c) => (
            <button key={c} onClick={() => setCalendar(c)}
              ...
              style={{
                backgroundColor: calendar === c ? "rgba(233,189,174,0.15)" : "rgba(255,255,255,0.04)",
...
function StepBirthTime({ onPrev, onNext, initial }: {
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>그대의 태어난 시간</p>
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold flex items-center justify-between"
              ...
              style={{
                backgroundColor: (btime && btime !== "시간 모름") ? "rgba(233,189,174,0.15)" : "rgba(255,255,255,0.04)",
                ...
                <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                  ...
                  {BIRTH_TIMES.map((t) => (
                    <button key={t} className="w-full text-left px-4 py-2.5 text-[14px] transition-colors"
                      ...
                      style={{ backgroundColor: btime === t ? "rgba(233,189,174,0.15)" : "transparent", color: btime === t ? "#fff" : "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          <button className="px-4 py-3 rounded-xl text-[14px] font-bold"
            ...
            style={{
              backgroundColor: btime === "시간 모름" ? "rgba(233,189,174,0.15)" : "rgba(255,255,255,0.04)",
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-4" style={{ color: TEXT_CLR }}>
          <span className="font-bold">{name ?? "그대"}{suffix} </span>
        ...
        <div className="rounded-2xl px-4 py-3" style={{ backgroundColor: "rgba(233,189,174,0.07)", border: "1px solid rgba(233,189,174,0.25)" }}>
          <p className="text-[13px] leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>
            <span style={{ color: TEXT_CLR, fontWeight: 700 }}>{name ?? "그대"}{suffix}</span>의 사주팔자를 살펴봤소.
            이 여덟 글자 안에 그대가 끌리는 사람의 유형, 인연이 찾아오는 시기, 그리고 사랑에서 반복되는 패턴이 모두 담겨 있소이다.
          </p>
        </div>
      </div>
...
function StepConcern({ onPrev, onNext, initial }: {
  ...
  const MAX = 200;
  const PLACEHOLDER = `예) 지금 솔로인데 언제쯤 연애를 시작할 수 있을지, 결혼은 할 수 있는 건지 걱정돼요. 제 운명의 사람은 어떤 사람일지도 너무 궁금해요.`;
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>그대의 사랑에 </span>
        ...
        <div className="rounded-2xl px-4 pt-4 pb-3" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(233,189,174,0.25)" }}>
              ...
              <div className="absolute bottom-full left-0 right-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                ...
                {EMAIL_DOMAINS.map((d) => (
                  <div key={d} onClick={() => { setDomain(d); setOpen(false); }}
                    ...
                    style={{ backgroundColor: domain === d ? "rgba(233,189,174,0.2)" : "transparent", color: domain === d ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {d}
                  </div>
                ))}
              </div>
                ...
                <div className="absolute bottom-full left-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                  ...
                  {PHONE_PREFIXES.map((p) => (
                    <div key={p} onClick={() => { setPhonePrefix(p); setPhoneOpen(false); }}
                      className="px-4 py-3 text-[14px] cursor-pointer"
                      style={{ backgroundColor: phonePrefix === p ? "rgba(233,189,174,0.2)" : "transparent", color: phonePrefix === p ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {p}
                    </div>
                  ))}
                </div>
...
function StepLoading({ name, date, time, calendar, gender, email, concern, phone }: {
  ...
  const goNext = () => {
    ...
    router.push(`/saju/saju_yeonae/checkout?${params.toString()}`);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center" style={{ backgroundColor: "#080a0f" }}>
      ...
      <div className="relative z-10 px-8 w-full max-w-sm text-center">
        ...
        <p className="text-[22px] font-black mb-1" style={{ color: "#fff" }}>{name}님의 연애사주</p>
