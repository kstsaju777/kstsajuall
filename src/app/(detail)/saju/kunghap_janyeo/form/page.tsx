...
function FormShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      {/* 배경 이미지 — 전체 화면 */}
      <img
        src="/media/cards/kunghap_janyeo/kunghap_janyeo-0.jpg"
        aria-hidden
                ...
                <div className="absolute bottom-full left-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                  ...
                  {PHONE_PREFIXES.map((p) => (
                    <div key={p} onClick={() => { setPhonePrefix(p); setPhoneOpen(false); }}
                      className="px-4 py-3 text-[14px] cursor-pointer"
                      style={{ backgroundColor: phonePrefix === p ? "rgba(0,180,216,0.25)" : "transparent", color: phonePrefix === p ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {p}
                    </div>
                  ))}
                </div>
...
function StepLoading({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, concern, phone }: {
  ...
  const goNext = () => {
    ...
    router.push(`/saju/kunghap_janyeo/checkout?${params.toString()}`);
  };
    ...
    <>
      ...
      {(step >= 1 && step <= 2) || (step >= 4 && step <= 6) || step === 8 ? (
        <FormShell>
          ...
          {step === 8 && (
            <StepEmail initial={form.email} initialPhone={form.phone} onPrev={() => setStep(7)} onNext={(email, phone) => next({ email, phone }, 9)} />
          )}
        </FormShell>
      ) : null}
      {step === 7 && (
