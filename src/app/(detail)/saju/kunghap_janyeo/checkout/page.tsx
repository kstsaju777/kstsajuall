...
const WHITE    = "#ffffff";
const RED      = "#0077b6";
const RED_SOFT = "#00b4d8";
const RED_PALE = "#e8f7fb";
const ROSE     = "#90e0ef";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCTS = [
  { id: "janyeo-compat", name: "자녀궁합", original: 59800, discount: 50, price: 29900 },
];

function PayBottomSheet({ open, onClose, onConfirm }: {
  open: boolean; onClose: () => void; onConfirm: (id: string) => void;
}) {
  const [selected] = useState("janyeo-compat");
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(0,180,216,0.16)", border: `1px solid ${ACCENT}55`, color: "#90e0ef" }}>
            ...
            <div className="w-full text-left rounded-2xl px-4 py-3.5" style={{ backgroundColor: DCARD, border: `1.5px solid ${ACCENT}`, boxShadow: `0 0 0 3px ${ACCENT}22` }}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 자녀궁합 이야기</p>
                </div>
          ...
          <div className="space-y-1.5 mb-4">
            ...
            <div className="flex items-center justify-between text-[13px]">
              <span style={{ color: "#00b4d8", fontWeight: 700 }}>지금 결제 시 할인 ({sel.discount}% 특가)</span>
          ...
          <button onClick={() => onConfirm(selected)} className="w-full py-4 rounded-2xl font-black text-[17px] text-white active:scale-[0.99] transition-transform"
            style={{ background: "linear-gradient(135deg, #00b4d8, #0077b6)", boxShadow: "0 6px 20px rgba(0,180,216,0.4)" }}>
            결제하기
          </button>
      ...
      {confirmExit && (
        <div className="fixed z-[60] px-6" style={{ left: "max(0px, calc(50vw - 240px))", width: "min(100%, 480px)", top: "34%", pointerEvents: "none" }}>
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)} className="w-full mt-3 py-2.5 rounded-xl text-[13.5px] font-bold text-white" style={{ background: "linear-gradient(135deg, #00b4d8, #0077b6)" }}>혜택 받고 계속하기</button>
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4"
      ...
      <button onClick={onPay}
        ...
        <span>🔓</span>
        <span>{name}님 · {partnerName}님 자녀궁합 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/kunghap_janyeo/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/kunghap_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/kunghap_janyeo/report?id=${d.resultId}` : `/saju/kunghap_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/kunghap_janyeo/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        {/* ① 헤더 이미지 + 제목 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_janyeo/jaehwe-1.jpg"
          bottomImgSrc="/media/cards/kunghap_janyeo/jaehwe-2.jpg"
          label="자녀궁합 · 정밀 리포트"
          headline={`${name}님과\n${partnerName}님의 인연,\n낱낱이 봤어요`}
          accentWord="낱낱이"
          bgColor={WHITE}
        ...
        {/* ③ 분석 섹션 헤더 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_janyeo/jaehwe-3.jpg"
          bottomImgSrc="/media/cards/kunghap_janyeo/jaehwe-1.jpg"
        ...
        <ImageDivider src="/media/cards/kunghap_janyeo/jaehwe-2.jpg" bgColor={CREAM} />
