  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-end text-[11px] mb-2" style={{ color: "#33ccee" }}>
          <span>{pct}%</span>
        </div>
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#003344" }}>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const imageTask = fetch("/api/kunghap_janyeo-report", {
      ...
      const poll = async () => {
        for (let i = 0; i < 150; i++) { // 최대 5분(2초 간격)
          if (cancelled) return;
          try {
            const r = await fetch(`/api/kunghap_janyeo-report?id=${encodeURIComponent(resultId)}`);
