...
//
// 미리보기: http://localhost:3000/saju/kunghap_imshin/report-preview
//
        ...
        {coords.map((c, i) => {
          ...
          return (
            <g key={i}>
              ...
              {isWorst && <text x={c.x} y={c.y + 16} fontSize="13" fill="#E24B4A" textAnchor="middle" fontWeight="700">주의</text>}
              {isBest  && <text x={c.x} y={c.y - 10} fontSize="13" fill="#5b9e6a" textAnchor="middle" fontWeight="700">길함</text>}
...
// 추천 상품(크로스셀) 그리드 (마무리) — 결혼궁합과 동일 구조
...
const SAJU_ORDER_IM = ["정통명리 종합사주", "영재발굴 자녀사주", "대박이야 재물사주", "나만솔로? 연애사주", "우리아가 유아사주", "오래살자 건강사주"];
...
function RecoGrid() {
  ...
  const all = (CATEGORY_CARDS["전체"] ?? []).filter((c) => !c.href.includes("kunghap_imshin") && !RECO_EXCLUDE.has(c.name));
  const sajuCards    = sortByIm(all.filter((c) => !c.href.includes("kunghap")), SAJU_ORDER_IM);
  const kunghapCards = sortByIm(all.filter((c) => c.href.includes("kunghap")), KUNGHAP_ORDER_IM);
...
function ReviewBox() {
  ...
  return (
    <div className="rounded-2xl p-5 mx-6 mb-8" style={{ background: WHITE, border: "1.5px solid #738e6f", boxShadow: "0 0 6px #738e6f80, 0 0 16px #738e6f40, 0 0 32px #738e6f20" }}>
...
const IM5_COLOR = "#2a7a6a"; const IM5_PALE  = "#edf8f5"; // 5장 — 틸 (임신 시기·자연 흐름)
...
const IM8_BOND  = "#8a3a5a"; const IM8_BOND_P = "#fdf0f4"; // 8장 — 딥로즈 (감정 유대)
const IM8_SUP   = "#5a3a8a"; const IM8_SUP_P  = "#f5eeff"; // 8장 — 보라 (상호 지지)
const IM9_DARK  = "#5a3a1a"; const IM9_DARK_P = "#fdf5ec"; // 9장 — 다크브라운 (어려움·도전)
const IM9_GOLD  = "#8a6200"; const IM9_GOLD_P = "#fdf8e0"; // 9장 — 골드 (극복·희망)
const IM10_STAR = "#b07200"; const IM10_STAR_P= "#fdf6da"; // 10장 — 브라이트골드 (최고 시기·황금창)
const IM10_TEAL = "#1a6a7a"; const IM10_TEAL_P= "#edf6f8"; // 10장 — 틸블루 (사주 근거·깊이)

/**
 * OhaengDonutRich — 오행 균형 도넛 + 세로 바차트 조합 카드
 *
 * 사주 원국의 천간·지지를 분석해 목·화·토·금·수 다섯 기운의 비율을
 * 도넛 차트와 세로 바차트로 동시에 보여줍니다. 왼쪽 도넛은 가장 강한
 * 오행을 한자로 강조해 직관적으로 전달하고, 오른쪽 바차트는 각 기운의
 * 상대적 크기를 높이로 비교합니다. 임신·출산 맥락에서 부모의 오행 분포는
 * 자녀와의 기운 조화나 임신 시기 선택에 중요한 배경이 됩니다.
 */
function OhaengDonutRich({ view, accentColor }: { view: MyeongsikView | null; accentColor: string }) {
  const counts: Record<string, number> = { 목: 0, 화: 0, 토: 0, 금: 0, 수: 0 };
  if (view) {
    for (const p of view.pillars) {
      if (p.ganEl && counts[p.ganEl] !== undefined) counts[p.ganEl]++;
  ...
  return (
    <div className="mx-5 my-2 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(135deg, ${dom.color}10 0%, ${WHITE} 55%)`, border: `1.5px solid ${dom.color}30` }}>
      ...
      {accentColor && false && <span style={{ color: accentColor }} />}
    </div>
  );
}

/**
 * INatureCard — 나의 기질 카드 (임신궁합 맥락)
 *
 * 사주 원국에서 읽히는 본연의 기질을 키워드와 함께 풀어냅니다.
 * '기질의 빛' 섹션은 임신·출산·양육 과정에서 이 기질이 발휘하는 긍정적인 힘을,
 * '주의해야 할 그림자' 섹션은 임신 과정이나 배우자와의 관계에서 이 기질이
 * 드러낼 수 있는 어려운 면을 솔직하게 짚습니다. 키워드는 색상 뱃지로 표시되어
 * 한눈에 이 사람의 기운을 파악할 수 있습니다. 임신 준비 중인 부모가 자신의
...
function INatureCard({ data, color, label }: {
  ...
  return (
    <div className="mx-5 mb-5 rounded-2xl overflow-hidden" style={{ background: WHITE, border: `1px solid ${INK}10`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
      ...
      {strengthDesc && (
        <div className="px-4 pt-3 pb-3" style={{ borderBottom: `1px solid ${INK}08` }}>
      ...
      {shadowDesc && (
        <div className="px-4 pt-3 pb-3" style={{ borderBottom: paragraphs.length > 0 ? `1px solid ${INK}08` : "none", background: "#fffaf9" }}>
          <div className="flex items-center gap-1.5 mb-1.5">
            <span className="text-[14px]">🌙</span>
      ...
      {paragraphs.length > 0 && (
        <div className="px-4 pt-3 pb-4">
          {paragraphs.map((p, i) => (
            <p key={i} className="text-[13.5px] leading-[1.85] mt-3 first:mt-0" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
          ))}
        </div>
      )}
    </div>
...
/**
 * ParentStyleCard — 나의 부모 성향 유형 카드 (임신궁합 맥락)
 *
 * 사주 원국을 통해 이 사람이 어떤 방식으로 임신·출산·육아를 맞이할 것인지,
 * 타고난 부모 성향 유형을 보여줍니다. 유형명(patternType)과 아이콘으로 직관적인
 * 인상을 먼저 전달하고, 한 줄 요약 인트로와 핵심 callout 문장으로 이 성향의
 * 본질을 짚습니다. 이어지는 단락들에서는 이 부모 성향이 배우자와의 관계,
 * 임신 준비 과정, 태교 방식, 출산 이후 양육 스타일에 어떻게 나타나는지를
 * 사주 원리와 연결해 구체적으로 풀어냅니다. 결혼궁합의 MarriagePatternCard와
 * 동일한 레이아웃 구조를 사용하되, 맥락은 임신·출산·부모됨으로 전환됩니다.
 */
function ParentStyleCard({ data, color, pale }: {
          ...
          return coords.map((c, i) => {
            ...
            return (
              <g key={i}>
                ...
                {isPeak && (
                  <text x={c.x} y={isMax ? c.y - 16 : c.y + 22} textAnchor="middle" fontSize="16" fontWeight="800"
                    fill={isMax ? "#1a4a9e" : "#c0306a"}>{isMax ? "길함" : "주의"}</text>
                )}
              </g>
            );
          });
...
function ChildFlowGraphLocal({ myView, partnerView, myYE, myHE, myGE, ptYE, ptHE, ptGE, myGender, ptGender, myName, partnerName }: {
  myView: MyeongsikView; partnerView: MyeongsikView;
  myYE: string; myHE: string; myGE: string;
  ptYE: string; ptHE: string; ptGE: string;
  myGender: string; ptGender: string;
  ...
  const scoreYear = (year: number) => {
    ...
    const ganEl = _S5_STEM_EL[gz[0] ?? ""] ?? "";
    const jiEl  = _S5_STEM_EL[_S5_BRANCH_MAIN[gz[1] ?? ""] ?? ""] ?? "";
    const base = (el: string, yE: string, hE: string, gE: string) =>
      (el === yE ? 2 : el === hE ? 1 : el === gE ? -2 : 0);
    const myBase = base(ganEl, myYE, myHE, myGE) + base(jiEl, myYE, myHE, myGE);
    const ptBase = base(ganEl, ptYE, ptHE, ptGE) + base(jiEl, ptYE, ptHE, ptGE);
    const childBonus = (ilgan: string, gender: string) => {
      const gSS = sipseongOfStem(ilgan, gz[0] ?? "");
      const jSS = sipseongOfBranch(ilgan, gz[1] ?? "");
      return (s5IsChildSS(gSS, gender) ? 1.5 : 0) + (s5IsChildSS(jSS, gender) ? 1.5 : 0);
    };
    const raw = myBase + ptBase + childBonus(myIlgan, myGender) + childBonus(ptIlgan, ptGender);
    // raw 범위 -8 ~ +11, 정규화 → 0~100
    return Math.min(100, Math.max(0, Math.round((raw + 8) / 19 * 100)));
  };
  ...
  return (
    <div className="mx-5 mb-2 rounded-2xl overflow-hidden" style={{ border: `1px solid rgba(0,0,0,0.08)`, background: "#f9f6ff" }}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", display: "block", overflow: "visible" }}>
        <defs>
          <linearGradient id="im3cfg2" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={IM3_COLOR} stopOpacity="0.22" />
            <stop offset="100%" stopColor={IM3_COLOR} stopOpacity="0.02" />
          </linearGradient>
        </defs>
        <path d={fillPath} fill="url(#im3cfg2)" />
        <path d={linePath} fill="none" stroke={IM3_COLOR} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round" />
        {coords.map((c, i) => (
          <line key={`vl-${i}`} x1={c.x} y1={c.y + 4} x2={c.x} y2={PT + CH} stroke={IM3_COLOR} strokeWidth="1" strokeDasharray="2 2" strokeOpacity="0.4" />
        ))}
          ...
          return (
            <g key={y}>
              ...
              <text x={xOf(y)} y={PT + CH + 18} textAnchor="middle" fontSize="10" fontWeight="600"
                fill={isPeak ? (isMax ? "#1a4a9e" : "#c0306a") : MUTE}>{String(y).slice(2)}</text>
              {isPeak && (
...
function ChildFlowPeriodsLocal({ myView, partnerView, myYE, myHE, myGE, ptYE, ptHE, ptGE, myGender, ptGender, myName, partnerName }: {
  myView: MyeongsikView; partnerView: MyeongsikView;
  myYE: string; myHE: string; myGE: string;
  ptYE: string; ptHE: string; ptGE: string;
  myGender: string; ptGender: string;
  ...
  const pts = YEARS.map(year => {
    ...
    const myChild = [myGanSS, myJiSS].filter(ss => s5IsChildSS(ss, myGender));
    const ptChild = [ptGanSS, ptJiSS].filter(ss => s5IsChildSS(ss, ptGender));
    return { year, score: scoreYear(year), gz, myChild, ptChild };
  });

  // 방향 분류 (THRESH 이상 차이만 방향으로 인정)
  const THRESH = 6;
  ...
  // 연속 같은 방향 → 그룹핑 (flat은 앞 방향에 흡수)
  type Group = { years: number[]; scores: number[]; dir: Dir; myChildYrs: string[][]; ptChildYrs: string[][]; gzList: string[] };
  const groups: Group[] = [];
  pts.forEach((p, i) => {
    const rawDir = dirs[i];
    ...
    if (groups.length === 0 || groups[groups.length - 1].dir !== effectiveDir) {
      groups.push({ years: [p.year], scores: [p.score], dir: effectiveDir, myChildYrs: [p.myChild], ptChildYrs: [p.ptChild], gzList: [p.gz] });
    } else {
      const g = groups[groups.length - 1];
      g.years.push(p.year); g.scores.push(p.score);
      g.myChildYrs.push(p.myChild); g.ptChildYrs.push(p.ptChild); g.gzList.push(p.gz);
    }
  });
  ...
  groups.forEach((g, i) => {
    if (g.years.length === 1 && merged.length > 0) {
      ...
      prev.years.push(...g.years); prev.scores.push(...g.scores);
      prev.myChildYrs.push(...g.myChildYrs); prev.ptChildYrs.push(...g.ptChildYrs);
      prev.gzList.push(...g.gzList);
    } else {
      merged.push({ ...g });
    }
  });
  void groups; // suppress unused warning

  // 각 그룹 → period 카드 데이터 생성
  ...
  const periods = merged.map(g => {
    ...
    const hasMyChild = g.myChildYrs.some(arr => arr.length > 0);
    const hasPtChild = g.ptChildYrs.some(arr => arr.length > 0);
    const myChildSS = g.myChildYrs.flat().filter((v, i, a) => a.indexOf(v) === i).join("·") || "";
    const ptChildSS = g.ptChildYrs.flat().filter((v, i, a) => a.indexOf(v) === i).join("·") || "";

    const isGlobalPeak = g.scores.includes(maxScore);
    const isGlobalTrough = g.scores.includes(minScore);

    const yearLabel = g.years.length === 1
      ? `${g.years[0]}년`
    ...
    if (isGlobalPeak && peakScore >= 55) {
      trend = "고점";
      title = "자녀 인연이 가장 강하게 열리는 시기";
      const childPart = hasMyChild && hasPtChild
        ? `특히 두 분 모두 자녀성(${myChildSS} / ${ptChildSS})이 드는 시기로, 임신·출산의 기운이 강하게 모입니다.`
        : hasMyChild
        ? `${myName}님 사주에 자녀성(${myChildSS})이 드는 시기로, 자녀 기운이 활발하게 열립니다.`
        : hasPtChild
        ? `${partnerName}님 사주에 자녀성(${ptChildSS})이 드는 시기로, 자녀 기운이 활발하게 열립니다.`
        : "용신과 희신의 기운이 함께 들어와 두 분 모두 운의 흐름이 좋게 맞아떨어지는 시기입니다.";
      text = `${gzDesc}의 기운이 두 사람의 사주에 활기를 불어넣습니다. ${childPart} 적극적으로 준비하기 좋은 때이오.`;
    } else if (isGlobalTrough && peakScore <= 45) {
      trend = "저점";
      title = "기운이 약해지는 시기 — 몸과 마음 돌보기";
      text = `${gzDesc}의 기운은 두 사람의 사주에 부담을 주는 오행이 겹쳐 들어오는 구간입니다. 무리한 시도보다는 체력과 마음의 안정을 우선하는 것이 좋소. 이 시기를 잘 넘기면 다음 상승 구간에서 더 좋은 흐름을 맞이할 수 있습니다.`;
    } else if (g.dir === "up") {
      trend = "상승 중";
      title = avgScore >= 55 ? "임신 인연 활성화" : "기운이 서서히 나아지는 시기";
      const childPart = hasMyChild || hasPtChild
        ? ` ${hasMyChild ? myName + "님" : partnerName + "님"}의 사주에 자녀성(${hasMyChild ? myChildSS : ptChildSS})이 겹쳐 들어오며 자녀 기운이 한층 강해집니다.`
        : "";
      text = `${gzDesc}의 기운이 나아가며 두 사람의 임신·출산 인연이 점차 활성화되는 구간입니다.${childPart} 이 흐름을 잘 활용하면 좋은 시기이오.`;
    } else if (g.dir === "down") {
      trend = "하락 중";
      title = avgScore >= 45 ? "잠시 숨 고르기" : "기운이 가라앉는 구간";
      text = `${gzDesc}의 기운이 두 사람의 사주와 충돌하거나 기신 오행이 강해지는 시기입니다. 두 분의 기운을 조화롭게 조정하는 것이 중요하오. 급하게 서두르기보다 안정적인 준비에 집중하면 다음 상승 구간을 더 잘 맞이할 수 있습니다.`;
    } else {
      trend = "유지";
      title = "평이하게 흐르는 시기";
      const childPart = hasMyChild || hasPtChild
        ? ` 다만 ${hasMyChild ? myName + "님" : partnerName + "님"}의 자녀성(${hasMyChild ? myChildSS : ptChildSS})이 드는 해가 포함되어 있으니 그 시점을 눈여겨보시오.`
        : "";
      text = `${gzDesc}의 기운이 큰 변화 없이 흐르는 구간입니다.${childPart} 꾸준히 준비하고 건강을 챙기는 것이 좋소.`;
    }
  ...
  return (
    <div className="mx-5 mb-4 space-y-2">
      {periods.map((p, i) => {
        const meta = FLOW_TREND_META[p.trend] ?? { icon: "•", color: MUTE };
        return (
          <div key={i} className="flex gap-3 items-start rounded-2xl px-4 py-3.5" style={{ background: `${meta.color}09`, border: `1px solid ${meta.color}22` }}>
            <div className="flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center text-[15px] font-black mt-0.5"
              style={{ background: `${meta.color}14`, border: `2px solid ${meta.color}30`, color: meta.color }}>
...
// ══════════════════════════════════════════════════════════
//  향후 5년 세운별 임신·출산 흐름 컴포넌트
// ══════════════════════════════════════════════════════════

const _S5_STEM_EL: Record<string, string> = {
  甲:"목",乙:"목",丙:"화",丁:"화",戊:"토",己:"토",庚:"금",辛:"금",壬:"수",癸:"수",
};
const _S5_BRANCH_MAIN: Record<string, string> = {
  子:"癸",丑:"己",寅:"甲",卯:"乙",辰:"戊",巳:"丙",午:"丁",未:"己",申:"庚",酉:"辛",戌:"戊",亥:"壬",
};
const _S5_GAN_KO: Record<string, string> = {
  甲:"갑",乙:"을",丙:"병",丁:"정",戊:"무",己:"기",庚:"경",辛:"신",壬:"임",癸:"계",
};
const _S5_JI_KO: Record<string, string> = {
  子:"자",丑:"축",寅:"인",卯:"묘",辰:"진",巳:"사",午:"오",未:"미",申:"신",酉:"유",戌:"술",亥:"해",
};

type S5Tier = "good" | "caution" | "neutral";

const S5_TIER_COLOR: Record<S5Tier, string> = { good: "#2e7d32", caution: "#c62828", neutral: "#5a5a7a" };
const S5_TIER_BG:    Record<S5Tier, string> = { good: "#e8f5e9", caution: "#ffebee", neutral: "#f3f4f6" };
const S5_TIER_LABEL: Record<S5Tier, string> = { good: "길운",    caution: "주의",    neutral: "보통"    };

const S5_CLS_STYLE: Record<string, { bg: string; border: string; text: string; label: string }> = {
...
function s5TierOf(gz: string, yE: string, hE: string, gE: string): S5Tier {
  const score = (el: string) => el === yE ? 2 : el === hE ? 1 : el === gE ? -2 : 0;
  const t = score(_S5_STEM_EL[gz[0]] ?? "") + score(_S5_STEM_EL[_S5_BRANCH_MAIN[gz[1]] ?? ""] ?? "");
  return t >= 2 ? "good" : t <= -1 ? "caution" : "neutral";
}

function s5ClsEl(el: string, yE: string, hE: string, gE: string): string {
  return el === yE ? "yong" : el === hE ? "heu" : el === gE ? "gi" : "neu";
}

const S5_CHILD_SS: Record<string, string[]> = {
  female: ["식신", "상관"],
  male:   ["편관", "정관"],
};

function s5IsChildSS(ss: string, gender: string): boolean {
  const isFemale = /female|여/.test(gender);
  return (S5_CHILD_SS[isFemale ? "female" : "male"] ?? []).includes(ss);
}

function ImshinSeunFiveCard({
  myView, partnerView,
  myYE, myHE, myGE, ptYE, ptHE, ptGE,
  myName, ptName, myGender, ptGender,
}: {
  myView: MyeongsikView; partnerView: MyeongsikView;
  myYE: string; myHE: string; myGE: string;
  ptYE: string; ptHE: string; ptGE: string;
  ...
  return (() => {
    const selYear = sel !== null ? years[sel] : years[0];
    const selGz = yearGanji(selYear);
    ...
    return (
      <div className="px-4 pb-6">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12,
          ...
          <span style={{ fontSize: 12, fontWeight: 600, color: "#3a5abf" }}>카드를 눌러 년별, 월별 상세 흐름을 확인하시오</span>
        </div>
        {/* 범례 */}
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#b07a00", background: "#fff8e1", border: "2px solid #f9a825", borderRadius: 5, padding: "1px 6px" }}>자녀성</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: "#7a6a50" }}>— 여성:식신·상관 / 남성:편관·정관</span>
          </div>
        </div>
        {/* 카드 스크롤 영역 */}
        <div style={{ position: "relative" }}>
        ...
        <div className="overflow-x-auto pb-2 -mx-1" style={{ scrollbarWidth: "none" }}>
          <div className="flex gap-2 min-w-max px-1">
            {years.map((year, idx) => {
              const gz = yearGanji(year);
              const myTier = s5TierOf(gz, myYE, myHE, myGE);
              const ptTier = s5TierOf(gz, ptYE, ptHE, ptGE);
              ...
              const ganEl = _S5_STEM_EL[gz[0] ?? ""] ?? "";
              const jiEl  = _S5_STEM_EL[_S5_BRANCH_MAIN[gz[1] ?? ""] ?? ""] ?? "";
              const myGanColor = S5_CLS_STYLE[s5ClsEl(ganEl, myYE, myHE, myGE)].text;
              const myJiColor  = S5_CLS_STYLE[s5ClsEl(jiEl,  myYE, myHE, myGE)].text;
              const ptGanColor = S5_CLS_STYLE[s5ClsEl(ganEl, ptYE, ptHE, ptGE)].text;
              const ptJiColor  = S5_CLS_STYLE[s5ClsEl(jiEl,  ptYE, ptHE, ptGE)].text;
              ...
              return (
                <div key={year} className="flex-shrink-0 flex flex-col items-center"
                  ...
                  <div style={{ display: "flex", gap: 3, justifyContent: "center", marginBottom: 3 }}>
                    {[{ ss: myGanSS, cls: s5ClsEl(ganEl, myYE, myHE, myGE), gender: myGender }, { ss: ptGanSS, cls: s5ClsEl(ganEl, ptYE, ptHE, ptGE), gender: ptGender }].map(({ ss, cls, gender }, i) => {
                      const s = S5_CLS_STYLE[cls];
                      const isChild = s5IsChildSS(ss, gender);
                      return <span key={i} style={{ fontSize: 11, fontWeight: 700, color: s.text, background: s.bg, borderRadius: 5, padding: "2px 6px", boxShadow: isChild ? "0 0 0 2px #fff, 0 0 0 5px #f9a825" : undefined }}>{ss || "—"}</span>;
                    })}
                  </div>
                  {/* 천간 이미지 */}
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <div style={{ display: "flex", gap: 3, justifyContent: "center", marginTop: 3, marginBottom: 2 }}>
                    {[{ ss: myJiSS, cls: s5ClsEl(jiEl, myYE, myHE, myGE), gender: myGender }, { ss: ptJiSS, cls: s5ClsEl(jiEl, ptYE, ptHE, ptGE), gender: ptGender }].map(({ ss, cls, gender }, i) => {
                      const s = S5_CLS_STYLE[cls];
                      const isChild = s5IsChildSS(ss, gender);
                      return <span key={i} style={{ fontSize: 11, fontWeight: 700, color: s.text, background: s.bg, borderRadius: 5, padding: "2px 6px", boxShadow: isChild ? "0 0 0 2px #fff, 0 0 0 5px #f9a825" : undefined }}>{ss || "—"}</span>;
                    })}
          ...
          return (
            <div style={{ marginTop: 20 }}>
                ...
                <div style={{ overflowX: "auto", scrollbarWidth: "none" as const, paddingBottom: 4 }}>
                  <div style={{ display: "flex", gap: 8, width: "max-content" }}>
                    {months.map((month) => {
                      ...
                      const mGanEl = _S5_STEM_EL[mgz[0] ?? ""] ?? "";
                      const mJiEl  = _S5_STEM_EL[_S5_BRANCH_MAIN[mgz[1] ?? ""] ?? ""] ?? "";
                      const myMTier = s5TierOf(mgz, myYE, myHE, myGE);
                      const ptMTier = s5TierOf(mgz, ptYE, ptHE, ptGE);
                      ...
                      return (
                        <div key={month}
                          ...
                          {/* 천간 십성 태그 */}
                          <div style={{ display: "flex", gap: 3, justifyContent: "center", marginBottom: 3 }}>
                            {[{ ss: myMGanSS, cls: s5ClsEl(mGanEl, myYE, myHE, myGE), gender: myGender }, { ss: ptMGanSS, cls: s5ClsEl(mGanEl, ptYE, ptHE, ptGE), gender: ptGender }].map(({ ss, cls, gender }, i) => {
                              const s = S5_CLS_STYLE[cls];
                              const isChild = s5IsChildSS(ss, gender);
                              return <span key={i} style={{ fontSize: 11, fontWeight: 700, color: s.text, background: s.bg, borderRadius: 5, padding: "2px 6px", boxShadow: isChild ? "0 0 0 2px #fff, 0 0 0 5px #f9a825" : undefined }}>{ss || "—"}</span>;
                            })}
                          ...
                          {/* 지지 십성 태그 */}
                          <div style={{ display: "flex", gap: 3, justifyContent: "center", marginTop: 3, marginBottom: 2 }}>
                            {[{ ss: myMJiSS, cls: s5ClsEl(mJiEl, myYE, myHE, myGE), gender: myGender }, { ss: ptMJiSS, cls: s5ClsEl(mJiEl, ptYE, ptHE, ptGE), gender: ptGender }].map(({ ss, cls, gender }, i) => {
                              const s = S5_CLS_STYLE[cls];
                              const isChild = s5IsChildSS(ss, gender);
                              return <span key={i} style={{ fontSize: 11, fontWeight: 700, color: s.text, background: s.bg, borderRadius: 5, padding: "2px 6px", boxShadow: isChild ? "0 0 0 2px #fff, 0 0 0 5px #f9a825" : undefined }}>{ss || "—"}</span>;
                            })}
              ...
              {selMonth !== null && (() => {
                ...
                const ptMGanSS = sipseongOfStem(ptIlgan, mgz[0] ?? "");
                const ptMJiSS  = sipseongOfBranch(ptIlgan, mgz[1] ?? "");
                const myGanCls = s5ClsEl(mGanEl, myYE, myHE, myGE);
                const myJiCls  = s5ClsEl(mJiEl,  myYE, myHE, myGE);
                const ptGanCls = s5ClsEl(mGanEl, ptYE, ptHE, ptGE);
                const ptJiCls  = s5ClsEl(mJiEl,  ptYE, ptHE, ptGE);
                const myGanIsChild = s5IsChildSS(myMGanSS, myGender);
                const myJiIsChild  = s5IsChildSS(myMJiSS,  myGender);
                const ptGanIsChild = s5IsChildSS(ptMGanSS, ptGender);
                const ptJiIsChild  = s5IsChildSS(ptMJiSS,  ptGender);
                const myChildSS = [myMGanSS, myMJiSS].filter(ss => s5IsChildSS(ss, myGender));
                const ptChildSS = [ptMGanSS, ptMJiSS].filter(ss => s5IsChildSS(ss, ptGender));
                const myHasChild = myChildSS.length > 0;
                const ptHasChild = ptChildSS.length > 0;
                const bothHasChild = myHasChild && ptHasChild;

                const mCombinedLabel = bothHasChild ? "두 사람 자녀성 ✦"
                  : myHasChild ? `${myName} 자녀성`
                  : ptHasChild ? `${ptName} 자녀성`
                  : bothMGood ? "두 사람 모두 길운"
                  : bothMBad  ? "두 사람 모두 주의"
                  : anyMBad   ? "엇갈리는 흐름"
                  : "평이한 흐름";
                const mPanelAccent = (myHasChild || ptHasChild) ? "#b07a00"
                  : bothMGood ? "#2e7d32" : anyMBad ? "#c62828" : "#5a5a7a";

                const chipStyle = (cls: string, isChild: boolean) => {
                  const s = S5_CLS_STYLE[cls] ?? S5_CLS_STYLE.neu;
                  return {
                    fontSize: 12, fontWeight: 800, padding: "4px 10px", borderRadius: 9,
                    background: s.bg, color: s.text,
                    border: `1.5px solid ${isChild ? "#f9a825" : s.border}`,
                    boxShadow: isChild ? "0 0 0 2px #fff8e1" : "none",
                ...
                if (bothHasChild && bothMGood) {
                  commentIcon = "✦"; commentColor = "#b07a00";
                  comment = `두 분 모두 자녀성(${myChildSS.join("·")} / ${ptChildSS.join("·")})이 들고 기운도 길운으로 맞아떨어지는 달입니다. 임신·출산을 시도하기에 이 달보다 좋은 때가 없으오. 적극적으로 준비하시기를 권합니다.`;
                } else if (bothHasChild && bothMBad) {
                  commentIcon = "⚠"; commentColor = "#b07a00";
                  comment = `두 분 모두 자녀성(${myChildSS.join("·")} / ${ptChildSS.join("·")})이 드는 달이나, 기신 오행도 함께 들어와 체력 부담이 있는 시기입니다. 자녀 기운은 열려 있으니 컨디션을 살피며 무리하지 않는 선에서 시도해볼 수 있소.`;
                } else if (bothHasChild) {
                  commentIcon = "✦"; commentColor = "#b07a00";
                  comment = `두 분 모두 자녀성(${myChildSS.join("·")} / ${ptChildSS.join("·")})이 드는 달입니다. 임신·출산의 기운이 두 사람 사주에 동시에 열리는 드문 시기이오. 적극적으로 시도해 보시기를 권합니다.`;
                } else if (myHasChild && myMTier === "good") {
                  commentIcon = "↗"; commentColor = "#1a6a4a";
                  comment = `${myName}님 사주에 자녀성(${myChildSS.join("·")})이 들고 기운도 길운인 달입니다. ${myName}님 쪽의 임신 기운이 강하게 활성화되는 시기이오. ${ptName}님도 무리가 없다면 함께 준비하기 좋은 달입니다.`;
                } else if (myHasChild && myMTier === "caution") {
                  commentIcon = "◎"; commentColor = "#b07a00";
                  comment = `${myName}님 사주에 자녀성(${myChildSS.join("·")})은 드나, 기신 오행도 함께 들어오는 달입니다. 자녀 기운은 열려 있지만 체력 소모가 클 수 있으오. 컨디션을 우선 확인하고 시도 여부를 결정하시기를 권합니다.`;
                } else if (myHasChild) {
                  commentIcon = "◎"; commentColor = "#b07a00";
                  comment = `${myName}님 사주에 자녀성(${myChildSS.join("·")})이 드는 달입니다. 운기 자체는 평이하지만 자녀 기운이 열려 있으니 두 분이 함께 주목해볼 만한 달입니다.`;
                } else if (ptHasChild && ptMTier === "good") {
                  commentIcon = "↗"; commentColor = "#1a6a4a";
                  comment = `${ptName}님 사주에 자녀성(${ptChildSS.join("·")})이 들고 기운도 길운인 달입니다. ${ptName}님 쪽의 임신 기운이 활성화되는 시기이오. ${myName}님도 무리가 없다면 함께 준비하기 좋은 달입니다.`;
                } else if (ptHasChild && ptMTier === "caution") {
                  commentIcon = "◎"; commentColor = "#b07a00";
                  comment = `${ptName}님 사주에 자녀성(${ptChildSS.join("·")})은 드나, 기신 오행도 함께 들어오는 달입니다. 자녀 기운은 열려 있지만 체력 부담이 있을 수 있으오. 무리하지 않는 선에서 시도를 검토해 보시기를 권합니다.`;
                } else if (ptHasChild) {
                  commentIcon = "◎"; commentColor = "#b07a00";
                  comment = `${ptName}님 사주에 자녀성(${ptChildSS.join("·")})이 드는 달입니다. 운기는 평이하나 자녀 기운이 열려 있으니 함께 주목해볼 만한 달입니다.`;
                } else if (bothMGood) {
                  commentIcon = "↗"; commentColor = "#1a6a4a";
                  comment = `두 분 모두 용신·희신 기운이 드는 길운의 달입니다. 자녀성은 들지 않으나 전반적인 기운이 좋으니, 꾸준히 준비하고 컨디션을 쌓기에 좋은 시기입니다.`;
                } else if (bothMBad) {
                  commentIcon = "↘"; commentColor = "#c62828";
                  comment = `두 분 모두 기신 오행이 강하게 드는 달입니다. 무리한 시도보다 몸과 마음의 회복에 집중하는 것이 좋소. 다음 좋은 달을 위해 체력을 비축하는 시기로 삼으시기를 권합니다.`;
                } else if (anyMBad) {
                  const badName = myMTier === "caution" ? myName : ptName;
                  const goodName = myMTier === "caution" ? ptName : myName;
                  commentIcon = "→"; commentColor = "#5a5a7a";
                  comment = `${badName}님은 기신 오행이 드는 달로 부담이 있고, ${goodName}님은 무난한 흐름입니다. ${badName}님의 컨디션을 특히 살피며 결정하시기를 권합니다.`;
                } else {
                  commentIcon = "→"; commentColor = "#5a5a7a";
                  comment = `두 분 모두 무난하게 흘러가는 달입니다. 자녀성이나 특별한 길운은 없으나 평온한 기운 속에서 꾸준히 준비하기에 적합한 달입니다.`;
                }
                  ...
                  <div style={{ marginTop: 12, borderRadius: 16, border: `1.5px solid ${mPanelAccent}35`,
                    ...
                    <div style={{ padding: "11px 14px", background: `${mPanelAccent}10`, borderBottom: `1px solid ${mPanelAccent}20`,
                      ...
                      <span style={{ fontSize: 13, fontWeight: 800, color: mPanelAccent }}>
                        ...
                        <span style={{ fontWeight: 600, color: "#6a5a50" }}>
                          {mgz[0]}{mgz[1]}월 ({_S5_GAN_KO[mgz[0] ?? ""] ?? ""}{_S5_JI_KO[mgz[1] ?? ""] ?? ""}월)
                        </span>
                      </span>
                    ...
                    <div style={{ padding: "12px 14px 10px", display: "flex", gap: 10 }}>
                      ...
                      <div style={{ flex: 1, borderRadius: 12, minWidth: 0,
                        border: `1.5px solid ${myHasChild ? "#f9a825" : myTierColor}30`,
                        background: myHasChild ? "#fffdf0" : myMTier === "good" ? "#e8f5e9" : myMTier === "caution" ? "#fdecea" : "#f4f4f4",
                        padding: "11px 12px" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          <span style={chipStyle(myGanCls, myGanIsChild)}>{myMGanSS || "—"}</span>
                          <span style={chipStyle(myJiCls,  myJiIsChild )}>{myMJiSS  || "—"}</span>
                        </div>
                        {myHasChild && (
                          <div style={{ marginTop: 5, fontSize: 10, fontWeight: 700, color: "#b07a00" }}>자녀성 ✦</div>
                        )}
                        <div style={{ marginTop: 6, fontSize: 10, color: "#9a8878" }}>천간 {mgz[0]} · 지지 {mgz[1]}</div>
                      </div>
                      {/* 체은 카드 */}
                      <div style={{ flex: 1, borderRadius: 12, minWidth: 0,
                        border: `1.5px solid ${ptHasChild ? "#f9a825" : ptTierColor}30`,
                        background: ptHasChild ? "#fffdf0" : ptMTier === "good" ? "#e8f5e9" : ptMTier === "caution" ? "#fdecea" : "#f4f4f4",
                        padding: "11px 12px" }}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          <span style={chipStyle(ptGanCls, ptGanIsChild)}>{ptMGanSS || "—"}</span>
                          <span style={chipStyle(ptJiCls,  ptJiIsChild )}>{ptMJiSS  || "—"}</span>
                        </div>
                        {ptHasChild && (
                          <div style={{ marginTop: 5, fontSize: 10, fontWeight: 700, color: "#b07a00" }}>자녀성 ✦</div>
                        )}
...
/**
 * PregnancyScoreCard — 임신·출산 궁합 점수 카드
 *
 * 두 사람의 사주가 얼마나 임신·출산의 기운을 잘 품고 있는지를 0~100점 점수와
 * 등급(S~D)으로 한눈에 보여줍니다. 반원형 게이지는 점수를 시각적으로 전달하고,
 * 점수 아래에 등급 뱃지와 대표 한마디(label)가 붙어 이 숫자가 어떤 의미인지
 * 즉각 이해할 수 있게 합니다. 점수 구간별로 게이지 색상이 달라지고(보라→로즈→골드→
 * 회색), 이어지는 단락들에서 이 점수가 나오게 된 사주 구조적 근거를 풀어냅니다.
 * 임신궁합의 핵심 지표이므로 이 카드는 이 장 전체의 무게 중심입니다.
 */
function PregnancyScoreCard({ score, tier, label, paragraphs }: {
  score: number; tier: string; label: string; paragraphs: string[];
}) {
  ...
  const arcColor = s >= 80 ? IM3_COLOR : s >= 60 ? ROSE : s >= 40 ? GOLD : MUTE;
  const TIER_STYLE: Record<string, { bg: string; text: string; label: string }> = {
    "S": { bg: IM3_COLOR, text: "#fff",     label: "천생연분" },
    "A": { bg: ROSE,      text: "#fff",     label: "매우 좋음" },
    "B": { bg: GOLD,      text: "#fff",     label: "좋음" },
    "C": { bg: "#7a6a5a", text: "#fff",     label: "보통" },
    "D": { bg: MUTE,      text: "#fff",     label: "주의 필요" },
  };
  const ts = TIER_STYLE[tier] ?? TIER_STYLE["B"];
  return (
    <div className="mx-5 mb-5 rounded-3xl overflow-hidden" style={{ background: `linear-gradient(160deg, ${IM3_COLOR}10 0%, ${IM3_PALE} 100%)`, border: `1.5px solid ${IM3_COLOR}30` }}>
      <div className="px-5 pt-5 pb-1 flex items-center justify-between">
        <p className="text-[11px] font-bold tracking-widest" style={{ color: IM3_COLOR }}>👶 임신·출산 궁합 점수</p>
        <div className="flex items-center gap-1.5">
          <span className="text-[11px] font-black px-2.5 py-0.5 rounded-full" style={{ background: ts.bg, color: ts.text }}>{tier}등급</span>
          <span className="text-[11px] font-bold" style={{ color: IM3_COLOR }}>{ts.label}</span>
        </div>
      </div>
      <svg viewBox="0 0 220 115" style={{ width: "100%", maxHeight: 120 }}>
        <defs>
          <linearGradient id="pgGrad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stopColor={MUTE} />
            <stop offset="40%"  stopColor={GOLD} />
            <stop offset="75%"  stopColor={ROSE} />
            <stop offset="100%" stopColor={IM3_COLOR} />
          </linearGradient>
        </defs>
        {/* 배경 트랙 */}
        <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="#e8e0f0" strokeWidth="18" strokeLinecap="round" />
        {/* 채워진 게이지 */}
        <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="url(#pgGrad)" strokeWidth="18" strokeLinecap="round"
          strokeDasharray={`${(s / 100) * Math.PI * r} ${Math.PI * r}`} />
        {/* 바늘 */}
        <line x1={cx} y1={cy} x2={needleX} y2={needleY} stroke={IM3_DEEP} strokeWidth="2.5" strokeLinecap="round" />
        <circle cx={cx} cy={cy} r="5" fill={IM3_DEEP} />
        {/* 점수 */}
        <text x={cx} y={cy - 16} textAnchor="middle" fontSize="28" fontWeight="900" fill={arcColor}>{s}</text>
        <text x={cx} y={cy}      textAnchor="middle" fontSize="10"  fill={MUTE}>점</text>
      </svg>
...
/**
 * PregnancyReasonCard — 임신 궁합 사주 근거 카드
 *
 * 두 사람의 점수가 도출된 사주 원리상의 근거를 구체적으로 풀어냅니다.
 * 단순히 "합이 있다 / 충이 있다" 수준이 아니라, 어떤 오행·합충·식상 구조가
 * 두 사람의 임신·출산 기운에 어떻게 작용하는지를 아이콘·제목·설명 형태의
 * 항목(reasons)으로 명확하게 제시합니다. 각 근거 항목은 임신궁합 전문 용어
 * (식상, 관성, 생지, 월령 등)를 쉬운 언어로 풀어 담아, 사주를 잘 모르는 독자도
 * 이해할 수 있도록 구성됩니다. 상단 callout으로 핵심을 먼저 제시하고,
 * 하단 마무리 단락으로 근거 전체를 종합하는 흐름을 취합니다.
 */
function PregnancyReasonCard({ intro, callout, reasons, paragraphs }: {
  intro: string;
  callout: string;
  ...
  return (
    <div className="mx-5 mb-5">
      {intro && (
        <p className="mb-4 text-[13px] leading-relaxed italic" style={{ color: INK_SOFT }}>&#8220;{intro}&#8221;</p>
      )}
      {callout && (
        <div className="mb-4 px-4 py-3 rounded-xl" style={{ background: `${IM3_COLOR}0c`, borderLeft: `3px solid ${IM3_COLOR}` }}>
          <p className="text-[13.5px] font-bold leading-relaxed" style={{ color: IM3_COLOR }}>{callout}</p>
        </div>
      )}
      {reasons.length > 0 && (
        <div className="flex flex-col gap-3 mb-4">
          {reasons.map((r, i) => (
            <div key={i} className="flex gap-3 p-3.5 rounded-2xl" style={{ background: "#fff", border: `1px solid ${IM3_COLOR}22` }}>
              <div className="text-[22px] flex-shrink-0 mt-0.5">{r.icon}</div>
              <div>
                <p className="text-[13px] font-bold mb-1" style={{ color: IM3_DEEP }}>{r.title}</p>
                <p className="text-[12.5px] leading-relaxed" style={{ color: INK_SOFT }}>{r.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
/**
 * ChildEnergyCard — 태어날 자녀의 기운 유형 카드
 *
 * 두 사람의 사주가 만나 빚어낼 자녀의 오행적 기운을 한눈에 담아내는 카드이오.
 * 상단에는 energyType(기운 유형명)과 energyIcon(대표 이모지)을 크게 배치하여
 * 아이가 지닐 기질의 핵심을 직관적으로 전하고, 그 아래에 keywords를 앰버 배지로
 * 나열하여 기운의 세부 결을 살피오. 배지는 진하기를 교차하여 첫 번째 키워드가
 * 가장 강하게 눈에 들어오도록 하였소. 하단의 desc 단락은 자녀 기운의 본질적 의미와
 * 이 기운이 삶에서 어떻게 발현될지를 풍성하게 풀어내오.
 */
function ChildEnergyCard({ energyType, energyIcon, keywords, desc, color, pale }: {
  energyType: string; energyIcon: string; keywords: string[]; desc: string;
  color: string; pale: string;
}) {
  const accents = [color, `${color}cc`, `${color}99`];
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}30` }}>
      {/* 기운 유형 헤더 */}
      <div className="px-5 pt-5 pb-3 flex items-center gap-3" style={{ borderBottom: `1px solid ${color}20` }}>
        <span className="text-[2.2rem] leading-none">{energyIcon}</span>
        <div>
          <p className="text-[10px] tracking-[0.2em] mb-0.5" style={{ color: `${color}99`, fontFamily: SERIF }}>자녀 기운 유형</p>
          <p className="text-[17px] font-black leading-tight" style={{ color, fontFamily: SERIF }}>{energyType}</p>
        </div>
      </div>
      {/* 키워드 배지 */}
      <div className="px-5 py-3 flex flex-wrap gap-2" style={{ borderBottom: `1px solid ${color}15` }}>
        {keywords.map((kw, i) => (
          <span key={i} className="px-3 py-1 rounded-full text-[12px] font-bold"
            style={{ background: `${accents[i] ?? color}22`, color: accents[i] ?? color, border: `1px solid ${accents[i] ?? color}44` }}>
            {kw}
          </span>
        ))}
      </div>
...
/**
 * ChildDescCard — 태어날 자녀 상세 풀이 카드
 *
 * 자녀의 성격·재능·인간관계 성향을 깊이 있게 서술하는 본문 카드이오.
 * 상단 intro는 이탤릭체로 아이의 본질을 한 문장으로 포착하여 독자의 마음을 먼저
 * 열고, callout 패널은 앰버 좌측 테두리와 함께 핵심 특성을 한눈에 잡아주오.
 * paragraphs는 각각 충분한 줄간격으로 배치되어 자녀의 다면적 면모를
 * 자연스러운 호흡으로 읽어내려갈 수 있게 하였소. 마지막 단락에는
 * 부드러운 앰버 그라데이션 배경을 깔아 여운을 남기오.
 */
function ChildDescCard({ intro, callout, paragraphs, color, pale }: {
  intro: string; callout: string; paragraphs: string[];
  color: string; pale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}25` }}>
      {/* 인트로 */}
      <div className="px-5 pt-5 pb-3" style={{ borderBottom: `1px solid ${color}18` }}>
        <p className="text-[14px] leading-[1.8] italic" style={{ color, fontFamily: SERIF }}>
          &ldquo;{intro}&rdquo;
        </p>
      </div>
      {/* 콜아웃 */}
      <div className="mx-5 my-4 pl-4 py-3 rounded-xl" style={{ background: `${color}12`, borderLeft: `3px solid ${color}` }}>
        <p className="text-[12px] font-bold leading-relaxed" style={{ color }}>{callout}</p>
      </div>
      {/* 단락들 */}
      <div className="px-5 pb-5 space-y-3">
        {paragraphs.map((p, i) => (
          <p key={i} className="text-[13px] leading-[1.9]"
            style={{
              color: INK_SOFT,
              ...(i === paragraphs.length - 1 ? { padding: "10px 12px", borderRadius: 12, background: `${color}0c` } : {}),
            }}>
            {p}
          </p>
        ))}
...
/**
 * ParenthoodTipsCard — 양육 방향 조언 카드
 *
 * 자녀의 기운에 맞춘 실질적 양육 지침을 아이콘 카드 형식으로 담아내오.
 * 각 tips 항목은 icon(이모지)·title(조언 제목)·desc(상세 설명)으로 구성되어
 * 단순 나열에서 벗어나 제목이 강조된 구조적 읽기를 가능하게 하오.
 * 상단의 녹색 헤더 배너는 "양육의 길"이라는 장의 맥락을 알리고,
 * 각 카드는 번호 배지를 초록 계열로 칠하여 성장·자연·생명이라는
 * 새 생명 주제의 색채를 일관되게 유지하오.
 */
function ParenthoodTipsCard({ tips, greenColor, greenPale }: {
  tips: Array<{ icon: string; title: string; desc: string }>;
  greenColor: string; greenPale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: greenPale, border: `1.5px solid ${greenColor}25` }}>
      {/* 헤더 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${greenColor}18`, borderBottom: `1px solid ${greenColor}20` }}>
        <span className="text-[16px]">🌱</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color: greenColor }}>양육의 방향 · 홍연이 전하는 지혜</p>
      </div>
      {/* 팁 카드 목록 */}
      <div className="px-4 py-4 space-y-3">
        {tips.map((tip, i) => (
          <div key={i} className="flex gap-3 p-3 rounded-2xl" style={{ background: "#fff", border: `1px solid ${greenColor}20` }}>
            {/* 번호 배지 + 아이콘 */}
            <div className="shrink-0 flex flex-col items-center gap-1">
              <span className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black" style={{ background: greenColor, color: "#fff" }}>{i + 1}</span>
              <span className="text-[18px] leading-none">{tip.icon}</span>
            </div>
            {/* 텍스트 */}
            <div className="flex-1">
...
/**
 * TimingFlowCard — 임신 시기 흐름 타임라인 카드
 *
 * 두 사람의 사주 운기 흐름에서 임신·출산에 유리한 시기를 순서대로 배치한
 * 세로 타임라인 카드이오. 좌측의 수직선과 원형 번호 마커가 각 시기를
 * 시간 순서로 연결하며, highlight가 true인 항목은 금색 마커와 금색 배경으로
 * "지금 가장 좋은 시기"임을 직관적으로 알려주오. 각 항목에는 icon(계절·운기 이모지)·
 * label(시기명)·desc(사주 원리 기반 설명)이 담기오. desc는 단순 나열이 아닌
 * 왜 이 시기가 임신 기운과 잘 맞는지 사주 원리를 녹여 충분히 서술하오.
 */
function TimingFlowCard({ items, color, pale, goldColor, goldPale }: {
  items: Array<{ icon?: string; label: string; desc: string; highlight?: boolean }>;
  color: string; pale: string; goldColor: string; goldPale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}25` }}>
      {/* 헤더 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${color}15`, borderBottom: `1px solid ${color}20` }}>
        <span className="text-[16px]">🗓️</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color }}>임신 기운의 흐름 · 시기별 운세</p>
      </div>
      {/* 타임라인 */}
      <div className="px-5 py-4">
        {items.map((item, i) => {
          const isLast = i === items.length - 1;
          const markerColor = item.highlight ? goldColor : color;
          const bg = item.highlight ? goldPale : "#fff";
          const border = item.highlight ? `${goldColor}40` : `${color}20`;
          return (
            <div key={i} className="flex gap-3">
              {/* 마커 + 수직선 */}
              <div className="flex flex-col items-center" style={{ minWidth: 28 }}>
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-black shrink-0"
                  style={{ background: markerColor, color: "#fff", boxShadow: item.highlight ? `0 0 0 3px ${goldColor}30` : "none" }}>
                  {i + 1}
                </div>
                {!isLast && <div className="flex-1 w-[2px] my-1" style={{ background: `${color}25`, minHeight: 12 }} />}
              </div>
              {/* 내용 */}
              <div className="flex-1 mb-3 p-3 rounded-2xl" style={{ background: bg, border: `1px solid ${border}` }}>
                <div className="flex items-center gap-1.5 mb-1">
                  {item.icon && <span className="text-[14px]">{item.icon}</span>}
                  <p className="text-[13px] font-bold" style={{ color: markerColor }}>{item.label}</p>
                  {item.highlight && (
...
/**
 * BestMonthsCard — 임신에 좋은 달 카드
 *
 * 한 해 중 임신·출산 기운이 특히 모이는 달을 카드 형식으로 펼쳐 보이오.
 * 각 항목은 season(계절·월령 이모지)·title(달 이름 또는 기간)·desc(사주 기반 이유)로
 * 구성되며, 두 열 그리드로 나란히 배치하여 시각적 비교가 쉽도록 하였소.
 * 상단 틸 헤더 배너가 "언제 임신하면 좋은가"라는 질문에 명확히 답하는 섹션임을
 * 알리고, 각 카드 안에서 계절 이모지가 시각적 리듬을 만들어 읽는 재미를 더하오.
 * desc는 단순 월 지칭에 그치지 않고 그 달의 오행·지지 기운이 임신 기운과 어떻게
 * 공명하는지를 충분한 문장으로 설명하오.
 */
function BestMonthsCard({ items, color, pale }: {
  items: Array<{ season?: string; title: string; desc: string }>;
  color: string; pale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}25` }}>
      ...
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${color}15`, borderBottom: `1px solid ${color}20` }}>
        <span className="text-[16px]">🌙</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color }}>좋은 달 · 임신 기운이 모이는 시간</p>
      </div>
      {/* 카드 그리드 */}
...
/**
 * TimingAdviceCard — 홍연의 시기 조언 카드
 *
 * 임신 시기에 관한 홍연의 종합 조언을 담은 마무리 카드이오.
 * 이전 TimingFlowCard·BestMonthsCard가 "언제"를 구체적으로 제시했다면,
 * 이 카드는 그 시기들을 어떤 마음가짐과 태도로 맞이해야 하는지
 * 홍연의 따뜻한 목소리로 전하오. headline(조언 제목)은 골드 색으로 크게 배치하여
 * 시선을 집중시키고, callout 패널이 핵심 메시지를 한 눈에 전달하며,
 * paragraphs는 사주 원리와 삶의 지혜를 엮어 충분한 분량으로 풀어내오.
 * 배경은 금빛 그라데이션으로 처리하여 "소중한 시기"라는 감성을 시각적으로 전하오.
 */
function TimingAdviceCard({ headline, callout, paragraphs, goldColor, goldPale }: {
  headline: string; callout: string; paragraphs: string[];
  goldColor: string; goldPale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden"
      style={{ background: `linear-gradient(160deg, ${goldPale} 0%, #fff 100%)`, border: `1.5px solid ${goldColor}30` }}>
      {/* 헤더 */}
      <div className="px-5 pt-5 pb-3" style={{ borderBottom: `1px solid ${goldColor}20` }}>
        <p className="text-[10px] tracking-[0.2em] mb-1" style={{ color: `${goldColor}99`, fontFamily: SERIF }}>홍연의 조언</p>
        <p className="text-[16px] font-black leading-snug" style={{ color: goldColor, fontFamily: SERIF }}>{headline}</p>
      </div>
      {/* 콜아웃 */}
      <div className="mx-5 my-4 pl-4 py-3 rounded-xl" style={{ background: `${goldColor}10`, borderLeft: `3px solid ${goldColor}` }}>
        <p className="text-[12px] font-bold leading-relaxed" style={{ color: goldColor }}>{callout}</p>
      </div>
      {/* 단락 */}
      <div className="px-5 pb-5 space-y-3">
        {paragraphs.map((p, i) => (
 ...
 * IHapCard — 합(合) 아이템 카드
 *
 * 두 사주 사이에서 발견된 합(合) 하나를 상세히 풀어내는 카드이오.
 * 합이란 두 사주 글자가 서로 끌어당겨 새로운 오행 기운을 만들어내는 현상으로,
 * 임신·출산 맥락에서는 두 사람의 기운이 자녀를 향해 수렴한다는 뜻이기도 하오.
 * 상단 헤더에는 icon(합의 상징 이모지)·type(합의 유형명)·strength(강도 배지)를 나란히 배치하여
 * 어떤 합인지 단번에 알 수 있게 하였소. hapEffect 패널은 초록 테두리 콜아웃으로
 * "임신·출산 기운에 어떤 긍정적 영향을 주는지"를 집약하고, desc는 사주 원리 풀이와
 * 두 사람의 실제 삶에서 어떻게 나타나는지를 충분히 서술하오.
 */
function IHapCard({ item }: {
  item: { icon?: string; type: string; strength?: string; hapEffect?: string; desc: string };
}) {
  return (
    <div className="mb-3 rounded-2xl overflow-hidden" style={{ background: IM6_HAP_P, border: `1.5px solid ${IM6_HAP}30` }}>
      {/* 헤더 */}
      <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${IM6_HAP}15`, borderBottom: `1px solid ${IM6_HAP}20` }}>
        {item.icon && <span className="text-[18px]">{item.icon}</span>}
        <p className="text-[14px] font-black flex-1" style={{ color: IM6_HAP }}>{item.type}</p>
        {item.strength && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: `${IM6_HAP}22`, color: IM6_HAP }}>{item.strength}</span>
        )}
      </div>
      {/* hapEffect 콜아웃 */}
      {item.hapEffect && (
        <div className="mx-4 mt-3 pl-3 py-2 rounded-lg" style={{ background: "#fff", borderLeft: `3px solid ${IM6_HAP}` }}>
          <p className="text-[11px] font-bold leading-relaxed" style={{ color: IM6_HAP }}>✦ {item.hapEffect}</p>
        </div>
      )}
      {/* 풀이 */}
...
/**
 * IChungCard — 충(沖) 아이템 카드
 *
 * 두 사주 사이의 충(沖) — 서로 부딪히고 밀어내는 글자 관계 — 을 풀어내는 카드이오.
 * 충은 두려워할 것이 아니라 제대로 알고 다스려야 할 기운이오. 임신·출산 맥락에서
 * 충은 두 사람의 기운이 충돌하는 지점, 즉 서로 조율이 필요한 영역을 알려주오.
 * 상단 헤더는 적색 계열로 충의 긴장감을 색으로 전하고, chungEffect 콜아웃은
 * "이 충이 임신·출산 과정에서 어떤 긴장을 만드는지"를 집약하오.
 * resolve 패널은 "어떻게 하면 이 충을 다스릴 수 있는지"를 초록 포인트로 따뜻하게
 * 제시하여 독자가 충을 보고 낙심하지 않도록 균형을 잡아주오.
 */
function IChungCard({ item }: {
  item: { icon?: string; type: string; strength?: string; chungEffect?: string; resolve?: string; desc: string };
}) {
  return (
    <div className="mb-3 rounded-2xl overflow-hidden" style={{ background: IM6_CHUNG_P, border: `1.5px solid ${IM6_CHUNG}25` }}>
      {/* 헤더 */}
      <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${IM6_CHUNG}12`, borderBottom: `1px solid ${IM6_CHUNG}18` }}>
        {item.icon && <span className="text-[18px]">{item.icon}</span>}
        <p className="text-[14px] font-black flex-1" style={{ color: IM6_CHUNG }}>{item.type}</p>
        {item.strength && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: `${IM6_CHUNG}18`, color: IM6_CHUNG }}>{item.strength}</span>
        )}
      </div>
      {/* chungEffect 콜아웃 */}
      {item.chungEffect && (
        <div className="mx-4 mt-3 pl-3 py-2 rounded-lg" style={{ background: "#fff", borderLeft: `3px solid ${IM6_CHUNG}` }}>
          <p className="text-[11px] font-bold leading-relaxed" style={{ color: IM6_CHUNG }}>⚡ {item.chungEffect}</p>
        </div>
      )}
      {/* 풀이 */}
      ...
      {item.resolve && (
        <div className="mx-4 mb-3 pl-3 py-2 rounded-lg" style={{ background: `${IM6_HAP}10`, borderLeft: `3px solid ${IM6_HAP}` }}>
          <p className="text-[11px] font-bold mb-0.5" style={{ color: IM6_HAP }}>🌿 다스리는 법</p>
          <p className="text-[11px] leading-relaxed" style={{ color: INK_SOFT }}>{item.resolve}</p>
        </div>
      )}
    </div>
  );
...
/**
 * HapChungSummaryCard — 합·충 종합 평가 카드
 *
 * 이 장의 결론 카드이오. 합 개수와 충 개수를 시각적 바로 대비시켜 두 사람의
 * 사주 관계가 전체적으로 조화로운지 긴장이 많은지를 직관적으로 전하오.
 * 상단의 score는 반원 게이지 없이 큰 숫자와 tier 배지로 단순하게 표현하되
 * label이 그 점수의 의미를 홍연 말투로 한 줄 요약하오. 합/충 카운터 바는
 * 초록(합)과 적색(충) 두 색의 너비 비율로 전체 균형을 시각화하오.
 * paragraphs는 이 합·충 구도가 임신·출산 과정 전체에 어떤 의미를 갖는지
 * 종합적 시각으로 3단락에 걸쳐 충분히 서술하오.
 */
function HapChungSummaryCard({ score, tier, label, hapCount, chungCount, paragraphs }: {
  score: number; tier: string; label: string;
  hapCount: number; chungCount: number;
  paragraphs: string[];
}) {
  const total = hapCount + chungCount || 1;
  const hapPct = Math.round((hapCount / total) * 100);
  const tierColor: Record<string, string> = { S: "#c0932a", A: "#2a6a3a", B: "#2a5a8a", C: "#6a3a8a", D: "#8a2a2a" };
  const tc = tierColor[tier] ?? "#555";
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: "#fff", border: "1.5px solid #e5e0d8" }}>
      {/* 점수 헤더 */}
      <div className="px-5 pt-5 pb-4 flex items-center gap-4" style={{ borderBottom: "1px solid #f0ece4" }}>
        <div className="text-center" style={{ minWidth: 64 }}>
          <p className="text-[36px] font-black leading-none" style={{ color: tc }}>{score}</p>
          <span className="text-[11px] px-2 py-0.5 rounded-full font-bold mt-1 inline-block" style={{ background: `${tc}18`, color: tc }}>{tier}급</span>
        </div>
        <div className="flex-1">
          <p className="text-[10px] tracking-[0.15em] mb-1" style={{ color: MUTE, fontFamily: SERIF }}>합·충 종합 궁합</p>
          <p className="text-[14px] font-bold leading-snug" style={{ color: "#2a2a2a", fontFamily: SERIF }}>{label}</p>
        </div>
      </div>
      {/* 합 vs 충 바 */}
      <div className="px-5 py-4" style={{ borderBottom: "1px solid #f0ece4" }}>
        <div className="flex justify-between text-[11px] font-bold mb-1.5">
          <span style={{ color: IM6_HAP }}>합(合) {hapCount}개</span>
          <span style={{ color: IM6_CHUNG }}>충(沖) {chungCount}개</span>
        </div>
        <div className="h-2.5 rounded-full overflow-hidden" style={{ background: `${IM6_CHUNG}30` }}>
          <div className="h-full rounded-full" style={{ width: `${hapPct}%`, background: `linear-gradient(to right, ${IM6_HAP}, ${IM6_HAP}aa)` }} />
        </div>
        <p className="text-[11px] mt-1.5 text-center" style={{ color: MUTE }}>
          {hapPct >= 60 ? "조화가 긴장보다 우세하오" : hapPct >= 40 ? "조화와 긴장이 균형을 이루오" : "긴장이 조화보다 우세하오"}
        </p>
      </div>
      {/* 종합 단락 */}
...
/**
 * HealthPersonCard — 개인별 건강 주의 카드
 *
 * 임신·출산 시기에 본인 또는 배우자가 사주 기운상 특히 주의해야 할
 * 건강 항목들을 인물 카드 형식으로 담아내오. 상단 헤더에는 이름·역할·
 * 오행 체질 유형(constitution)을 표시하여 "어떤 기운의 몸인가"를 먼저
 * 알게 하오. 각 items 항목은 icon(신체 이모지)·title(주의 제목)·
 * bodyPart(연관 신체 부위)·caution(핵심 경고 한 줄)·desc(사주 원리 풀이)로
 * 구성되오. caution은 색깔 있는 콜아웃 박스로 강조하여 독자가 바로
 * 위험 신호를 인지할 수 있게 하고, desc는 왜 이 기운이 이 신체 부위에
 * 영향을 미치는지 사주 오행 원리와 함께 충분히 설명하오.
 */
function HealthPersonCard({ name, role, constitution, items, color, pale }: {
  name: string; role: string; constitution?: string;
  items: Array<{ icon?: string; title: string; bodyPart?: string; caution?: string; desc: string }>;
  color: string; pale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}25` }}>
      ...
      <div className="px-4 py-3 space-y-3">
        {items.map((item, i) => (
          <div key={i} className="rounded-2xl overflow-hidden" style={{ background: "#fff", border: `1px solid ${color}18` }}>
            {/* 항목 헤더 */}
            <div className="px-4 py-2.5 flex items-center gap-2" style={{ borderBottom: `1px solid ${color}12` }}>
              {item.icon && <span className="text-[16px]">{item.icon}</span>}
              <p className="text-[13px] font-bold flex-1" style={{ color }}>{item.title}</p>
              {item.bodyPart && (
                <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ background: `${color}15`, color }}>{item.bodyPart}</span>
              )}
            </div>
            {/* 경고 콜아웃 */}
            {item.caution && (
              <div className="mx-3 mt-2.5 pl-3 py-1.5 rounded-lg" style={{ background: `${color}0c`, borderLeft: `2.5px solid ${color}` }}>
                <p className="text-[11px] font-bold leading-relaxed" style={{ color }}>⚠ {item.caution}</p>
              </div>
            )}
            {/* 풀이 */}
            <p className="px-4 py-3 text-[12px] leading-[1.85]" style={{ color: INK_SOFT }}>{item.desc}</p>
...
/**
 * HealthTipsCard — 건강 관리 조언 카드
 *
 * 두 사람이 임신·출산 시기에 함께 실천할 수 있는 건강 관리 지침을
 * 아이콘 카드 형태로 담아내오. 이 카드가 HealthPersonCard와 다른 점은
 * "개인별 주의"가 아닌 "함께 만들어가는 건강 루틴"이라는 점이오.
 * 각 팁은 icon(행동 이모지)·title(조언 제목)·desc(구체적 방법과 사주 근거)로
 * 구성되며, 번호 배지 대신 아이콘 + 초록 제목으로 생동감 있게 배열하오.
 * 상단 초록 헤더 배너는 "두 사람이 함께"라는 협력의 메시지를 담고,
 * 각 팁 카드는 흰 배경 위에 초록 포인트 색을 써서 건강·생명·성장의
 * 에너지를 일관되게 전하오.
 */
function HealthTipsCard({ tips, color, pale }: {
  tips: Array<{ icon: string; title: string; desc: string }>;
  color: string; pale: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: pale, border: `1.5px solid ${color}25` }}>
      ...
      <div className="px-4 py-4 space-y-3">
        {tips.map((tip, i) => (
          <div key={i} className="flex gap-3 p-3 rounded-2xl" style={{ background: "#fff", border: `1px solid ${color}18` }}>
...
/**
 * EmotionalBondCard — 감정 유대 분석 카드
 *
 * 두 사람의 사주 기운이 만들어내는 감정적 연결의 본질을 심층 분석하는 카드이오.
 * 임신·출산은 단순히 몸의 일이 아니라 두 사람의 마음이 얼마나 깊이 연결되어
 * 있는가가 결정적으로 중요하오. 이 카드는 그 감정 유대의 성격을 bondType(유대 유형명)과
 * bondIcon(상징 이모지)으로 먼저 직관적으로 제시하고, intro 인용구가 두 사람의
 * 연결 방식의 본질을 한 문장으로 포착하오. callout 패널은 딥로즈 좌테두리와 함께
 * 이 유대의 핵심 특성을 압축하며, paragraphs는 사주 오행이 어떻게 두 사람의
 * 감정적 공명을 만들어내는지 — 기쁠 때, 힘들 때, 위기의 순간에 — 를
 * 충분한 분량으로 서술하오. 독자가 "우리가 이렇게 연결되어 있구나"를
 * 깨닫게 하는 것이 이 카드의 목적이오.
 */
function EmotionalBondCard({ bondType, bondIcon, intro, callout, paragraphs }: {
  bondType: string; bondIcon: string; intro: string; callout: string; paragraphs: string[];
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM8_BOND_P, border: `1.5px solid ${IM8_BOND}25` }}>
      {/* 유대 유형 헤더 */}
      <div className="px-5 pt-5 pb-3 flex items-center gap-3" style={{ borderBottom: `1px solid ${IM8_BOND}18` }}>
        <span className="text-[2.2rem] leading-none">{bondIcon}</span>
        <div>
          <p className="text-[17px] font-black leading-tight" style={{ color: IM8_BOND, fontFamily: SERIF }}>{bondType}</p>
        </div>
      </div>
      {/* 단락들 */}
...
/**
 * SupportStyleCard — 상호 지지 방식 카드
 *
 * 임신·출산 과정에서 두 사람이 서로를 어떻게 지지하는지,
 * 각자의 사주 기운에서 우러나오는 자연스러운 지지 방식을 대비하여 보여주오.
 * 사람마다 위로하는 방식이 다르고 받고 싶은 지지의 형태가 다르오.
 * 이 카드는 좌우 두 패널로 나뉘어 — 나의 지지 방식(로즈)과 배우자의 지지 방식(보라) —
 * 각자의 style(지지 유형명)·icon·desc를 나란히 보여주오. 독자는 이를 통해
 * "배우자는 이런 방식으로 나를 돕고 싶어 하는구나"를 이해하고,
 * 서로 다른 지지 방식을 오해 없이 받아들이는 법을 배우게 되오.
 * 하단 bridgeDesc는 두 가지 지지 방식이 어떻게 서로를 보완하는지 연결해주오.
 */
function SupportStyleCard({ myName, partnerName, myStyle, partnerStyle, bridgeDesc }: {
  myName: string; partnerName: string;
  myStyle: { style: string; icon: string; desc: string };
  partnerStyle: { style: string; icon: string; desc: string };
  bridgeDesc?: string;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: "#fff", border: "1.5px solid #e8e0f0" }}>
      {/* 두 패널 */}
      <div className="p-4 grid grid-cols-2 gap-3">
        {/* 나 */}
        <div className="rounded-2xl p-3" style={{ background: IM8_BOND_P, border: `1px solid ${IM8_BOND}20` }}>
          <div className="text-center mb-2">
            <span className="text-[22px]">{myStyle.icon}</span>
        ...
        <div className="rounded-2xl p-3" style={{ background: IM8_SUP_P, border: `1px solid ${IM8_SUP}20` }}>
          <div className="text-center mb-2">
            <span className="text-[22px]">{partnerStyle.icon}</span>
      ...
      {/* 연결 메시지 */}
      {bridgeDesc && (
        <div className="mx-4 mb-4 px-4 py-3 rounded-2xl" style={{ background: `${IM8_SUP}08`, border: `1px solid ${IM8_SUP}18` }}>
          <p className="text-[12px] leading-[1.8] text-center italic" style={{ color: IM8_SUP }}>🌸 {bridgeDesc}</p>
        </div>
      )}
    </div>
  );
...
/**
 * CoupleTipsCard — 부부 관계 조언 카드
 *
 * 임신·출산 시기에 두 사람의 감정적 유대를 더욱 단단히 하기 위한
 * 실질적 부부 관계 지침을 담아내오. 앞의 EmotionalBondCard와 SupportStyleCard가
 * "두 사람이 어떻게 연결되어 있는가"를 분석했다면, 이 카드는 "어떻게 하면
 * 그 연결을 더 잘 가꾸어 나갈 수 있는가"를 실천 지침으로 제시하오.
 * 각 팁은 icon·title·desc 구조로 이루어지며, 보라 테마 아래 서로 돕고
 * 이해하고 소통하는 방법을 따뜻하게 일러주오. 임신이라는 생의 특별한 시간을
 * 함께 만들어가는 두 사람에게 홍연이 건네는 마음 깊은 당부이오.
 */
function CoupleTipsCard({ tips }: {
  tips: Array<{ icon: string; title: string; desc: string }>;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM8_SUP_P, border: `1.5px solid ${IM8_SUP}25` }}>
      {/* 팁 목록 */}
      <div className="px-4 py-4 space-y-3">
        {tips.map((tip, i) => (
          <div key={i} className="flex gap-3 p-3 rounded-2xl" style={{ background: "#fff", border: `1px solid ${IM8_SUP}18` }}>
            <span className="text-[22px] leading-none shrink-0 mt-0.5">{tip.icon}</span>
            <div className="flex-1">
...
/**
 * ChallengeItemCard — 어려움 아이템 카드
 *
 * 임신·출산 과정에서 이 부부의 사주 기운상 찾아올 수 있는 어려움 하나하나를
 * 구체적으로 풀어내는 카드이오. 어려움을 미리 안다는 것은 두려움이 아니라
 * 준비의 시작이오. 상단 헤더에는 icon(어려움의 상징 이모지)·title(어려움 제목)·
 * phase(어려움이 주로 나타나는 임신 단계 — 예: "임신 초기", "출산 직전")를 배치하여
 * "언제 어떤 어려움이 오는가"를 한눈에 파악하게 하오. caution 콜아웃은 다크브라운
 * 좌테두리로 핵심 경고를 강조하고, desc는 이 어려움의 사주 원리적 근거와
 * 실제 경험될 수 있는 구체적 양상을 충분히 서술하오.
 */
function ChallengeItemCard({ item }: {
  item: { icon?: string; title: string; phase?: string; caution?: string; desc: string };
}) {
  return (
    <div className="mb-3 rounded-2xl overflow-hidden" style={{ background: IM9_DARK_P, border: `1.5px solid ${IM9_DARK}25` }}>
      {/* 헤더 */}
      <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${IM9_DARK}12`, borderBottom: `1px solid ${IM9_DARK}18` }}>
        {item.icon && <span className="text-[18px]">{item.icon}</span>}
        <p className="text-[13px] font-black flex-1" style={{ color: IM9_DARK }}>{item.title}</p>
        {item.phase && (
          <span className="text-[10px] px-2 py-0.5 rounded-full font-bold shrink-0" style={{ background: `${IM9_DARK}18`, color: IM9_DARK }}>{item.phase}</span>
        )}
      </div>
      {/* 경고 콜아웃 */}
      {item.caution && (
        <div className="mx-4 mt-3 pl-3 py-2 rounded-lg" style={{ background: "#fff", borderLeft: `3px solid ${IM9_DARK}` }}>
          <p className="text-[11px] font-bold leading-relaxed" style={{ color: IM9_DARK }}>⚠ {item.caution}</p>
        </div>
      )}
      {/* 풀이 */}
      <p className="px-4 py-3 text-[12px] leading-[1.85]" style={{ color: INK_SOFT }}>{item.desc}</p>
...
/**
 * ChallengeReasonCard — 어려움의 사주 원리 카드
 *
 * "왜 이런 어려움들이 오는가?"라는 질문에 사주 원리로 답하는 카드이오.
 * ChallengeItemCard가 어려움의 현상을 하나씩 보여줬다면, 이 카드는 그 어려움들의
 * 공통된 뿌리 — 두 사람의 사주 구조 어디서 이 어려움들이 비롯되는지 — 를
 * 짚어주오. intro 인용구는 어려움에 대한 홍연의 철학적 시각을 담고,
 * callout은 이 부부 사주 구조에서 어려움을 만드는 핵심 지점을 한 줄로 압축하오.
 * paragraphs는 오행·충·대운 흐름 등 구체적 사주 원리를 들어 어려움의 배경을
 * 충분히 설명하되, 독자가 "이 어려움은 우리 탓이 아니라 기운의 흐름"임을
 * 이해하게끔 따뜻한 관점을 잃지 않도록 하오.
 */
function ChallengeReasonCard({ intro, callout, paragraphs }: {
  intro: string; callout: string; paragraphs: string[];
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM9_DARK_P, border: `1.5px solid ${IM9_DARK}20` }}>
      {/* 헤더 배너 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${IM9_DARK}12`, borderBottom: `1px solid ${IM9_DARK}18` }}>
        <span className="text-[15px]">🔍</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color: IM9_DARK }}>어려움의 사주 원리 · 뿌리를 알면 두렵지 않소</p>
      </div>
      {/* 인트로 인용구 */}
      <div className="px-5 py-4" style={{ borderBottom: `1px solid ${IM9_DARK}12` }}>
        <p className="text-[14px] leading-[1.8] italic" style={{ color: IM9_DARK, fontFamily: SERIF }}>
          &ldquo;{intro}&rdquo;
        </p>
      </div>
      {/* 콜아웃 */}
      <div className="mx-5 my-4 pl-4 py-3 rounded-xl" style={{ background: "#fff", borderLeft: `3px solid ${IM9_DARK}` }}>
        <p className="text-[12px] font-bold leading-relaxed" style={{ color: IM9_DARK }}>{callout}</p>
      </div>
      {/* 단락들 */}
      <div className="px-5 pb-5 space-y-3">
        {paragraphs.map((p, i) => (
...
/**
 * OvercomeTipsCard — 극복 조언 카드
 *
 * 어려움을 알았으니 이제 어떻게 넘어설 것인가 — 홍연이 전하는 극복의 지혜이오.
 * 이 카드는 앞 두 카드의 어두운 색조와 대비하여 골드(금) 테마를 사용하오.
 * 사주에서 금(金)은 단련을 통해 빛을 발하는 기운이고, 어려움을 넘어선 뒤의
 * 강인함을 상징하오. 상단 헤더 배너에 "어둠을 넘어 빛으로"라는 방향성을 제시하고,
 * 각 tips 항목은 icon·title·desc 구조로 어려움을 구체적으로 다스리는 방법을
 * 안내하오. 마지막 팁은 두 사람이 함께 이 시간을 어떤 마음으로 통과할지
 * 홍연의 따뜻한 위로로 마무리하오.
 */
function OvercomeTipsCard({ tips }: {
  tips: Array<{ icon: string; title: string; desc: string }>;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM9_GOLD_P, border: `1.5px solid ${IM9_GOLD}30` }}>
      {/* 헤더 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${IM9_GOLD}18`, borderBottom: `1px solid ${IM9_GOLD}22` }}>
        <span className="text-[16px]">✨</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color: IM9_GOLD }}>어둠을 넘어 빛으로 · 극복의 지혜</p>
      </div>
      {/* 팁 목록 */}
      <div className="px-4 py-4 space-y-3">
        {tips.map((tip, i) => (
          <div key={i} className="flex gap-3 p-3 rounded-2xl"
            style={{ background: "#fff", border: `1px solid ${IM9_GOLD}22`,
              ...(i === tips.length - 1 ? { background: `${IM9_GOLD}10` } : {}) }}>
            <span className="text-[22px] leading-none shrink-0 mt-0.5">{tip.icon}</span>
            <div className="flex-1">
...
/**
 * GoodTimeFlowCard — 빛나는 시기 타임라인 카드
 *
 * 두 사람에게 가장 임신·출산 기운이 집중되는 "황금 창(Golden Window)"을
 * 시간 순서로 펼쳐 보이는 카드이오. ch5의 TimingFlowCard가 일반적 시기 흐름을
 * 보여줬다면, 이 카드는 특히 "가장 강한 임신 기운이 모이는 최고의 시기"에
 * 초점을 맞추오. highlight 항목은 황금 링 후광(box-shadow)과 "🌟 황금 창"
 * 배지로 페이지에서 가장 빛나는 카드가 되게 하오. 각 시기마다 icon으로
 * 계절·오행 기운을 시각화하고, desc는 왜 이 시기에 임신 기운이 절정에
 * 이르는지 대운·세운·월운의 교차 관점에서 충분히 서술하오.
 */
function GoodTimeFlowCard({ items }: {
  items: Array<{ icon?: string; label: string; desc: string; highlight?: boolean }>;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM10_STAR_P, border: `1.5px solid ${IM10_STAR}30` }}>
      {/* 헤더 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${IM10_STAR}18`, borderBottom: `1px solid ${IM10_STAR}22` }}>
        <span className="text-[16px]">⭐</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color: IM10_STAR }}>임신 기운이 절정에 이르는 시기</p>
      </div>
      {/* 타임라인 */}
      <div className="px-5 py-4">
        {items.map((item, i) => {
          const isLast = i === items.length - 1;
          const markerColor = item.highlight ? IM10_STAR : IM10_TEAL;
          const bg  = item.highlight ? "#fff" : "#fff";
          const border = item.highlight ? `${IM10_STAR}55` : `${IM10_TEAL}20`;
          const shadow = item.highlight ? `0 0 0 3px ${IM10_STAR}30, 0 2px 8px ${IM10_STAR}25` : "none";
          return (
            <div key={i} className="flex gap-3">
              {/* 마커 + 수직선 */}
              <div className="flex flex-col items-center" style={{ minWidth: 28 }}>
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-black shrink-0"
                  style={{ background: markerColor, color: "#fff", boxShadow: shadow }}>
                  {item.highlight ? "★" : i + 1}
                </div>
                {!isLast && <div className="flex-1 w-[2px] my-1" style={{ background: `${IM10_TEAL}25`, minHeight: 12 }} />}
              </div>
              {/* 내용 */}
              <div className="flex-1 mb-3 p-3 rounded-2xl" style={{ background: bg, border: `1px solid ${border}`, boxShadow: item.highlight ? `0 1px 6px ${IM10_STAR}18` : "none" }}>
...
/**
 * GoodTimeReasonCard — 좋은 시기의 사주 근거 카드
 *
 * "왜 이 시기가 가장 좋은가?"를 사주 원리로 하나씩 논증하는 카드이오.
 * 단순히 "이 달이 좋소"라고 선언하는 것이 아니라, 두 사람의 사주 원국에서
 * 어떤 글자·오행·대운이 맞물려 이 시기에 임신 기운이 절정에 이르는지를
 * 각각의 근거 카드로 명확히 제시하오. 틸블루(IM10_TEAL) 헤더 배너가
 * "사주 원리의 논리적 영역"임을 색으로 알리고, 각 items는 icon·title·desc로
 * 구성된 미니 카드로 나열되어 독자가 "이래서 지금이 좋은 시기구나"를
 * 오행 원리와 함께 이해하게 하오. desc는 실제 두 사람의 사주 글자를
 * 언급하며 3~4문장으로 충분히 설명하오.
 */
function GoodTimeReasonCard({ items }: {
  items: Array<{ icon?: string; title: string; desc: string }>;
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden" style={{ background: IM10_TEAL_P, border: `1.5px solid ${IM10_TEAL}25` }}>
      {/* 헤더 */}
      <div className="px-5 py-3 flex items-center gap-2" style={{ background: `${IM10_TEAL}15`, borderBottom: `1px solid ${IM10_TEAL}20` }}>
        <span className="text-[15px]">🔭</span>
        <p className="text-[12px] font-bold tracking-wide" style={{ color: IM10_TEAL }}>이 시기가 빛나는 사주 근거</p>
      </div>
      {/* 근거 카드 목록 */}
      <div className="px-4 py-4 space-y-3">
        {items.map((item, i) => (
          <div key={i} className="rounded-2xl overflow-hidden" style={{ background: "#fff", border: `1px solid ${IM10_TEAL}20` }}>
            <div className="px-4 py-2.5 flex items-center gap-2" style={{ borderBottom: `1px solid ${IM10_TEAL}12` }}>
              {item.icon && <span className="text-[16px]">{item.icon}</span>}
...
/**
 * PeakTimingCard — 황금 창을 맞이하는 종합 조언 카드
 *
 * 이 장의 결론 카드이오. 최고의 시기와 그 근거를 모두 살핀 뒤,
 * "이 황금의 시간을 어떻게 맞이해야 하는가"를 홍연이 종합하여 전하오.
 * 이 카드는 ch5의 TimingAdviceCard와 달리 황금 그라데이션에서 더 밝고
 * 빛나는 분위기를 연출하오. headline은 이 시기의 특별함을 선언하는 제목,
 * callout은 두 사람이 이 시기에 반드시 기억해야 할 핵심 한 줄,
 * paragraphs는 몸·마음·관계 세 측면에서 이 최고의 시기를 어떻게
 * 준비하고 활용할지 충분히 안내하오. 독자가 이 카드를 읽고
 * "우리에게 빛나는 시간이 오고 있구나"를 가슴 깊이 느끼게 하는 것이
 * 이 카드의 목적이오.
 */
function PeakTimingCard({ headline, callout, paragraphs }: {
  headline: string; callout: string; paragraphs: string[];
}) {
  return (
    <div className="mx-4 mb-4 rounded-3xl overflow-hidden"
      style={{ background: `linear-gradient(150deg, ${IM10_STAR_P} 0%, #fff 60%)`, border: `1.5px solid ${IM10_STAR}35` }}>
      {/* 헤더 */}
      <div className="px-5 pt-5 pb-3" style={{ borderBottom: `1px solid ${IM10_STAR}20` }}>
        <p className="text-[10px] tracking-[0.2em] mb-1" style={{ color: `${IM10_STAR}88`, fontFamily: SERIF }}>홍연의 종합 당부</p>
        <p className="text-[17px] font-black leading-snug" style={{ color: IM10_STAR, fontFamily: SERIF }}>{headline}</p>
      </div>
      {/* 콜아웃 */}
      <div className="mx-5 my-4 pl-4 py-3 rounded-xl" style={{ background: `${IM10_STAR}10`, borderLeft: `3px solid ${IM10_STAR}` }}>
        <p className="text-[12px] font-bold leading-relaxed" style={{ color: IM10_STAR }}>⭐ {callout}</p>
      </div>
      {/* 단락 */}
      <div className="px-5 pb-5 space-y-3">
        {paragraphs.map((p, i) => (
          <p key={i} className="text-[13px] leading-[1.9]"
            style={{
              color: INK_SOFT,
              ...(i === paragraphs.length - 1 ? { padding: "10px 12px", borderRadius: 12, background: `${IM10_STAR}0a` } : {}),
            }}>
            {p}
          </p>
        ))}
...
const CHAPTER_TITLES: Record<string, string> = {
  ...
  "4":  "제4장 · 자녀는 아들일까 딸일까?",
  "5":  "제5장 · 두 사람의 자녀운 흐름",
  "6":  "제6장 · 임신·출산 시 주의할 것들",
  "7":  "제7장 · 우린 어떤 가정이 될까",
  "8":  "마무리 · 홍연이 남기는 마지막 서신",
};
...
function isYeonaeChapterReady(content: Record<string, unknown> | null | undefined, chapter: number): boolean {
  return isImshinKunghapChapterReady(content, chapter);
}

// ─── 연애궁합 섹션 렌더 헬퍼 ─────────────────────────────────────
function ReportSec({ data }: { data: { intro?: string; callout?: string; paragraphs?: string[] } | null | undefined }) {
  if (!data) return null;
  const str = (v: unknown): string => typeof v === "string" ? v : (v && typeof v === "object" ? ((v as Record<string, string>).paragraph ?? (v as Record<string, string>).text ?? "") : "");
  return (
    <>
      {data.intro && <P>{str(data.intro)}</P>}
      {data.callout && <Callout>{str(data.callout)}</Callout>}
      {(data.paragraphs as unknown[])?.map((p, i) => <P key={i}>{str(p)}</P>)}
    </>
  );
}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5">
        <div className="min-w-0 flex-1 mr-3">
          ...
          {subtitle && <p className="text-[14px] font-bold leading-snug mt-1 whitespace-pre-line" style={{ color: INK }}>{subtitle}</p>}
        </div>
...
const TOC_A: TocEntry[] = [
  ...
  { disp: "제1장",  chip: "나의 사주팔자",    title: "나는 어떤 부모인가",        no: "1" },
  { disp: "제2장",  chip: "배우자의 사주팔자", title: "배우자는 어떤 부모인가",    no: "2" },
  { disp: "제3장",  chip: "글자들간의 관계",  title: "두 사람 글자들의 합과 충",            no: "3" },
  { disp: "제4장",  chip: "자녀의 성별",      title: "자녀는 아들일까 딸일까?",             no: "4" },
  { disp: "제5장",  chip: "자녀운 흐름",      title: "두 사람의 자녀운 흐름",               no: "5" },
  { disp: "제6장",  chip: "주의할점",         title: "임신·출산 시 주의할 것들",            no: "6" },
  { disp: "제7장",  chip: "가정분위기",       title: "우린 어떤 가정이 될까",    no: "7" },
  { disp: "마무리", chip: "당부",    title: "홍연이 남기는 마지막 서신",       no: "8" },
];
        ...
        <div className="px-5 py-6">
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[15px]" style={{ color: MUTE }}>[임신궁합]</span>
            </div>
            <button
              onClick={onClose}
...
function Heading({ children, color }: { children: React.ReactNode; color?: string }) {
  return (
    <h2 className="text-[19px] font-black mb-4" style={{ color: color ?? INK }}>
      {children}
    </h2>
  );
}
...
function ReportPreviewInner() {
  ...
  const needGen = !!id && !!IMSHIN_KUNGHAP_CHAPTER_SECTIONS[chNum] && !isYeonaeChapterReady(report?.content as Record<string, unknown>, chNum);
  ...
  const next = (n: string) => router.push(`/saju/kunghap_imshin/report-preview?${id ? `id=${id}&` : ""}ch=${n}`);
...
<>
      ...
      {ch === "0" && (
        <>
          ...
          <div className="relative overflow-hidden" style={{ height: 520 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/kunghap_imshin/kunghap_imshin_intro/kunghap_imshin_intro_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 30%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>
      ...
      {ch === "1" && (() => {
        ...
        return (
          <>
            {/* ── 다크 헤더 ── */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 1 장 · 나의 사주팔자</p>
              <h1 className="text-[20px] font-black leading-snug whitespace-pre-line" style={{ color: "#fff", fontFamily: SERIF }}>{"나는 어떤 부모인가"}</h1>
            </div>

            {/* ── 커버 이미지 ── */}
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_1/kunghap_imshin_1_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 30%" }} />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
            </div>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_1/kunghap_imshin_1_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "22%", left: "69%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{name.slice(1) || name}님의 사주팔자로<br />한폭의 그림을<br />그려봤소.</p>
              </div>
            </div>
            ...
            <section className="pt-2 pb-2">
              ...
              <OhaengDonutRich view={report?.view ?? null} accentColor={IM1_COLOR} />
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_1/kunghap_imshin_1_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "40%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{name.slice(1) || name}님이 타고난 사주,<br />밝은 면과 어두운 면을<br />들여다 보겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <section className="pt-4 pb-4">
              ...
              <ParentStyleCard data={myParentStyle} color={IM1_COLOR} pale={IM1_PALE} />
            </section>
      ...
      {/* ═══════════ 제2장 · 배우자는 부모로써 어떤 사람인가? ═══════════ */}
      {ch === "2" && (() => {
        const partnerName        = report?.partnerName || "배우자";
        const partnerShort       = stripSurname(partnerName);
        ...
        const partnerParentStyle = (jc.partnerParentStyle as Record<string, unknown> | undefined) ?? null;
        const wongukParas        = (wonguk.paragraphs as string[] | undefined) ?? [];
        return (
          <>
            {/* ── 다크 헤더 ── */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 2 장 · 배우자의 사주팔자</p>
              <h1 className="text-[20px] font-black leading-snug whitespace-pre-line" style={{ color: "#fff", fontFamily: SERIF }}>{"배우자는 어떤 부모인가"}</h1>
            </div>

            {/* ── 커버 이미지 ── */}
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_2/kunghap_imshin_2_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 30%" }} />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
            </div>
            ...
            <section className="pb-4">
              <MyeongsikTable
                ...
                header={
                  <div className="text-center">
                    <p className="text-[22px] font-black mb-1" style={{ color: "#2a2320" }}>{partnerShort}님의 사주팔자</p>
                    {report?.partnerBirth?.date && (
                      <p className="text-[13px]" style={{ color: "#5b504a" }}>
                        {report.partnerBirth.date}{" "}
                        {report.partnerBirth.calendar === "lunar" ? "(음력)" : "(양력)"}{" "}
                        {report?.partnerGender === "female" || report?.partnerGender === "여성" || report?.partnerGender === "여자" ? "여자" : "남자"}
                      </p>
                    )}
                  </div>
                }
            ...
            {(() => {
              ...
              return (
                <Quote>{"풀이를 읽다 명식이 궁금할 때면\n상단 "}<span style={{ display: "inline-flex", alignItems: "center", verticalAlign: "middle", gap: 0 }}>
                  <span style={{ width: 7, height: 24, flexShrink: 0, background: rodColor, borderRadius: 3, boxShadow: `1px 0 3px ${shadowColor}` }} />
                  <span style={{ padding: "4px 8px", background: bgColor, borderTop: `1.5px solid ${borderColor}`, borderBottom: `1.5px solid ${borderColor}`, color: textColor, fontFamily: SERIF, fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", lineHeight: 1, whiteSpace: "nowrap" }}>{`명식(${partnerName})`}</span>
                  <span style={{ width: 7, height: 24, flexShrink: 0, background: rodColor, borderRadius: 3, boxShadow: `-1px 0 3px ${shadowColor}` }} />
                </span>{" 버튼을 누르면\n언제든 다시 꺼내볼 수 있소."}</Quote>
              );
            })()}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_2/kunghap_imshin_2_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "20%", left: "62%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{partnerShort}님의 사주팔자로<br />한폭의 그림을 그려봤소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <section className="pt-6 pb-10">
              {report?.partnerSajuImageUrl ? (
                <div className="px-5">
                  ...
                  <div style={{ display: "flex", justifyContent: "center", marginTop: 10 }}>
                    <div style={{ background: "linear-gradient(135deg, #d8b428 0%, #a87c10 45%, #d0aa24 100%)", padding: "5px 22px", boxShadow: "0 2px 6px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,240,100,0.45), inset 0 -1px 0 rgba(0,0,0,0.3)", border: "1px solid #7a5808" }}>
                      <p style={{ fontSize: 11, color: "#1e1000", fontFamily: SERIF, letterSpacing: "0.12em", margin: 0 }}>{partnerShort}님의 사주화</p>
                    </div>
                  </div>
                  {(() => {
                    const meaning = buildSajuImageMeaning(report?.partnerView?.pillars ?? [], `${partnerShort}님`);
                    if (!meaning) return null;
                    return (
                      <div style={{ maxWidth: 420, margin: "16px auto 0", padding: "14px 16px", background: "#faf6ea", border: "1px solid #e5d9b0", borderRadius: 8 }}>
                        <p style={{ fontSize: 11, color: "#a07018", fontWeight: 700, letterSpacing: "0.08em", margin: "0 0 6px", fontFamily: SERIF }}>이 그림이 담고 있는 뜻</p>
            ...
            <section className="pt-2 pb-2">
              ...
              <OhaengDonutRich view={report?.partnerView ?? null} accentColor={IM2_COLOR} />
              <div className="px-5 mt-4">
                {wongukParas.map((p, i) => (
                  <p key={i} className="text-[13.5px] leading-[1.85] mb-4" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
                ))}
            ...
            {/* ── 용신/희신/기신 (배우자) ── */}
            {jc.partnerYongsin && (() => {
              const YS = jc.partnerYongsin as { yongsinEl?: string; heusinEl?: string; gisinEl?: string; yongsinReason?: string; heusinReason?: string; gisinReason?: string; intro?: string; desc?: string };
              const OHAENG_META: Record<string, { hanja: string; color: string; bg: string; desc: string }> = {
                목: { hanja: "木", color: "#222", bg: "#e8f5e9", desc: "성장·추진" },
                화: { hanja: "火", color: "#222", bg: "#ffebee", desc: "열정·표현" },
                토: { hanja: "土", color: "#222", bg: "#efebe9", desc: "안정·신뢰" },
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_2/kunghap_imshin_2_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "42%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[20px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{partnerShort}님이 타고난 사주,<br />밝은 면과 어두운 면을<br />들여다 보겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <section className="pt-4 pb-4">
              <div className="px-5 mb-3">
                <h2 className="text-[19px] font-black" style={{ color: INK }}>부모로서 배우자는 어떤 사람인가</h2>
              </div>
              <ParentStyleCard data={partnerParentStyle} color={IM2_COLOR} pale={IM2_PALE} />
            </section>

            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`배우자의 기운도 살펴보았으니,\n이제 두 사람의 임신 궁합을\n살펴보겠소.`}</p>
            </div>
      ...
      {ch === "4" && (() => {
        ...
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 4 장 · 자녀의 성별</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>자녀는 아들일까 딸일까?</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_4/kunghap_imshin_4_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "25%", left: "33%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>두사람 사이에<br />맞이하게될 자녀의<br />성별을 알아보았소.</p>
              </div>
            </div>
            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`이제 두 사람의 자녀운 흐름을\n꼼꼼히 살펴보겠소.`}</p>
            </div>
      ...
      {ch === "3" && (() => {
        ...
        return (
          <>
            {/* 커버 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 3 장 · 글자들간의 관계</p>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_3/kunghap_imshin_3_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "73%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[16px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>그대와 배우자의<br />사주 글자끼리<br />맞닿는 기운이 있소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_3/kunghap_imshin_3_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "20%", left: "30%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>이번에는 두사람<br />사주 글자끼리<br />충돌하는 기운을<br />살펴보겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`두 사람 글자의 합과 충을\n살펴보았소.\n\n이제 두 사람의\n자녀 인연을 살펴보겠소.`}</p>
            </div>
      ...
      {ch === "5" && (() => {
        const cf = (jc.childFlow as Record<string, unknown> | undefined) ?? null;
        const myFirstName3      = stripSurname(name);
        const partnerFirstName3 = stripSurname((report?.partnerName ?? "배우자"));
        const myYs = (jc.myYongsin      as Record<string, string> | undefined) ?? {};
        const ptYs = (jc.partnerYongsin as Record<string, string> | undefined) ?? {};
        const myYE = myYs.yongsinEl ?? ""; const myHE = myYs.heusinEl ?? ""; const myGE = myYs.gisinEl ?? "";
        const ptYE = ptYs.yongsinEl ?? ""; const ptHE = ptYs.heusinEl ?? ""; const ptGE = ptYs.gisinEl ?? "";
        const myView = report?.view ?? null;
        const ptView = report?.partnerView ?? null;
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 5 장 · 자녀운 흐름</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>두 사람의 자녀운 흐름</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <section className="px-6 pt-2 pb-2">
              <Heading>향후 10년 세운별 임신·출산 흐름</Heading>
            </section>
            {myView && ptView && (
              <ChildFlowGraphLocal
                myView={myView} partnerView={ptView}
                myYE={myYE} myHE={myHE} myGE={myGE}
                ptYE={ptYE} ptHE={ptHE} ptGE={ptGE}
                myGender={report?.gender ?? ""} ptGender={report?.partnerGender ?? ""}
            ...
            <section className="px-6 pt-6 pb-2">
              <Heading>향후 5년 세운별 임신·출산 흐름</Heading>
              <p className="text-[14.5px] leading-[1.95] mt-2 mb-6 whitespace-pre-line" style={{ color: INK_SOFT, fontFamily: SERIF }}>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_5/kunghap_imshin_5_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "23%", left: "68%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>사주 속 자녀성이<br />빛나는 해와 달이<br />따로 있소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            {myView && ptView && (
              <ImshinSeunFiveCard
                ...
                myYE={myYE} myHE={myHE} myGE={myGE}
                ptYE={ptYE} ptHE={ptHE} ptGE={ptGE}
                myName={myFirstName3} ptName={partnerFirstName3}
                myGender={report?.gender ?? ""} ptGender={report?.partnerGender ?? ""}
              />
            )}
            ...
            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`아이를 맞이할\n좋은 시기를 알았으니,\n이제 준비를 할 차례겠소.\n\n두 사람이 준비할 것들에 대해\n하나씩 짚어드리겠소.`}</p>
            </div>
      ...
      {/* ═══════════ 제6장 · 건강 관리 ═══════════ */}
      {ch === "6" && (() => {
        const hm  = (jc.healthMine    as Record<string, unknown> | undefined) ?? null;
        const hp  = (jc.healthPartner as Record<string, unknown> | undefined) ?? null;
        const ht  = (jc.healthTips    as Record<string, unknown> | undefined) ?? null;

        type HealthItem = { icon?: string; title: string; bodyPart?: string; caution?: string; desc: string };
        type HealthTip  = { icon: string; title: string; desc: string };

        const myItems      = Array.isArray(hm?.items) ? (hm.items as HealthItem[]) : [];
        const partnerItems = Array.isArray(hp?.items) ? (hp.items as HealthItem[]) : [];
        const tips         = Array.isArray(ht?.tips)  ? (ht.tips  as HealthTip[])  : [];

        const myConstitution      = typeof hm?.constitution === "string" ? hm.constitution : undefined;
        const partnerConstitution = typeof hp?.constitution === "string" ? hp.constitution : undefined;
        const myName      = name || "나";
        const partnerName = report?.partnerName || "배우자";
        ...
        return (
          <>
            {/* 커버 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 6 장 · 주의할점</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>임신·출산 시 주의할 것들</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {/* 오프닝 인용 */}
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "24px auto -24px" }} />
            <Quote>{`자녀를 맞이할 시기에\n두 사람이 준비할 것들이오.`}</Quote>

            {/* ── 삽화1 ── */}
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_6/kunghap_imshin_6_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "20%", left: "73%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>우리 아이를<br />맞이하기 위해<br />{name.slice(1) || name}님이 준비할<br />사항들이오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            {myItems.length > 0 && (
              <>
                ...
                <HealthPersonCard
                  ...
                  items={myItems}
                  color={/^(female|여성|여자|여아)$/.test(report?.gender ?? "") ? "#6b0030" : "#0d2b5e"}
                  pale={/^(female|여성|여자|여아)$/.test(report?.gender ?? "") ? "#fce8f0" : "#e8f0fc"}
                />
              </>
            )}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_6/kunghap_imshin_6_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "16%", left: "44%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>우리 아이를<br />맞이하기 위해<br />{stripSurname(partnerName)}님이 준비할<br />사항들이오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            {partnerItems.length > 0 && (
              <>
                ...
                <HealthPersonCard
                  ...
                  items={partnerItems}
                  color={/^(female|여성|여자|여아)$/.test(report?.partnerGender ?? "") ? "#6b0030" : "#0d2b5e"}
                  pale={/^(female|여성|여자|여아)$/.test(report?.partnerGender ?? "") ? "#fce8f0" : "#e8f0fc"}
                />
              </>
            )}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_6/kunghap_imshin_6_3.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "16%", left: "30%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[16px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>두 사람이 함께<br />준비할 것들은<br />무엇인지 말해주겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            {tips.length > 0 && (
              <>
                <div className="px-5"></div>
                <HealthTipsCard tips={tips} color={IM7_TIP} pale={IM7_TIP_P} />
              </>
            )}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_6/kunghap_imshin_6_4.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "17%", left: "50%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[20px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>임신·출산의 여정,<br />두 사람이 함께<br />걸어가는 법이 있소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`몸도 마음도 준비가 됐으니,\n이제 그 이후를 보겠소.\n\n아이와 함께할\n두 사람의 가정 이야기요.`}</p>
            </div>
      ...
      {/* ═══════════ 제7장 · 감정적 유대 ═══════════ */}
      {ch === "7" && (() => {
        const eb  = (jc.emotionalBond as Record<string, unknown> | undefined) ?? null;
        const ss  = (jc.supportStyle  as Record<string, unknown> | undefined) ?? null;
        const ct  = (jc.coupleTips    as Record<string, unknown> | undefined) ?? null;

        type SupportItem = { style: string; icon: string; desc: string };
        type CoupleTip   = { icon: string; title: string; desc: string };

        const bondType   = typeof eb?.bondType  === "string" ? eb.bondType  : "깊은 감응형";
        const bondIcon   = typeof eb?.bondIcon  === "string" ? eb.bondIcon  : "💞";
        const bondIntro  = typeof eb?.intro     === "string" ? eb.intro     : "";
        const bondCallout= typeof eb?.callout   === "string" ? eb.callout   : "";
        const bondParas  = Array.isArray(eb?.paragraphs) ? (eb.paragraphs as string[]) : [];

        const ssRaw     = (ss as { mySupport?: unknown; partnerSupport?: unknown; bridgeDesc?: unknown }) ?? {};
        const mySupport  = (ssRaw.mySupport      as SupportItem | undefined) ?? null;
        const ptSupport  = (ssRaw.partnerSupport as SupportItem | undefined) ?? null;
        const bridgeDesc = typeof ssRaw.bridgeDesc === "string" ? ssRaw.bridgeDesc : undefined;

        const coupleTips = Array.isArray(ct?.tips) ? (ct.tips as CoupleTip[]) : [];
        ...
        return (
          <>
            {/* 커버 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 7 장 · 가정분위기</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>우린 어떤 가정이 될까</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_7/kunghap_imshin_7_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 30%" }} />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
            </div>
            ...
            {/* 감정 유대 분석 카드 */}
            {(bondIntro || bondCallout || bondParas.length > 0) && (
              <>
                <section className="px-6 pt-2 pb-2">
                  <Heading>두 사람이 맺은 마음의 인연</Heading>
                </section>
                <EmotionalBondCard
                  bondType={bondType}
                  bondIcon={bondIcon}
                  intro={bondIntro || "두 사람의 마음은 보이지 않는 실로 연결되어 있소."}
                  callout={bondCallout || ""}
                  paragraphs={bondParas}
                />
              </>
            )}
            ...
            {mySupport && ptSupport && (
              <>
                <section className="px-6 pt-2 pb-2">
                  <Heading>서로를 지지하는 방식</Heading>
                </section>
                <SupportStyleCard
                  myName={myName}
                  partnerName={partnerName}
                  myStyle={mySupport}
                  partnerStyle={ptSupport}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_imshin/kunghap_imshin_7/kunghap_imshin_7_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "17%", left: "50%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>아이를 맞이하기 전<br />두 사람 사이를<br />더 단단히 해두시오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "#ccc" }} />
            ...
            {coupleTips.length > 0 && (
              <>
                <section className="px-6 pt-2 pb-2">
                  <Heading>두 사람의 유대를 위한 방법</Heading>
                </section>
                <CoupleTipsCard tips={coupleTips} />
              </>
            )}
            ...
            <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`아이와 함께할\n두 사람의 모습을 보았소.\n\n마지막으로\n홍연이 전하고 싶은 말이 있소.`}</p>
            </div>
            <div style={{ background: PINK_PALE }}><ChapterNav cur="7" go={next} /></div>
          </>
        );
      })()}
      ...
      {ch === "8" && (
        ...
        <div style={{ filter: eventOpen ? "blur(5px)" : "none", transition: "filter 0.25s ease", pointerEvents: eventOpen ? "none" : "auto" }}>
          <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>마무리 · 당부</p>
            <h1 className="text-[20px] font-black leading-snug whitespace-pre-line" style={{ color: "#fff", fontFamily: SERIF }}>{"홍연이 남기는 마지막 서신"}</h1>
          </div>
          <div className="relative overflow-hidden" style={{ height: 520 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/kunghap_imshin/kunghap_imshin_8/kunghap_imshin_8_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 20%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>
