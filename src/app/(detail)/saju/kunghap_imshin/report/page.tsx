...
//
// 미리보기: http://localhost:3000/saju/kunghap_imshin/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/kunghap_imshin-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/kunghap_imshin/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/kunghap_imshin/report?${id ? `id=${id}&` : ""}ch=${n}`);
