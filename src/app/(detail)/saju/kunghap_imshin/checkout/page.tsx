  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      <div className="relative overflow-hidden" style={{ height: 240 }}>
        <img src="/media/cards/kunghap_imshin/imshin-0.jpg" alt=""
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/kunghap_imshin/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/kunghap_imshin/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/kunghap_imshin/report?id=${d.resultId}` : `/saju/kunghap_imshin/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/kunghap_imshin/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
