...
function CreatingScreen({ doneCount, currentChapter }: { doneCount: number; currentChapter: number }) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#8faa8b", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#8faa8b" }}>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await fetch("/api/kunghap_imshin-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: resultId, content: {} }) }).catch(() => {});
