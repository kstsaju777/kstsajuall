      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>이혼사주 결제 안내</h3>
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(124,106,246,0.16)", border: `1px solid ${ACCENT}55`, color: "#b8b0fb" }}>
            총 <span style={{ color: "#9b8dff" }}>{saved.toLocaleString()}원</span> 할인받았어요!
          </div>
          <div className="mb-5">
            <div className="w-full text-left rounded-2xl px-4 py-3.5" style={{ backgroundColor: DCARD, border: `1.5px solid ${ACCENT}`, boxShadow: `0 0 0 3px ${ACCENT}22` }}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>이혼사주</span>
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 이혼사주 이야기</p>
                </div>
          ...
          <div className="space-y-1.5 mb-4">
            ...
            <div className="flex items-center justify-between text-[13px]">
              <span style={{ color: "#9b8dff", fontWeight: 700 }}>지금 결제 시 할인 ({sel.discount}% 특가)</span>
              <span style={{ color: "#9b8dff", fontWeight: 700 }}>-{saved.toLocaleString()}</span>
            </div>
          </div>
          <button onClick={() => onConfirm(selected)} className="w-full py-4 rounded-2xl font-black text-[17px] text-white active:scale-[0.99] transition-transform"
            style={{ background: "linear-gradient(135deg, #9b8dff, #7c6af6)", boxShadow: "0 6px 20px rgba(124,106,246,0.4)" }}>
            결제하기
          </button>
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4"
      ...
      <button onClick={onPay}
        ...
        <span>🔓</span>
        <span>{name}님 · {partnerName}님 이혼사주 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/kunghap_ehon/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/kunghap_ehon/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/kunghap_ehon/report?id=${d.resultId}` : `/saju/kunghap_ehon/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/kunghap_ehon/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
