...
const SAJU_ORDER_E   = ["정통명리 종합사주", "영재발굴 자녀사주", "대박이야 재물사주", "나만솔로? 연애사주", "우리아가 유아사주", "오래살자 건강사주"];
...
function DivorceScoreGauge({ score, label, basis }: { score: number; label: string; basis?: string }) {
  ...
  const ZONE_BG = s >= 70 ? `${ECH3_COLOR}18` : s >= 45 ? `${ECH3_WARN}18` : `${ECH3_HOPE}18`;
  return (
    <div className="mx-5 my-3 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(160deg, ${ECH3_COLOR}0a 0%, ${WHITE} 60%)`, border: `1.5px solid ${ECH3_COLOR}22` }}>
      <div className="px-5 pt-5 pb-1 flex items-center justify-between">
        <span className="inline-block px-4 py-1.5 rounded-full text-[14px] font-black" style={{ background: ZONE_BG, color: COLOR }}>{ZONE_LABEL}</span>
      ...
      <svg viewBox="0 0 260 118" style={{ width: "100%", maxHeight: 130 }}>
        ...
        <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="url(#divorceGaugeGrad)" strokeWidth="20" strokeLinecap="round"
          strokeDasharray={`${(s / 100) * Math.PI * r} ${Math.PI * r}`} />
        <line x1={cx} y1={cy} x2={needleX} y2={needleY} stroke={INK} strokeWidth="2.5" strokeLinecap="round" />
        <circle cx={cx} cy={cy} r="6" fill={INK} />
        <text x={cx - r - 4} y={cy + 16} textAnchor="middle" fontSize="9" fill={MUTE}>안정</text>
        <text x={cx + r + 4} y={cy + 16} textAnchor="middle" fontSize="9" fill={MUTE}>위험</text>
      </svg>
      {label && (
        <div className="mt-6 py-4 text-center" style={{ background: ZONE_BG }}>
...
function DivorceReasonCard({ item }: { item: Record<string, unknown> }) {
  const title  = (item.title  as string | undefined) ?? "";
  const desc   = (item.desc   as string | undefined) ?? "";
  const effect = (item.effect as string | undefined) ?? "";
  const type   = (item.type   as string | undefined) ?? "main";
  const COLOR  = ECH4_COLOR;
  const PALE   = ECH4_PALE;
  return (
    <div className="mx-5 mb-4 rounded-xl overflow-hidden" style={{ borderLeft: `4px solid ${COLOR}`, background: WHITE, boxShadow: `0 1px 6px ${COLOR}12` }}>
      <div className="px-4 py-3 flex items-center gap-2.5" style={{ background: `${COLOR}0d` }}>
        <p className="text-[14px] font-black truncate" style={{ color: INK }}>{title}</p>
      </div>
      {effect && (
        <div className="mx-4 mt-3 px-3 py-2 rounded-lg" style={{ background: PALE }}>
          <p className="text-[12.5px] font-bold leading-relaxed" style={{ color: COLOR }}>{effect}</p>
        </div>
      )}
...
function ConflictStyleBanner({ data, myName, partnerName, myGender, partnerGender }: {
  data: Record<string, unknown> | null; myName: string; partnerName: string; myGender?: string; partnerGender?: string;
}) {
  if (!data) return null;
  const isFem = (g?: string) => g === "female" || g === "여성" || g === "여자" || g === "여아";
  const BLUE  = { color: "#0d2b5e", bg: "#e8f0fc", border: "#1a4a9e" };
  const PINK  = { color: "#6b0030", bg: "#fce8f0", border: "#c0306a" };
  const myChip  = isFem(myGender)      ? PINK : BLUE;
  const ptChip  = isFem(partnerGender) ? PINK : BLUE;
  const callout      = (data.callout      as string | undefined) ?? "";
  const paragraphs   = (data.paragraphs   as string[] | undefined) ?? [];
  ...
  const ptIcon       = (partnerStyle?.icon as string | undefined) ?? "";
  const ptDesc       = (partnerStyle?.desc as string | undefined) ?? "";
  return (
    <div className="mx-5 mb-5">
      {/* 나 카드 */}
      {myType && (
        <div className="mb-0 rounded-xl overflow-hidden" style={{ background: WHITE, boxShadow: `0 1px 6px ${myChip.border}20`, border: `1px solid ${myChip.border}30`, borderLeft: `4px solid ${myChip.border}` }}>
          <div className="px-4 py-3 flex items-center gap-2.5 flex-wrap" style={{ background: `${myChip.bg}` }}>
            <span className="px-2.5 py-1 rounded-full text-[11px] font-black" style={{ background: WHITE, color: myChip.color, border: `1px solid ${myChip.border}60` }}>{myName}님</span>
      ...
      {(myType && ptType) && (
        <div className="flex items-center justify-center py-3">
          <span className="px-3 py-1 rounded-full text-[11px] font-black" style={{ background: `${ECH4_COLOR}12`, color: ECH4_COLOR }}>⚡ 이 두 방식이 부딪히면</span>
        </div>
      )}
      {/* 상대 카드 */}
      {ptType && (
        <div className="mb-4 rounded-xl overflow-hidden" style={{ background: WHITE, boxShadow: `0 1px 6px ${ptChip.border}20`, border: `1px solid ${ptChip.border}30`, borderLeft: `4px solid ${ptChip.border}` }}>
          <div className="px-4 py-3 flex items-center gap-2.5 flex-wrap" style={{ background: `${ptChip.bg}` }}>
            <span className="px-2.5 py-1 rounded-full text-[11px] font-black" style={{ background: WHITE, color: ptChip.color, border: `1px solid ${ptChip.border}60` }}>{partnerName}님</span>
...
function ConflictTriggerCard({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  return (
    <div className="mx-5 mb-3 rounded-xl overflow-hidden" style={{ borderLeft: `4px solid ${t.color}`, background: WHITE, boxShadow: `0 1px 6px ${t.color}12` }}>
...
// 두 사람 재물운 흐름 비교 차트 (2026-2035)
function DualWealthLineChart({ myView, partnerView, myName, partnerName }: {
  myView: MyeongsikView | null; partnerView: MyeongsikView | null;
  myName: string; partnerName: string;
}) {
  ...
  function calcScores(view: MyeongsikView | null) {
    const ilEl = STEM_EL[(view?.ilgan ?? "")[0]] ?? "목";
    const seunMap: Record<number,string> = {};
    (view?.seun ?? []).forEach(s => { const y = Number(s.label); if (y >= 2026 && y <= 2035) seunMap[y] = s.gz; });
    TARGET_YEARS.forEach(y => { if (!seunMap[y]) seunMap[y] = GANJIS[(baseIdx + (y - BASE_YEAR)) % 60]; });
    return TARGET_YEARS.map(y => {
      ...
      const sScore = sEl ? SIPSEONG_SCORE[toSipseong(ilEl, sEl)] ?? 65 : 65;
      const bScore = bEl ? SIPSEONG_SCORE[toSipseong(ilEl, bEl)] ?? 65 : 65;
      const gz2k = (GAN_KOR[gz[0]] ?? gz[0] ?? "") + (JI_KOR[gz[1]] ?? gz[1] ?? "");
      return { score: Math.min(95, Math.max(40, Math.round(sScore * 0.6 + bScore * 0.4))), gz2k };
    });
  }
  const myData  = calcScores(myView);
  const ptData  = calcScores(partnerView);
  const MY_COLOR = "#3a6abf";
  const PT_COLOR = "#c0404a";
  const W = 320, H = 170, padX = 24, padTop = 16, padBot = 56;
  const allScores = [...myData, ...ptData].map(d => d.score);
  const minS = Math.min(...allScores);
  const maxS = Math.max(...allScores);
  const range = maxS - minS || 1;
  const n = TARGET_YEARS.length;
  const xp = (i: number) => padX + ((W - padX * 2) * i) / (n - 1);
  const yp = (v: number) => padTop + (H - padTop - padBot) * (1 - (v - minS) / range);
  const myPts  = myData.map((d, i) => `${xp(i)},${yp(d.score)}`).join(" ");
  const ptPts  = ptData.map((d, i) => `${xp(i)},${yp(d.score)}`).join(" ");
  const myArea = `${xp(0)},${yp(minS - range * 0.08)} ${myPts} ${xp(n-1)},${yp(minS - range * 0.08)}`;
  const ptArea = `${xp(0)},${yp(minS - range * 0.08)} ${ptPts} ${xp(n-1)},${yp(minS - range * 0.08)}`;
  const myPeakI = myData.reduce((m, d, i) => d.score > myData[m].score ? i : m, 0);
  const ptPeakI = ptData.reduce((m, d, i) => d.score > ptData[m].score ? i : m, 0);
  return (
    <div className="mx-5 mb-5 rounded-2xl overflow-hidden" style={{ background: WHITE, border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full px-1" style={{ overflow: "visible" }}>
        <defs>
          <linearGradient id="dwMyArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={MY_COLOR} stopOpacity="0.18" />
            <stop offset="100%" stopColor={MY_COLOR} stopOpacity="0.02" />
          </linearGradient>
          <linearGradient id="dwPtArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={PT_COLOR} stopOpacity="0.18" />
            <stop offset="100%" stopColor={PT_COLOR} stopOpacity="0.02" />
          </linearGradient>
        </defs>
        {[0,1,2,3].map(i => {
          const yy = padTop + ((H - padTop - padBot) * i) / 3;
          return <line key={i} x1={padX} x2={W-padX} y1={yy} y2={yy} stroke="rgba(0,0,0,0.05)" strokeWidth="1" />;
        })}
        {/* 나 영역 */}
        <polygon points={myArea} fill="url(#dwMyArea)" />
        {/* 상대방 영역 */}
        <polygon points={ptArea} fill="url(#dwPtArea)" />
        {/* 나 선 */}
        <polyline points={myPts} fill="none" stroke={MY_COLOR} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        {/* 상대방 선 */}
        <polyline points={ptPts} fill="none" stroke={PT_COLOR} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {/* 점 */}
        {myData.map((d, i) => (
          <circle key={`m${i}`} cx={xp(i)} cy={yp(d.score)} r={2.5} fill={WHITE} stroke={MY_COLOR} strokeWidth="2" />
        ))}
        {ptData.map((d, i) => (
          <circle key={`p${i}`} cx={xp(i)} cy={yp(d.score)} r={2.5} fill={WHITE} stroke={PT_COLOR} strokeWidth="2" />
        ))}
        {/* x축 수직 점선 + 레이블 */}
        {TARGET_YEARS.map((y, i) => {
          const cx = xp(i);
          const bw = 24, bh = 30, bx = cx - bw / 2, by = H - 46;
          const myY = yp(myData[i].score);
          const ptY = yp(ptData[i].score);
          const lineEnd = by - 2;
          return (
            <g key={y}>
              {/* 나 점선 */}
              <line x1={cx} y1={myY + 3} x2={cx} y2={lineEnd} stroke={MY_COLOR} strokeWidth="0.8" strokeDasharray="3 2" strokeOpacity="0.5" />
              {/* 상대 점선 */}
              <line x1={cx} y1={ptY + 3} x2={cx} y2={lineEnd} stroke={PT_COLOR} strokeWidth="0.8" strokeDasharray="3 2" strokeOpacity="0.5" />
              <rect x={bx} y={by} width={bw} height={bh} rx="4" fill="none" stroke="rgba(0,0,0,0.2)" strokeWidth="0.8" />
              <text x={cx} y={by + 13} fontSize="11" fill={INK_SOFT} textAnchor="middle" fontWeight="700">{String(y).slice(2)}</text>
              <text x={cx} y={by + 26} fontSize="10" fill={MUTE} textAnchor="middle">{myData[i].gz2k}</text>
            </g>
          );
        })}
      </svg>
      ...
      <div className="flex items-center justify-center gap-6 pb-3 pt-1">
        <div className="flex items-center gap-1.5">
          <div className="w-7 h-[2.5px] rounded-full" style={{ background: MY_COLOR }} />
          <span className="text-[11px] font-bold" style={{ color: MY_COLOR }}>{myName}님</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-7 h-[2.5px] rounded-full" style={{ background: PT_COLOR }} />
          <span className="text-[11px] font-bold" style={{ color: PT_COLOR }}>{partnerName}님</span>
        </div>
      </div>
    </div>
  );
...
// 이혼 후 운명 흐름 카드 — 나 또는 상대방 단독
function FutureFlowCard({ data, name, color }: {
  data: Record<string, unknown> | null;
  name: string;
  color: string;
}) {
  ...
  const desc       = (data.desc       as string | undefined) ?? "";
  const brightSide = (data.brightSide as string | undefined) ?? "";
  return (
    <div className="mx-5 mb-4 rounded-2xl overflow-hidden" style={{ border: `1.5px solid ${color}25` }}>
      <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: `${color}0e` }}>
        <div className="w-2 h-2 rounded-full" style={{ background: color }} />
        <p className="text-[12px] font-black tracking-wide" style={{ color }}>{name}님</p>
        {keyword && (
          <span className="ml-auto px-2.5 py-0.5 rounded-full text-[11px] font-black" style={{ background: `${color}18`, color }}>
            {keyword}
          </span>
        )}
      </div>
...
// 각자가 빛나는 방향 카드
function ShiningPathCard({ data, myName, partnerName }: {
  data: Record<string, unknown> | null;
  myName: string;
  partnerName: string;
}) {
  ...
  return (
    <div className="mx-5 mb-5 flex flex-col gap-4">
      {items.map((item, i) => {
        const theme       = (item.theme       as string | undefined) ?? "";
        const myPath      = (item.myPath      as string | undefined) ?? "";
        const partnerPath = (item.partnerPath as string | undefined) ?? "";
        return (
          <div key={i} className="rounded-2xl overflow-hidden" style={{ border: "1px solid #e8e4de" }}>
            <div className="px-5 py-3 text-center" style={{ background: "#3d3028" }}>
              <p className="text-[13px] font-black tracking-widest" style={{ color: "#f0e8dc" }}>{theme}</p>
            </div>
      ...
      {closing && (
        <div className="px-4 py-3.5 rounded-xl" style={{ background: "#f5f0ea", border: "1px solid #e0d8ce" }}>
          <p className="text-[13.5px] font-bold leading-relaxed text-center" style={{ color: "#5a4a38", fontFamily: SERIF }}>{closing}</p>
        </div>
      )}
    </div>
  );
...
function PropertyFlowCard({ data, myName, partnerName }: {
  ...
  return (
    <div className="mx-5 mb-5 flex flex-col gap-3">
      {/* 나 카드 */}
      <div className="rounded-2xl overflow-hidden" style={{ border: `1.5px solid ${MY_C}25` }}>
        <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: `${MY_C}0e` }}>
          <div className="w-2 h-2 rounded-full" style={{ background: MY_C }} />
          <p className="text-[12px] font-black tracking-wide" style={{ color: MY_C }}>{myName}님</p>
          {myFlow && (
            <span className="ml-auto px-2.5 py-0.5 rounded-full text-[11px] font-black" style={{ background: `${MY_C}18`, color: MY_C }}>
              {myFlow}
            </span>
          )}
        </div>
      ...
      {/* 상대방 카드 */}
      <div className="rounded-2xl overflow-hidden" style={{ border: `1.5px solid ${PT_C}25` }}>
        <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: `${PT_C}0e` }}>
          <div className="w-2 h-2 rounded-full" style={{ background: PT_C }} />
          <p className="text-[12px] font-black tracking-wide" style={{ color: PT_C }}>{partnerName}님</p>
          {partnerFlow && (
            <span className="ml-auto px-2.5 py-0.5 rounded-full text-[11px] font-black" style={{ background: `${PT_C}18`, color: PT_C }}>
              {partnerFlow}
            </span>
          )}
        </div>
      ...
      {paragraphs.length > 0 && (
        <div className="pt-2">
          {paragraphs.map((p, i) => (
            <p key={i} className="mb-5 text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
          ))}
        </div>
      )}
    </div>
...
function ReconcileScoreGauge({ score, label, basis }: { score: number; label: string; basis?: string }) {
  ...
  const needleX = cx + r * Math.cos(rad(angle));
  const needleY = cy + r * Math.sin(rad(angle));
  const COLOR = s >= 61 ? ECH9_YES : s >= 26 ? ECH9_MID : ECH9_NO;
  const ZONE_LABEL = s >= 91 ? "재결합 강력 권함" : s >= 76 ? "재결합 가능성 매우 높음" : s >= 61 ? "재결합 가능성 있음" : s >= 41 ? "조건부 화해 가능" : s >= 26 ? "화해 쉽지 않음" : s >= 11 ? "각자의 길이 맞음" : "이별이 맞는 인연";
  const ZONE_BG = s >= 61 ? `${ECH9_YES}18` : s >= 26 ? `${ECH9_MID}18` : `${ECH9_NO}18`;
  return (
    <div className="mx-5 my-3 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(160deg, ${COLOR}0a 0%, ${WHITE} 60%)`, border: `1.5px solid ${COLOR}22` }}>
      <div className="px-5 pt-5 pb-1 flex items-center justify-between">
        <span className="inline-block px-4 py-1.5 rounded-full text-[14px] font-black" style={{ background: ZONE_BG, color: COLOR }}>{ZONE_LABEL}</span>
        <p className="font-black" style={{ color: COLOR, fontFamily: SERIF }}>
          <span className="text-[42px] leading-none">{s}</span>
      ...
      <svg viewBox="0 0 260 118" style={{ width: "100%", maxHeight: 130 }}>
        <defs>
          <linearGradient id="reconcileGaugeGrad" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="#ece8f0" strokeWidth="20" strokeLinecap="round" />
        <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="url(#reconcileGaugeGrad)" strokeWidth="20" strokeLinecap="round"
          strokeDasharray={`${(s / 100) * Math.PI * r} ${Math.PI * r}`} />
        <line x1={cx} y1={cy} x2={needleX} y2={needleY} stroke={INK} strokeWidth="2.5" strokeLinecap="round" />
        <circle cx={cx} cy={cy} r="6" fill={INK} />
        <text x={cx - r - 4} y={cy + 16} textAnchor="middle" fontSize="9" fill={MUTE}>각자의 길</text>
        <text x={cx + r + 4} y={cy + 16} textAnchor="middle" fontSize="9" fill={MUTE}>재결합</text>
      </svg>
      {label && (
        <div className="mt-6 py-4 text-center" style={{ background: ZONE_BG }}>
          <p className="text-[14px] font-black tracking-tight" style={{ color: COLOR }}>{label}</p>
...
function ReconcileFactorCard({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  type FactorItem = { title?: string; desc?: string } | string;
  const enablers = (data.enablers as FactorItem[] | undefined) ?? [];
  const barriers = (data.barriers as FactorItem[] | undefined) ?? [];
  const verdict    = (data.verdict    as string  | undefined) ?? "";
  const callout    = (data.callout    as string  | undefined) ?? "";
  const paragraphs = (data.paragraphs as string[] | undefined) ?? [];
  return (
    <div className="mx-5 mb-5">
      {/* 화해 가능 요소 카드 */}
      {enablers.length > 0 && (
        <div className="rounded-2xl overflow-hidden mb-3" style={{ border: `1px solid ${ECH9_YES}28` }}>
          <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${ECH9_YES}12` }}>
            <span className="text-[14px]">🌿</span>
            <p className="text-[12px] font-black" style={{ color: ECH9_YES }}>화해를 가능하게 하는 것</p>
          </div>
          <div className="px-4 py-3" style={{ background: `${ECH9_YES}05` }}>
            {enablers.map((e, i) => {
              const title = typeof e === "string" ? e : (e.title ?? "");
              const desc  = typeof e === "string" ? "" : (e.desc ?? "");
              return (
      ...
      {/* 장벽 요소 카드 */}
      {barriers.length > 0 && (
        <div className="rounded-2xl overflow-hidden mb-3" style={{ border: `1px solid ${ECH9_NO}28` }}>
          <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${ECH9_NO}12` }}>
            <span className="text-[14px]">🚧</span>
            <p className="text-[12px] font-black" style={{ color: ECH9_NO }}>화해를 막는 것</p>
          </div>
          <div className="px-4 py-3" style={{ background: `${ECH9_NO}05` }}>
            {barriers.map((b, i) => {
      ...
      {(verdict || callout) && (
        <div className="flex justify-center mb-3">
          <span className="text-[20px]" style={{ color: `${ECH9_COLOR}60` }}>↓</span>
        </div>
      )}
      {/* 홍연의 종합 판단 + callout 통합 */}
      {(verdict || callout) && (
        <div className="rounded-2xl px-4 py-4 mb-4" style={{ background: `${ECH9_COLOR}06`, border: `1px solid ${ECH9_COLOR}22` }}>
          {verdict && <p className="text-[13.5px] font-bold leading-relaxed mb-3" style={{ color: ECH9_COLOR }}>{verdict}</p>}
          {callout && <p className="text-[13px] leading-relaxed" style={{ color: INK_SOFT, fontFamily: SERIF }}>{callout}</p>}
        </div>
      )}
      {/* 풀이 단락 */}
      {paragraphs.map((p, i) => (
        <p key={i} className="mb-4 text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
      ))}
    </div>
  );
}
...
function ReconcileTipCard({ item, index }: { item: Record<string, unknown>; index: number }) {
  ...
  return (
    <div className="mx-5 mb-4 rounded-2xl overflow-hidden" style={{ border: `1px solid ${tm.color}22` }}>
      <div className="px-4 py-3" style={{ background: `${tm.color}10`, borderBottom: `1px solid ${tm.color}18` }}>
...
// 연도별 이혼 에너지 꺾은선 차트 (결혼궁합 MarriageTimingLineChart와 동일 구조)
function DivorceTimingLineChart({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  const rawItems = (data.items as Record<string, unknown>[] | undefined) ?? [];
  if (!rawItems.length) return null;
  const ECH5_NORMAL = "#3d3d3d";
  ...
  const items = rawItems.map((it) => {
    ...
    const hanja     = (full.split(/\s+/)[1] ?? "").replace("년", "");
    const zodiac    = (GAN_KOR[hanja[0]] ?? hanja[0] ?? "") + (JI_KOR[hanja[1]] ?? hanja[1] ?? "");
  ...
  const secondMinIdx = items.findIndex((d, i) => i !== minIdx && d.score === (items.filter((_, j) => j !== minIdx).reduce((a, d) => Math.min(a, d.score), Infinity)));
  const BLUE_PEAK = "#3a6abf"; const BLUE_2ND = "#4a9e6a"; const RED_CAUTION = "#c0404a";
  ...
  return (
    <div>
      <div className="mx-5 mb-5 rounded-2xl p-4" style={{ background: WHITE, border: `1px solid ${ECH5_COLOR}18`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ overflow: "visible" }}>
          <defs>
            <linearGradient id="dtArea" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={ECH5_COLOR} stopOpacity="0.18" />
              <stop offset="100%" stopColor={ECH5_COLOR} stopOpacity="0.01" />
            </linearGradient>
          </defs>
          {[0,1,2,3].map(gi => {
            const gy = padTop + ((H - padTop - padBot) * gi) / 3;
            return <line key={gi} x1={padX} x2={W - padX} y1={gy} y2={gy} stroke={`${ECH5_COLOR}12`} strokeWidth="1" />;
          })}
          <polygon points={area} fill="url(#dtArea)" />
          {items.map((d, i) => {
            if (i === 0) return null;
            const prev = items[i - 1];
            return <line key={i} x1={x(i-1)} y1={y(prev.score)} x2={x(i)} y2={y(d.score)} stroke={ECH5_NORMAL} strokeWidth="2.5" strokeLinecap="round" />;
          })}
          {items.map((d, i) => {
            const isPeak = i === maxIdx; const is2nd = i === secondIdx; const isLow = i === minIdx || i === secondMinIdx;
            const col = isPeak ? BLUE_PEAK : is2nd ? BLUE_2ND : isLow ? RED_CAUTION : ECH5_NORMAL;
            return (
              <g key={i}>
                <line x1={x(i)} y1={y(d.score) + 5} x2={x(i)} y2={H - padBot} stroke={col} strokeWidth="1" strokeDasharray="3 3" strokeOpacity="0.4" />
                ...
                <rect x={x(i) - 12} y={H - padBot + 16} width="24" height="27" rx="5" ry="5" fill={col} fillOpacity={isPeak ? 0.15 : 0.10} stroke={col} strokeOpacity={isPeak ? 0.5 : 0.30} strokeWidth={isPeak ? 1.2 : 0.8} />
        ...
        <div className="flex items-center justify-center gap-3 mt-2 flex-nowrap">
          {[{ color: BLUE_PEAK, label: "최적" }, { color: BLUE_2ND, label: "길함" }, { color: ECH5_NORMAL, label: "보통" }, { color: RED_CAUTION, label: "주의" }].map(({ color, label }) => (
      ...
      <div className="px-6 pt-3 pb-1"><Heading>이혼하기 좋은 시기</Heading></div>
      <div className="px-5 space-y-3 mb-2">
        {items.map((d, i) => {
          if (i !== maxIdx && i !== secondIdx) return null;
          const col = i === maxIdx ? BLUE_PEAK : BLUE_2ND;
          return (
            <div key={i} className="rounded-xl overflow-hidden" style={{
              border: `${i === maxIdx ? "2px" : "1.5px"} solid ${col}${i === maxIdx ? "cc" : "99"}`,
              background: `linear-gradient(135deg, ${col}08 0%, ${WHITE} 60%)`,
              boxShadow: i === maxIdx ? `0 0 8px 2px ${col}55, 0 0 18px 4px ${col}22` : `0 0 6px 2px ${col}44`,
            }}>
              <div className="flex items-center gap-2.5 px-3.5 pt-2.5 pb-2" style={{ background: `${col}10` }}>
                ...
                {i === maxIdx && <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: col, color: WHITE }}>최적</span>}
                {i === secondIdx && <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: col, color: WHITE }}>길함</span>}
...
// 이혼하기 최적 시기 배너 — timingItems 최고점 연도 자동 식별
function DivorceBestTimingBanner({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  const rawItems = (data.items as Record<string, unknown>[] | undefined) ?? [];
  if (!rawItems.length) return null;
  const GAN_KOR: Record<string,string> = {甲:"갑",乙:"을",丙:"병",丁:"정",戊:"무",己:"기",庚:"경",辛:"신",壬:"임",癸:"계"};
  ...
  const items = rawItems.map((it) => {
    ...
    const hanja     = (full.split(/\s+/)[1] ?? "").replace("년", "");
    const zodiac    = (GAN_KOR[hanja[0]] ?? hanja[0] ?? "") + (JI_KOR[hanja[1]] ?? hanja[1] ?? "");
    return { label: full, shortYear, zodiac, score, desc: (it.desc as string | undefined) ?? "" };
  });
  const maxIdx    = items.reduce((mi, d, i) => d.score > items[mi].score ? i : mi, 0);
  const secondIdx = items.reduce((mi, d, i) => i !== maxIdx && d.score > items[mi].score ? i : mi, maxIdx === 0 ? 1 : 0);
  const best   = items[maxIdx];
  const second = items[secondIdx];
  const BLUE_PEAK = "#3a6abf";
  const BLUE_2ND  = "#4a9e6a";
  return (
    <div className="mx-5 mb-5">
      {/* 최적 시기 배너 */}
      <div className="rounded-2xl px-5 py-5 mb-4" style={{ background: `linear-gradient(135deg, ${ECH5_COLOR} 0%, ${ECH5_DEEP} 100%)` }}>
        <p className="text-[10px] font-bold mb-1.5" style={{ color: "rgba(255,255,255,0.65)" }}>사주가 가리키는 이혼하기 가장 좋은 시기</p>
        <p className="text-[24px] font-black leading-tight mb-2" style={{ color: WHITE, fontFamily: SERIF }}>20{best.shortYear}년 {best.zodiac}</p>
        <div className="flex items-center gap-2">
          <span className="inline-block px-2.5 py-1 rounded-full text-[11px] font-black" style={{ background: "rgba(255,255,255,0.2)", color: WHITE }}>{best.score}점 · 최적</span>
        </div>
      </div>
      {/* 최적 + 차선 카드 */}
      {[{ item: best, col: BLUE_PEAK, badge: "최적" }, { item: second, col: BLUE_2ND, badge: "차선" }].map(({ item, col, badge }) => (
        <div key={badge} className="rounded-xl overflow-hidden mb-3" style={{
          border: `${badge === "최적" ? "2px" : "1.5px"} solid ${col}${badge === "최적" ? "cc" : "99"}`,
          background: `linear-gradient(135deg, ${col}08 0%, ${WHITE} 60%)`,
          boxShadow: badge === "최적" ? `0 0 8px 2px ${col}55, 0 0 18px 4px ${col}22` : `0 0 6px 2px ${col}44`,
        }}>
          <div className="flex items-center gap-2.5 px-3.5 pt-2.5 pb-2" style={{ background: `${col}10` }}>
            <span className="text-[13px]">{badge === "최적" ? "⚡" : "🌙"}</span>
            <p className="text-[12.5px] font-black" style={{ color: col }}>20{item.shortYear}년 {item.zodiac}</p>
            <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: col, color: WHITE }}>{badge}</span>
            <span className="ml-auto text-[12px] font-black tabular-nums" style={{ color: col }}>{item.score}점</span>
          </div>
          <div className="px-3.5 pt-2.5 pb-3">
...
// 피해야 할 이혼 시기 카드 — timingItems에서 점수 최하위 2개 자동 식별
const DIVORCE_CAUTION_RED = "#c0404a";
function DivorceCautionCard({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  const rawItems = (data.items as Record<string, unknown>[] | undefined) ?? [];
  if (!rawItems.length) return null;
  const GAN_KOR: Record<string,string> = {甲:"갑",乙:"을",丙:"병",丁:"정",戊:"무",己:"기",庚:"경",辛:"신",壬:"임",癸:"계"};
  ...
  const items = rawItems.map((it) => {
    ...
    const hanja     = (full.split(/\s+/)[1] ?? "").replace("년", "");
    const zodiac    = (GAN_KOR[hanja[0]] ?? hanja[0] ?? "") + (JI_KOR[hanja[1]] ?? hanja[1] ?? "");
    return { score, shortYear, zodiac, desc: (it.desc as string | undefined) ?? "" };
  });
  const minIdx = items.reduce((mi, d, i) => d.score < items[mi].score ? i : mi, 0);
  const secondMinIdx = items.reduce((mi, d, i) => i !== minIdx && d.score < items[mi].score ? i : mi, minIdx === 0 ? 1 : 0);
  const cautionItems = [minIdx, secondMinIdx].filter((i, pos, arr) => arr.indexOf(i) === pos).map(i => ({ ...items[i], rank: i === minIdx ? 1 : 2 }));
  return (
    <div className="px-5 space-y-3 mb-5">
      {cautionItems.map((d, i) => (
        <div key={i} className="rounded-xl overflow-hidden" style={{
          border: `${d.rank === 1 ? "2px" : "1.5px"} solid ${DIVORCE_CAUTION_RED}${d.rank === 1 ? "cc" : "99"}`,
          background: `linear-gradient(135deg, ${DIVORCE_CAUTION_RED}08 0%, ${WHITE} 60%)`,
          boxShadow: d.rank === 1 ? `0 0 8px 2px ${DIVORCE_CAUTION_RED}55, 0 0 18px 4px ${DIVORCE_CAUTION_RED}22` : `0 0 6px 2px ${DIVORCE_CAUTION_RED}44`,
        }}>
          <div className="flex items-center gap-2.5 px-3.5 pt-2.5 pb-2" style={{ background: `${DIVORCE_CAUTION_RED}10` }}>
            <span className="text-[13px]">⚠️</span>
            <p className="text-[12.5px] font-black" style={{ color: DIVORCE_CAUTION_RED }}>20{d.shortYear}년 {d.zodiac}</p>
            <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: DIVORCE_CAUTION_RED, color: WHITE }}>주의</span>
            <span className="ml-auto text-[12px] font-black tabular-nums" style={{ color: DIVORCE_CAUTION_RED }}>{d.score}점</span>
          </div>
          <div className="px-3.5 pt-2.5 pb-3">
            <p className="text-[13px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{d.desc}</p>
          </div>
  ...
  return (
    <div className="mx-5 mb-5">
      {tips.map((tip, i) => {
        ...
        return (
          <div key={i} className="mb-3 rounded-xl overflow-hidden" style={{ borderLeft: `4px solid ${ECH5_COLOR}` }}>
            <div className="px-4 py-2" style={{ background: `${ECH5_COLOR}0e` }}>
...
// 이혼 후 두 사람 인연운 흐름 비교 차트 (2026-2035)
function DualLoveLineChart({ myView, partnerView, myName, partnerName, myGender, partnerGender }: {
  myView: MyeongsikView | null; partnerView: MyeongsikView | null;
  myName: string; partnerName: string;
  myGender?: string; partnerGender?: string;
}) {
  const TARGET_YEARS = [2026,2027,2028,2029,2030,2031,2032,2033,2034,2035];
  const STEM_EL: Record<string,string> = { 甲:"목",乙:"목",丙:"화",丁:"화",戊:"토",己:"토",庚:"금",辛:"금",壬:"수",癸:"수" };
  const BRANCH_EL: Record<string,string> = { 子:"수",丑:"토",寅:"목",卯:"목",辰:"토",巳:"화",午:"화",未:"토",申:"금",酉:"금",戌:"토",亥:"수" };
  ...
  function loveScore(gender: string | undefined): Record<string,number> {
    const isFem = gender === "female";
    return isFem
      ? { 관성:90, 편관:88, 정관:92, 재성:70, 편재:68, 정재:72, 식상:75, 식신:76, 상관:74, 인성:60, 편인:58, 정인:62, 비겁:50, 비견:50, 겁재:48 }
      : { 재성:90, 편재:88, 정재:92, 관성:70, 편관:68, 정관:72, 식상:75, 식신:76, 상관:74, 인성:60, 편인:58, 정인:62, 비겁:50, 비견:50, 겁재:48 };
  }
  function calcScores(view: MyeongsikView | null, gender?: string) {
    const SCORE = loveScore(gender);
    const ilEl = STEM_EL[(view?.ilgan ?? "")[0]] ?? "목";
    const seunMap: Record<number,string> = {};
    (view?.seun ?? []).forEach(s => { const y = Number(s.label); if (y >= 2026 && y <= 2035) seunMap[y] = s.gz; });
    TARGET_YEARS.forEach(y => { if (!seunMap[y]) seunMap[y] = GANJIS[(baseIdx + (y - BASE_YEAR)) % 60]; });
    return TARGET_YEARS.map(y => {
      const gz = seunMap[y] ?? "";
      const sEl = STEM_EL[gz[0]]; const bEl = BRANCH_EL[gz[1]];
      const sScore = sEl ? (SCORE[toSipseong(ilEl, sEl)] ?? 60) : 60;
      const bScore = bEl ? (SCORE[toSipseong(ilEl, bEl)] ?? 60) : 60;
      const gz2k = (GAN_KOR[gz[0]] ?? "") + (JI_KOR[gz[1]] ?? "");
      return { score: Math.min(95, Math.max(40, Math.round(sScore * 0.6 + bScore * 0.4))), gz2k };
    });
  }
  const myData = calcScores(myView, myGender);
  const ptData = calcScores(partnerView, partnerGender);
  const isFem = (g?: string) => g === "female" || g === "여성" || g === "여자" || g === "여아";
  const MY_COLOR = isFem(myGender) ? "#c0306a" : "#3f63c4";
  const PT_COLOR = isFem(partnerGender) ? "#c0306a" : "#3f63c4";
  const W = 320, H = 180, padX = 24, padTop = 16, padBot = 58;
  const allScores = [...myData, ...ptData].map(d => d.score);
  const minS = Math.min(...allScores);
  const maxS = Math.max(...allScores);
  const range = maxS - minS || 1;
  ...
  return (
    <div className="mx-5 mb-5 rounded-2xl overflow-hidden" style={{ background: WHITE, border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full px-1" style={{ overflow: "visible" }}>
        <defs>
          <linearGradient id="dualLoveMyArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={MY_COLOR} stopOpacity="0.18" />
            <stop offset="100%" stopColor={MY_COLOR} stopOpacity="0.02" />
          </linearGradient>
          <linearGradient id="dualLovePtArea" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={PT_COLOR} stopOpacity="0.18" />
            <stop offset="100%" stopColor={PT_COLOR} stopOpacity="0.02" />
          </linearGradient>
        </defs>
        ...
        <polygon points={myArea} fill="url(#dualLoveMyArea)" />
        <polygon points={ptArea} fill="url(#dualLovePtArea)" />
        <polyline points={myPts} fill="none" stroke={MY_COLOR} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        <polyline points={ptPts} fill="none" stroke={PT_COLOR} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
        {/* 점 */}
        {myData.map((d, i) => (
    ...
    <div className="mx-5 mb-5">
      ...
      <div className="rounded-2xl overflow-hidden mb-4" style={{ border: `1px solid ${ECH11_MY}18` }}>
        <div className="px-4 py-3 flex items-center gap-3" style={{ background: `${ECH11_MY}0a` }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-black" style={{ color: ECH11_MY }}>{myName}님의 새로운 출발</p>
            {myNewStartKey && (
        ...
        {myNewStart && (
          <div className="px-4 pt-3 pb-1">
            <p className="text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{myNewStart}</p>
          </div>
        )}
        {myItems.length > 0 && (
      ...
      <div className="rounded-2xl overflow-hidden mb-4" style={{ border: `1px solid ${ECH11_PT}18` }}>
        <div className="px-4 py-3 flex items-center gap-3" style={{ background: `${ECH11_PT}0a` }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[12px] font-black" style={{ color: ECH11_PT }}>{partnerName}님의 새로운 출발</p>
            {partnerNewStartKey && (
        ...
        {partnerNewStart && (
          <div className="px-4 pt-3 pb-1">
            <p className="text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{partnerNewStart}</p>
          </div>
        )}
      ...
      {/* 풀이 단락 */}
      {paragraphs.map((p, i) => (
        <p key={i} className="mb-4 text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF, wordBreak: "keep-all" }}>{p}</p>
      ))}
    </div>
  );
}

// 새 인연 카드 — 나/상대방 각각에게 찾아올 새 인연을 사주로 조명
function NewBeginningCard({ data, myName, partnerName }: {
  ...
  if (!data) return null;
  const myTiming      = (data.myTiming      as string | undefined) ?? "";
  const myLoveKey     = (data.myLoveKey     as string | undefined) ?? "";
  const myLove        = (data.myLove        as string | undefined) ?? "";
  const myPersonType  = (data.myPersonType  as string | undefined) ?? "";
  const myMeetHow     = (data.myMeetHow     as string | undefined) ?? "";
  const ptTiming      = (data.ptTiming      as string | undefined) ?? "";
  const ptLoveKey     = (data.ptLoveKey     as string | undefined) ?? "";
  const partnerLove   = (data.partnerLove   as string | undefined) ?? "";
  const ptPersonType  = (data.ptPersonType  as string | undefined) ?? "";
  const ptMeetHow     = (data.ptMeetHow     as string | undefined) ?? "";
  const paragraphs    = (data.paragraphs    as string[] | undefined) ?? [];
  ...
  const SectionLabel = ({ label, color }: { label: string; color: string }) => (
    <div className="flex items-center gap-2 mb-3">
      ...
      <span className="text-[13px] font-black tracking-wide" style={{ color }}>{label}</span>
    </div>
  );

  const renderBlock = (
    personName: string, color: string, pale: string,
    timing: string, loveKey: string, love: string, personType: string, meetHow: string,
  ) => (
    <div className="rounded-2xl overflow-hidden mb-5" style={{ border: `1.5px solid ${color}22`, boxShadow: `0 2px 16px ${color}0a` }}>
      {/* 헤더 */}
      <div className="px-4 py-3 flex items-center gap-2" style={{ background: `${color}20`, borderBottom: `1px solid ${color}28` }}>
        <span className="text-[14px] font-black" style={{ color }}>{personName}님에게 찾아올 새 인연</span>
        {timing && (
          <span className="ml-auto px-3 py-1 rounded-full text-[10.5px] font-black flex-shrink-0" style={{ background: color, color: WHITE }}>
            {timing}
          </span>
        )}
      </div>
      {/* 인연 방향 — 메인 파트: 연한 컬러 배경 */}
      <div className="px-4 pt-5 pb-4" style={{ background: `${color}0c` }}>
        {loveKey && (
          <div className="inline-flex items-center gap-1.5 mb-3 px-3 py-1 rounded-full" style={{ background: `${color}18`, border: `1px solid ${color}35` }}>
            <span className="text-[12px] font-black" style={{ color }}>{loveKey}</span>
          </div>
        )}
        {love.split('\n\n').map((para, i) => (
      ...
      {(personType || meetHow) && (
        <div style={{ borderTop: `1.5px solid ${color}18` }}>
          {personType && (
  ...
  return (
    <div className="mx-5 mb-5">
      {renderBlock(myName, ECH11_MY, ECH11_MY_P, myTiming, myLoveKey, myLove, myPersonType, myMeetHow)}
      {renderBlock(partnerName, ECH11_PT, ECH11_PT_P, ptTiming, ptLoveKey, partnerLove, ptPersonType, ptMeetHow)}
      {paragraphs.map((p, i) => (
        <p key={i} className="mb-4 text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF, wordBreak: "break-all" }}>{p}</p>
      ))}
    </div>
  );
}
...
const TOC_A: TocEntry[] = [
  ...
  { disp: "제4장",  chip: "흔들림",  title: "이혼하게 될 인연일까",                  no: "4" },
  { disp: "제5장",  chip: "이혼시기", title: "언제 이혼하면 좋을까",           no: "5" },
  { disp: "제6장",  chip: "재산자녀", title: "이혼 후 재산과 자녀",                     no: "6" },
  { disp: "제7장",  chip: "새로운나",  title: "두 사람의 이혼 후 인생",                   no: "7" },
  { disp: "제8장",  chip: "화해가능", title: "좋아질 가능성은 없을까",                  no: "8" },
  { disp: "제9장",  chip: "새출발",  title: "그대들의 새로운 출발",                    no: "9" },
  { disp: "마무리", chip: "당부",    title: "홍연이 드리는 마지막 서신",            no: "10" },
];
        ...
        <div className="px-5 py-6">
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[15px]" style={{ color: MUTE }}>[이혼궁합]</span>
            </div>
            <button
              onClick={onClose}
...
<>
      ...
      {/* ═══════════ 제1장 · 나의 사주 원국 ═══════════ */}
      {ch === "1" && (() => {
        const myWonguk        = (jc.myWonguk        as Record<string, unknown> | undefined) ?? {};
        ...
        const wongukParas = (myWonguk.paragraphs as string[] | undefined) ?? [];
        return (
          <>
            {/* ── 다크 헤더 ── */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            ...
            <section className="pt-4 pb-4">
              <div className="px-5 mb-3">
                <h2 className="text-[19px] font-black" style={{ color: INK }}>이혼할 때 나는 어떤 사람인가</h2>
              </div>
...
{`이번에는 ${partnerName}님의\n사주를 펼쳐보겠소.`}
            ...
            {report?.partnerSajuImageUrl && (
              <div>
                <div className="px-5">
                  ...
                  <div style={{ display: "flex", justifyContent: "center", marginTop: 10 }}>
                    <div style={{ background: "linear-gradient(135deg, #d8b428 0%, #a87c10 45%, #d0aa24 100%)", padding: "5px 22px", boxShadow: "0 2px 6px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,240,100,0.45), inset 0 -1px 0 rgba(0,0,0,0.3)", border: "1px solid #7a5808" }}>
                      <p style={{ fontSize: 11, color: "#1e1000", fontFamily: SERIF, letterSpacing: "0.12em", margin: 0 }}>{partnerName}님의 사주화</p>
                    </div>
                  </div>
                  {(() => {
                    const meaning = buildSajuImageMeaning(report?.partnerView?.pillars ?? [], `${partnerName}님`);
                    if (!meaning) return null;
                    return (
                      <div style={{ maxWidth: 420, margin: "16px auto 0", padding: "14px 16px", background: "#faf6ea", border: "1px solid #e5d9b0", borderRadius: 8 }}>
                        <p style={{ fontSize: 11, color: "#a07018", fontWeight: 700, letterSpacing: "0.08em", margin: "0 0 6px", fontFamily: SERIF }}>이 그림이 담고 있는 뜻</p>
            ...
            <section className="pt-4 pb-4">
              <div className="px-5 mb-3">
                <h2 className="text-[19px] font-black" style={{ color: INK }}>이혼할 때 상대는 어떤 사람인가</h2>
              </div>
      ...
      {/* ═══════════ 제4장 · 이혼 가능성 ═══════════ */}
      {ch === "4" && (() => {
        const myName4      = name || "나";
        const partnerName4 = report?.partnerName || "상대방";
        // divorceScore
        ...
        const drItems      = (dr?.items      as Record<string, unknown>[] | undefined) ?? [];
        const drIntro      = (dr?.intro      as string   | undefined) ?? "";
        const drCallout    = (dr?.callout    as string   | undefined) ?? "";
        const drParas      = (dr?.paragraphs as string[] | undefined) ?? [];
        const triggerItems = (cs?.triggerItems as Record<string, unknown>[] | undefined) ?? [];
        const csParas      = (cs?.paragraphs   as string[] | undefined) ?? [];
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 4 장 · 이혼 가능성 · 갈등 · 원인</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>이혼하게 될 인연일까</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            <section className="px-6 pt-6 pb-2">
              <Heading>두 사람이 이혼하는 이유</Heading>
            </section>
            {drItems.map((item, i) => <DivorceReasonCard key={i} item={item} />)}
            {drParas.length > 0 && (
              <section className="px-5 pt-2 pb-4">
                {drParas.map((p, i) => (
                  <p key={i} className="text-[13.5px] leading-[1.85] mb-4" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
            ...
            {triggerItems.length > 0 && (
              <>
                <section className="px-6 pt-4 pb-2">
                  <Heading>두 사람이 반복해서 싸우는 이유</Heading>
                  <P>두 사람 사이에서 반복적으로 갈등에 불을 지피는 사주적 뇌관이오.</P>
                </section>
            ...
            {report?.view && report?.partnerView && (
              <section className="pb-4">
                ...
                <KunghapRelationCardG
                  ...
                  myColor={ECH1_COLOR} partnerColor={ECH2_COLOR}
                />
              </section>
            )}
      ...
      {ch === "6" && (() => {
        ...
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 6 장 · 재산·자녀</p>
            ...
            {/* 재산 흐름 */}
            <section className="px-6 pt-2 pb-2">
              <Heading>이혼 후 재물운 흐름</Heading>
              <P>이혼 후 두 사람 각자의 사주 재물운이 어떻게 흘러가는지를 10년 그래프로 살펴보겠소.</P>
            </section>
            <DualWealthLineChart myView={report?.view ?? null} partnerView={report?.partnerView ?? null} myName={myName} partnerName={partnerName} />
            <section className="px-6 pt-2 pb-2">
              <Heading>두 사람의 재물 성향</Heading>
              <P>이혼은 재산의 분리를 수반하게 되는데, 두사람의 사주에 재물흐름이 어떻게 될지 살펴보겠소.</P>
            </section>
      ...
      {ch === "7" && (() => {
        const myName      = name || "나";
        const partnerName = report?.partnerName || "상대방";
        const mf = (jc.myFuture      as Record<string, unknown> | undefined) ?? null;
        const pf = (jc.partnerFuture as Record<string, unknown> | undefined) ?? null;
        const sp = (jc.shiningPath   as Record<string, unknown> | undefined) ?? null;
        const MY_C = "#3a6abf";
        const PT_C = "#c0404a";
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 7 장 · 두 사람의 이혼 후 인생</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>두 사람의 이혼 후 인생</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {/* 나의 운명 흐름 */}
            <section className="px-6 pt-2 pb-2">
              <Heading>이혼 후 {myName}님의 운명 흐름</Heading>
            </section>
            <FutureFlowCard data={mf} name={myName} color={MY_C} />

            {/* 상대방의 운명 흐름 */}
            <section className="px-6 pt-4 pb-2">
              <Heading>이혼 후 {partnerName}님의 운명 흐름</Heading>
            </section>
            ...
            {/* 각자가 빛나는 방향 */}
            <section className="px-6 pt-4 pb-2">
              <Heading>각자가 빛나는 방향</Heading>
              <P>두 사람이 이혼 후 각자의 사주에서 가장 잘 살 수 있는 삶의 방향이오.</P>
            </section>
      ...
      {ch === "8" && (() => {
        ...
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 8 장 · 화해의 가능성</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>좋아질 가능성은 없을까</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {/* 재결합 가능성 점수 게이지 */}
            <section className="px-6 pt-2 pb-2">
              <Heading>화해 가능성 점수는?</Heading>
              <P>두 사람이 다시 좋아질 수 있는가를 점수로 나타내 봤소. 점수가 높을수록 화해의 기운이 살아있고, 낮을수록 각자의 길이 더 나을것이오.</P>
            </section>
            ...
            {/* 화해 가능 요소 vs 장벽 */}
            <section className="px-6 pt-4 pb-2">
              <Heading>화해를 가능하게 하는것과 막는것</Heading>
            </section>
      ...
      {ch === "5" && (() => {
        ...
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 5 장 · 이혼 시기</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>언제 이혼하면 좋을까</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {/* 연도별 이혼 에너지 흐름 */}
            <section className="px-6 pt-2 pb-2">
              <Heading>연도별 이혼 에너지 흐름</Heading>
              <P>두 사람의 사주팔자와 세운의 흐름을 분석하여, 향후 10년간의 이혼의 흐름을 그래프로 나타내 주겠소. 이혼하기 가장 좋은 시기가 언제인지 확인해 보시오.</P>
            </section>
            <DivorceTimingLineChart data={ti} />

            {/* 피해야 할 시기 */}
            <section className="px-6 pt-4 pb-2">
              <Heading>이혼을 피해야 할 시기</Heading>
            </section>
      ...
      {ch === "9" && (() => {
        ...
        return (
          <>
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 9 장 · 그대들의 새로운 출발</p>
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>그대들의 새로운 출발</h1>
            </div>
            <div className="relative overflow-hidden" style={{ height: 520 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
            ...
            {/* 인연운 흐름 차트 */}
            <section className="px-6 pt-2 pb-2">
              <Heading>이혼 후에 찾아오게 될 새 인연흐름</Heading>
            </section>
            <DualLoveLineChart
              myView={report?.view ?? null}
              partnerView={report?.partnerView ?? null}
              myName={myName}
              partnerName={partnerName}
              myGender={report?.gender}
              partnerGender={report?.partnerGender}
            />
      ...
      {ch === "10" && (
        ...
        <div style={{ filter: eventOpen ? "blur(5px)" : "none", transition: "filter 0.25s ease", pointerEvents: eventOpen ? "none" : "auto" }}>
          <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            ...
            <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>홍연이 드리는 마지막 서신</h1>
          </div>
          <div className="relative overflow-hidden" style={{ height: 520 }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/media/report/kunghap_ehon/kunghap_ehon_10/kunghap_ehon_10_cover.jpg" alt="" className="absolute inset-0 w-full h-full object-cover" style={{ objectPosition: "center 20%" }} />
            <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(17,17,17,1) 0%, rgba(17,17,17,0.3) 35%, transparent 60%, transparent 70%, rgba(253,248,244,1) 100%)" }} />
          </div>
