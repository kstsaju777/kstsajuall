...
const TOTAL_CH = Object.keys(EHON_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "이혼사주 들어가며",
  1: "두 사람의 타고난 기질과 결혼 인연",
  2: "부부 궁합과 갈등의 뿌리",
  3: "배우자의 속마음",
  4: "이혼 시기와 운의 흐름",
  5: "이혼 후 삶과 새로운 인연",
...
const CH_SUBS: Record<number, string> = {
  0: "",
  1: "두 사람이 왜 만났는지, 사주에 이미 담겨 있어요",
  2: "지금의 갈등이 왜 반복되는지 살펴봅니다",
  3: "배우자는 지금 어떤 마음일까요",
  4: "이혼이 맞는 시기인지, 사주로 확인합니다",
  5: "이혼 후 삶의 흐름과 새 인연의 시기",
  6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/kunghap_ehon/ehon-0.jpg",
  1: "/media/cards/kunghap_ehon/ehon-0.jpg",
  2: "/media/cards/kunghap_ehon/ehon-0.jpg",
  3: "/media/cards/kunghap_ehon/ehon-0.jpg",
  4: "/media/cards/kunghap_ehon/ehon-0.jpg",
  5: "/media/cards/kunghap_ehon/ehon-0.jpg",
  6: "/media/cards/kunghap_ehon/ehon-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isEhonChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/kunghap_ehon/ehon-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: MAROON }}>✦ 이혼사주 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, partnerName, onRetry, onNext }: {
  ...
  const sectionKeys = EHON_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isEhonChapterReady(content, ch) : false;
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: MAROON }}>✦ 이혼사주 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/kunghap_ehon/ehon-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>이혼사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/ehon-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date && partnerDate) {
      fetch("/api/ehon-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          if (rid) {
            ...
            router.replace(`/saju/kunghap_ehon/report-preview?id=${rid}&${base}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(EHON_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isEhonChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/kunghap_ehon/report-preview?${base}&ch=${n}`);
  };
