...
//
// 미리보기: http://localhost:3000/saju/saju_youare/report-preview
//
...
const RECO_SAJU_ORDER = ["정통명리 종합사주", "영재발굴 자녀사주", "대박이야 재물사주", "나만솔로? 연애사주", "오래살자 건강사주"];
        ...
        <div className="px-5 py-6">
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[15px]" style={{ color: MUTE }}>[유아사주]</span>
            </div>
            <button
              onClick={onClose}
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/saju_youare-report?id=${encodeURIComponent(id)}`)
        ...
        .then(async (d) => {
          ...
          if (concern && caEmpty) {
            try {
              const r = await fetch("/api/saju_youare-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, concernOnly: true }) });
      ...
      fetch("/api/saju_youare-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/saju_youare/report-preview?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const genChapter = (n: number, force = false) => {
    ...
    fetch("/api/saju_youare-report", {
      ...
      .then((d) => {
        ...
        if (!sec || !isYouareChapterReady(sec, toApiChapter(n))) return;
        setReport((p) => {
          if (!p) return p;
  ...
  const needGen = !!id && !!YOUARE_CHAPTER_SECTIONS[chNum] && !isYouareChapterReady(report?.content as Record<string, unknown>, chNum);
  ...
  const next = (n: string) => router.push(`/saju/saju_youare/report-preview?${id ? `id=${id}&` : ""}ch=${n}`);
...
<>
        ...
        <>
          ...
          <section className="pt-6 pb-12">
            ...
            <div className="relative w-full mt-8">
              ...
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "73%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[16px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{honorShort}의 사주팔자로<br />한폭의 그림을<br />그려봤소.</p>
              </div>
            </div>
            ...
            <div className="px-6 pt-4">
              {(jc.wonguk as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
            </div>
          </section>
          ...
          <div className="relative w-full mt-8">
            ...
            <div className="absolute pointer-events-none" style={{ top: "18%", left: "30%", transform: "translate(-50%, -50%)" }}>
              <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>네 기둥 속에<br />아이의 인생 전체가<br />담겨 있소.</p>
            </div>
          </div>
          <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
            <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
          ...
          <section className="px-6 pt-2 pb-4">
            ...
            {(jc.chonyeongi as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
            ...
            {(jc.cheongneongi as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
            ...
            {(jc.jungnyeongi as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
            ...
            {report?.birth?.time !== "시간 모름" && (() => {
              ...
              return (
                <>
                  ...
                  {(jc.nonyeongi as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
                </>
              );
            })()}
          ...
          <div className="relative w-full mt-8">
            ...
            <div className="absolute pointer-events-none" style={{ top: "18%", left: "72%", transform: "translate(-50%, -50%)" }}>
              <p className="text-[16px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>이 아이가 특히<br />강한 기운이<br />무엇인지 말해주겠소.</p>
            </div>
          </div>
          <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
            <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
          ...
          <section className="px-6 pt-2 pb-4">
            ...
            {(jc.ability as { paragraphs?: string[] } | undefined)?.paragraphs?.map((p, i) => <P key={i}>{p}</P>)}
          </section>
        ...
        <>
          ...
          {(() => {
            const hl = (jc.health as {
              ...
              healthTipCards?: { icon: string; title: string; desc: string }[];
              healthTips?: string[];
            } | undefined) ?? {};
            ...
            const bodyCategories = OHAENG_BODY.map(cat => ({
              ...
              items: cat.items.map(([label, primary, secondary, w]) => ({
                label, value: Math.round(elScore(primary) * w + elScore(secondary) * (1 - w)),
              })),
            }));
            const tipCards = hl.healthTipCards ?? [];
              ...
              <>
                ...
                <div className="relative w-full mt-8">
                  ...
                  <div className="absolute pointer-events-none" style={{ top: "22%", left: "31%", transform: "translate(-50%, -50%)" }}>
                    <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>타고난 기운으로<br />주의할 신체 부위를<br />알 수 있소.</p>
                  </div>
                </div>
                <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
                  <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
          ...
          <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
            <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`아이의 건강을 알았으니\n이제 양육 이야기를 하겠소.\n\n${honor}에게\n맞는 양육 방식을\n살펴보겠소.`}</p>
          </div>
      ...
      {ch === "3" && (
        <>
              ...
              <>
                ...
                <div className="relative w-full mt-8">
                  ...
                  <div className="absolute pointer-events-none" style={{ top: "23%", left: "70%", transform: "translate(-50%, -50%)" }}>
                    <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>이 아이를 위해<br />부모가 할 수 있는<br />것들이 있소.</p>
                  </div>
                </div>
                <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
                  <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
                  ...
                  <section className="px-6 pt-4 pb-4">
                    ...
                    <div className="mt-3 flex flex-col gap-3">
                      {pt.parentingTips.map((tip, i) => (
                        <div key={i} className="flex gap-3 items-start rounded-2xl px-4 py-4" style={{ background: `${TEAL}08`, border: `1.5px solid ${TEAL}22` }}>
                          <span className="flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-black" style={{ background: TEAL, color: "#fff" }}>
                            {String(i + 1).padStart(2, "0")}
                          </span>
                          <p className="text-[12.5px] leading-relaxed flex-1 pt-0.5" style={{ color: INK_SOFT }}>{tip}</p>
                        </div>
                      ))}
                    </div>
                  </section>
          ...
          <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
            <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`양육 방향을 알았으니\n이제 재능 이야기를 하겠소.\n\n타고난 재능은\n사주에 이미 새겨져 있소.\n\n${honor}의\n타고난 재능을 살펴보겠소.`}</p>
          </div>
      ...
      {ch === "4" && (
        <>
              ...
              <>
                ...
                <div className="relative w-full mt-8">
                  ...
                  <div className="absolute pointer-events-none" style={{ top: "25%", left: "70%", transform: "translate(-50%, -50%)" }}>
                    <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>이 아이의 재능,<br />이렇게 키워주면<br />더 빛나오.</p>
                  </div>
                </div>
                <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
                  <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
          ...
          <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
            <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`아이의 성장에도\n파고가 있소.\n\n${honor}의 성장 시기별\n주의할 때를 알려드리겠소.`}</p>
          </div>
        ...
        <>
          ...
          {(() => {
            ...
            function phaseYearRange(phaseIdx: number, total: number): [number, number] {
              const span = CHART_END - CHART_START + 1;
                    ...
                    <section className="px-6 pt-0 pb-4">
                      ...
                      <div className="rounded-2xl p-4 mt-3" style={{ background: WHITE, border: `1px solid ${INK}12`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
                        ...
                        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ overflow:"visible" }}>
                          ...
                          <polygon points={area} fill="url(#cautionAreaTop5)" clipPath="url(#cautionClipTop5)" />
                          <polygon points={area} fill="rgba(91,158,106,0.12)" clipPath="url(#cautionClipBot5)" />
                          <polyline points={pts} fill="none" stroke={GOLD} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                          {data.map((d, i) => {
          ...
          <div className="px-8 py-10 text-center" style={{ background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
            <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>{`아이의 기운을 살리는\n또 하나의 방법이 있소.\n\n부족한 오행을 채우는\n개운법을 알려드리겠소.`}</p>
          </div>
      ...
      {ch === "6" && (
        <>
          ...
          {(() => {
            ...
            const FOREST = "#3d6b4f";
            const OHAENG_META: Record<string, { desc: string }> = {
              목: { desc: "성장·추진" }, 화: { desc: "열정·표현" },
              토: { desc: "안정·신뢰" }, 금: { desc: "절제·결단" }, 수: { desc: "지혜·유연" },
            };
            const OHAENG_IMG: Record<string, string> = { 목:"mok", 화:"hwa", 토:"to", 금:"geum", 수:"su" };
            // LLM이 ohaeng 필드를 빠뜨린 경우 intro 텍스트에서 추출
            const OHAENG_LIST = ["금","목","화","토","수"] as const;
            const yText = [rd.intro, ...(rd.paragraphs ?? [])].filter(Boolean).join(" ");
            const extractEl = (role: string, text: string) => { const m = text.match(new RegExp(`${role}[은이가]?\\s*(?:오행인\\s*)?(금|목|화|토|수)`)); return m?.[1] ?? ""; };
            const yongsinEl = rd.yongsinEl || extractEl("용신", yText) || (() => { const f: string[] = []; for (const ch of yText) { if ((OHAENG_LIST as readonly string[]).includes(ch) && !f.includes(ch)) f.push(ch); if (f.length) return f[0]; } return ""; })();
            const heusinEl  = rd.heusinEl  || extractEl("희신", yText) || (() => { const f: string[] = []; for (const ch of yText) { if ((OHAENG_LIST as readonly string[]).includes(ch) && !f.includes(ch)) f.push(ch); if (f.length >= 2) return f[1]; } return ""; })();
            const gisinEl   = rd.gisinEl   || extractEl("기신", yText) || (() => { const f: string[] = []; for (const ch of yText) { if ((OHAENG_LIST as readonly string[]).includes(ch) && !f.includes(ch)) f.push(ch); if (f.length >= 3) return f[2]; } return ""; })();
            const rows = [
              { role: "용신", el: yongsinEl, reason: rd.yongsinReason, badge: { bg: "#fff3cd", border: "#e6a817", text: "#7a4f00", label: "★★★" } },
              { role: "희신", el: heusinEl,  reason: rd.heusinReason,  badge: { bg: "#e8f5e9", border: "#43a047", text: "#1b5e20", label: "★★" } },
              { role: "기신", el: gisinEl,   reason: rd.gisinReason,   badge: { bg: "#ffeaea", border: "#e53935", text: "#7f0000", label: "✕" } },
            ];
            return (
              <>
                {/* ① 용신·희신·기신 + 인트로 단락 */}
                <section className="px-6 pt-6 pb-4">
                  <Heading>아이에게 필요한 기운</Heading>
                  <div className="my-4 rounded-2xl overflow-hidden" style={{ border: "1px solid #e0d8cc" }}>
                    {rows.map((r, i) => {
                      const m = OHAENG_META[r.el] ?? { desc: r.el };
                      const imgKey = OHAENG_IMG[r.el];
                      ...
                      return (
                        <div key={r.role} className="flex items-center gap-3 px-4 py-3" style={{ background: isGisin ? "#fff5f5" : i === 0 ? "#fffcf0" : "#f6faf6", borderBottom: i < 2 ? "1px solid #e8dfd0" : "none" }}>
                          ...
                          <div className="flex flex-col gap-1">
                            <div className="text-[14px] font-black" style={{ color: "#333" }}>{m.desc}</div>
                  ...
                  {(rd.intro || rd.paragraphs?.[0]) && (
                    <P>{[rd.intro, rd.paragraphs?.[0]].filter(Boolean).join(" ")}</P>
                  )}
                </section>
                ...
                <div className="relative w-full mt-8">
                  ...
                  <div className="absolute pointer-events-none" style={{ top: "23%", left: "35%", transform: "translate(-50%, -50%)" }}>
                    <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>일상 속 작은 습관이<br />이 아이의 기운을<br />바꿔주오.</p>
                  </div>
                </div>
                <div className="flex justify-center" style={{ paddingTop: 40, paddingBottom: 32 }}>
                  <div style={{ width: 1, height: 60, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
                      ...
                      {rd.remedies.map((r, i) => (
                        <div key={i} className="rounded-2xl overflow-hidden flex" style={{ border: `1.5px solid ${FOREST}28` }}>
                          ...
                          <div className="flex-1 p-3.5" style={{ background: `${FOREST}05` }}>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-black px-2 py-0.5 rounded-full text-[11px]" style={{ background: FOREST, color: "#fff" }}>{r.category}</span>
                            </div>
                            <p className="text-[13px] font-black mb-1" style={{ color: FOREST }}>{r.title}</p>
                            <p className="text-[12px] leading-relaxed" style={{ color: INK_SOFT }}>{r.desc}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* ③ 분야별 실천 체크리스트 */}
                {rd.remedyCats && rd.remedyCats.length > 0 && (
                  <section className="px-6 pt-2 pb-4">
                    <Heading>부모 실천 체크리스트</Heading>
                    <div className="space-y-3 mt-3">
                      {rd.remedyCats.map((cat, ci) => (
                        <div key={ci} className="rounded-2xl p-4" style={{ background: `${FOREST}07`, border: `1.5px solid ${FOREST}20` }}>
                          <div className="flex items-center gap-2 mb-3">
                            <span className="text-[18px]">{cat.icon}</span>
                            <span className="text-[12px] font-black" style={{ color: FOREST }}>{cat.category}</span>
                          </div>
                          <div className="space-y-2">
                            {cat.items.map((item, ii) => (
                              <div key={ii} className="flex items-start gap-2">
                                <span className="flex-shrink-0 w-4 h-4 rounded border-2 mt-0.5" style={{ borderColor: `${FOREST}50` }} />
                                <p className="text-[12px] leading-snug" style={{ color: INK_SOFT }}>{item}</p>
                              </div>
                            ))}
                          </div>
                        </div>
