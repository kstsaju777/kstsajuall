...
const TOTAL_CH = Object.keys(YOUARE_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "유아사주 들어가며", 1: "아이의 타고난 기질", 2: "숨겨진 재능과 잠재력", 3: "공부 방식과 진로", 4: "운의 흐름", 5: "건강과 육아 주의사항", 6: "홍연의 편지",
};

const CH_SUBS: Record<number, string> = {
  0: "", 1: "아이의 사주에 담긴 천성을 살펴봅니다", 2: "아이만의 특별한 재능을 찾아봅니다", 3: "아이에게 맞는 학습법과 진로를 안내합니다", 4: "아이의 운이 펼쳐지는 시기를 확인합니다", 5: "건강하게 키우기 위한 주의사항입니다", 6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/saju_youare/youare-0.jpg",
  1: "/media/cards/saju_youare/youare-0.jpg",
  2: "/media/cards/saju_youare/youare-0.jpg",
  3: "/media/cards/saju_youare/youare-0.jpg",
  4: "/media/cards/saju_youare/youare-0.jpg",
  5: "/media/cards/saju_youare/youare-0.jpg",
  6: "/media/cards/saju_youare/youare-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isYOUAREChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/saju_youare/youare-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: PINKD }}>✦ 유아사주 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, onRetry, onNext }: {
  ...
  const sectionKeys = YOUARE_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isYOUAREChapterReady(content, ch) : false;
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: PINKD }}>✦ 유아사주 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/saju_youare/youare-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>유아사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/youare-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date) {
      fetch("/api/youare-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          setResultId(rid);
          if (rid) {
            router.replace(`/saju/saju_youare/report-preview?id=${rid}&name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(YOUARE_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isYOUAREChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/saju_youare/report-preview?${base}&ch=${n}`);
  };
