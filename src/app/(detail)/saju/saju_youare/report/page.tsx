...
//
// 미리보기: http://localhost:3000/saju/saju_youare/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/saju_youare-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/saju_youare/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/saju_youare/report?${id ? `id=${id}&` : ""}ch=${n}`);
