...
const WHITE    = "#ffffff";
const RED      = "#e4d1b2";
const RED_SOFT = "#c9b68a";
const RED_PALE = "#fdf9f2";
const ROSE     = "#c9b68a";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
...
const FAQS = [
  ...
  { q: "아이 생년월일 시간을 모르면요?", a: "시주까지 입력할 수 있으나, 시간을 모르는 경우도 분석이 가능하오. 다만 시주 관련 항목의 정확도는 다소 낮을 수 있소." },
  { q: "어떤 항목을 분석하나요?", a: "타고난 기질, 건강, 양육 방식, 오행 균형, 이름·색·방향, 재능, 주의 시기 등 총 8장에 걸쳐 상세히 풀이하오." },
];
          ...
          <div key={i} className="rounded-2xl overflow-hidden"
            ...
            <button
              ...
              <span className="flex-shrink-0 text-[18px] font-light transition-transform duration-200"
                style={{ color: "#7a6030", transform: openIdx === i ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
            </button>
            {openIdx === i && (
...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCT = { name: "유아사주", original: 39800, discount: 50, price: 19900 };
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(228,209,178,0.16)", border: `1px solid ${ACCENT}55`, color: "#e4d1b2" }}>
            ...
            <div className="w-full text-left rounded-2xl px-4 py-3.5"
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 아이의 이야기</p>
                </div>
          ...
          <button onClick={onConfirm}
            className="w-full py-4 rounded-2xl font-black text-[17px] active:scale-[0.99] transition-transform"
            style={{ background: "linear-gradient(135deg, #e4d1b2, #c9b68a)", color: "#1a1000", boxShadow: "0 6px 20px rgba(228,209,178,0.4)" }}>
            결제하기
          </button>
          <div className="flex items-center justify-center gap-2 mt-3.5">
            <span className="flex-shrink-0 flex items-center justify-center rounded"
              style={{ width: 16, height: 16, background: ACCENT, color: "#1a1000", fontSize: 10 }}>✓</span>
      ...
      {confirmExit && (
        <div className="fixed z-[60] px-6"
          ...
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)}
              className="w-full mt-3 py-2.5 rounded-xl text-[13.5px] font-bold"
              style={{ background: "linear-gradient(135deg, #e4d1b2, #c9b68a)", color: "#1a1000" }}>혜택 받고 계속하기</button>
...
const TIME_COLORS: Record<string, string> = {
  "방금": "#e4d1b2", "방금 전": "#e4d1b2",
  "1분 전": "#c9b68a", "2분 전": "#c9b68a",
      ...
      <div style={{ position: "fixed", bottom: 100, right: "max(8px, calc(50vw - 232px))", zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, pointerEvents: "none" }}>
        {toasts.map((t) => (
          <div key={t.id} style={{ animation: "toastInRight 0.35s ease", display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(0,0,0,0.7)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 9999, padding: "6px 14px", fontSize: 12, color: "#fff", whiteSpace: "nowrap", backdropFilter: "blur(6px)" }}>
            <span style={{ backgroundColor: TIME_COLORS[t.time] ?? "#c9b68a", color: "#1a1000", fontSize: 10, fontWeight: 700, borderRadius: 9999, padding: "2px 7px" }}>{t.time}</span>
...
function StickyPayCTA({ onPay }: { onPay: () => void }) {
  ...
  return (
    <div className="fixed bottom-0 flex items-center gap-3 px-4 z-50"
      ...
      <div className="flex flex-col items-start flex-shrink-0">
        ...
        <span className="text-[18px] font-black tabular-nums" style={{ color: "#ffffff" }}>{timeLeft}</span>
      </div>
      {/* 구분선 */}
      <div style={{ width: 1, height: 36, backgroundColor: "rgba(255,255,255,0.15)", flexShrink: 0 }} />
      {/* 오른쪽: 버튼 */}
      <style>{`
        @keyframes youareBtnNeon {
          0%   { background: #e4d1b2; box-shadow: 0 0 12px 3px rgba(228,209,178,0.7); }
          33%  { background: #c9b68a; box-shadow: 0 0 12px 3px rgba(201,182,138,0.7); }
          66%  { background: #f5edd8; box-shadow: 0 0 12px 3px rgba(245,237,216,0.7); }
          100% { background: #e4d1b2; box-shadow: 0 0 12px 3px rgba(228,209,178,0.7); }
        }
        @keyframes youareBtnBeat {
      ...
      <button onClick={onPay}
        className="flex-1 py-3.5 rounded-2xl font-black text-[18px] flex items-center justify-center"
        style={{ color: "#1a1000", animation: "youareBtnNeon 3s ease-in-out infinite, youareBtnBeat 2s ease-in-out infinite" }}>
        유아사주 확인하기
      </button>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #e4d1b2aa,0 0 20px 4px #c9b68a55} 50%{box-shadow:0 0 16px 4px #f5edd8cc,0 0 40px 10px #e4d1b288} }
...
const PRODUCT_INFO = { name: "유아사주", price: 19900 };
...
function CheckoutContent() {
  ...
  const buildOrderBody = useCallback(() => {
    ...
    const birthTime = time === "시간 모름" ? null : time;
    const timeUnknown = time === "시간 모름";
    const calApi = calendar === "음력" ? "lunar" : "solar";
    const genderApi: "male" | "female" = gender === "여자" || gender === "여성" || gender === "female" || gender === "여아" ? "female" : "male";
  ...
  useEffect(() => {
    ...
    fetch("/api/orders/create-guest", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
      .then(r => r.json())
    ...
    <div className="w-full h-full" style={{ backgroundColor: WHITE }}>
      <div className="w-full h-full overflow-y-auto" style={{ scrollbarWidth: "none", paddingBottom: 80 }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_youare/s1.jpg" alt="" className="w-full block" />
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "26%", left: "69%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 보호자님,`}</p>
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "54.8%", left: "42%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>자녀분 사주풀이에 앞서,</p>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 사주팔자가`}</p>
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "95.5%", left: "60%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            ...
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 사주팔자요.`}</p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <img src="/media/checkout/saju_youare/s2.jpg" alt="" className="w-full block" />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "13.1%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"} 사주가 과연`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>신강한지 신약한지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "19.2%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"} 사주로 그린`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>사주화를 보여주며 시작되오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "28.25%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 사주에`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>어떤 능력과 재능이 있는지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "34.95%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}에게 필요한 기운과`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>피해야할 기운이 어떤건지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "48.0%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 건강관리 팁과`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>부위별 건강운까지 알려드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "57.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}에게 어떤 환경이`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>가장 잘 맞는지.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "66.9%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}에게`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>숨겨진 재능이 무엇인지.</p>
          </div>
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "4.6%", left: "66%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "91.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>언제 더 세심하게</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>돌봐야 하는지도 짚어드리겠소.</p>
          </div>
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "82.3%", left: "35%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_youare/s3.jpg" alt="" className="w-full block" />
          <div className="absolute bottom-0 left-0 right-0 h-16 pointer-events-none" style={{ background: `linear-gradient(to bottom, transparent, ${WHITE})` }} />
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "2.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}${gender === "female" || gender === "여자" || gender === "여성" || gender === "여아" ? "양" : "군"}의 모든것에 대해`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>낱낱히 파헤쳐 드리겠소.</p>
          </div>
        </div>
      ...
      <ToastLayer />
      <StickyPayCTA onPay={() => setShowSheet(true)} />
          ...
          <div className="fixed bottom-0 z-[60] rounded-t-3xl overflow-hidden"
            ...
            <div className="toss-widget-wrap">
              <TossWidget
                ...
                successUrl={`${typeof window !== 'undefined' ? window.location.origin : 'https://www.hongyeondang.com'}/saju/saju_youare/checkout/success`}
...
export default function SajuYouareCheckoutPage() {
  return (
    <Suspense fallback={
