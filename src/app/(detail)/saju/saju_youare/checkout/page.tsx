...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch("/api/saju_youare-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, date, time, calendar, gender, email }) });
      const d = res.ok ? await res.json() : null;
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/saju_youare/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/saju_youare/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/saju_youare/report?id=${d.resultId}` : `/saju/saju_youare/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/saju_youare/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
