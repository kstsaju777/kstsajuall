  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-end text-[11px] mb-2" style={{ color: "#c9b68a" }}>
          <span>{pct}%</span>
        </div>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const imageTask = fetch("/api/saju_youare-report", {
      ...
      const poll = async () => {
        for (let i = 0; i < 150; i++) { // 최대 5분(2초 간격)
          if (cancelled) return;
          try {
            const r = await fetch(`/api/saju_youare-report?id=${encodeURIComponent(resultId)}`);
