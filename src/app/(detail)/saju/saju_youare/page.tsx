...
function StickyCTA() {
  ...
  return (
    <div className="fixed bottom-0 z-40 px-5 pb-6" style={{ left:"max(0px,calc(50vw - 240px))", width:"min(100%,480px)", paddingTop:"180px", background:"linear-gradient(to top, #0a0a0a 65%, transparent)" }}>
      ...
      <style>{`@keyframes btnNeon { 0%,100% { background:#e4d1b2; box-shadow:0 0 12px 3px #e4d1b288; } 50% { background:#b09060; box-shadow:0 0 12px 3px #b0906088; } }`}</style>
      <button onClick={()=>router.push("/saju/saju_youare/form")} className="w-full py-2 rounded-2xl font-bold active:scale-95 transition-transform" style={{color:"#000",fontSize:"22px",animation:"btnNeon 3s ease-in-out infinite"}}>
        유아사주 보러가기
      </button>
    </div>
  );
