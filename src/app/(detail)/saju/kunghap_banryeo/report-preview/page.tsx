...
import { BANRYEO_CHAPTER_SECTIONS, isBanryeoChapterReady } from "@/lib/saju/banryeo-report-content";
...
const TOTAL_CH = Object.keys(BANRYEO_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "반려궁합 들어가며", 1: "두 사람의 타고난 기질", 2: "반려 관계 궁합 분석", 3: "상대방의 마음", 4: "관계 시기와 운의 흐름", 5: "관계를 방해하는 요인", 6: "홍연의 편지",
};

const CH_SUBS: Record<number, string> = {
  0: "", 1: "두 사람이 함께하는 이유, 사주에 담겨 있어요", 2: "두 사람의 반려 궁합을 깊이 살펴봅니다", 3: "지금 이 순간, 상대방의 마음속을 들여다봅니다", 4: "두 사람의 관계가 깊어지는 때를 찾습니다", 5: "관계를 가로막는 것들, 미리 알고 피하세요", 6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  1: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  2: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  3: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  4: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  5: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
  6: "/media/cards/kunghap_banryeo/banryeo-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isBanryeoChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/kunghap_banryeo/banryeo-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: MAROON }}>✦ 반려궁합 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, partnerName, onRetry, onNext }: {
  ...
  const sectionKeys = BANRYEO_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isBanryeoChapterReady(content, ch) : false;
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: MAROON }}>✦ 반려궁합 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/kunghap_banryeo/banryeo-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>반려궁합 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/banryeo-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date && partnerDate) {
      fetch("/api/banryeo-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          if (rid) {
            ...
            router.replace(`/saju/kunghap_banryeo/report-preview?id=${rid}&${base}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(BANRYEO_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isBanryeoChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/kunghap_banryeo/report-preview?${base}&ch=${n}`);
  };
