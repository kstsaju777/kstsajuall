...
export default function BanryeoPage() {
  ...
  return (
    <div className="w-full relative" style={{height:"100dvh",backgroundColor:BG,overflow:"hidden"}}>
      ...
      `}</style>
      {[
        {top:"72px",left:"5%",appearDelay:"1s",floatDelay:"1.6s",floatAnim:"floatA",name:"박*현",stars:5,text:"강아지랑 저랑 궁합이 이렇게 잘 맞는 줄 몰랐어요 🐾"},
        {top:"24%",left:"52%",appearDelay:"2s",floatDelay:"2.6s",floatAnim:"floatB",name:"이*진",stars:5,text:"반려묘 입양 전에 봤는데 정말 딱 맞는 아이 만났어요!"},
        {top:"45%",left:"4%",appearDelay:"3s",floatDelay:"3.6s",floatAnim:"floatC",name:"김*서",stars:5,text:"왜 우리 애가 나만 따르는지 이유를 알게 됐어요 ㅎㅎ"},
      ].map((r,i)=>(
      ...
      <div style={{position:"fixed",bottom:"240px",right:"max(8px,calc(50vw - 232px))",zIndex:50,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,pointerEvents:"none"}}>
        {toasts.map(t=>(
