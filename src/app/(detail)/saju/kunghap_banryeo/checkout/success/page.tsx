  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #d49030aa,0 0 20px 4px #e8c07055} 50%{box-shadow:0 0 16px 4px #e8c070cc,0 0 40px 10px #d4903088} }
      ...
      <div className="relative w-20 h-20 mb-6">
        <div className="absolute inset-0 rounded-full" style={{ background: "radial-gradient(circle, #d4903022 0%, transparent 70%)" }} />
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            ...
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#d49030" : "#e8c070"}`,
      ...
      <p className="text-[18px] font-bold mb-1" style={{ color: "#fff5f5", fontFamily: "'Noto Serif KR', serif", textShadow: "0 0 20px #d4903088" }}>
        결과지를 완성하고 있소…
      </p>
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#e8c070", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#e8c070" }}>
        ...
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#1a1000" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #6b4010, #d49030, #e8c070)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(212,144,48,0.15)", border: "1px solid rgba(212,144,48,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#e8c070" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#e8c070" }}>
...
export default function KunghapBanryeoCheckoutSuccessPage() {
  return (
    <Suspense>
      <SuccessInner />
    </Suspense>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const confirmRes = await fetch("/api/kunghap_banryeo/payment-confirm", {
