  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      ...
      <div className="px-6 pt-10 pb-5 text-center">
        <p className="text-[11px] tracking-[0.2em] mb-3 font-medium" style={{ color: RED }}>반려궁합 · 정밀 리포트</p>
      ...
      <div className="relative overflow-hidden w-full" style={{ aspectRatio: "9/16" }}>
        ...
        <video
          src="/media/cards/kunghap_banryeo/banryeo-0.mp4"
...
const PREVIEW_SECTIONS = [
  {
    icon: "🐾", title: "두 사람의 반려 인연", free: true,
    content: "각자의 일간과 오행 구성을 바탕으로 두 사람의 타고난 성향과 기질을 분석합니다. 서로의 차이가 갈등을 만드는지, 아니면 오히려 보완이 되는지 사주 속에 답이 있습니다.\n\n함께 살아갈 때 빛나는 순간, 그리고 오래도록 곁에 머물 수 있는 이유까지 사주는 조용히 기록하고 있습니다.",
    blurLines: [],
  },
  {
    icon: "🏠", title: "동반자 궁합 분석", free: false,
    content: "",
    blurLines: [
      "두 사람의 ████간 상생·상극 관계가 동반자 궁합의 핵심입니다.",
      "████년 ████월, 운의 흐름이 두 사람을 더 가깝게 합니다.",
      "이 시기의 선택은 사주상 ████한 영향을 받고 있습니다.",
    ],
  },
  {
    icon: "🐶", title: "반려 인연의 깊이", free: false,
    content: "",
    blurLines: [
      "상대방의 일간 기준으로 당신은 ████에 해당합니다.",
      "현재 상대방의 마음속에 당신은 ████한 존재로 남아있습니다.",
      "함께하는 일상을 깊게 하려면 ████한 접근이 효과적입니다.",
    ],
  },
  {
    icon: "📅", title: "동반 시기와 운의 흐름", free: false,
    content: "",
    blurLines: [
...
const REVIEWS = [
  ...
  { star: 5, text: "반려 인연이 딱 맞았어요. 홍연이 알려준 대로 접근했더니 관계가 더 깊어졌습니다.", name: "20대 대학생 이○○", date: "2025.04.28" },
...
const FAQS = [
  ...
  { q: "어떤 궁합 항목을 분석하나요?", a: "두 사람의 기질 분석, 반려 인연의 깊이, 동반자 궁합, 동반 시기, 관계를 방해하는 요인 등 총 14장에 걸쳐 상세히 풀이하오." },
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          ...
          <div className="space-y-1.5 mb-4">
            ...
            <div className="flex items-center justify-between text-[13px]">
              <span style={{ color: "#b47221", fontWeight: 700 }}>지금 결제 시 할인 ({PRODUCT.discount}% 특가)</span>
              <span style={{ color: "#b47221", fontWeight: 700 }}>-{saved.toLocaleString()}</span>
            </div>
          </div>
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        router.push(`/saju/kunghap_banryeo/report-preview?${new URLSearchParams({ name, gender, partnerName, partnerGender }).toString()}`);
        return;
      }
