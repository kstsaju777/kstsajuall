...
const WHITE    = "#ffffff";
const RED      = "#b47221";
const RED_SOFT = "#b47221";
const RED_PALE = "#fdf6ec";
const ROSE     = "#d4a06a";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
...
const SECTIONS = [
  {
    icon: "🔮", title: "두 사람의 타고난 기질과 궁합", free: true,
    content: "각자의 일간과 오행 구성을 바탕으로 두 사람의 타고난 성향과 기질을 분석합니다. 서로의 차이가 갈등을 만드는지, 아니면 오히려 보완이 되는지 사주 속에 답이 있습니다.\n\n처음 만났을 때 끌렸던 이유, 그리고 함께할수록 더 좋아지는 부분까지 사주는 조용히 담고 있습니다.",
    blurLines: ["", "", ""],
  },
  {
    icon: "💘", title: "두 사람이 끌리는 사주적 이유", free: false,
    content: "",
    blurLines: [
      "두 사람의 ████간 관계가 서로를 끌어당기는 핵심입니다.",
      "████의 오행이 상대방의 ████을 자극하고 있습니다.",
      "이 끌림은 사주상 ████한 인연으로 분석됩니다.",
    ],
  },
  {
    ...
    blurLines: [
      ...
      "현재 상대방의 마음속에 당신은 ████한 존재입니다.",
      "████한 방식으로 다가가면 더 깊은 신뢰를 얻을 수 있습니다.",
    ],
  },
  {
    icon: "📅", title: "함께하기 좋은 시기와 주의할 시기", free: false,
    content: "",
    blurLines: [
      "두 사람의 운이 가장 잘 맞는 시기는 ████년 ████월입니다.",
      "현재 두 사람의 대운은 ████으로 ████한 기운이 작용합니다.",
      "████년 ████월은 갈등이 생기기 쉬운 시기이니 조심하세요.",
    ],
  },
  {
    icon: "⚡", title: "반복되는 갈등 패턴과 해결법", free: false,
    content: "",
    blurLines: [
      "두 사람 사이에서 반복되는 갈등은 ████ 때문입니다.",
      "████ 방식의 소통은 오히려 관계를 악화시킵니다.",
      "████한 접근이 두 사람의 관계를 더 깊게 만들어 줍니다.",
    ],
  },
];
...
// ─── 후기 ─────────────────────────────────────────────────────────────────────
const REVIEWS = [
  { star: 5, text: "남자친구랑 자꾸 같은 이유로 싸우는데 사주로 보니 진짜 이유가 있더라고요. 갈등 패턴을 알고 나니까 대화 방식이 바뀌었어요.", name: "30대 직장인 김○○", date: "2025.05.12" },
  { star: 5, text: "상대방이 나를 어떻게 생각하는지 사주로 보니 너무 정확해서 소름이었어요. 덕분에 더 자신있게 다가갈 수 있었습니다.", name: "20대 대학생 이○○", date: "2025.04.28" },
  { star: 5, text: "두 사람이 끌리는 이유부터 함께하면 좋은 시기까지 나와서 정말 도움됐어요. 이 분석 보고 나서 관계가 훨씬 편안해졌습니다.", name: "30대 자영업자 박○○", date: "2025.05.03" },
];
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(180,114,33,0.16)", border: `1px solid ${ACCENT}55`, color: "#d4a06a" }}>
            ...
            <div className="w-full text-left rounded-2xl px-4 py-3.5" style={{ backgroundColor: DCARD, border: `1.5px solid ${ACCENT}`, boxShadow: `0 0 0 3px ${ACCENT}22` }}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 반려궁합 이야기</p>
                </div>
          ...
          <div className="space-y-1.5 mb-4">
            ...
            <div className="flex items-center justify-between text-[13px]">
              <span style={{ color: "#b47221", fontWeight: 700 }}>지금 결제 시 할인 ({sel.discount}% 특가)</span>
          ...
          <button onClick={() => onConfirm(selected)} className="w-full py-4 rounded-2xl font-black text-[17px] text-white active:scale-[0.99] transition-transform"
            style={{ background: "#b47221", boxShadow: "0 6px 20px rgba(180,114,33,0.4)" }}>
            결제하기
          </button>
      ...
      {confirmExit && (
        <div className="fixed z-[60] px-6" style={{ left: "max(0px, calc(50vw - 240px))", width: "min(100%, 480px)", top: "34%", pointerEvents: "none" }}>
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)} className="w-full mt-3 py-2.5 rounded-xl text-[13.5px] font-bold text-white" style={{ background: "#b47221" }}>혜택 받고 계속하기</button>
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4"
      ...
      <div className="flex items-center justify-between mb-3 px-1">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold px-2 py-0.5 rounded-full"
            style={{ backgroundColor: "rgba(180,114,33,0.12)", color: RED_SOFT, border: `1px solid ${ROSE}` }}>특가 -50%</span>
        </div>
      ...
      <button onClick={onPay}
        ...
        <span>🔓</span>
        <span>{name}님 · {partnerName}님 반려궁합 확인하기</span>
      </button>
    </div>
  );
}
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        {/* ① 헤더 이미지 + 제목 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_banryeo/banryeo-0.jpg"
          bottomImgSrc="/media/cards/kunghap_banryeo/banryeo-0.jpg"
          label="반려궁합 · 정밀 리포트"
          headline={`${name}님과\n${partnerName}님의 인연,\n낱낱이 봤어요`}
          accentWord="낱낱이"
          bgColor={WHITE}
        ...
        <ImageDivider src="/media/cards/kunghap_banryeo/banryeo-0.jpg" bgColor={CREAM} />
