...
function FormShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <img
        src="/media/cards/kunghap_banryeo/banryeo-0.jpg"
        aria-hidden
...
// ─── Step 4: 반려동물 이름 ────────────────────────────────────────────────────
function StepPetName({ onPrev, onNext, petType, initial }: { onPrev: () => void; onNext: (name: string) => void; petType?: string; initial?: string }) {
  const [name, setName] = useState(initial ?? "");
  const label = petType ?? "반려동물";
  const valid = !!name.trim() && /^[가-힣ㄱ-ㅎㅏ-ㅣa-zA-Z\s]+$/.test(name.trim());
  return (
    <>
      <style>{`@keyframes slideUp { from { opacity:0; transform:translateY(24px); } to { opacity:1; transform:translateY(0); } }`}</style>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>{label} 이름</p>
        <h2 className="text-[20px] mb-6" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{label}의 </span>
          <span className="font-bold">이름을 알려주시오.</span>
        </h2>
        <input
          type="text"
          placeholder="예) 초코, 미미, 콩이..."
          autoFocus
          value={name}
      ...
      <BottomNav onPrev={onPrev} onNext={() => valid && onNext(name.trim())} nextLabel="다음으로" nextDisabled={!valid} />
    </>
  );
}
...
// ─── Step 1/3: 성별 + 생년월일 ────────────────────────────────────────────────
function StepGender({ onPrev, onNext, initial, isPartner, petType, initialName, petName }: { onPrev: () => void; onNext: (gender: string, date: string, time: string, calendar: string, name: string, neutered?: string) => void; initial?: string; isPartner?: boolean; petType?: string; initialName?: string; petName?: string }) {
  const partnerLabel = (isPartner && petName) ? petName : (petType ?? "상대방");
  const prefix = isPartner ? `${partnerLabel}의` : "보호자의";
  const partnerSubject = isPartner ? `${partnerLabel}${josa(partnerLabel, "이", "가")}` : "보호자가";
  ...
  const nameValid = initialName ? true : (!!name.trim() && /^[가-힣ㄱ-ㅎㅏ-ㅣ\s]+$/.test(name.trim()));
  const needsNeutered = isPartner && gender === "수컷";
  const neuteredValid = !needsNeutered || !!neutered;
  const dateValid = timeSelected && nameValid && neuteredValid;
  ...
  const handleGender = (g: string) => { setGender(g); setNeutered(""); setShowDate(false); setTimeout(() => setShowDate(true), 10); };
    ...
    <>
      ...
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{isPartner ? `${partnerLabel}의 ` : "보호자의 "}</span>
          <span className="font-bold">성별은 무엇이오?</span>
        </h2>
        <div className="flex gap-3 mb-4">
          {(isPartner ? ["수컷", "암컷"] : ["남성", "여성"]).map((label) => (
            <button key={label} onClick={() => handleGender(label)}
              className="flex-1 py-2 rounded-2xl text-[16px] font-bold transition-all"
              style={{
                backgroundColor: gender === label ? "rgba(180,114,33,0.15)" : "rgba(255,255,255,0.04)",
        ...
        {needsNeutered && (
          <div style={{ animation: "slideUp 0.35s ease" }}>
            ...
            <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>중성화 여부</p>
            <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
              <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{partnerLabel}의 </span>
              <span className="font-bold">중성화 여부는?</span>
            </h2>
            <div className="flex gap-2 mb-4">
              {["했어요 ○", "안 했어요 ✕", "관계없음"].map((opt) => (
                <button key={opt} onClick={() => setNeutered(opt)}
                  className="flex-1 py-2 rounded-2xl text-[13px] font-bold transition-all"
                  style={{
                    backgroundColor: neutered === opt ? "rgba(180,114,33,0.15)" : "rgba(255,255,255,0.04)",
                    border: `2px solid ${neutered === opt ? NAVY : "rgba(255,255,255,0.1)"}`,
                    color: neutered === opt ? "#fff" : "rgba(255,255,255,0.6)",
                  }}>
                  {opt}
                </button>
              ))}
        ...
        {showDate && neuteredValid && (
          <div style={{ animation: "slideUp 0.35s ease" }}>
            ...
            <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
              {isPartner ? (
                <><span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{partnerSubject} </span><span className="font-bold">태어난 날짜는?</span></>
              ) : (
                <><span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>보호자가 </span><span className="font-bold">태어난 날짜는?</span></>
              )}
            </h2>
            {isPartner && (
              <p className="text-[12px] mb-3" style={{ color: "rgba(255,255,255,0.38)", lineHeight: 1.6 }}>
                생일을 모른다면, 입양일이나 처음 만난 날을 적어도 되오.
              </p>
            )}
            ...
            <div className="flex gap-2 mb-3">
              {["양력", "음력", "윤달"].map((c) => (
                <button key={c} onClick={() => setCalendar(c)}
                  ...
                  style={{
                    backgroundColor: calendar === c ? "rgba(180,114,33,0.15)" : "rgba(255,255,255,0.04)",
            ...
            {dateEntered && (
              <div style={{ animation: "slideUp 0.35s ease" }}>
                ...
                <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
                  {isPartner ? (
                    <><span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{partnerSubject} </span><span className="font-bold">태어난 시간은?</span></>
                  ) : (
                    <><span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>보호자가 </span><span className="font-bold">태어난 시간은?</span></>
                  )}
                </h2>
                <div className="flex gap-3">
                  <div className="flex-1 relative">
                    <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold transition-all flex items-center justify-between"
                      ...
                      style={{
                        backgroundColor: (btime && btime !== "모름") ? "rgba(180,114,33,0.15)" : "rgba(255,255,255,0.04)",
                      ...
                      <>
                        ...
                        <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                          style={{
                            ...
                            backgroundColor: "rgba(18,20,26,0.92)", backdropFilter: "blur(16px)",
                            border: "1px solid rgba(255,255,255,0.12)",
                            animation: "slideUp 0.25s ease", maxHeight: 260, overflowY: "auto",
                          }}>
                          {[
                            ...
                            return (
                              <button key={t.label} className="w-full px-4 py-3 flex items-center justify-between transition-all"
                                ...
                                style={{ backgroundColor: selected ? "rgba(180,114,33,0.18)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <button onClick={() => { setBtime("모름"); setTimeOpen(false); }}
                    ...
                    style={{
                      backgroundColor: btime === "모름" ? "rgba(180,114,33,0.15)" : "rgba(255,255,255,0.04)",
      ...
      <BottomNav onPrev={onPrev} onNext={() => gender && dateValid && onNext(gender, dateStr, btime, calendar, name.trim(), neutered || undefined)} nextLabel="다음으로" nextDisabled={!gender || !dateValid} />
    </>
  );
}
...
function StepPetType({ onPrev, onNext, initial }: { onPrev: () => void; onNext: (v: string) => void; initial?: string }) {
  const [selected, setSelected] = useState<string | null>(initial ?? null);
  const [customPet, setCustomPet] = useState("");
  const isEtc = selected === "기타";
  const finalValue = isEtc ? (customPet.trim() || null) : selected;
  return (
    <>
      <div className="px-6 pt-4 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>반려동물</p>
        <h2 className="text-[20px] mb-4" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>어떤 </span>
          <span className="font-bold">반려동물이오?</span>
        </h2>
        <div className="grid grid-cols-4 gap-2">
          {PET_TYPES.map(({ emoji, label }) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: `1.5px solid ${active ? NAVY : "rgba(255,255,255,0.12)"}`,
                }}
              >
                <span style={{ fontSize: 26 }}>{emoji}</span>
                <span className="text-[12px] font-semibold mt-1" style={{ color: active ? "#fff" : "rgba(255,255,255,0.6)" }}>{label}</span>
              </button>
            );
          })}
        </div>
        {isEtc && (
          <div className="mt-4" style={{ animation: "slideUp 0.3s ease" }}>
            <p className="text-[12px] font-medium mb-1" style={{ color: "#8a8a8a" }}>반려동물 종류를 적어주시오</p>
            <input
              type="text"
              placeholder="예) 이구아나, 미니피그..."
              autoFocus
              value={customPet}
              onChange={(e) => setCustomPet(e.target.value)}
              className="w-full bg-transparent text-[18px] font-bold pb-1 outline-none"
              style={{ borderBottom: customPet ? `2px solid ${NAVY}` : `2px solid ${BORDER_CLR}`, color: TEXT_CLR, caretColor: NAVY }}
            />
          </div>
        )}
      </div>
      <BottomNav onPrev={onPrev} onNext={() => finalValue && onNext(finalValue)} nextLabel="다음으로" nextDisabled={!finalValue} />
    </>
  );
}
...
function StepConcern({ onPrev, onSubmit, initial, date, btime, calendar, name, isPartner }: {
  ...
  const timeUnknown = !btime || btime === "모름";
  const saju = useMemo(() => {
    if (!date) return null;
    try { return calcSaju(date, timeUnknown ? "모름" : btime!, calendar ?? "양력"); } catch { return null; }
  }, [date, btime, calendar, timeUnknown]);
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        {name && (
          <div className="mb-2">
            <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>{isPartner ? "반려동물의" : "보호자의"} 사주팔자</p>
            <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
              <span className="font-bold" style={{ color: TEXT_CLR }}>{name}{isPartner ? "" : "님"}의 </span>
            ...
            <div className="grid grid-cols-4 gap-2 rounded-2xl p-4" style={{ backgroundColor: "rgba(255,255,255,0.05)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.08)" }}>
              {(pillars ?? Array(4).fill(null)).map((p, i) => {
                const isUnknownTime = i === 0 && timeUnknown;
                return (
                  <div key={i} className="flex flex-col items-center gap-1">
                    <p className="text-[15px] font-medium tracking-wide" style={{ color: "#8a8a8a" }}>{PILLAR_LABELS_JW[i]}</p>
                    <span className="text-[15px]" style={{ color: LABEL_CLR }}>{isUnknownTime ? "—" : (p?.stemSs || "")}</span>
                    <div className="w-full rounded-2xl flex items-center justify-center overflow-hidden" style={{ aspectRatio: "1", backgroundColor: "rgba(255,255,255,0.06)" }}>
                      {isUnknownTime
                        ? <img src={ganCharImage("—")} alt="—" style={{ width: "80%", height: "80%", objectFit: "contain" }} />
                    ...
                    <div className="w-full rounded-2xl flex items-center justify-center overflow-hidden" style={{ aspectRatio: "1", backgroundColor: "rgba(255,255,255,0.06)" }}>
                      {isUnknownTime
                        ? <img src={jiCharImage("—")} alt="—" style={{ width: "80%", height: "80%", objectFit: "contain" }} />
                    ...
                    <span className="text-[15px]" style={{ color: LABEL_CLR }}>{isUnknownTime ? "—" : (p?.branchSs || "")}</span>
                  </div>
                );
              })}
            </div>
...
// ─── 미니 명식 ────────────────────────────────────────────────────────────────
function MiniMyeongsik({ date, time, calendar, name, showNim }: { date?: string; time?: string; calendar?: string; name?: string; showNim?: boolean }) {
  const timeUnknown = !time || time === "모름";
  const saju = useMemo(() => {
    if (!date) return null;
    try { return calcSaju(date, timeUnknown ? "모름" : time!, calendar ?? "양력"); } catch { return null; }
  }, [date, time, calendar, timeUnknown]);
  const pillars = saju ? [saju.pillars.time, saju.pillars.day, saju.pillars.month, saju.pillars.year] : null;
  return (
    <div className="flex-1">
      <p className="text-[10px] font-bold mb-1 text-center px-2 py-0.5 rounded-full self-center w-fit mx-auto" style={{ backgroundColor: "rgba(180,114,33,0.18)", color: "#d4a06a" }}>{showNim ? "보호자" : "반려동물"}</p>
      <p className="text-[14px] font-bold mb-2 text-center" style={{ color: TEXT_CLR }}>{name ?? "—"}{showNim ? "님" : ""} 사주팔자</p>
        ...
        {(pillars ?? Array(4).fill(null)).map((p, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <span style={{ fontSize: 12, color: LABEL_CLR, lineHeight: 1 }}>{isUnknownTime ? "—" : (p?.stemSs || " ")}</span>
              <div className="w-full flex items-center justify-center" style={{ aspectRatio: "1" }}>
                {isUnknownTime
                  ? <img src={ganCharImage("—")} alt="—" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
              ...
              <div className="w-full flex items-center justify-center" style={{ aspectRatio: "1" }}>
                {isUnknownTime
                  ? <img src={jiCharImage("—")} alt="—" style={{ width: "100%", height: "100%", objectFit: "contain" }} />
              ...
              <span style={{ fontSize: 12, color: LABEL_CLR, lineHeight: 1 }}>{isUnknownTime ? "—" : (p?.branchSs || " ")}</span>
            </div>
          );
        })}
      </div>
    ...
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <div className="flex gap-2 mb-5">
          <MiniMyeongsik date={myDate} time={myTime} calendar={myCalendar} name={myName} showNim />
        ...
        <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{partnerName ? `${partnerName}${josa(partnerName, "과", "와")}의 궁합, ` : "반려동물과의 궁합, "}</span>
          <span className="font-bold">궁금한 점이 있소?</span>
        </h2>
        <p className="text-[12px] mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>자세히 적을수록 풀이가 깊어지오. (없어도 괜찮소.)</p>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value.slice(0, MAX))}
          placeholder="예) 강아지가 유독 낯을 가리고 예민한데 저랑 궁합이 잘 맞는 건지, 이 아이가 우리 집에 오게 된 게 인연인지 궁금해요."
          rows={5}
      ...
      <BottomNav onPrev={onPrev} onNext={() => onSubmit(text)} nextLabel={filled ? "입력했어요" : "궁금한 건 없소"} />
    </>
  );
}
          ...
          <div className="relative" style={{ flex: "1.2 1 0", minWidth: 0 }}>
              ...
              <div className="absolute bottom-full left-0 right-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                ...
                {EMAIL_DOMAINS.map((d) => (
                  <div key={d} onClick={() => { setDomain(d); setOpen(false); }}
                    ...
                    style={{ backgroundColor: domain === d ? "rgba(180,114,33,0.25)" : "transparent", color: domain === d ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {d}
                  </div>
                ))}
              </div>
            ...
            {isCustom ? (
              <div className="flex items-center" style={{ borderBottom: `1.5px solid ${BORDER_CLR}` }}>
                <input type="text" placeholder="직접입력" value={custom} onChange={(e) => setCustom(e.target.value)}
...
function StepLoading({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, concern }: {
  ...
  const goNext = () => {
    ...
    router.push(`/saju/kunghap_banryeo/checkout?${params.toString()}`);
  };
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ backgroundColor: "#0a0a0a" }}>
      ...
      <img src="/media/cards/kunghap_banryeo/banryeo-0.jpg" className="absolute inset-0 w-full h-full object-cover" style={{ filter: "blur(8px)", transform: "scale(1.1)" }} />
      ...
      <div className="absolute bottom-0 left-0 right-0 px-6 pb-12">
        <p className="text-center text-[15px] font-bold mb-3" style={{ color: "#ffffff" }}>
          사주팔자 정밀분석중... <span style={{ color: "#b47221" }}>{pct}%</span>
        </p>
        <div className="w-full h-2.5 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.2)" }}>
          <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: "#b47221", transition: "width 0.2s" }} />
        </div>
      </div>
    </div>
  );
    ...
    <>
      {((step >= 1 && step <= 7) || step === 9) && (
        <FormShell>
          ...
          {step === 3 && (
            <StepPetType initial={form.petType} onPrev={() => setStep(2)} onNext={(petType) => next({ petType }, 4)} />
          )}
          {step === 4 && (
            <StepPetName petType={form.petType} initial={form.partnerName} onPrev={() => setStep(3)} onNext={(partnerName) => next({ partnerName }, 5)} />
          )}
          {step === 5 && (
            <StepGender isPartner petType={form.petType} petName={form.partnerName} initialName={form.partnerName} initial={form.partnerGender} onPrev={() => setStep(4)} onNext={(gender, date, time, calendar, _name, neutered) => next({ partnerGender: gender, partnerDate: date, partnerTime: time, partnerCalendar: calendar, partnerNeutered: neutered ?? "" }, 6)} />
          )}
          {step === 6 && (
            <StepConcern isPartner initial="" onPrev={() => setStep(5)} onSubmit={() => next({}, 7)}
              date={form.partnerDate} btime={form.partnerTime} calendar={form.partnerCalendar} name={form.partnerName} />
          )}
          {step === 7 && (
            <StepConcernInput initial={form.concern} onPrev={() => setStep(6)} onSubmit={(concern) => next({ concern }, 8)}
