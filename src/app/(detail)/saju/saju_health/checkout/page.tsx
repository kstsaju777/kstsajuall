...
const PRODUCT = { name: "건강운사주", original: 49800, discount: 50, price: 24900 };
  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      <div className="relative overflow-hidden" style={{ height: 240 }}>
        <img src="/media/cards/saju_health/health-0.jpg" alt=""
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>건강운사주 결제 안내</h3>
          ...
          <div className="w-full text-left rounded-2xl px-4 py-3.5 mb-5" style={{ backgroundColor: DCARD, border: `1.5px solid ${PINK_D}`, boxShadow: `0 0 0 3px ${PINK_D}22` }}>
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>건강운사주</span>
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    setCreating(true);
    try {
      const res = await fetch("/api/saju_health-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, date, time, calendar, gender, email }) });
      const d = res.ok ? await res.json() : null;
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/saju_health/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`;
      router.push(d?.resultId ? `/saju/saju_health/report?id=${d.resultId}` : `/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
