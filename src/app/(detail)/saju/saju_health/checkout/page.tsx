...
const WHITE    = "#ffffff";
const RED      = "#ca884b";
const RED_SOFT = "#d89a5a";
const RED_PALE = "#fdf6ee";
const ROSE     = "#d89a5a";
const GRAY1    = "#1a1a1a";
const GRAY2    = "#444444";
const GRAY3    = "#888888";
...
const FAQS = [
  ...
  { q: "어떤 건강 항목을 분석하나요?", a: "타고난 체질, 약한 신체 부위, 맞는 식습관·생활, 건강 흐름과 시기, 개운법 등 총 6장에 걸쳐 상세히 풀이하오." },
];
          ...
          <div key={i} className="rounded-2xl overflow-hidden"
            ...
            <button
              ...
              <span className="flex-shrink-0 text-[18px] font-light transition-transform duration-200"
                style={{ color: "#8a5c20", transform: openIdx === i ? "rotate(45deg)" : "rotate(0deg)" }}>+</span>
            </button>
            {openIdx === i && (
...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCT = { name: "건강사주", original: 39800, discount: 50, price: 19900 };
      ...
      <div className="fixed bottom-0 z-50 overflow-y-auto rounded-t-3xl"
        ...
        <div className="px-5 pt-2 pb-7">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-[18px] font-black" style={{ color: DTXT }}>건강사주 결제 안내</h3>
          ...
          <div className="inline-block text-[13px] font-bold px-3.5 py-1.5 rounded-full mb-5"
            style={{ background: "rgba(202,136,75,0.16)", border: `1px solid ${ACCENT}55`, color: "#d89a5a" }}>
          ...
          <div className="mb-5">
            <div className="w-full text-left rounded-2xl px-4 py-3.5"
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  <span className="text-[14.5px] font-bold" style={{ color: DTXT }}>건강사주</span>
                  <p className="text-[11.5px] mt-1" style={{ color: DMUTE }}>홍연이 들려주는 나의 건강 이야기</p>
                </div>
          ...
          <button onClick={onConfirm}
            ...
            style={{ background: "linear-gradient(135deg, #ca884b, #d89a5a)", boxShadow: "0 6px 20px rgba(202,136,75,0.4)" }}>
            결제하기
          </button>
        ...
        <div className="fixed z-[60] px-6"
          ...
          <div className="relative mx-auto rounded-2xl px-5 py-4"
            ...
            <button onClick={() => setConfirmExit(false)}
              ...
              style={{ background: "linear-gradient(135deg, #ca884b, #d89a5a)" }}>혜택 받고 계속하기</button>
...
const TIME_COLORS: Record<string, string> = {
  "방금": "#ca884b", "방금 전": "#ca884b",
  "1분 전": "#d89a5a", "2분 전": "#d89a5a",
      ...
      <div style={{ position: "fixed", bottom: 100, right: "max(8px, calc(50vw - 232px))", zIndex: 49, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, pointerEvents: "none" }}>
        {toasts.map((t) => (
          <div key={t.id} style={{ animation: "toastInRight 0.35s ease", display: "flex", alignItems: "center", gap: 6, backgroundColor: "rgba(0,0,0,0.7)", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 9999, padding: "6px 14px", fontSize: 12, color: "#fff", whiteSpace: "nowrap", backdropFilter: "blur(6px)" }}>
            <span style={{ backgroundColor: TIME_COLORS[t.time] ?? "#d89a5a", color: "#fff", fontSize: 10, fontWeight: 700, borderRadius: 9999, padding: "2px 7px" }}>{t.time}</span>
...
function StickyPayCTA({ onPay, name }: { onPay: () => void; name: string }) {
  ...
  return (
    <div className="fixed bottom-0 flex items-center gap-3 px-4 z-50"
      ...
      {/* 오른쪽: 버튼 */}
      <style>{`
        @keyframes healthBtnNeon {
          0%   { background: #ca884b; box-shadow: 0 0 12px 3px rgba(202,136,75,0.7); }
          33%  { background: #d89a5a; box-shadow: 0 0 12px 3px rgba(216,154,90,0.7); }
          66%  { background: #f5c97a; box-shadow: 0 0 12px 3px rgba(245,201,122,0.7); }
          100% { background: #ca884b; box-shadow: 0 0 12px 3px rgba(202,136,75,0.7); }
        }
        @keyframes healthBtnBeat {
      ...
      <button onClick={onPay}
        ...
        style={{ animation: "healthBtnNeon 3s ease-in-out infinite, healthBtnBeat 2s ease-in-out infinite" }}>
        건강사주 확인하기
      </button>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #ca884baa,0 0 20px 4px #d89a5055} 50%{box-shadow:0 0 16px 4px #f5c97acc,0 0 40px 10px #ca884b88} }
      ...
      <div className="relative w-20 h-20 mb-6">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span style={{ fontSize: 28, filter: "drop-shadow(0 0 8px #ca884b)" }}>✦</span>
        </div>
      </div>
      ...
      <p className="text-[11px] text-center leading-relaxed mt-4" style={{ color: "#886644" }}>
...
const PRODUCT_INFO = { name: "건강사주", price: 19900 };
    ...
    <div className="w-full h-full" style={{ backgroundColor: WHITE }}>
      <div className="w-full h-full overflow-y-auto" style={{ scrollbarWidth: "none", paddingBottom: 80 }}>
        ...
        <div className="relative">
          <img src="/media/checkout/saju_health/s1.jpg" alt="" className="w-full block" />
          ...
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "54.8%", left: "42%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
            <p style={{ fontSize: 22, fontWeight: 400, color: "#000000" }}>건강운 풀이에 앞서,</p>
        ...
        <div className="relative">
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "13.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "19.8%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>타고난 오행 균형이</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>건강에 어떻게 작용하는지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "31.3%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>건강에 도움되는 기운과</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>해로운 기운까지 짚어드리오.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "44.7%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님만을 위한`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>건강 처방을 준비해두었소.</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "54.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님 타고난 사주에 맞는`}</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>음식과 생활 루틴들까지</p>
          </div>
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "66%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>타고난 기운을 풀어낼</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님만의 개운법도 알려주겠소.`}</p>
          </div>
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "91.2%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>평생 흘러가는 건강운의</p>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>오르내림까지 짚어보겠소.</p>
          </div>
          <div className="absolute flex flex-col items-center gap-1 pointer-events-none" style={{ top: "81.7%", left: "35%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap" }}>
        ...
        <div className="relative">
          ...
          <div className="absolute flex flex-col items-center pointer-events-none" style={{ top: "2.5%", left: "50%", transform: "translate(-50%, -50%)", textAlign: "center", whiteSpace: "nowrap", fontFamily: "'GmarketSans', sans-serif", fontWeight: 700 }}>
            <p style={{ fontSize: 22, color: "#000000", lineHeight: 1.4 }}>{`${stripSurname(name)}님의 건강운에 대해`}</p>
          ...
          <div className="fixed bottom-0 z-[60] rounded-t-3xl overflow-hidden"
            ...
            <div className="toss-widget-wrap">
              <TossWidget
                ...
                successUrl={`${typeof window !== 'undefined' ? window.location.origin : 'https://www.hongyeondang.com'}/saju/saju_health/checkout/success`}
...
export default function SajuHealthCheckoutPage() {
  return (
    <Suspense fallback={
