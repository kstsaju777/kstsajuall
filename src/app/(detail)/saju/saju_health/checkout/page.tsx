...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok || !res.body) { router.push(`/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); return; }
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (data.done && data.resultId) { router.push(`/saju/saju_health/report?id=${data.resultId}`); return; }
            if (data.error) { router.push(`/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); return; }
          } catch { /* JSON 파싱 실패 무시 */ }
        }
      }
    } catch { router.push(`/saju/saju_health/report?${new URLSearchParams({ name, date, time, calendar, gender, email, ch: "0" }).toString()}`); }
  };
