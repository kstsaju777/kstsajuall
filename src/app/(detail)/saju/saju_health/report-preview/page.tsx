...
const TOTAL_CH = Object.keys(HEALTH_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "건강운사주 들어가며", 1: "나의 타고난 체질과 에너지", 2: "주의해야 할 건강 부위", 3: "건강 위험 시기", 4: "올바른 생활습관", 5: "건강을 방해하는 요인", 6: "홍연의 편지",
};

const CH_SUBS: Record<number, string> = {
  0: "", 1: "나의 사주에 담긴 체질과 건강 기운입니다", 2: "약한 장기와 만성 질환 위험을 살펴봅니다", 3: "건강이 흔들릴 수 있는 시기를 확인합니다", 4: "나의 체질에 맞는 생활습관을 안내합니다", 5: "건강을 가로막는 것들, 미리 알고 피하세요", 6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/saju_health/health-0.jpg",
  1: "/media/cards/saju_health/health-0.jpg",
  2: "/media/cards/saju_health/health-0.jpg",
  3: "/media/cards/saju_health/health-0.jpg",
  4: "/media/cards/saju_health/health-0.jpg",
  5: "/media/cards/saju_health/health-0.jpg",
  6: "/media/cards/saju_health/health-0.jpg",
};
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/saju_health/health-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: PINKD }}>✦ 건강운사주 리포트 ✦</p>
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: PINKD }}>✦ 건강운사주 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/saju_health/health-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>건강운사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
  ...
  useEffect(() => {
    ...
    if (id) {
      fetch(`/api/health-saju-report?id=${encodeURIComponent(id)}`)
        .then((r) => r.ok ? r.json() : Promise.reject())
        .then((d) => { setContent(d.content ?? {}); })
        .catch(() => {})
        .finally(() => setLoading(false));
    } else if (date) {
      fetch("/api/health-saju-report", {
        method: "POST",
        ...
        .then((d) => {
          ...
          setResultId(rid);
          if (rid) {
            router.replace(`/saju/saju_health/report-preview?id=${rid}&name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/saju_health/report-preview?${base}&ch=${n}`);
  };
