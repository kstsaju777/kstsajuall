...
const PINK      = "#ca884b";
const CARD_BG   = "#0d0905";
const BORDER_CLR = "#3d2a15";
const PH_CLR    = "#7a8590";
  ...
  return (
    <div className="relative w-full overflow-hidden" style={{ height: vh ? `${vh}px` : "100%", maxHeight: "100%", backgroundColor: "#0d0f14" }}>
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.3) 0%, rgba(0,0,0,0.15) 30%, rgba(13,9,5,0.55) 55%, rgba(13,9,5,0.92) 72%, rgba(13,9,5,1) 82%)" }} />
  ...
  return (
    <div className="flex items-center gap-3 px-5 pb-8 pt-4" style={{ backgroundColor: CARD_BG }}>
      ...
      <button onClick={onNext} disabled={nextDisabled} className="flex-1 py-3.5 rounded-2xl text-[16px] font-bold transition-all"
        style={{ backgroundColor: PINK, color: "#fff", opacity: nextDisabled ? 0.35 : 1, letterSpacing: "-0.3px" }}>{nextLabel}</button>
    </div>
  );
}
...
function StepIntro({ scenes, onNext }: { scenes: string[]; onNext: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center" style={{ backgroundColor: "#0d0905" }}>
      <img src={IMG_BG} className="absolute inset-0 w-full h-full object-cover object-top opacity-30" alt="" />
      <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(13,9,5,0.3) 0%, rgba(13,9,5,0.7) 60%, rgba(13,9,5,1) 100%)" }} />
          ...
          {["남성", "여성"].map((g) => {
            ...
            return (
              <button key={g} onClick={() => { setGender(g); setTimeout(() => onNext(g), 350); }}
                ...
                style={{
                  backgroundColor: active ? "rgba(202,136,75,0.18)" : "rgba(255,255,255,0.04)",
...
// ─── Step 2: 생년월일 ─────────────────────────────────────────────────────────
function StepBirthDate({ onPrev, onNext, gender, initial, initialCalendar }: {
  onPrev: () => void; onNext: (date: string, calendar: string) => void;
  gender?: string; initial?: string; initialCalendar?: string;
}) {
        ...
        <div className="flex gap-2 mb-5">
          {["양력", "음력"].map((c) => (
            <button key={c} onClick={() => setCalendar(c)}
              ...
              style={{
                backgroundColor: calendar === c ? "rgba(202,136,75,0.15)" : "rgba(255,255,255,0.04)",
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold flex items-center justify-between"
              ...
              style={{
                backgroundColor: (btime && btime !== "시간 모름") ? "rgba(202,136,75,0.15)" : "rgba(255,255,255,0.04)",
                ...
                <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                  ...
                  {BIRTH_TIMES.map((t) => (
                    <button key={t} className="w-full text-left px-4 py-2.5 text-[14px] transition-colors"
                      ...
                      style={{ backgroundColor: btime === t ? "rgba(202,136,75,0.15)" : "transparent", color: btime === t ? "#fff" : "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          <button className="px-4 py-3 rounded-xl text-[14px] font-bold"
            ...
            style={{
              backgroundColor: btime === "시간 모름" ? "rgba(202,136,75,0.15)" : "rgba(255,255,255,0.04)",
...
function StepConcern({ onPrev, onNext, initial }: {
  ...
  const MAX = 200;
  const PLACEHOLDER = `예) 허리가 자주 아프고 소화가 잘 안 되는데 사주와 관련이 있을까요? 건강 관리를 어떻게 해야 할지 알고 싶어요.`;
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>건강에 대해 </span>
        ...
        <style>{`textarea.health-concern::placeholder { color: rgba(232,184,130,0.3); }`}</style>
        <div className="rounded-2xl px-4 pt-4 pb-3" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(202,136,75,0.25)" }}>
          <textarea value={text} onChange={(e) => setText(e.target.value.slice(0, MAX))}
            placeholder={PLACEHOLDER} rows={5}
            className="health-concern w-full bg-transparent outline-none resize-none text-[13px] leading-relaxed"
              ...
              <div className="absolute bottom-full left-0 right-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                ...
                {EMAIL_DOMAINS.map((d) => (
                  <div key={d} onClick={() => { setDomain(d); setOpen(false); }}
                    ...
                    style={{ backgroundColor: domain === d ? "rgba(202,136,75,0.2)" : "transparent", color: domain === d ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {d}
                  </div>
                ))}
              </div>
                ...
                <div className="absolute bottom-full left-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                  ...
                  {PHONE_PREFIXES.map((p) => (
                    <div key={p} onClick={() => { setPhonePrefix(p); setPhoneOpen(false); }}
                      className="px-4 py-3 text-[14px] cursor-pointer"
                      style={{ backgroundColor: phonePrefix === p ? "rgba(202,136,75,0.2)" : "transparent", color: phonePrefix === p ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {p}
                    </div>
                  ))}
                </div>
    ...
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center" style={{ backgroundColor: "#080a0f" }}>
      ...
      <div className="relative z-10 px-8 w-full max-w-sm text-center">
        <div className="w-16 h-16 rounded-full mx-auto mb-5 flex items-center justify-center"
          style={{ backgroundColor: "rgba(202,136,75,0.15)", border: "2px solid rgba(202,136,75,0.4)" }}>
          <span style={{ fontSize: 28 }}>🌿</span>
        </div>
        <p className="text-[22px] font-black mb-1" style={{ color: "#fff" }}>{name}{suffix}의 건강사주</p>
  ...
  return (
    <>
      ...
      {[1, 2, 3, 4, 5, 6, 8].includes(step) && (
        <FormShell>
          {step === 1 && (
            <StepGender initial={form.gender} onNext={(gender) => next({ gender }, 2)} />
          )}
          {step === 2 && (
            <StepBirthDate
              onPrev={() => setStep(1)}
