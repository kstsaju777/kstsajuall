...
const IMG_BG    = "/media/cards/saju_jaemul/jaemul-0.jpg";
...
const GOLD_BTN_STYLE = `
  @keyframes goldGlow {
    0%, 100% { box-shadow: 0 0 12px 3px rgba(249,220,100,0.45), 0 0 28px 6px rgba(212,160,23,0.2), inset 0 1px 0 rgba(255,240,120,0.4); }
    50% { box-shadow: 0 0 22px 8px rgba(255,220,50,0.65), 0 0 48px 14px rgba(212,160,23,0.3), inset 0 1px 0 rgba(255,255,160,0.6); }
  }
  @keyframes shimmer {
...
function BottomNav({ onPrev, onNext, nextLabel, nextDisabled = false }: {
  ...
  return (
    <div className="flex items-center gap-3 px-5 pb-8 pt-4" style={{ backgroundColor: CARD_BG }}>
      ...
      {nextDisabled ? (
        <button disabled className="flex-1 py-3.5 rounded-2xl text-[16px] font-bold"
          style={{ backgroundColor: "#333", color: "#666", letterSpacing: "-0.3px" }}>{nextLabel}</button>
      ) : (
        <div style={{ flex: 1, padding: "2px", borderRadius: "18px", background: "linear-gradient(135deg, #ffe066 0%, #d4a017 30%, #fff5a0 50%, #d4a017 70%, #ffe066 100%)", boxShadow: "0 0 14px 3px rgba(212,160,23,0.35)" }}>
    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50" style={{ backgroundColor: "#080a0f" }}>
      ...
      <div className="relative z-10 px-8 text-center">
        <p className="text-[26px] font-bold leading-relaxed whitespace-pre-line" style={{ color: "#fff", minHeight: "2.2em" }}>
          {displayed}
          <span className="inline-block w-[2px] h-[1.1em] ml-1 align-middle animate-pulse" style={{ backgroundColor: GOLD, verticalAlign: "middle" }} />
        </p>
      </div>
    </div>
  );
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>성별</p>
        <h2 className="text-[24px] font-bold mb-6" style={{ color: TEXT_CLR }}>그대의 성별은 무엇이오?</h2>
          ...
          {["남성", "여성"].map((g) => {
            ...
            return (
              <button key={g} onClick={() => { setGender(g); setTimeout(() => onNext(g), 350); }}
                ...
                style={{
                  backgroundColor: active ? "rgba(249,220,100,0.12)" : "rgba(255,255,255,0.04)",
                  border: `1.5px solid ${active ? GOLD : "rgba(255,255,255,0.18)"}`,
                  color: active ? GOLD : "#dddddd",
...
function StepBirthDate({ onPrev, onNext, initial, initialCalendar }: {
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        <p className="text-[13px] font-medium mb-1" style={{ color: "#8a8a8a" }}>생년월일</p>
        <h2 className="text-[20px] font-bold mb-6" style={{ color: TEXT_CLR }}>그대가 태어난 날짜를 알려주시오</h2>
        <div className="flex gap-2 mb-5">
          {["양력", "음력"].map((c) => (
            <button key={c} onClick={() => setCalendar(c)}
              className="flex-1 py-2 rounded-xl text-[13px] font-bold transition-all"
              style={{
                backgroundColor: calendar === c ? "rgba(249,220,100,0.12)" : "rgba(255,255,255,0.04)",
                border: `1.5px solid ${calendar === c ? GOLD : "rgba(255,255,255,0.1)"}`,
                color: calendar === c ? GOLD : "rgba(255,255,255,0.6)",
              }}>{c}</button>
          ))}
        </div>
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-end gap-1">
              <input type="text" inputMode="numeric" placeholder="1990" autoComplete="off" value={year}
                ...
                style={{ width: 70, borderBottom: yearErr ? EB : year ? AB : NB, color: yearErr ? "#ff4444" : TEXT_CLR, caretColor: GOLD }} />
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-end gap-1">
              <input ref={monthRef} type="text" inputMode="numeric" placeholder="01" autoComplete="off" value={month}
                ...
                className="bg-transparent text-[22px] font-bold pb-1 outline-none text-center"
                style={{ width: 42, borderBottom: monthErr ? EB : month ? AB : NB, color: monthErr ? "#ff4444" : TEXT_CLR, caretColor: GOLD }} />
          ...
          <div className="flex flex-col items-center">
            <div className="flex items-end gap-1">
              <input ref={dayRef} type="text" inputMode="numeric" placeholder="01" autoComplete="off" value={day}
                ...
                className="bg-transparent text-[22px] font-bold pb-1 outline-none text-center"
                style={{ width: 42, borderBottom: dayErr ? EB : day ? AB : NB, color: dayErr ? "#ff4444" : TEXT_CLR, caretColor: GOLD }} />
  ...
  return (
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[24px] font-bold mb-6" style={{ color: TEXT_CLR }}>그대가 태어난 시간은?</h2>
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold flex items-center justify-between"
              onClick={() => setTimeOpen((v) => !v)}
              style={{
                backgroundColor: (btime && btime !== "시간 모름") ? "rgba(249,220,100,0.12)" : "rgba(255,255,255,0.04)",
                border: `1.5px solid ${(btime && btime !== "시간 모름") ? GOLD : "rgba(255,255,255,0.1)"}`,
                color: (btime && btime !== "시간 모름") ? GOLD : "rgba(255,255,255,0.45)",
              }}>
                ...
                <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                  ...
                  {BIRTH_TIMES.map((t) => (
                    <button key={t} className="w-full text-left px-4 py-2.5 text-[14px] transition-colors"
                      ...
                      style={{ backgroundColor: btime === t ? "rgba(249,220,100,0.12)" : "transparent", color: btime === t ? GOLD : "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {t}
                    </button>
                  ))}
                </div>
          ...
          <button className="px-4 py-3 rounded-xl text-[14px] font-bold"
            ...
            style={{
              backgroundColor: btime === "시간 모름" ? "rgba(249,220,100,0.12)" : "rgba(255,255,255,0.04)",
              border: `1.5px solid ${btime === "시간 모름" ? GOLD : "rgba(255,255,255,0.1)"}`,
              color: btime === "시간 모름" ? GOLD : "rgba(255,255,255,0.45)",
            }}>
            시간 모름
          </button>
        </div>
...
// ─── Step 4: 이름 ─────────────────────────────────────────────────────────────
function StepName({ onPrev, onNext, initial }: {
  onPrev: () => void; onNext: (v: string) => void; initial?: string;
}) {
  ...
  return (
    <>
      ...
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[24px] font-bold mb-6" style={{ color: TEXT_CLR }}>그대의 이름을 알려주시오</h2>
        <input type="text" placeholder="홍길동" autoComplete="off" lang="ko" value={name}
          ...
          style={{ borderBottom: name ? AB : NB, color: TEXT_CLR, caretColor: GOLD }}
          autoFocus />
        {name && !nameValid && (
...
// ─── Step 5: 사주 미리보기 ────────────────────────────────────────────────────
function StepSajuPreview({ onPrev, onNext, date, time, calendar, name }: {
  onPrev: () => void; onNext: () => void;
  date?: string; time?: string; calendar?: string; name?: string;
}) {
  const saju = useMemo(() => {
    if (!date) return null;
  ...
  return (
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <div className="rounded-2xl px-4 py-3" style={{ backgroundColor: "rgba(249,220,100,0.07)", border: "1px solid rgba(249,220,100,0.2)" }}>
          <p className="text-[13px] leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>
            ...
            이 여덟 글자 안에 재물 그릇과 돈이 오고 가는 흐름이 담겨 있소이다.
          </p>
        </div>
      </div>
...
function StepConcern({ onPrev, onNext, initial }: {
  ...
  const MAX = 200;
  const PLACEHOLDER = `예) 사업을 시작하려 하는데 지금이 맞는 시기인지, 재물운이 언제 오는지 궁금해요. 투자를 해도 될지도 알고 싶어요.`;
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>재물에 대해 </span>
          <span className="font-bold">궁금한 점이 있소?</span>
        </h2>
        <p className="text-[12px] mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>소인이 작은 조언을 해주겠소. (안적어도 괜찮소.)</p>
        <div className="rounded-2xl px-4 pt-4 pb-3" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(249,220,100,0.2)" }}>
          <textarea value={text} onChange={(e) => setText(e.target.value.slice(0, MAX))}
            ...
            style={{ color: TEXT_CLR, caretColor: GOLD, border: "none" }} />
      ...
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        ...
        <div className="flex items-end gap-2">
          <input type="text" inputMode="email" placeholder="아이디" value={local}
            ...
            style={{ flex: "1 1 0", minWidth: 0, fontFamily: MONO_FONT, borderBottom: `1.5px solid ${BORDER_CLR}`, color: local ? TEXT_CLR : PH_CLR, caretColor: GOLD }}
            autoFocus />
              ...
              <div className="absolute bottom-full left-0 right-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                ...
                {EMAIL_DOMAINS.map((d) => (
                  <div key={d} onClick={() => { setDomain(d); setOpen(false); }}
                    ...
                    style={{ backgroundColor: domain === d ? "rgba(249,220,100,0.15)" : "transparent", color: domain === d ? GOLD : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {d}
                  </div>
                ))}
              </div>
            ...
            {isCustom ? (
              <div className="flex items-center" style={{ borderBottom: `1.5px solid ${BORDER_CLR}` }}>
                <input type="text" placeholder="직접입력" value={custom}
                  ...
                  style={{ flex: "1 1 0", minWidth: 0, fontFamily: MONO_FONT, color: custom ? TEXT_CLR : PH_CLR, caretColor: GOLD }} />
    ...
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50" style={{ backgroundColor: "#080a0f" }}>
      ...
      <div className="relative z-10 px-8 w-full max-w-sm text-center">
        <div className="w-16 h-16 rounded-full mx-auto mb-5 flex items-center justify-center"
          style={{ backgroundColor: "rgba(249,220,100,0.15)", border: "2px solid rgba(249,220,100,0.4)" }}>
          <span style={{ fontSize: 28 }}>💰</span>
        </div>
        <p className="text-[22px] font-black mb-1" style={{ color: "#fff" }}>{name}{suffix}의 재물사주</p>
