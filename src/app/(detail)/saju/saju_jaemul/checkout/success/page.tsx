  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-end text-[11px] mb-2" style={{ color: "#c9a800" }}>
          <span>{pct}%</span>
        </div>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(249,220,100,0.15)", border: "1px solid rgba(249,220,100,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#f9dc64" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#f9dc64" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있소.<br />오래 걸려도 정상이니 그대로 기다려 주시오.<br />새로고침하면 처음부터 다시 시작되어<br />시간이 더 걸리게 되오.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#886820" }}>
    ...
    (async () => {
      ...
      const poll = async () => {
        for (let i = 0; i < 150; i++) { // 최대 5분(2초 간격)
          if (cancelled) return;
          try {
            const r = await fetch(`/api/jaemul-report?id=${encodeURIComponent(resultId)}`);
