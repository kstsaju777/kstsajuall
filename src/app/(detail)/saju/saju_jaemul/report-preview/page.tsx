...
import { JAEMUL_CHAPTER_SECTIONS, isJaemulChapterReady as isJAEMULChapterReady } from "@/lib/saju/jaemul-report-content";
...
const TOTAL_CH = Object.keys(JAEMUL_CHAPTER_SECTIONS).length;

const CH_TITLES: Record<number, string> = {
  0: "재물운사주 들어가며", 1: "나의 타고난 기질과 재물관", 2: "재물운과 돈의 흐름", 3: "돈이 들어오는 시기", 4: "직업운과 투자 스타일", 5: "재물을 방해하는 요인", 6: "홍연의 편지",
};

const CH_SUBS: Record<number, string> = {
  0: "", 1: "나의 사주에 담긴 재물 기운을 살펴봅니다", 2: "사주로 본 나의 재물운과 돈의 흐름입니다", 3: "돈이 크게 들어오는 시기를 찾습니다", 4: "나에게 맞는 직업과 투자 방향을 안내합니다", 5: "재물을 가로막는 것들, 미리 알고 피하세요", 6: "홍연이 당신에게 전하는 마지막 이야기",
};

const CH_IMAGES: Record<number, string> = {
  0: "/media/cards/saju_jaemul/jaemul-0.jpg",
  1: "/media/cards/saju_jaemul/jaemul-0.jpg",
  2: "/media/cards/saju_jaemul/jaemul-0.jpg",
  3: "/media/cards/saju_jaemul/jaemul-0.jpg",
  4: "/media/cards/saju_jaemul/jaemul-0.jpg",
  5: "/media/cards/saju_jaemul/jaemul-0.jpg",
  6: "/media/cards/saju_jaemul/jaemul-0.jpg",
};
    ...
    <>
      ...
      <div className="fixed z-50 top-0 left-0 bottom-0 flex flex-col"
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {items.map((n) => {
            const ready = content ? isJAEMULChapterReady(content, n) : false;
  ...
  return (
    <div>
      <div className="relative overflow-hidden" style={{ height: 320 }}>
        <img src="/media/cards/saju_jaemul/jaemul-0.jpg" alt="" className="absolute inset-0 w-full h-full object-cover object-top" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8 text-center">
          <p className="text-[11px] tracking-widest mb-2" style={{ color: PINKD }}>✦ 재물운사주 리포트 ✦</p>
...
function ChapterView({ ch, content, generating, name, onRetry, onNext }: {
  ...
  const sectionKeys = JAEMUL_CHAPTER_SECTIONS[ch] ?? [];
  const ready = content ? isJAEMULChapterReady(content, ch) : false;
  ...
  if (isLast) {
    return (
      <div className="mt-8 text-center rounded-2xl py-8 px-6" style={{ backgroundColor: CALLOUT, border: `1px solid ${ROSE}40` }}>
        <p className="text-[11px] tracking-widest mb-3" style={{ color: PINKD }}>✦ 재물운사주 완료 ✦</p>
...
function InitLoading() {
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      <img src="/media/cards/saju_jaemul/jaemul-0.jpg"
      ...
      <div className="relative z-10 flex flex-col items-center gap-4">
        ...
        <p className="text-[15px] font-bold" style={{ color: "rgba(255,255,255,0.9)" }}>재물운사주 리포트 준비 중</p>
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/jaemul-report", {
        ...
        .then((d) => {
          ...
          setResultId(rid);
          if (rid) {
            router.replace(`/saju/saju_jaemul/report-preview?id=${rid}&name=${encodeURIComponent(name)}&gender=${encodeURIComponent(gender)}&ch=0`);
          }
        })
        .catch(() => {})
        .finally(() => setLoading(false));
  ...
  useEffect(() => {
    ...
    const allNums = Object.keys(JAEMUL_CHAPTER_SECTIONS).map(Number);
    const missing = allNums.filter((n) => !isJAEMULChapterReady(content, n));
  ...
  const goTo = (n: number) => {
    ...
    router.push(`/saju/saju_jaemul/report-preview?${base}&ch=${n}`);
  };
