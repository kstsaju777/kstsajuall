...
//
// 미리보기: http://localhost:3000/saju/kunghap_yeonae/report-preview
//
...
// 장별 테마 색상
const CH1_COLOR  = "#8b3a5c"; // 딥 로즈 — 1장 테마
const CH1_PALE   = "#f9eef3";
...
const CH5_COLOR  = "#5a3e8b"; // 딥 퍼플 — 5장 테마 (상대의 시각·거울)
const CH5_PALE   = "#f3f0fa";
const CH6_COLOR  = "#1a5c8a"; // 딥 인디고 블루 — 6장 테마 (합·충, 두 기운의 만남)
...
const CH9_COLOR  = "#6b3a2a"; // 딥 브릭 — 9장 테마 (위기·긴장·극복)
const CH9_PALE   = "#fdf3f1";
const CRISIS_COLOR  = "#c0392b"; // 위기 — 진한 빨강
const OVERCOME_COLOR = "#1e8449"; // 극복 — 진한 초록
const CH10_COLOR = "#7b2d5a"; // 딥 버건디 로즈 — 10장 테마 (결혼·영원한 인연)
const CH10_PALE  = "#fdf0f5";
const CH11_COLOR = "#1e5f3a"; // 딥 포레스트 그린 — 11장 테마 (봄날·좋은 시기·성장)
const CH11_PALE  = "#eef7f2";
...
// 오행 도넛 + 세로 바차트 (자녀사주 스타일)
function OhaengDonutCh1({ view }: { view: MyeongsikView | null }) {
  const counts: Record<string, number> = { 목: 0, 화: 0, 토: 0, 금: 0, 수: 0 };
  if (view) {
    for (const p of view.pillars) {
      if (p.ganEl && counts[p.ganEl] !== undefined) counts[p.ganEl]++;
...
// 오행 도넛 + 세로 바차트 — 상대방용 (CH2_COLOR 팔레트)
function OhaengDonutCh2({ view }: { view: MyeongsikView | null }) {
  const counts: Record<string, number> = { 목: 0, 화: 0, 토: 0, 금: 0, 수: 0 };
  if (view) {
    for (const p of view.pillars) {
      if (p.ganEl && counts[p.ganEl] !== undefined) counts[p.ganEl]++;
...
// 기질 카드 — 키워드 태그 + 강점/그림자 분리 표시 (color 테마 지원)
function NatureCard({ data, color = CH1_COLOR, label = "나를 대표하는 기질" }: { data: Record<string, unknown> | null; color?: string; label?: string }) {
  if (!data) return null;
...
// 연애 패턴 유형 카드 + 단락 (color/pale 테마 지원)
function LovePatternCard({ data, color = CH1_COLOR, pale = CH1_PALE }: { data: Record<string, unknown> | null; color?: string; pale?: string }) {
  if (!data) return null;
  ...
  return (
    <div className="mx-5 mb-5">
      ...
      {paragraphs.map((p, i) => (
        <p key={i} className="text-[13.5px] leading-[1.85] mt-4" style={{ color: INK_SOFT, fontFamily: SERIF }}>{p}</p>
      ))}
    </div>
  );
}
...
// 추천 상품(크로스셀) 그리드 (마무리) — 재물사주와 동일 구조
...
const SAJU_ORDER = ["정통명리 종합사주", "영재발굴 자녀사주", "대박이야 재물사주", "나만솔로? 연애사주", "우리아가 유아사주", "오래살자 건강사주"];
...
function RecoGrid() {
  const all = (CATEGORY_CARDS["전체"] ?? []).filter((c) => !c.href.includes("kunghap_yeonae") && !RECO_EXCLUDE.has(c.name));
...
const KIND_MEANING: Record<RelKind, { verb: string; effect: string }> = {
  "천간합": { verb: "서로 끌어당겨 합을 이루고 있소.", effect: "천간합은 두 사람의 하늘 기운이 서로를 향해 강하게 끌어당기는 것이오. 마치 자석처럼 처음 만나는 순간부터 묘한 친밀감과 편안함이 느껴지오. 함께하면 서로의 부족한 기운을 채워주는 느낌이 들며, 자연스럽게 한 팀이 되어 움직이게 되오. 다만 천간합은 두 기운이 하나로 녹아드는 과정에서 각자의 본래 성질이 옅어질 수 있소. 너무 깊이 합해지면 경계가 무너지고 자아를 잃는 방향으로 흐를 수 있으니, 서로를 향한 끌림을 소중히 여기되 각자의 뿌리도 지켜나가는 것이 중요하오." },
  "천간충": { verb: "정면으로 충돌하고 있소.", effect: "천간충은 두 사람의 생각과 의지가 정면으로 맞부딪히는 것이오. 같은 상황을 보면서도 서로 전혀 다른 판단을 내리거나, 한 사람이 전진하려 할 때 다른 사람이 제동을 거는 식으로 마찰이 생기기 쉽소. 대화 중에 이유 없이 날이 서거나, 작은 의견 차이가 큰 언쟁으로 번지는 경험을 할 수도 있소. 그러나 천간충은 서로를 자극하여 각자가 더 선명하게 자신의 방향을 다듬게 만드는 힘도 있소. 서로의 충돌을 두려워하기보다, 다름을 인정하고 표현의 방식을 부드럽게 조율한다면 오히려 날카로운 협력 관계가 될 수 있소." },
  "육합":   { verb: "음양으로 짝을 이뤄 합을 이루고 있소.", effect: "육합은 하늘이 정해준 짝처럼 두 지지가 음양으로 끌어당기는 것이오. 두 사람 사이에 표현하지 않아도 통하는 무언가가 있고, 함께 있으면 마음이 고요하게 안정되는 느낌을 받게 되오. 육합은 천간합보다 더 내밀하고 감정적인 층에서 작용하오. 오랜 시간을 함께할수록 더욱 깊어지는 유형의 인연이며, 억지로 이어지려 하지 않아도 자연스럽게 가까워지는 힘이 있소. 이 합이 있는 자리에서 두 사람은 서로에게 따뜻한 안식처가 될 수 있소." },
  "삼합":   { verb: "기운이 모여 하나의 세력을 이루고 있소.", effect: "삼합은 같은 방향을 바라보는 세 지지가 힘을 합쳐 하나의 강한 오행 기운을 만들어내는 것이오. 두 사람의 지지가 삼합 세트의 일부를 이루고 있다면, 그 합화 오행의 기운이 두 사람 관계 전반에 강하게 흐르게 되오. 함께 있을 때 특정 영역 — 예컨대 재물, 또는 활동성, 또는 사교성 — 이 강해지는 느낌을 받을 수 있소. 방향이 맞으면 두 사람이 힘을 모았을 때 큰 결실을 이루어낼 수 있는 구조이오. 다만 그 오행이 지나치게 강해지면 다른 기운이 눌릴 수 있으니, 관계 안에서 균형을 의식하는 것도 필요하오." },
  "충":     { verb: "정반대로 부딪히고 있소.", effect: "지지충은 두 사람의 감정적·본능적 기반이 정면으로 충돌하는 것이오. 천간충이 생각과 의지의 충돌이라면, 지지충은 살아온 환경·습관·기질의 충돌이오. 한 사람이 편안하게 여기는 방식을 상대방은 불편하게 느끼는 장면이 반복될 수 있소. 이동이나 변화의 시기에 두 사람의 방향이 엇갈리기 쉽고, 특히 생활 리듬과 공간 감각에서 마찰이 생기는 경우가 많소. 그러나 충은 정체된 기운을 흔들어 깨우는 힘이기도 하오. 서로에게 익숙해지려는 노력과 함께, 각자의 다름을 변화의 에너지로 전환할 수 있다면 오히려 역동적인 관계로 발전할 수 있소." },
  "형":     { verb: "서로 형을 가하고 있소.", effect: "형(刑)은 충처럼 드러나는 충돌이 아니라, 내부에서 서서히 쌓이는 마찰이오. 겉으로는 사이가 나쁘지 않은데 묘하게 피곤하거나, 함께 있으면 긴장이 풀리지 않는 느낌이 드는 경우가 있소. 형이 있는 사이에서는 서로에게 무의식적으로 상처를 주는 말이나 행동이 나오기 쉬우며, 나중에서야 '그 말이 아팠는데'하고 돌아보는 패턴이 반복되기도 하오. 특히 가까워질수록 서로의 날카로운 면이 자극받을 수 있소. 형이 있다고 반드시 나쁜 관계는 아니지만, 서로의 예민한 부분을 인식하고 의도치 않은 상처를 줄이려는 세심한 노력이 필요하오." },
  "파":     { verb: "서로 깨지는 관계에 있소.", effect: "파(破)는 시작은 좋은데 어느 순간 흐지부지되는 기운이오. 처음 만날 때는 잘 맞는 것 같고 기대감도 크지만, 중간 어딘가에서 맥이 끊기거나 어긋나는 경험이 반복될 수 있소. 함께 무언가를 계획하거나 실행할 때 결정적인 순간에 방향이 어긋나는 경우가 있고, 잡힐 듯 잡히지 않는 어긋남이 관계 안에 쌓이기도 하오. 파가 있다면 서로를 향한 기대치를 무리하게 높이지 않는 것이 좋소. 작은 성취를 함께 확인하고 쌓아가는 방식으로 관계를 다져나가면, 파의 기운에 흔들리지 않고 단단하게 이어나갈 수 있소." },
  "해":     { verb: "서로 해를 끼치는 관계에 있소.", effect: "해(害)는 선의로 한 말과 행동이 상대방에게 예상치 못한 방식으로 상처가 되는 기운이오. 의도한 것이 아닌데 오해가 생기거나, 도우려 했는데 오히려 방해가 되는 상황이 반복될 수 있소. 특히 제삼자를 통해 오해나 갈등이 불거지는 패턴이 나타나기도 하오. 해가 있는 관계에서는 말보다 행동으로 신뢰를 쌓는 것이 효과적이오. 직접 소통하고, 뒤에서 이야기가 돌지 않도록 투명하게 표현하는 습관이 관계를 지키는 데 도움이 되오." },
  "원진":   { verb: "서로 싫어하고 멀어지려는 관계에 있소.", effect: "원진(怨嗔)은 겉으로는 별 문제가 없어 보이는데, 함께하면 이유를 설명하기 어려운 불편함이 쌓이는 기운이오. 상대방이 틀린 것도 아니고 나쁜 것도 아닌데 어딘가 맞지 않는 느낌 — 그것이 원진의 작용이오. 같은 공간에 오래 있으면 피로감이 생기거나, 사소한 말투와 습관이 과하게 신경 쓰이는 경험을 할 수 있소. 원진은 억지로 극복하려 할수록 오히려 더 강해지는 경향이 있소. 서로에게 일정한 거리와 여유를 허용하고, 함께하는 시간만큼 혼자만의 공간도 확보하는 것이 이 기운을 다스리는 가장 현명한 방법이오." },
};
...
// 명식 + 합충 인터랙티브 컴포넌트
function MiniMyeongsikReport({ pillars, name, color }: { pillars: MyeongsikView["pillars"] | undefined; name: string; color: string }) {
  const COL_LABELS = ["시주", "일주", "월주", "년주"];
  const ps = pillars ?? [];
  return (
    <div className="flex-1">
      <p className="text-[13px] font-bold mb-2 text-center" style={{ color }}>{name}님</p>
      <div className="rounded-2xl p-3" style={{ background: WHITE, border: `2px solid ${color}30`, boxShadow: `0 2px 10px ${color}10` }}>
        <div className="grid grid-cols-4 gap-1.5">
          {COL_LABELS.map((col, i) => {
            const p = ps[i];
            return (
              <div key={i} className="flex flex-col items-center gap-1">
                <span className="text-[10px]" style={{ color: MUTE }}>{col}</span>
                <span className="text-[10px]" style={{ color: INK_SOFT }}>{(p?.sipTop ?? "-").replace("(나)", "")}</span>
                <div className="w-full" style={{ aspectRatio: "1" }}>
                  {p ? (
                    ...
                    <div className="w-full h-full animate-pulse rounded" style={{ background: `${color}10` }} />
                  )}
                </div>
                <div className="w-full" style={{ aspectRatio: "1" }}>
                  {p ? (
                ...
                <span className="text-[10px]" style={{ color: INK_SOFT }}>{p?.sipBot ?? "-"}</span>
              </div>
            );
          })}
        </div>
...
function calcCrossRelations(myView: MyeongsikView, ptView: MyeongsikView): Rel[] {
...
const CROSS_DESC: Record<string, string> = {
  ...
  "충_오자": "자(子)와 오(午)가 충하오. 수(水)와 화(火)가 정면으로 부딪히는 자오충이오. 자수는 깊이 담아두고 내면으로 흘러가는 기운이고, 오화는 밖으로 터뜨리고 표현하려는 기운이오. 한 사람은 감정을 충분히 삭힌 뒤 말하고 싶고, 다른 사람은 즉각적으로 표현하고 반응받고 싶어하는 구조가 충돌을 만들어내오. 서로의 표현 시간표가 다름을 인식하고, 상대방의 방식이 틀린 것이 아님을 받아들이는 것이 이 충을 다스리는 핵심이오.",
  "충_축미": "축(丑)과 미(未)가 충하오. 같은 토(土)끼리의 충이지만, 한쪽은 겨울 토(축)이고 다른 쪽은 여름 토(미)로 기질이 전혀 다르오. 둘 다 자신의 자리를 굳건히 지키려는 성질이 강해, 서로 상대방의 방식에 맞추는 것을 불편하게 느끼기 쉽소. 생활 방식·식습관·일상의 리듬에서 작지만 끊임없는 마찰이 생길 수 있소. 서로의 다름이 틀림이 아님을 인식하고, 공유하는 공간과 개인 공간의 경계를 명확히 해두는 것이 도움이 되오.",
  "충_미축": "축(丑)과 미(未)가 충하오. 같은 토(土)끼리의 충이지만, 한쪽은 겨울 토(축)이고 다른 쪽은 여름 토(미)로 기질이 전혀 다르오. 둘 다 자신의 자리를 굳건히 지키려는 성질이 강해, 서로 상대방의 방식에 맞추는 것을 불편하게 느끼기 쉽소. 생활 방식·식습관·일상의 리듬에서 작지만 끊임없는 마찰이 생길 수 있소. 서로의 다름이 틀림이 아님을 인식하고, 공유하는 공간과 개인 공간의 경계를 명확히 해두는 것이 도움이 되오.",
  "충_인신": "인(寅)과 신(申)이 충하오. 목(木)과 금(金)의 충으로, 인목의 뻗어나가는 추진력과 신금의 자르고 다듬는 기운이 부딪히오. 한 사람은 직관적으로 빠르게 움직이려 하고, 다른 사람은 논리적으로 따져보고 결정하려 해 함께 무언가를 진행할 때 속도가 맞지 않는 경우가 많소. 이동과 변화의 시기에 두 사람의 방향이 엇갈리기도 하오. 각자의 결정 방식을 존중하고 중요한 선택에서는 충분히 이야기 나누는 시간을 갖는 것이 필요하오.",
  "충_신인": "인(寅)과 신(申)이 충하오. 목(木)과 금(金)의 충으로, 인목의 뻗어나가는 추진력과 신금의 자르고 다듬는 기운이 부딪히오. 한 사람은 직관적으로 빠르게 움직이려 하고, 다른 사람은 논리적으로 따져보고 결정하려 해 함께 무언가를 진행할 때 속도가 맞지 않는 경우가 많소. 이동과 변화의 시기에 두 사람의 방향이 엇갈리기도 하오. 각자의 결정 방식을 존중하고 중요한 선택에서는 충분히 이야기 나누는 시간을 갖는 것이 필요하오.",
  "충_묘유": "묘(卯)와 유(酉)가 충하오. 목(木)과 금(金)의 충이오. 묘목의 부드럽고 섬세한 감수성과 유금의 날카롭고 정갈한 기운이 충돌하오. 한 사람은 관계 안에서 자유롭고 유연하게 흐르고 싶어하고, 다른 사람은 질서와 원칙 안에서 안정감을 찾으려 하오. 서로의 다름이 처음에는 매력으로 느껴질 수 있지만, 가까워질수록 부딪히는 지점이 드러나오. 상대방의 방식을 변화시키려 하기보다 다름을 인정하고 균형점을 찾는 것이 이 관계를 단단하게 만드오.",
  "충_유묘": "묘(卯)와 유(酉)가 충하오. 목(木)과 금(金)의 충이오. 묘목의 부드럽고 섬세한 감수성과 유금의 날카롭고 정갈한 기운이 충돌하오. 한 사람은 관계 안에서 자유롭고 유연하게 흐르고 싶어하고, 다른 사람은 질서와 원칙 안에서 안정감을 찾으려 하오. 서로의 다름이 처음에는 매력으로 느껴질 수 있지만, 가까워질수록 부딪히는 지점이 드러나오. 상대방의 방식을 변화시키려 하기보다 다름을 인정하고 균형점을 찾는 것이 이 관계를 단단하게 만드오.",
  "충_진술": "진(辰)과 술(戌)이 충하오. 같은 토(土)끼리의 충으로, 둘 다 강한 자기 기반과 주관을 가지고 있소. 진토는 창의적이고 다변적인 기질, 술토는 열정적이고 집중력 강한 기질을 지니고 있어 방향이 다를 때 서로 좀처럼 물러서지 않으려 하오. 특히 장기적인 계획이나 방향성을 정하는 과정에서 충돌이 커지기 쉽소. 서로 옳고 그름으로 판단하기보다, 두 가지 관점을 모두 고려한 제3의 길을 함께 만들어가는 접근이 필요하오.",
  "충_술진": "진(辰)과 술(戌)이 충하오. 같은 토(土)끼리의 충으로, 둘 다 강한 자기 기반과 주관을 가지고 있소. 진토는 창의적이고 다변적인 기질, 술토는 열정적이고 집중력 강한 기질을 지니고 있어 방향이 다를 때 서로 좀처럼 물러서지 않으려 하오. 특히 장기적인 계획이나 방향성을 정하는 과정에서 충돌이 커지기 쉽소. 서로 옳고 그름으로 판단하기보다, 두 가지 관점을 모두 고려한 제3의 길을 함께 만들어가는 접근이 필요하오.",
  "충_사해": "사(巳)와 해(亥)가 충하오. 화(火)와 수(水)의 충이오. 사화는 집중적이고 목적 지향적인 기운, 해수는 포용적이고 방향이 자유로운 기운이오. 한 사람은 목표를 향해 직선으로 나아가려 하고, 다른 사람은 다양한 가능성을 탐색하며 넓게 흘러가려 해 속도와 방향이 자주 어긋날 수 있소. 삶을 대하는 방식의 근본적인 차이가 가끔 크게 느껴지는 구조이오. 서로의 방식이 상호 보완적일 수 있음을 인식하고, 함께할 수 있는 교차점을 꾸준히 찾아가는 것이 중요하오.",
  "충_해사": "사(巳)와 해(亥)가 충하오. 화(火)와 수(水)의 충이오. 사화는 집중적이고 목적 지향적인 기운, 해수는 포용적이고 방향이 자유로운 기운이오. 한 사람은 목표를 향해 직선으로 나아가려 하고, 다른 사람은 다양한 가능성을 탐색하며 넓게 흘러가려 해 속도와 방향이 자주 어긋날 수 있소. 삶을 대하는 방식의 근본적인 차이가 가끔 크게 느껴지는 구조이오. 서로의 방식이 상호 보완적일 수 있음을 인식하고, 함께할 수 있는 교차점을 꾸준히 찾아가는 것이 중요하오.",

  // ── 형 ──────────────────────────────────────────────────────────────────────
  "형_인사신": "인(寅)·사(巳)·신(申)의 지세지형(持勢之刑)이오. 세 기운 모두 강한 기세와 주도력을 가지고 있어, 같은 공간에서 서로의 힘이 충돌하는 구조이오. 의도하지 않아도 상대방의 영역을 침범하거나 자신의 방식을 관철하려는 기운이 강하게 작용하오. 두 사람 사이에 보이지 않는 주도권 다툼이 생기기 쉬우며, 서로 '내가 맞다'는 확신이 부딪히는 상황이 반복될 수 있소. 각자의 강함을 인정하고 서로의 영역을 존중해주는 것이 이 형을 다스리는 방법이오.",
  "형_축술미": "축(丑)·술(戌)·미(未)의 무은지형(無恩之刑)이오. 은혜를 베풀었는데 돌아오는 것이 없거나, 배려가 오히려 부담이 되는 형이오. 두 사람 중 한쪽이 먼저 마음을 쏟았는데 기대한 만큼의 반응이 오지 않아 서운함이 쌓이는 패턴이 생기기 쉽소. 또는 서로를 위한다고 한 행동이 상대방에게 간섭이나 부담으로 느껴지는 경우도 있소. 배려의 방식을 상대방이 원하는 방식으로 맞추어 가려는 노력이 필요하오.",
  "형_자묘": "자(子)와 묘(卯)의 무례지형(無禮之刑)이오. 예의·경계·매너에 관한 기운의 충돌이오. 의도하지 않게 상대방이 무례하게 느끼는 말이나 행동이 나오기 쉬우며, 가까워질수록 오히려 예의 없이 구는 패턴이 생길 수 있소. 서로에게 익숙해진다는 것이 경계를 허무는 방향으로 흐르지 않도록 주의가 필요하오. 친밀함과 예의는 함께 갈 수 있다는 인식 아래, 서로를 소중히 대하는 태도를 꾸준히 유지하는 것이 중요하오.",
  "형_진진": "진(辰)과 진(辰)의 자형(自刑)이오. 같은 기운끼리 스스로를 자극하는 형이오. 두 사람 모두 진토의 기운을 가지고 있어, 비슷한 특성이 부딪히는 구조이오. 서로의 약점을 너무 잘 아는 만큼 무심코 그 부분을 건드릴 수 있고, 자신에게 엄격한 기준을 상대방에게도 적용하는 경향이 생기기 쉽소.",
  "형_오오": "오(午)와 오(午)의 자형(自刑)이오. 같은 기운끼리 스스로를 자극하는 형이오. 두 사람 모두 오화의 뜨거운 기운을 가지고 있어, 감정이 함께 달아오를 때 서로를 소진시키는 방향으로 흐르기 쉽소. 열정이 충돌로 이어지지 않도록 감정의 온도를 서로 조율하는 것이 필요하오.",
  "형_유유": "유(酉)와 유(酉)의 자형(自刑)이오. 유금의 날카롭고 원칙적인 기운이 맞부딪히는 형이오. 서로에게 높은 기준을 요구하게 되고, 상대방의 미세한 실수나 어긋남에 예민하게 반응하기 쉽소. 완벽함을 향한 두 기운이 서로를 지치게 만들 수 있으니, 때로는 느슨하게 놓아주는 연습이 필요하오.",
  "형_해해": "해(亥)와 해(亥)의 자형(自刑)이오. 해수의 깊고 포용적인 기운이 맞부딪히는 형이오. 두 사람 모두 많은 것을 담아두고 표현하지 않는 경향이 있어, 서로가 서로에게 쌓아두는 것이 많아질 수 있소. 속으로 담은 것이 어느 순간 한꺼번에 터지는 패턴이 생기지 않도록, 작은 것도 꾸준히 나누는 소통 습관을 만들어가는 것이 중요하오.",

  // ── 파 ──────────────────────────────────────────────────────────────────────
  "파_자유": "자(子)와 유(酉)의 파(破)이오. 수와 금의 만남이지만 서로 생하는 관계임에도 파로 이어지오. 처음에는 서로를 도와주는 듯한 관계로 시작하지만, 어느 순간 기대가 어긋나거나 방향이 다르게 흘러가는 경험이 반복될 수 있소. 잘 될 것 같은데 중간에 맥이 끊기는 느낌이 드는 관계이오. 작은 약속부터 꾸준히 지켜나가며 신뢰를 쌓는 것이 이 파를 극복하는 방법이오.",
  "파_유자": "자(子)와 유(酉)의 파(破)이오. 수와 금의 만남이지만 서로 생하는 관계임에도 파로 이어지오. 처음에는 서로를 도와주는 듯한 관계로 시작하지만, 어느 순간 기대가 어긋나거나 방향이 다르게 흘러가는 경험이 반복될 수 있소. 잘 될 것 같은데 중간에 맥이 끊기는 느낌이 드는 관계이오. 작은 약속부터 꾸준히 지켜나가며 신뢰를 쌓는 것이 이 파를 극복하는 방법이오.",
  "파_오묘": "오(午)와 묘(卯)의 파(破)이오. 화와 목의 만남으로 목이 화를 생하는 관계인데도 파가 작용하오. 서로 잘 어울릴 것 같고 함께 있으면 즐거운데, 어느 순간 엇갈리고 마는 아쉬운 패턴이 반복되기 쉽소. 감정적으로는 맞는 것 같은데 실제 생활의 결이 어긋나는 경우가 생기오. 서로의 기대를 명확하게 표현하고 확인하는 소통이 이 파를 줄이는 데 도움이 되오.",
  "파_묘오": "오(午)와 묘(卯)의 파(破)이오. 화와 목의 만남으로 목이 화를 생하는 관계인데도 파가 작용하오. 서로 잘 어울릴 것 같고 함께 있으면 즐거운데, 어느 순간 엇갈리고 마는 아쉬운 패턴이 반복되기 쉽소. 감정적으로는 맞는 것 같은데 실제 생활의 결이 어긋나는 경우가 생기오. 서로의 기대를 명확하게 표현하고 확인하는 소통이 이 파를 줄이는 데 도움이 되오.",
  "파_인해": "인(寅)과 해(亥)의 파(破)이오. 육합 관계이기도 한데 파가 함께 작용하는 복잡한 조합이오. 서로 깊이 끌리고 합이 느껴지지만, 동시에 어긋나고 멀어지는 기운도 공존하오. 시작이 아름다웠다가 중간에 흐지부지되거나, 다시 가까워지면 또 잘 될 것 같은 반복이 생길 수 있소. 합의 기운을 더 키우고 파의 기운을 줄이려면 서로의 관계를 명확하게 정의하고 방향을 함께 정하는 것이 필요하오.",
  "파_해인": "인(寅)과 해(亥)의 파(破)이오. 육합 관계이기도 한데 파가 함께 작용하는 복잡한 조합이오. 서로 깊이 끌리고 합이 느껴지지만, 동시에 어긋나고 멀어지는 기운도 공존하오. 시작이 아름다웠다가 중간에 흐지부지되거나, 다시 가까워지면 또 잘 될 것 같은 반복이 생길 수 있소. 합의 기운을 더 키우고 파의 기운을 줄이려면 서로의 관계를 명확하게 정의하고 방향을 함께 정하는 것이 필요하오.",
  "파_사신": "사(巳)와 신(申)의 파(破)이오. 육합 관계이기도 한 복잡한 조합이오. 함께하면 강한 에너지가 오가지만, 그 에너지가 조화보다 마찰로 이어지는 경우가 많소. 서로를 향한 집착이나 통제 욕구가 강해지면 파의 기운이 커지오. 상대방을 있는 그대로 받아들이고 각자의 방식을 인정해주는 것이 이 관계를 지키는 핵심이오.",
  "파_신사": "사(巳)와 신(申)의 파(破)이오. 육합 관계이기도 한 복잡한 조합이오. 함께하면 강한 에너지가 오가지만, 그 에너지가 조화보다 마찰로 이어지는 경우가 많소. 서로를 향한 집착이나 통제 욕구가 강해지면 파의 기운이 커지오. 상대방을 있는 그대로 받아들이고 각자의 방식을 인정해주는 것이 이 관계를 지키는 핵심이오.",
  "파_축진": "축(丑)과 진(辰)의 파(破)이오. 같은 토 기운끼리의 파이오. 서로 비슷한 듯하면서도 실상은 다른 방식으로 안정을 추구하는 두 기운이 어긋나오. 큰 다툼 없이 조용히 멀어지거나, 유지하려는 노력이 어딘가에서 계속 어긋나는 패턴이 생기기 쉽소. 관계를 지속하고 싶다면 명확한 소통과 함께하는 목표를 꾸준히 만들어 가는 것이 중요하오.",
  "파_진축": "축(丑)과 진(辰)의 파(破)이오. 같은 토 기운끼리의 파이오. 서로 비슷한 듯하면서도 실상은 다른 방식으로 안정을 추구하는 두 기운이 어긋나오. 큰 다툼 없이 조용히 멀어지거나, 유지하려는 노력이 어딘가에서 계속 어긋나는 패턴이 생기기 쉽소. 관계를 지속하고 싶다면 명확한 소통과 함께하는 목표를 꾸준히 만들어 가는 것이 중요하오.",
  "파_술미": "술(戌)과 미(未)의 파(破)이오. 같은 토 기운끼리의 파이오. 술토의 뜨거운 열정과 미토의 온화한 기운이 만나지만, 서로의 기대 방식이 달라 어긋나기 쉬운 구조이오. 한 사람이 관계에 쏟는 에너지에 비해 상대방의 반응이 시원치 않게 느껴지는 상황이 반복될 수 있소. 서로가 관계에서 원하는 것을 솔직하게 나누고 균형을 맞추어 가는 것이 필요하오.",
  "파_미술": "술(戌)과 미(未)의 파(破)이오. 같은 토 기운끼리의 파이오. 술토의 뜨거운 열정과 미토의 온화한 기운이 만나지만, 서로의 기대 방식이 달라 어긋나기 쉬운 구조이오. 한 사람이 관계에 쏟는 에너지에 비해 상대방의 반응이 시원치 않게 느껴지는 상황이 반복될 수 있소. 서로가 관계에서 원하는 것을 솔직하게 나누고 균형을 맞추어 가는 것이 필요하오.",

  // ── 해 ──────────────────────────────────────────────────────────────────────
  "해_자미": "자(子)와 미(未)의 해(害)이오. 자수의 깊고 고요한 기운과 미토의 따뜻하고 온화한 기운이 서로를 방해하는 구조이오. 선의로 한 말이 상대방에게 엉뚱하게 받아들여지거나, 도와주려 했는데 오히려 부담이 되는 경우가 생기기 쉽소. 특히 제삼자를 통해 오해가 생기거나 말이 변형되어 전달되는 패턴을 주의하시오. 직접 소통하고 중간에서 이야기가 돌지 않도록 하는 것이 중요하오.",
  "해_미자": "자(子)와 미(未)의 해(害)이오. 자수의 깊고 고요한 기운과 미토의 따뜻하고 온화한 기운이 서로를 방해하는 구조이오. 선의로 한 말이 상대방에게 엉뚱하게 받아들여지거나, 도와주려 했는데 오히려 부담이 되는 경우가 생기기 쉽소. 특히 제삼자를 통해 오해가 생기거나 말이 변형되어 전달되는 패턴을 주의하시오. 직접 소통하고 중간에서 이야기가 돌지 않도록 하는 것이 중요하오.",
  "해_축오": "축(丑)과 오(午)의 해(害)이오. 축토의 묵직하고 조용한 기운이 오화의 뜨겁고 표현적인 기운과 맞부딪혀 서로를 방해하오. 한 사람이 조용히 있고 싶을 때 다른 사람이 분위기를 달구거나, 반대로 한 사람이 감정을 드러내고 싶을 때 다른 사람이 차갑게 느껴지는 상황이 반복될 수 있소. 서로의 감정 표현 방식이 다름을 인정하고, 상대방의 방식을 억지로 바꾸려 하지 않는 것이 해를 줄이는 핵심이오.",
  "해_오축": "축(丑)과 오(午)의 해(害)이오. 축토의 묵직하고 조용한 기운이 오화의 뜨겁고 표현적인 기운과 맞부딪혀 서로를 방해하오. 한 사람이 조용히 있고 싶을 때 다른 사람이 분위기를 달구거나, 반대로 한 사람이 감정을 드러내고 싶을 때 다른 사람이 차갑게 느껴지는 상황이 반복될 수 있소. 서로의 감정 표현 방식이 다름을 인정하고, 상대방의 방식을 억지로 바꾸려 하지 않는 것이 해를 줄이는 핵심이오.",
  "해_인사": "인(寅)과 사(巳)의 해(害)이오. 인목의 뻗어나가려는 기운과 사화의 집중적인 기운이 서로를 방해하오. 두 사람 모두 자기 방향이 뚜렷한 경우가 많아, 상대방의 방식이 내 길을 막는다는 느낌이 들 수 있소. 도와주려 한 것이 간섭이 되거나, 자신의 방향을 지키려는 것이 상대방에게 외면으로 느껴지기도 하오. 서로의 독립성을 인정하면서 함께 있을 수 있는 공간을 의식적으로 만들어 가시오.",
  "해_사인": "인(寅)과 사(巳)의 해(害)이오. 인목의 뻗어나가려는 기운과 사화의 집중적인 기운이 서로를 방해하오. 두 사람 모두 자기 방향이 뚜렷한 경우가 많아, 상대방의 방식이 내 길을 막는다는 느낌이 들 수 있소. 도와주려 한 것이 간섭이 되거나, 자신의 방향을 지키려는 것이 상대방에게 외면으로 느껴지기도 하오. 서로의 독립성을 인정하면서 함께 있을 수 있는 공간을 의식적으로 만들어 가시오.",
  "해_묘진": "묘(卯)와 진(辰)의 해(害)이오. 이웃한 지지끼리의 해로, 가까운 만큼 보이지 않는 곳에서 마찰이 쌓이는 구조이오. 서로 잘 맞는 것 같은데 어느 순간 작은 것에서 상처받는 패턴이 반복될 수 있소. 특히 묘의 섬세한 감수성과 진의 다양한 기운이 부딪혀 의도치 않은 오해가 생기기 쉽소. 서로에게 솔직하게 감정을 표현하고, 쌓아두기 전에 터놓는 소통 습관이 이 해를 다스리는 방법이오.",
  "해_진묘": "묘(卯)와 진(辰)의 해(害)이오. 이웃한 지지끼리의 해로, 가까운 만큼 보이지 않는 곳에서 마찰이 쌓이는 구조이오. 서로 잘 맞는 것 같은데 어느 순간 작은 것에서 상처받는 패턴이 반복될 수 있소. 특히 묘의 섬세한 감수성과 진의 다양한 기운이 부딪혀 의도치 않은 오해가 생기기 쉽소. 서로에게 솔직하게 감정을 표현하고, 쌓아두기 전에 터놓는 소통 습관이 이 해를 다스리는 방법이오.",
  "해_신해": "신(申)과 해(亥)의 해(害)이오. 신금의 날카롭고 결단적인 기운과 해수의 포용적이고 흐르는 기운이 서로를 방해하오. 한 사람은 명확하게 정하고 싶어하고, 다른 사람은 여지를 두고 흘러가길 원하는 구조이오. 서로의 다름이 보완이 될 수도 있지만, 중요한 결정의 순간에 방향이 엇갈리면 서로에게 방해가 되는 느낌이 들 수 있소. 결정적 순간에는 충분히 이야기 나누고 합의하는 과정이 필요하오.",
  "해_해신": "신(申)과 해(亥)의 해(害)이오. 신금의 날카롭고 결단적인 기운과 해수의 포용적이고 흐르는 기운이 서로를 방해하오. 한 사람은 명확하게 정하고 싶어하고, 다른 사람은 여지를 두고 흘러가길 원하는 구조이오. 서로의 다름이 보완이 될 수도 있지만, 중요한 결정의 순간에 방향이 엇갈리면 서로에게 방해가 되는 느낌이 들 수 있소. 결정적 순간에는 충분히 이야기 나누고 합의하는 과정이 필요하오.",
  "해_유술": "유(酉)와 술(戌)의 해(害)이오. 이웃한 지지끼리의 해로, 매우 가까운 기운이지만 서로 어긋나는 구조이오. 유금의 세련되고 정교한 기운과 술토의 열정적이고 깊은 기운이 맞닿아 있지만, 기대하는 방식과 반응하는 방식이 다르게 흐르오. 서로에게 가장 가까이 있는 사람이 가장 상처 주기 쉬운 법이오. 이웃한 만큼 더 세심하게 배려하고, 상대방의 방식을 존중하는 태도를 유지하시오.",
  "해_술유": "유(酉)와 술(戌)의 해(害)이오. 이웃한 지지끼리의 해로, 매우 가까운 기운이지만 서로 어긋나는 구조이오. 유금의 세련되고 정교한 기운과 술토의 열정적이고 깊은 기운이 맞닿아 있지만, 기대하는 방식과 반응하는 방식이 다르게 흐르오. 서로에게 가장 가까이 있는 사람이 가장 상처 주기 쉬운 법이오. 이웃한 만큼 더 세심하게 배려하고, 상대방의 방식을 존중하는 태도를 유지하시오.",

  // ── 원진 ────────────────────────────────────────────────────────────────────
  "원진_자미": "자(子)와 미(未)의 원진이오. 자수의 깊고 내성적인 기운과 미토의 따뜻하고 감성적인 기운이 묘하게 어긋나오. 상대방이 나쁜 것도 아니고 틀린 것도 아닌데, 함께 있으면 설명하기 어려운 불편함이 쌓이는 조합이오. 특히 자수는 혼자만의 공간이 필요하고, 미토는 감정적 연결을 원하는 경향이 있어 서로의 욕구가 어긋나기 쉽소. 억지로 맞추려 하기보다 각자의 공간을 존중하며 부담 없이 함께할 수 있는 방식을 찾아가시오.",
  "원진_미자": "자(子)와 미(未)의 원진이오. 자수의 깊고 내성적인 기운과 미토의 따뜻하고 감성적인 기운이 묘하게 어긋나오. 상대방이 나쁜 것도 아니고 틀린 것도 아닌데, 함께 있으면 설명하기 어려운 불편함이 쌓이는 조합이오. 특히 자수는 혼자만의 공간이 필요하고, 미토는 감정적 연결을 원하는 경향이 있어 서로의 욕구가 어긋나기 쉽소. 억지로 맞추려 하기보다 각자의 공간을 존중하며 부담 없이 함께할 수 있는 방식을 찾아가시오.",
  "원진_축오": "축(丑)과 오(午)의 원진이오. 축토의 조용하고 참을성 있는 기운과 오화의 활발하고 표현적인 기운이 근본적으로 어긋나오. 한 사람은 조용히 있는 것이 편안하고, 다른 사람은 활발하게 움직여야 활기를 찾는 구조이오. 함께 있는 공간의 온도와 분위기를 서로 다르게 원하는 경우가 많소. 서로의 에너지 수준이 다름을 인정하고, 각자의 방식으로 충전할 시간을 확보하는 것이 이 원진을 다스리는 방법이오.",
  "원진_오축": "축(丑)과 오(午)의 원진이오. 축토의 조용하고 참을성 있는 기운과 오화의 활발하고 표현적인 기운이 근본적으로 어긋나오. 한 사람은 조용히 있는 것이 편안하고, 다른 사람은 활발하게 움직여야 활기를 찾는 구조이오. 함께 있는 공간의 온도와 분위기를 서로 다르게 원하는 경우가 많소. 서로의 에너지 수준이 다름을 인정하고, 각자의 방식으로 충전할 시간을 확보하는 것이 이 원진을 다스리는 방법이오.",
  "원진_인유": "인(寅)과 유(酉)의 원진이오. 인목의 거침없이 뻗어나가는 기운과 유금의 정교하고 절제된 기운이 서로를 불편하게 느끼는 구조이오. 인목은 자유롭고 직선적으로 나아가고 싶고, 유금은 질서와 미감을 중시하는 경향이 있어 서로의 방식을 답답하게 느끼기 쉽소. 특히 계획을 세우거나 실행하는 방식에서 마찰이 커질 수 있소. 서로의 스타일이 다름을 미리 인식하고 큰 결정에서는 충분히 소통하는 것이 중요하오.",
  "원진_유인": "인(寅)과 유(酉)의 원진이오. 인목의 거침없이 뻗어나가는 기운과 유금의 정교하고 절제된 기운이 서로를 불편하게 느끼는 구조이오. 인목은 자유롭고 직선적으로 나아가고 싶고, 유금은 질서와 미감을 중시하는 경향이 있어 서로의 방식을 답답하게 느끼기 쉽소. 특히 계획을 세우거나 실행하는 방식에서 마찰이 커질 수 있소. 서로의 스타일이 다름을 미리 인식하고 큰 결정에서는 충분히 소통하는 것이 중요하오.",
  "원진_묘신": "묘(卯)와 신(申)의 원진이오. 묘목의 부드럽고 감성적인 기운과 신금의 날카롭고 현실적인 기운이 서로를 낯설게 느끼는 구조이오. 한 사람은 관계 안에서 감정과 분위기를 중시하고, 다른 사람은 효율과 논리를 우선하는 경향이 있어 대화의 결이 자주 어긋날 수 있소. 상대방이 차갑다거나, 너무 감성적이라는 판단을 내리기 전에 서로의 방식을 먼저 이해하려는 노력이 필요하오.",
  "원진_신묘": "묘(卯)와 신(申)의 원진이오. 묘목의 부드럽고 감성적인 기운과 신금의 날카롭고 현실적인 기운이 서로를 낯설게 느끼는 구조이오. 한 사람은 관계 안에서 감정과 분위기를 중시하고, 다른 사람은 효율과 논리를 우선하는 경향이 있어 대화의 결이 자주 어긋날 수 있소. 상대방이 차갑다거나, 너무 감성적이라는 판단을 내리기 전에 서로의 방식을 먼저 이해하려는 노력이 필요하오.",
  "원진_진해": "진(辰)과 해(亥)의 원진이오. 진토의 다양하고 창의적인 기운과 해수의 깊고 자유로운 기운이 서로를 불편하게 느끼는 구조이오. 두 기운 모두 자기만의 방식이 뚜렷해 상대방의 방식을 받아들이는 것이 쉽지 않소. 함께 있으면 피로감이 쌓이거나, 상대방이 이해되지 않는 느낌이 드는 경우가 있소. 각자의 세계를 존중하고 너무 깊이 파고들지 않는 적절한 거리가 오히려 관계를 유지하는 힘이 되오.",
  "원진_해진": "진(辰)과 해(亥)의 원진이오. 진토의 다양하고 창의적인 기운과 해수의 깊고 자유로운 기운이 서로를 불편하게 느끼는 구조이오. 두 기운 모두 자기만의 방식이 뚜렷해 상대방의 방식을 받아들이는 것이 쉽지 않소. 함께 있으면 피로감이 쌓이거나, 상대방이 이해되지 않는 느낌이 드는 경우가 있소. 각자의 세계를 존중하고 너무 깊이 파고들지 않는 적절한 거리가 오히려 관계를 유지하는 힘이 되오.",
  "원진_사술": "사(巳)와 술(戌)의 원진이오. 사화의 집중적이고 목적 지향적인 기운과 술토의 열정적이고 고집스러운 기운이 서로를 어긋나게 느끼는 구조이오. 두 사람 모두 자신의 방식에 대한 확신이 강하고, 상대방의 방식이 비효율적이거나 맞지 않는다고 느끼기 쉽소. 무엇보다 서로에게 변화를 요구하기보다 상대방이 왜 그 방식을 선택하는지 이해하려는 자세가 이 원진을 가장 부드럽게 만드는 방법이오.",
  "원진_술사": "사(巳)와 술(戌)의 원진이오. 사화의 집중적이고 목적 지향적인 기운과 술토의 열정적이고 고집스러운 기운이 서로를 어긋나게 느끼는 구조이오. 두 사람 모두 자신의 방식에 대한 확신이 강하고, 상대방의 방식이 비효율적이거나 맞지 않는다고 느끼기 쉽소. 무엇보다 서로에게 변화를 요구하기보다 상대방이 왜 그 방식을 선택하는지 이해하려는 자세가 이 원진을 가장 부드럽게 만드는 방법이오.",
};

function buildCrossRelDesc(r: Rel): string {
  const key = `${r.kind}_${r.chars.join("")}`;
  const reverseKey = `${r.kind}_${[...r.chars].reverse().join("")}`;
  return CROSS_DESC[key] ?? CROSS_DESC[reverseKey] ?? KIND_MEANING[r.kind].effect;
}
...
function calcKunghapScore(rels: Rel[]): { score: number; label: string } {
  const raw = rels.reduce((acc, r) => acc + (REL_SCORE[r.kind] ?? 0), 70);
  const score = Math.min(100, Math.max(0, raw));
  const label = (SCORE_LABELS.find(([min]) => score >= min) ?? SCORE_LABELS[SCORE_LABELS.length - 1])[1];
  return { score, label };
}

function buildHapChungDesc(rels: Rel[], score: number): string {
  const HAP_KINDS: RelKind[] = ["천간합", "육합", "삼합"];
  ...
  if (hapCount > 0) {
    const hapRole = (() => {
      ...
      return `합(合)은 두 사람 사이에서 ${roles.join(", ")}의 기운으로 작용하오. 이 합이 있는 영역에서 두 사람은 억지로 맞추지 않아도 자연스럽게 통하게 되며, 함께 있을 때 편안함과 연결감의 근원이 되오.`;
    })();
    if (hapRole) parts.push(hapRole);
  }
  ...
  const conclusion = (() => {
    if (score >= 90)
      return `총평하자면, 이 관계는 사주 궁합상 매우 드문 강한 조화를 이루고 있소. 기운의 연결이 여러 겹으로 탄탄하게 쌓여 있어, 함께할수록 서로에게 힘이 되고 안정이 되는 인연이오. 이 기운을 믿고 함께 나아가시오.`;
    if (score >= 80)
      return `총평하자면, 이 관계는 조화로운 기운이 마찰보다 더 강하게 흐르고 있소. 함께할 때 자연스러운 편안함이 있고, 서로를 이해하려는 마음이 있다면 마찰도 충분히 소화해낼 수 있는 구조이오. 좋은 인연을 잘 가꾸어 나가시오.`;
    if (score >= 70)
      return `총평하자면, 이 관계는 맞는 부분과 어긋나는 부분이 공존하는 현실적인 궁합이오. 무조건 잘 맞는 것도, 무조건 힘든 것도 아니오. 서로의 다름을 인정하고 조율하는 노력 위에서 이 관계는 단단해질 수 있소.`;
    if (score >= 60)
      return `총평하자면, 이 관계는 조화보다 마찰의 기운이 더 강하게 작용하고 있소. 자연스럽게 흐르기 위해서는 의식적인 소통과 배려가 꾸준히 필요하오. 어렵더라도 서로를 이해하려는 노력을 멈추지 않는다면, 이 관계는 충분히 의미 있는 인연이 될 수 있소.`;
    if (score >= 50)
      return `총평하자면, 이 관계는 여러 층위에서 충돌하는 기운이 강하오. 쉽게 흐르는 관계는 아니지만, 가장 어려운 궁합이 가장 강렬한 성장을 이끌기도 하오. 서로를 변화시키는 관계가 될 수 있소. 다만 그 무게를 감당할 준비가 되어 있는지 솔직하게 들여다볼 필요가 있소.`;
    return `총평하자면, 이 관계는 사주 궁합상 매우 도전적인 구조이오. 서로를 끌어당기는 기운보다 부딪히는 기운이 훨씬 강하여, 자연스럽게 흐르기 어려운 관계이오. 이 관계를 유지하고 발전시키려면 서로에 대한 깊은 이해와 지속적인 노력이 전제되어야 하오. 그 노력이 뒷받침된다면 이 관계는 두 사람 모두를 크게 성장시키는 인연이 될 수 있소.`;
  })();
...
function KunghapRelationCard({ myView, partnerView, myName, partnerName, myColor, partnerColor }: {
  ...
  const rels = calcCrossRelations(myView, partnerView);
  ...
  return (
    <div className="mx-5 mb-4 rounded-2xl p-3.5" style={{ background: CREAM, border: `1px solid ${INK}12` }}>
      ...
      {rels.length === 0 ? (
        ...
        const HAP_KINDS: RelKind[] = ["천간합", "육합", "삼합"];
        const CHUNG_KINDS: RelKind[] = ["천간충", "충", "형", "파", "해", "원진"];
        ...
        const renderBtn = ({ r, i }: { r: Rel; i: number }) => {
          const m = KIND_META[r.kind];
            ...
            {selected !== null && (() => {
              ...
              return (
                <div className="rounded-xl px-4 py-3 mt-2" style={{ background: `${m.color}10`, borderLeft: `3px solid ${m.color}` }}>
                  ...
                  <p className="text-[13px] leading-[1.9]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{buildCrossRelDesc(r)}</p>
                </div>
              );
            })()}
          </>
...
// ─── 연애궁합 전용 섹션 정의 ──────────────────────────────────────
// YEONAE_KUNGHAP_CHAPTER_SECTIONS / isYeonaeKunghapChapterReady 는 kunghap_yeonae-report-content.ts 에서 import
function isYeonaeChapterReady(content: Record<string, unknown> | null | undefined, chapter: number): boolean {
  if (chapter === 0) return true; // 인트로 — 정적
  return isYeonaeKunghapChapterReady(content, chapter);
}
...
function AttractionGauge({ score, label }: { score: number; label: string }) {
  ...
  // 단일 붉은 톤: 점수 낮을수록 연한 장밋빛, 높을수록 진한 심홍색
  const gaugeColorLight = "#f0c4cc"; // 연한 장밋빛 (시작)
  const gaugeColorDark  = MAROON;    // 진한 심홍색 (끝)
  const COLOR = s >= 80 ? MAROON : s >= 60 ? ROSE : s >= 40 ? "#d4828f" : "#c9a0a8";
  return (
...
// 두 사람 첫인상 카드 — 방향 화살표 버전
function FirstImpressionDuelCard({ data, myName, partnerName, myGender, partnerGender }: { data: Record<string, unknown> | null; myName: string; partnerName: string; myGender?: string; partnerGender?: string }) {
  if (!data) return null;
  ...
  const partnerColor = isFemale(partnerGender) ? PINK : BLUE;

  const NameChip = ({ name, color }: { name: string; color: string }) => (
    <span className="text-[12px] font-black px-2.5 py-1 rounded-full" style={{ background: `${color}18`, color, border: `1.5px solid ${color}40` }}>
      {name}님
    </span>
  );
    ...
    <div className="mb-4">
      ...
      <div className="mx-5 mb-4 rounded-2xl overflow-hidden" style={{ border: `1px solid ${myColor}20`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ background: `${myColor}08`, borderBottom: `1px solid ${myColor}15` }}>
          <NameChip name={myName} color={myColor} />
          <span className="text-[16px] font-black" style={{ color: MUTE }}>→</span>
          <NameChip name={partnerName} color={partnerColor} />
        </div>
        <div className="px-4 py-4" style={{ background: WHITE }}>
          {mineDesc && <p className="text-[13px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{mineDesc}</p>}
        </div>
      </div>
      {/* 상대 → 나 */}
      <div className="mx-5 mb-5 rounded-2xl overflow-hidden" style={{ border: `1px solid ${partnerColor}20`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center gap-2 px-4 py-3" style={{ background: `${partnerColor}08`, borderBottom: `1px solid ${partnerColor}15` }}>
          <NameChip name={partnerName} color={partnerColor} />
          <span className="text-[16px] font-black" style={{ color: MUTE }}>→</span>
          <NameChip name={myName} color={myColor} />
        </div>
        <div className="px-4 py-4" style={{ background: WHITE }}>
          {partnerDesc && <p className="text-[13px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{partnerDesc}</p>}
        </div>
      </div>
    </div>
  );
...
function SipseongMeaningCard({ sipseong, desc, color, myIlgan, partnerIlgan, myName, partnerName, myGender, partnerGender }: { sipseong: string; desc: string; color: string; myIlgan?: string; partnerIlgan?: string; myName?: string; partnerName?: string; myGender?: string; partnerGender?: string }) {
  const BLUE = "#4a7cc9"; const PINK = "#c9607a";
  const isFemaleG = (g?: string) => g === "female" || g === "여성" || g === "여자";
  const myGenderColor = isFemaleG(myGender) ? PINK : BLUE;
  const partnerGenderColor = isFemaleG(partnerGender) ? PINK : BLUE;
  ...
  const ilganLabel = (gan?: string) => gan ? `${GAN_KOR[gan] ?? gan}${GAN_OHAENG[gan] ?? ""}일간` : "";
  const ilganShort = (gan?: string) => gan ? `${GAN_KOR[gan] ?? gan}${GAN_OHAENG[gan] ?? ""}` : "";
  return (
      ...
      <div className="px-5 py-5" style={{ background: `linear-gradient(135deg, ${color}10 0%, ${color}04 100%)`, borderBottom: `1px solid ${color}15` }}>
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <div className="w-12 h-12 flex items-center justify-center">
              {myIlgan
                ...
                : <span className="text-[26px] font-black" style={{ color: myGenderColor, fontFamily: SERIF }}>?</span>}
            </div>
            <div className="mt-1 px-2 py-0.5 rounded-full text-[11px] font-bold" style={{ background: `${myGenderColor}12`, color: myGenderColor, border: `1px solid ${myGenderColor}30` }}>
              {ilganLabel(myIlgan)}
            </div>
          </div>
          {/* 관계 표시 */}
          <div className="flex flex-col items-center gap-1 flex-1">
            {/* 십성 뱃지 — 화살표 위 */}
            <div className="px-2 py-1 rounded-full text-[10px] font-black" style={{ background: `${color}15`, color, border: `1.5px solid ${color}40` }}>
              {sipseong}
            </div>
            {/* 화살표 라인 */}
            <div className="w-full flex items-center">
              ...
              <svg width="8" height="10" viewBox="0 0 8 10" fill="none">
                <path d="M0 0L8 5L0 10V0Z" fill={color} fillOpacity="0.5" />
              </svg>
            </div>
          </div>
          {/* 상대 일간 */}
          <div className="flex flex-col items-center gap-1">
            <p className="text-[13px] font-bold mb-1" style={{ color: partnerGenderColor }}>{partnerName ?? "상대"}님</p>
            <div className="w-12 h-12 flex items-center justify-center">
              {partnerIlgan
                ? <img src={ganCharImage(partnerIlgan)} alt={partnerIlgan} className="w-full h-full object-contain" />
                : <span className="text-[26px] font-black" style={{ color: partnerGenderColor, fontFamily: SERIF }}>?</span>}
            </div>
            <div className="mt-1 px-2 py-0.5 rounded-full text-[11px] font-bold" style={{ background: `${partnerGenderColor}12`, color: partnerGenderColor, border: `1px solid ${partnerGenderColor}30` }}>
              {ilganLabel(partnerIlgan)}
            </div>
          </div>
        </div>
      </div>
...
function MyViewCard({ data, color = CH4_COLOR, pale = CH4_PALE }: { data: Record<string, unknown> | null; color?: string; pale?: string }) {
  ...
  return (
    <div className="mb-2">
      {intro && (
        <p className="px-6 text-[14px] leading-[1.85] mb-4 font-medium" style={{ color: INK_SOFT, fontFamily: SERIF }}>{intro}</p>
      )}
      <div className="px-6">
        {paragraphs.map((p, i) => (
...
// 합(合) 상세 카드 — 합 종류·강도·합화 오행·관계 영향·연애 효과
function HapCard({ item, index }: { item: { type: string; hapEl?: string; desc?: string; strength?: string; effect?: string }; index: number }) {
...
// 충(沖) 상세 카드 — 충 종류·강도·관계 영향·극복법
function ChungCard({ item, index }: { item: { type: string; desc?: string; strength?: string; effect?: string; overcome?: string }; index: number }) {
  const strengthStars = item.strength === "강함" ? "▲▲▲" : item.strength === "보통" ? "▲▲△" : "▲△△";
  return (
...
// 합·충 종합 스코어 카드
function HapChungScoreCard({ data, hapCount, chungCount }: { data: Record<string, unknown> | null; hapCount: number; chungCount: number }) {
  if (!data) return null;
    ...
    <div className="mx-5 mb-5 rounded-2xl overflow-hidden" style={{ background: WHITE, border: `1px solid ${CH6_COLOR}15`, boxShadow: "0 2px 16px rgba(0,0,0,0.05)" }}>
      ...
      <div className="px-5 pt-4 pb-2">
        <svg viewBox="0 10 220 100" style={{ width: "100%", maxHeight: 120 }}>
          ...
          <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="#eee" strokeWidth="18" strokeLinecap="round" />
          <path d={`M${cx - r},${cy} A${r},${r} 0 0,1 ${cx + r},${cy}`} fill="none" stroke="url(#gaugeGrad6)" strokeWidth="18" strokeLinecap="round" strokeDasharray={`${(s / 100) * Math.PI * r} ${Math.PI * r}`} />
          <line x1={cx} y1={cy} x2={needleX} y2={needleY} stroke={INK} strokeWidth="2.5" strokeLinecap="round" />
          <circle cx={cx} cy={cy} r="5" fill={INK} />
          <text x={cx} y={cy - 12} textAnchor="middle" fontSize="24" fontWeight="900" fill={COLOR}>{s}</text>
        </svg>
...
function RelationStrengthCard({ item, index }: { item: { title: string; icon?: string; desc?: string; effect?: string }; index: number }) {
  const KOR_ORD = ["첫번째","두번째","세번째","네번째","다섯번째","여섯번째"];
  const ordinal = KOR_ORD[index] ?? `${index + 1}번째`;
  return (
    <div className="mx-5 mb-4 rounded-2xl overflow-hidden" style={{ background: "linear-gradient(160deg, #fffbee 0%, #fff8e0 100%)", border: `1px solid #e8c06040`, boxShadow: "0 1px 10px rgba(0,0,0,0.04)" }}>
      {/* 헤더 */}
      <div className="px-5 py-2.5 flex items-center gap-2" style={{ borderBottom: `1px solid #e8c06030` }}>
        <span className="text-[18px] flex-shrink-0">{item.icon ?? "✨"}</span>
        <p className="text-[14px] font-black leading-tight" style={{ color: "#5a3e10" }}>
          <span style={{ color: "#c9a040" }}>{ordinal} 빛</span> — {item.title}
        </p>
      </div>
      {/* 풀이 */}
      {item.desc && (
        <div className="px-5 py-4" style={{ borderBottom: item.effect ? `1px solid #e8c06020` : "none" }}>
          <p className="text-[13px] leading-relaxed" style={{ color: "#7a6030" }}>{item.desc}</p>
        </div>
      )}
      {/* 빛나는 장면 */}
      {item.effect && (
        <div className="px-5 py-2.5 flex items-start gap-2" style={{ background: "#e8c06015" }}>
          <span className="text-[12px] font-black flex-shrink-0 mt-0.5" style={{ color: "#c9a040" }}>✦</span>
          <p className="text-[12px] leading-relaxed font-medium" style={{ color: "#7a6030" }}>{item.effect}</p>
        </div>
      )}
    </div>
  );
...
function RelationShadowCard({ item, index }: { item: { title: string; icon?: string; desc?: string; trigger?: string; overcome?: string }; index: number }) {
  ...
  const ordinal = KOR_ORD[index] ?? `${index + 1}번째`;
  return (
    <div className="mx-5 mb-4 rounded-2xl overflow-hidden" style={{ background: "linear-gradient(160deg, #f4f5f8 0%, #eaedf3 100%)", border: `1px solid #b0b8cc40`, boxShadow: "0 1px 10px rgba(0,0,0,0.04)" }}>
      {/* 헤더 */}
      <div className="px-5 py-2.5 flex items-center gap-2" style={{ borderBottom: `1px solid #b0b8cc30` }}>
        <span className="text-[18px] flex-shrink-0">{item.icon ?? "🌙"}</span>
        <p className="text-[14px] font-black leading-tight" style={{ color: "#2d3748" }}>
          <span style={{ color: "#6b7a99" }}>{ordinal} 그림자</span> — {item.title}
        </p>
      </div>
      {/* 풀이 */}
      {item.desc && (
        <div className="px-5 py-4" style={{ borderBottom: (item.trigger || item.overcome) ? `1px solid #b0b8cc20` : "none" }}>
          <p className="text-[13px] leading-relaxed" style={{ color: "#4a5568" }}>{item.desc}</p>
        </div>
      )}
      {/* 발생 상황 */}
      {item.trigger && (
        <div className="px-5 py-2.5 flex items-start gap-2" style={{ background: "#b0b8cc15", borderBottom: item.overcome ? `1px solid #b0b8cc20` : "none" }}>
          <span className="text-[12px] font-black flex-shrink-0 mt-0.5" style={{ color: "#6b7a99" }}>▲</span>
          <p className="text-[12px] leading-relaxed" style={{ color: "#4a5568" }}>{item.trigger}</p>
        </div>
      )}
      {/* 극복법 */}
      {item.overcome && (
        <div className="px-5 py-2.5 flex items-start gap-2" style={{ background: "#b0b8cc10" }}>
          <span className="text-[12px] font-black flex-shrink-0 mt-0.5" style={{ color: "#6b7a99" }}>✦</span>
          <p className="text-[12px] leading-relaxed font-medium" style={{ color: "#4a5568" }}>{item.overcome}</p>
        </div>
      )}
    </div>
  );
...
function LightShadowSummaryBanner({ lightSummary, shadowSummary }: { lightSummary: string; shadowSummary: string }) {
  ...
  const trimPrefix = (text: string, prefixes: string[]) => {
    for (const p of prefixes) {
      if (text.startsWith(p)) return text.slice(p.length).trimStart();
    }
    return text;
  };
  const lightText = trimPrefix(lightSummary, ["이 관계의 빛은 ", "이 관계의 빛은", "빛은 "]);
  const shadowText = trimPrefix(shadowSummary, ["이 관계의 그림자는 ", "이 관계의 그림자는", "그림자는 "]);
  return (
      ...
      <div className="flex">
        ...
        {lightSummary && (
          <div className="flex-1 px-4 py-5" style={{ background: "linear-gradient(160deg, #fffbee 0%, #fff8e0 100%)", borderRight: `1px solid ${INK}08` }}>
            ...
            <p className="text-[14px] font-black mb-2 text-center" style={{ color: LIGHT_COLOR }}>이 관계의 빛</p>
        ...
        {shadowSummary && (
          <div className="flex-1 px-4 py-5" style={{ background: "linear-gradient(160deg, #f4f5f8 0%, #eaedf3 100%)" }}>
            ...
            <p className="text-[14px] font-black mb-2 text-center" style={{ color: SHADOW_COLOR }}>이 관계의 그림자</p>
...
// 위기 요소 카드 — 번호 뱃지 + icon + 제목 + 사주 근거 풀이 + 신호 + 찾아오는 시기
function CrisisCard({ item, index }: {
  item: { title: string; icon?: string; desc?: string; when?: string; signal?: string };
  index: number;
}) {
  const icon = item.icon ?? "⚠️";
  return (
    <div className="rounded-2xl overflow-hidden mb-4" style={{ background: WHITE, border: `1.5px solid ${CRISIS_COLOR}25`, boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 pt-4 pb-3" style={{ borderBottom: `1px solid ${CRISIS_COLOR}15` }}>
        <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: CRISIS_COLOR, color: WHITE }}>
          <span className="text-[11px] font-black">{index + 1}</span>
        </div>
        <span className="text-[18px]">{icon}</span>
        <p className="text-[14px] font-black leading-tight" style={{ color: CH9_COLOR }}>{item.title}</p>
      </div>
      {/* 사주 근거 풀이 */}
      {item.desc && (
        <div className="px-4 pt-3 pb-2">
          <p className="text-[13.5px] leading-[1.9]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{item.desc}</p>
        </div>
      )}
    </div>
  );
...
// 극복법 카드 — icon + 제목 + 설명 풀이 + 단계별 실천
function OvercomeCard({ item, index }: {
  item: { title: string; icon?: string; desc?: string; steps?: string[] };
  index: number;
}) {
  const icon = item.icon ?? "🌱";
  const steps = item.steps ?? [];
  return (
    <div className="rounded-2xl overflow-hidden mb-4" style={{ background: WHITE, border: `1.5px solid ${OVERCOME_COLOR}25`, boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 pt-4 pb-3" style={{ borderBottom: `1px solid ${OVERCOME_COLOR}15` }}>
        <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: OVERCOME_COLOR, color: WHITE }}>
          <span className="text-[11px] font-black">{index + 1}</span>
        </div>
        <span className="text-[18px]">{icon}</span>
        <p className="text-[14px] font-black leading-tight" style={{ color: OVERCOME_COLOR }}>{item.title}</p>
      </div>
      {/* 설명 풀이 */}
      {item.desc && (
      ...
      {/* 단계별 실천 */}
      {steps.length > 0 && (
        <div className="mx-4 mb-4 rounded-xl overflow-hidden" style={{ border: `1px solid ${OVERCOME_COLOR}20` }}>
          <div className="px-3 py-2" style={{ background: `${OVERCOME_COLOR}10`, borderBottom: `1px solid ${OVERCOME_COLOR}15` }}>
            <p className="text-[10px] font-black tracking-wide" style={{ color: OVERCOME_COLOR }}>실천 단계</p>
          </div>
          <div className="px-3 py-2.5 space-y-2">
            {steps.map((step, i) => (
              <div key={i} className="flex items-start gap-2.5">
                <span className="w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 text-[9px] font-black" style={{ background: OVERCOME_COLOR, color: WHITE }}>{i + 1}</span>
                <p className="text-[12.5px] leading-relaxed" style={{ color: INK_SOFT }}>{step}</p>
              </div>
            ))}
          </div>
        </div>
...
// 시기별 위기·극복 타임라인 — 연도 라벨 + tone 색상 + 기운 설명 + 이 시기 팁
function CrisisFlowTimeline({ items, overviewDesc }: {
  items: Array<{ label: string; tone: string; text: string; tip?: string }>;
  overviewDesc?: string;
}) {
  if (!items.length) return null;
  const toneStyle = (tone: string) => {
    if (tone === "good") return { dot: OVERCOME_COLOR, bg: `${OVERCOME_COLOR}0c`, border: `${OVERCOME_COLOR}30`, label: OVERCOME_COLOR };
    if (tone === "warn") return { dot: CRISIS_COLOR, bg: `${CRISIS_COLOR}0c`, border: `${CRISIS_COLOR}30`, label: CRISIS_COLOR };
    return { dot: CH9_COLOR, bg: `${CH9_COLOR}0c`, border: `${CH9_COLOR}30`, label: CH9_COLOR }; // caution
  };
  const toneIcon = (tone: string) => tone === "good" ? "☀️" : tone === "warn" ? "⛈️" : "🌤️";
  return (
    <div>
      {overviewDesc && (
        <div className="mx-5 mb-5 rounded-xl px-4 py-3.5" style={{ background: CALLOUT_BG, borderLeft: `3px solid ${CH9_COLOR}` }}>
          <p className="text-[13.5px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{overviewDesc}</p>
        </div>
      )}
      <div className="relative px-5">
        {/* 세로 연결선 */}
        <div className="absolute left-[38px] top-3 bottom-3 w-px" style={{ background: `${CH9_COLOR}20` }} />
        <div className="space-y-4">
          {items.map((item, i) => {
            const s = toneStyle(item.tone ?? "caution");
            return (
              <div key={i} className="flex items-start gap-3">
                {/* 점 */}
                <div className="flex-shrink-0 mt-1 w-8 flex justify-center">
                  <div className="w-4 h-4 rounded-full flex items-center justify-center" style={{ background: s.dot, boxShadow: `0 0 0 3px ${s.dot}22` }}>
                    <div className="w-2 h-2 rounded-full" style={{ background: WHITE }} />
                  </div>
                </div>
                {/* 카드 */}
                <div className="flex-1 rounded-xl overflow-hidden mb-1" style={{ background: s.bg, border: `1px solid ${s.border}` }}>
                  {/* 시기 라벨 */}
                  <div className="flex items-center gap-2 px-3 pt-2.5 pb-2" style={{ borderBottom: `1px solid ${s.border}` }}>
                    <span className="text-[13px]">{toneIcon(item.tone)}</span>
                    <p className="text-[11px] font-black tracking-wide" style={{ color: s.label }}>{item.label}</p>
                  </div>
                  {/* 기운 설명 */}
                  <div className="px-3 pt-2.5 pb-2">
                    <p className="text-[13px] leading-[1.8]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{item.text}</p>
                  </div>
                  {/* 이 시기 팁 */}
                  {item.tip && (
                    <div className="mx-3 mb-3 rounded-lg px-3 py-2" style={{ background: `rgba(255,255,255,0.5)` }}>
                      <p className="text-[11px] font-black mb-0.5" style={{ color: s.label }}>이 시기의 방법</p>
                      <p className="text-[12px] leading-relaxed" style={{ color: INK_SOFT }}>{item.tip}</p>
                    </div>
                  )}
                </div>
              </div>
...
// 결혼 조건 카드 — 번호 뱃지 + icon + 조건 제목 + 사주 근거 풀이 + 충족 가능성
function MarriageConditionCard({ item, index }: {
  item: { title: string; icon?: string; desc?: string; possibility?: string };
  index: number;
}) {
  const icon = item.icon ?? "💍";
  return (
    <div className="rounded-2xl overflow-hidden mb-4" style={{ background: WHITE, border: `1.5px solid ${CH10_COLOR}25`, boxShadow: "0 2px 12px rgba(0,0,0,0.05)" }}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 pt-4 pb-3" style={{ borderBottom: `1px solid ${CH10_COLOR}15` }}>
        <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: CH10_COLOR, color: WHITE }}>
          <span className="text-[11px] font-black">{index + 1}</span>
        </div>
        <span className="text-[18px]">{icon}</span>
        <p className="text-[14px] font-black leading-tight" style={{ color: CH10_COLOR }}>{item.title}</p>
      </div>
      {/* 사주 근거 풀이 */}
      {item.desc && (
        <div className="px-4 pt-3 pb-2">
      ...
      {/* 충족 가능성 */}
      {item.possibility && (
        <div className="mx-4 mb-4 rounded-xl px-3 py-2.5 flex items-start gap-2" style={{ background: `${CH10_COLOR}08`, border: `1px solid ${CH10_COLOR}18` }}>
          <span className="text-[13px] flex-shrink-0 mt-0.5">✦</span>
          <div>
            <p className="text-[10px] font-black mb-0.5 tracking-wide" style={{ color: CH10_COLOR }}>충족 가능성</p>
            <p className="text-[12.5px] leading-relaxed" style={{ color: INK_SOFT }}>{item.possibility}</p>
          </div>
        </div>
      )}
    </div>
...
// 결혼 시기 카드 — 가장 좋은 시기 하이라이트 + 사주 근거 + 조심할 시기 + 조언
function MarriageTimingCard({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  const bestPeriod   = (data.bestPeriod   as string | undefined) ?? "";
  const desc         = (data.desc         as string | undefined) ?? "";
  const advice       = (data.advice       as string | undefined) ?? "";
  const cautionPeriod = (data.cautionPeriod as string | undefined) ?? "";
  const cautionDesc  = (data.cautionDesc  as string | undefined) ?? "";
  return (
    <div className="space-y-4">
      {/* 가장 좋은 시기 하이라이트 */}
      {bestPeriod && (
        <div className="mx-0 rounded-2xl overflow-hidden" style={{ background: `linear-gradient(135deg, ${CH10_COLOR} 0%, #a03070 100%)` }}>
          <div className="px-5 pt-5 pb-4 text-center">
            <p className="text-[11px] tracking-[0.2em] mb-1.5" style={{ color: "rgba(255,255,255,0.65)" }}>결혼하기 가장 좋은 시기</p>
            <p className="text-[22px] font-black" style={{ color: WHITE, fontFamily: SERIF }}>{bestPeriod}</p>
            <div className="mt-3 text-[18px]">💐</div>
          </div>
        </div>
      )}
      {/* 사주 근거 풀이 */}
      {desc && (
        <div className="rounded-xl px-4 py-4" style={{ background: CH10_PALE, border: `1px solid ${CH10_COLOR}20` }}>
          <p className="text-[13.5px] leading-[1.9]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{desc}</p>
        </div>
      )}
      {/* 실천 조언 */}
      {advice && (
        <div className="rounded-xl px-4 py-3.5 flex items-start gap-2.5" style={{ background: WHITE, border: `1.5px solid ${CH10_COLOR}30` }}>
          <span className="text-[15px] flex-shrink-0 mt-0.5">💌</span>
          <div>
            <p className="text-[10px] font-black mb-0.5 tracking-wide" style={{ color: CH10_COLOR }}>홍연의 조언</p>
            <p className="text-[13px] leading-relaxed" style={{ color: INK_SOFT }}>{advice}</p>
          </div>
        </div>
      )}
      {/* 조심할 시기 */}
      {cautionPeriod && (
        <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${WARN}25` }}>
          <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: `${WARN}08`, borderBottom: `1px solid ${WARN}18` }}>
            <span className="text-[13px]">⚠️</span>
            <p className="text-[11px] font-black" style={{ color: WARN }}>조심해야 할 시기 — {cautionPeriod}</p>
          </div>
          {cautionDesc && (
            <div className="px-4 py-3">
              <p className="text-[13px] leading-[1.8]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{cautionDesc}</p>
            </div>
          )}
        </div>
      )}
...
// 좋은 시기 카드 — 시기 뱃지 + icon + 설명 3~4문장 + 함께하면 좋은 것 + 사주 근거
function GoodPeriodCard({ item, index }: {
  item: { label: string; icon?: string; desc?: string; activity?: string; sajuBasis?: string };
  index: number;
}) {
  const icon = item.icon ?? "🌸";
  return (
    <div className="rounded-2xl overflow-hidden mb-4" style={{ background: WHITE, border: `1.5px solid ${CH11_COLOR}28`, boxShadow: "0 2px 14px rgba(0,0,0,0.05)" }}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 pt-4 pb-3" style={{ borderBottom: `1px solid ${CH11_COLOR}15`, background: `${CH11_COLOR}06` }}>
        <div className="flex-shrink-0 px-3 py-1 rounded-full text-[11px] font-black" style={{ background: CH11_COLOR, color: WHITE }}>{`${index + 1}번째 봄날`}</div>
        <span className="text-[18px]">{icon}</span>
        <p className="text-[13px] font-black leading-tight" style={{ color: CH11_COLOR }}>{item.label}</p>
      </div>
      {/* 기운 설명 */}
      {item.desc && (
      ...
      {/* 함께하면 좋은 것 */}
      {item.activity && (
        <div className="mx-4 mb-3 rounded-xl px-3 py-2.5 flex items-start gap-2" style={{ background: `${CH11_COLOR}0c`, border: `1px solid ${CH11_COLOR}20` }}>
          <span className="text-[13px] flex-shrink-0 mt-0.5">✨</span>
          <div>
            <p className="text-[10px] font-black mb-0.5 tracking-wide" style={{ color: CH11_COLOR }}>이 시기에 함께하면 좋은 것</p>
            <p className="text-[12.5px] leading-relaxed" style={{ color: INK_SOFT }}>{item.activity}</p>
          </div>
        </div>
      )}
      {/* 사주 근거 */}
      {item.sajuBasis && (
        <div className="mx-4 mb-4 rounded-xl px-3 py-2.5 flex items-start gap-2" style={{ background: `${CH11_COLOR}06`, border: `1px solid ${CH11_COLOR}15` }}>
          <span className="text-[11px] font-black flex-shrink-0 mt-0.5" style={{ color: CH11_COLOR }}>☯</span>
          <div>
            <p className="text-[10px] font-black mb-0.5 tracking-wide" style={{ color: CH11_COLOR }}>사주 근거</p>
            <p className="text-[12px] leading-relaxed" style={{ color: MUTE }}>{item.sajuBasis}</p>
          </div>
        </div>
      )}
    </div>
...
// 조심할 시기 카드 — 경고 뱃지 + icon + 이유 2~3문장 + 대처법
function CautionPeriodCard({ item, index }: {
  item: { label: string; icon?: string; desc?: string; cope?: string };
  index: number;
}) {
  const icon = item.icon ?? "🌧️";
  return (
    <div className="rounded-2xl overflow-hidden mb-3" style={{ background: WHITE, border: `1.5px solid ${WARN}28` }}>
      {/* 헤더 */}
      <div className="flex items-center gap-3 px-4 pt-3.5 pb-2.5" style={{ borderBottom: `1px solid ${WARN}15`, background: `${WARN}07` }}>
        <div className="flex-shrink-0 px-3 py-0.5 rounded-full text-[10px] font-black" style={{ background: `${WARN}20`, color: WARN }}>{`주의 ${index + 1}`}</div>
        <span className="text-[16px]">{icon}</span>
        <p className="text-[13px] font-black leading-tight" style={{ color: WARN }}>{item.label}</p>
      </div>
      {/* 조심 이유 */}
      {item.desc && (
        <div className="px-4 pt-3 pb-2">
          <p className="text-[13px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{item.desc}</p>
        </div>
      )}
      {/* 대처법 */}
      {item.cope && (
        <div className="mx-4 mb-3.5 rounded-lg px-3 py-2.5 flex items-start gap-2" style={{ background: `${GREEN}08`, border: `1px solid ${GREEN}20` }}>
          <span className="text-[12px] flex-shrink-0 mt-0.5">🌱</span>
          <div>
            <p className="text-[10px] font-black mb-0.5" style={{ color: GREEN }}>이 시기의 대처법</p>
            <p className="text-[12.5px] leading-relaxed" style={{ color: INK_SOFT }}>{item.cope}</p>
          </div>
        </div>
      )}
    </div>
...
// 연간 흐름 종합 요약 — 전체 설명 + 핵심 팁 리스트
function YearlyFlowSummary({ data }: { data: Record<string, unknown> | null }) {
  if (!data) return null;
  const overviewDesc = (data.overviewDesc as string | undefined) ?? "";
  const tips = (data.tips as string[] | undefined) ?? [];
  return (
    <div>
      {overviewDesc && (
        <div className="mb-5 rounded-xl px-4 py-4" style={{ background: CH11_PALE, borderLeft: `3px solid ${CH11_COLOR}` }}>
          <p className="text-[13.5px] leading-[1.9]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{overviewDesc}</p>
        </div>
      )}
      {tips.length > 0 && (
        <div className="rounded-2xl overflow-hidden" style={{ background: WHITE, border: `1px solid ${CH11_COLOR}20` }}>
          <div className="px-4 py-3" style={{ background: `${CH11_COLOR}0c`, borderBottom: `1px solid ${CH11_COLOR}15` }}>
            <p className="text-[12px] font-black" style={{ color: CH11_COLOR }}>🌿 두 사람이 이 시기를 잘 보내는 법</p>
          </div>
          <div className="px-4 py-3.5 space-y-3.5">
            {tips.map((tip, i) => (
              <div key={i} className="flex items-start gap-3">
                <span className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center mt-0.5 text-[9px] font-black" style={{ background: CH11_COLOR, color: WHITE }}>{i + 1}</span>
                <p className="text-[13px] leading-[1.8]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{tip}</p>
              </div>
            ))}
          </div>
        </div>
...
// 연애 흐름 꺾은선 차트 — crisisFlow.items의 score 기반
function LoveFlowLineChart({ items }: {
  items: Array<{ label: string; tone: string; score?: number; text: string; tip?: string }>;
}) {
  if (!items.length) return null;
  ...
  const toneScore = (tone: string, fallback?: number) => {
    ...
    if (tone === "good") return 80;
    if (tone === "caution") return 55;
    return 30; // warn
  };
  const data = items.map((it) => ({
    label: it.label,
    tone: it.tone,
    score: toneScore(it.tone, it.score),
    text: it.text,
    tip: it.tip,
  }));

  const toneColor = (tone: string) =>
    tone === "good" ? OVERCOME_COLOR : tone === "warn" ? CRISIS_COLOR : CH9_COLOR;
  const toneIcon = (tone: string) =>
    tone === "good" ? "☀️" : tone === "warn" ? "⛈️" : "🌤️";

  const n = data.length;
  const W = 320, H = 170, padX = 28, padTop = 18, padBot = 46;
  const minS = Math.min(...data.map((d) => d.score));
  const maxS = Math.max(...data.map((d) => d.score));
  ...
  return (
    <div>
      {/* 차트 */}
      <div className="rounded-2xl p-4 mx-5 mb-5" style={{ background: WHITE, border: `1px solid ${CH9_COLOR}18`, boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center gap-3 mb-3">
          <div className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-0.5 rounded" style={{ background: OVERCOME_COLOR }} />
            <span className="text-[10px]" style={{ color: OVERCOME_COLOR }}>좋은 기운</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-0.5 rounded" style={{ background: CH9_COLOR }} />
            <span className="text-[10px]" style={{ color: CH9_COLOR }}>주의 기운</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-0.5 rounded" style={{ background: CRISIS_COLOR }} />
            <span className="text-[10px]" style={{ color: CRISIS_COLOR }}>위기 기운</span>
          </div>
        </div>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ overflow: "visible" }}>
          <defs>
            <linearGradient id="loveFlowArea" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={CH9_COLOR} stopOpacity="0.18" />
              <stop offset="100%" stopColor={CH9_COLOR} stopOpacity="0.01" />
            </linearGradient>
          </defs>
          {/* 수평 그리드 */}
          {[0, 1, 2, 3].map((i) => {
            const yy = padTop + ((H - padTop - padBot) * i) / 3;
            return <line key={i} x1={padX} x2={W - padX} y1={yy} y2={yy} stroke={`${CH9_COLOR}12`} strokeWidth="1" />;
          })}
          {/* 영역 필 */}
          <polygon points={area} fill="url(#loveFlowArea)" />
          {/* 꺾은선 — 세그먼트별로 tone 색상 적용 */}
          {data.map((d, i) => {
            if (i === 0) return null;
            const prev = data[i - 1];
          ...
          {data.map((d, i) => {
            const col = toneColor(d.tone);
            // 라벨 줄바꿈 처리 (최대 8자 이후 줄바꿈)
            const parts = d.label.split(/[\s~·]/);
            return (
              <g key={i}>
                <circle cx={x(i)} cy={y(d.score)} r={5} fill={WHITE} stroke={col} strokeWidth="2.5" />
                <circle cx={x(i)} cy={y(d.score)} r={2.5} fill={col} />
                {/* 점수 값 */}
                <text x={x(i)} y={y(d.score) - 12} fontSize="11" fill={col} textAnchor="middle" fontWeight="700">{d.score}</text>
                {/* 시기 라벨 — 최대 2줄 */}
                {parts.slice(0, 2).map((p, pi) => (
                  <text key={pi} x={x(i)} y={H - padBot + 15 + pi * 12} fontSize="9.5" fill={INK_SOFT} textAnchor="middle">{p}</text>
                ))}
                {/* tone 아이콘 */}
                <text x={x(i)} y={H - padBot + 45} fontSize="16" textAnchor="middle">{toneIcon(d.tone)}</text>
              </g>
            );
          })}
        </svg>
        ...
        {data.map((d, i) => {
          ...
          return (
            <div key={i} className="rounded-xl overflow-hidden" style={{ border: `1.5px solid ${col}22`, background: WHITE }}>
              {/* 시기 헤더 */}
              <div className="flex items-center gap-2.5 px-3.5 pt-3 pb-2" style={{ borderBottom: `1px solid ${col}15`, background: `${col}06` }}>
                <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: col }}>
                ...
                <span className="ml-auto text-[11px] font-black px-2 py-0.5 rounded-full" style={{ background: `${col}18`, color: col }}>{d.score}점</span>
              </div>
              {/* 기운 설명 */}
              <div className="px-3.5 pt-2.5 pb-2">
                <p className="text-[13px] leading-[1.85]" style={{ color: INK_SOFT, fontFamily: SERIF }}>{d.text}</p>
              </div>
              {/* 이 시기 팁 */}
              {d.tip && (
                <div className="mx-3.5 mb-3 rounded-lg px-3 py-2" style={{ background: `${col}08`, border: `1px solid ${col}18` }}>
                  <p className="text-[10px] font-black mb-0.5" style={{ color: col }}>이 시기의 방법</p>
...
const TOC_A: TocEntry[] = [
  ...
  { disp: "마무리", chip: "당부",   title: "홍연이 남기는 마지막 서신",        no: "11" },
];
        ...
        <div className="px-5 py-6">
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[15px]" style={{ color: MUTE }}>[연애궁합]</span>
            </div>
            <button
              onClick={onClose}
...
function ReportPreviewInner() {
  ...
  const needGen = !!id && (chNum === 0 ? false : !!YEONAE_KUNGHAP_CHAPTER_SECTIONS[chNum]) && !isYeonaeChapterReady(report?.content as Record<string, unknown>, chNum);
  ...
  const next = (n: string) => router.push(`/saju/kunghap_yeonae/report-preview?${id ? `id=${id}&` : ""}ch=${n}`);
...
<>
      ...
      {ch === "0" && (
        <>
          ...
          <section className="px-6 pt-10 pb-6">
            <P>안녕하시오.</P>
            <P>본격적으로 궁합분석에 들어가기 전에,<br />사주궁합이 무엇인지 먼저 짚고 넘어가겠소.</P>
          </section>
          ...
          {/* 맺음말 */}
          <div className="px-8 pb-10 text-center" style={{ paddingTop: 24, background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
      ...
      {ch === "1" && (() => {
        ...
        const singang = (wonguk.singang as string | undefined);
        const dominantEl = (wonguk.dominantEl as string | undefined);
        const wongukParas = (wonguk.paragraphs as string[] | undefined) ?? [];
        const callout = (wonguk.callout as string | undefined) ?? "";
        return (
          <>
            {/* ── 다크 헤더 ── */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_1/kunghap_yeonae_1_1.jpg" alt="" className="w-full block" />
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_1/kunghap_yeonae_1_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "20%", left: "62%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{name.slice(1) || name}님의 기질 속에는<br />빛과 그림자가 공존하오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
      ...
      {ch === "2" && (() => {
        ...
        const partnerLovePattern = (jc.partnerLovePattern as Record<string, unknown> | undefined) ?? null;
        const partnerWongukParas = (partnerWonguk.paragraphs as string[] | undefined) ?? [];
        return (
          <>
            {/* ── 다크 헤더 ── */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
                  ...
                  <div className="text-center">
                    ...
                    {report?.partnerBirth?.date && (
                      <p className="text-[13px]" style={{ color: "#5b504a" }}>
                        ...
                        {report.partnerBirth.gender === "female" ? "여자" : "남자"}
                      </p>
                    )}
                  </div>
                }
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_2/kunghap_yeonae_2_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "25%", left: "31%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{partnerName.slice(1) || partnerName}님의 사주팔자로<br />한폭의 그림을<br />그려봤소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_2/kunghap_yeonae_2_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "23%", left: "68%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[15px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{partnerName.slice(1) || partnerName}님의 기질 속에는<br />빛과 그림자가 공존하오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            <div className="px-8 pb-10 text-center" style={{ paddingTop: 24, background: `linear-gradient(to bottom, ${CREAM}, ${PINK_PALE})` }}>
              <div style={{ width: 1, height: 48, background: "#ccc", margin: "0 auto 32px" }} />
              <p className="text-[17px] leading-[2.1] whitespace-pre-line" style={{ color: INK, fontFamily: SERIF }}>
                {`${partnerFirstName}님의 사주도 살펴보았으니,\n이제 두 사람 사이의 끌림을\n이야기 해보겠소.`}
              </p>
            </div>
            <div style={{ background: PINK_PALE }}>
      ...
      {ch === "3" && (() => {
        ...
        return (
          <>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_3/kunghap_yeonae_3_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "15%", left: "54%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>두 사람의 첫인상,<br />사주 속에 새겨져 있소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
      ...
      {ch === "4" && (() => {
        ...
        const sipseongName = computedSipseong || mySipseong?.sipseong || "";
        ...
        return (
          <>
            ...
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "24px auto -24px" }} />
            <Quote>{`${partnerFirstName}님이 나에게\n어떤 존재로 들어와 있는지\n자세히 들여다보겠소.`}</Quote>

            {/* 십성 의미 카드 */}
            {sipseongName && (
              <section className="pb-2">
                <div className="px-5"><Heading>나에게 {partnerFirstName}님은</Heading></div>
                <SipseongMeaningCard sipseong={sipseongName} desc={sipseongDesc} color={sipseongColor} myIlgan={report?.view?.pillars[1].gan} partnerIlgan={report?.partnerView?.pillars[1].gan} myName={myFirstName} partnerName={partnerFirstName} myGender={report?.gender} partnerGender={report?.partnerGender} />
              </section>
            )}

            {/* 내가 보는 상대 풀이 */}
            <section className="pb-4">
              <div className="px-5"><Heading>내 사주로 보는 {partnerFirstName}님</Heading></div>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_4/kunghap_yeonae_4_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "24%", left: "33%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>내가 {partnerName.slice(1) || partnerName}님에게<br />기대하는 것과<br />주의할 점이 무엇인지<br />말해보겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            {myExpectation?.items && myExpectation.items.length > 0 && (
              <section className="pb-4">
                <div className="px-5"><Heading>내가 {partnerFirstName}님에게 기대하는 것</Heading></div>
            ...
            {myWarning && (
              <section className="pb-4">
                <div className="px-5"><Heading>내가 {partnerFirstName}님에게 주의할 점</Heading></div>
      ...
      {ch === "5" && (() => {
        ...
        const partnerName = report?.partnerName || "상대방";
        const partnerFirstName = partnerName.slice(1) || partnerName;
        const partnerViewData = (jc.partnerView as Record<string, unknown> | undefined) ?? null;
        const partnerExpectation = (jc.partnerExpectation as { items?: Array<{ label: string; icon?: string; desc: string; detailDesc?: string }> } | undefined) ?? null;
        const partnerWarning = (jc.partnerWarning as Record<string, unknown> | undefined) ?? null;
        const partnerSipseong = (jc.partnerSipseong as { sipseong?: string; desc?: string } | undefined) ?? null;

        // 십성 자동 계산 — 항상 계산값 우선 (LLM 오류 방지)
        let computedSipseong = "";
        let sipseongColor = CH5_COLOR;
        if (report?.partnerView?.pillars?.[1] && report?.view?.pillars?.[1]) {
          computedSipseong = sipseongOfStem(report.partnerView.pillars[1].gan, report.view.pillars[1].gan);
          sipseongColor = SIPSEONG_COLOR_MAP[computedSipseong] ?? CH5_COLOR;
        }
        const sipseongName = computedSipseong || partnerSipseong?.sipseong || "";
        const sipseongDesc = partnerSipseong?.desc ?? "";
        ...
        return (
          <>
            ...
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              ...
              <h1 className="text-[20px] font-black leading-snug" style={{ color: "#fff", fontFamily: SERIF }}>상대는 나를 어떻게 보는가</h1>
            </div>
            ...
            <div style={{ width: 1, height: 48, background: "#ccc", margin: "24px auto -24px" }} />
            <Quote>{`${partnerFirstName}님의 눈에는\n내가 어떻게 보이는지\n자세히 들여다보겠소.`}</Quote>

            {/* 십성 의미 카드 */}
            {sipseongName && (
              <section className="pb-2">
                <div className="px-5"><Heading>{partnerFirstName}님에게 나는</Heading></div>
                <SipseongMeaningCard sipseong={sipseongName} desc={sipseongDesc} color={sipseongColor} myIlgan={report?.partnerView?.pillars[1].gan} partnerIlgan={report?.view?.pillars[1].gan} myName={partnerFirstName} partnerName={myFirstName} myGender={report?.partnerGender} partnerGender={report?.gender} />
              </section>
            )}

            {/* 상대가 보는 나 풀이 */}
            <section className="pb-4">
              <div className="px-5"><Heading>{partnerFirstName}님의 사주로 보는 나</Heading></div>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_5/kunghap_yeonae_5_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "14%", left: "55%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[1.5] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>{partnerName.slice(1) || partnerName}님이 나에게<br />바라는 것과 주의할 점이<br />무엇일지 알려주겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            {partnerExpectation?.items && partnerExpectation.items.length > 0 && (
              <section className="pb-4">
                <div className="px-5"><Heading>{partnerFirstName}님이 나에게 기대하는 것</Heading></div>
                <ExpectationListCard items={partnerExpectation.items} color={CH5_COLOR} />
              </section>
            )}
            ...
            {partnerWarning && (
              <section className="pb-4">
                <div className="px-5"><Heading>{partnerFirstName}님이 나에게 주의할 점</Heading></div>
                <RelationWarningCard data={partnerWarning} color={CH5_COLOR} />
              </section>
            )}
      ...
      {ch === "6" && (() => {
        ...
        const partnerName = report?.partnerName || "상대방";
        const partnerFirstName = partnerName.slice(1) || partnerName;
        const hapItems = ((jc.hapList as { items?: Array<{ type: string; hapEl?: string; desc?: string; strength?: string; effect?: string }> } | undefined)?.items) ?? [];
        const chungItems = ((jc.chungList as { items?: Array<{ type: string; desc?: string; strength?: string; effect?: string; overcome?: string }> } | undefined)?.items) ?? [];
        const overallScoreLlm = (jc.overallScore as Record<string, unknown> | undefined) ?? null;
        const crossRels = (report?.view && report?.partnerView) ? calcCrossRelations(report.view, report.partnerView) : [];
        const { score: computedScore, label: computedLabel } = calcKunghapScore(crossRels);
        const computedDesc = buildHapChungDesc(crossRels, computedScore);
        ...
        return (
          <>
            ...
            {report?.view && report?.partnerView && (
              <section className="pb-4">
                ...
                <KunghapRelationCard
                  ...
                  myColor={CH4_COLOR} partnerColor={CH5_COLOR}
                />
              </section>
            )}
            ...
            <section className="pb-4">
              ...
              <HapChungScoreCard data={overallScore} hapCount={crossRels.filter(r => ["천간합","육합","삼합"].includes(r.kind)).length} chungCount={crossRels.filter(r => !["천간합","육합","삼합"].includes(r.kind)).length} />
            </section>

            {/* ── 일러스트 이미지 ── */}
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_6/kunghap_yeonae_6_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "43%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>두 사람의 사주가 만나<br />어떤 기운을 만들어내는가.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
      ...
      {ch === "7" && (() => {
        ...
        const MY_STYLE_COLOR = isFemale7(report?.gender) ? "#c9607a" : "#4a7cc9";
        const PARTNER_STYLE_COLOR = isFemale7(report?.partnerGender) ? "#c9607a" : "#4a7cc9";
        return (
          <>
            {/* 다크 헤더 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            ...
            <section className="pb-2">
              ...
              <div className="flex mx-5 mb-1.5">
                <div className="flex-1 flex justify-center">
                  <span className="text-[11px] font-bold px-3 py-1 rounded-full" style={{ color: MY_STYLE_COLOR, border: `1px solid ${MY_STYLE_COLOR}50`, background: `${MY_STYLE_COLOR}10` }}>{myFirstName}님의 스타일</span>
                </div>
                <div style={{ minWidth: 72 }} />
                <div className="flex-1 flex justify-center">
                  <span className="text-[11px] font-bold px-3 py-1 rounded-full" style={{ color: PARTNER_STYLE_COLOR, border: `1px solid ${PARTNER_STYLE_COLOR}50`, background: `${PARTNER_STYLE_COLOR}10` }}>{partnerFirstName}님의 스타일</span>
                </div>
              </div>
              <StyleCompareBar
                myLabel={myLabel} myIcon={myIcon} myColor={MY_STYLE_COLOR} myName={myFirstName}
                partnerLabel={partnerLabel} partnerIcon={partnerIcon} partnerColor={PARTNER_STYLE_COLOR} partnerName={partnerFirstName}
            ...
            {/* 나의 연애 스타일 카드 */}
            <section className="pb-2">
              <div className="px-5"><Heading>{myFirstName}님의 연애스타일</Heading></div>
              <LoveStyleCard data={myStyle} color={MY_STYLE_COLOR} pale={`${MY_STYLE_COLOR}12`} label={`${myFirstName}님의 연애 스타일 유형`} />
            </section>

            {/* 상대방 연애 스타일 카드 */}
            <section className="pb-2">
              <div className="px-5"><Heading>{partnerFirstName}님의 연애스타일</Heading></div>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_7/kunghap_yeonae_7_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "25%", left: "32%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>두 사람이<br />사랑을 표현하는<br />방식이 다르오.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
        ...
        return (
          <>
            ...
            {(lightSummary || shadowSummary) && (
              <section className="pb-4">
                <div className="px-5"><Heading>두 사람의 빛과 그림자</Heading></div>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_8/kunghap_yeonae_8_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "25%", left: "70%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>빛을 키우고<br />그림자를 보완하면<br />더 깊어질 수 있소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
      ...
      {/* ═══════════ 제9장 · 위기와 극복 ═══════════ */}
      {ch === "9" && (() => {
        type CrisisItem = { title: string; icon?: string; desc?: string; when?: string; signal?: string };
        type OvercomeItem = { title: string; icon?: string; desc?: string; steps?: string[] };
        type FlowItem = { label: string; tone: string; text: string; tip?: string };
        const crisisData  = (jc.crisisPoints  as { items?: CrisisItem[];  crisisSummary?: string } | undefined) ?? null;
        const overcomeData = (jc.overcomeTips as { items?: OvercomeItem[] } | undefined) ?? null;
        const flowData     = (jc.crisisFlow   as { items?: FlowItem[]; overviewDesc?: string } | undefined) ?? null;
        const crisisItems  = crisisData?.items ?? [];
        const overcomeItems = overcomeData?.items ?? [];
        const flowItems    = flowData?.items ?? [];
        return (
          <>
            {/* 헤더 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_9/kunghap_yeonae_9_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "18%", left: "45%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>위기가 오기 전에<br />먼저 알 수 있다면<br />지킬 수 있을 텐데.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            {crisisItems.length > 0 && (
              <section className="px-5 pt-2 pb-2">
                <Heading>이 관계의 위기 요소</Heading>
                <p className="text-[13px] leading-[1.85] mb-5" style={{ color: INK_SOFT, fontFamily: SERIF }}>
                  사주명리에서 위기란 두 사람의 기운이 서로 맞부딪히거나, 각자의 약한 오행이 겹치는 순간에 찾아오오. 위기를 피하는 것이 능사가 아니오. 어느 시기에, 어떤 모습으로 위기가 오는지를 먼저 아는 것이 이 관계를 오래 지키는 첫걸음이오.
                </p>
                {crisisItems.map((item, i) => (
                  <CrisisCard key={i} item={item} index={i} />
                ))}
              </section>
            )}
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_9/kunghap_yeonae_9_2.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "25%", left: "70%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[16px] font-bold leading-[2] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>위기를 알았다면<br />이제 극복하면 되오.<br />방법을 알려드리겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            {overcomeItems.length > 0 && (
              <section className="px-5 pt-2 pb-2">
                <Heading>위기를 극복하는 법</Heading>
                <p className="text-[13px] leading-[1.85] mb-5" style={{ color: INK_SOFT, fontFamily: SERIF }}>
                  위기를 알았다면 이제 극복하면 되오. 사주에서 비롯된 갈등은 사주적 해법으로 풀 수 있소. 두 사람이 함께 노력할 수 있는 구체적인 방법들을 짚어드리겠소. 하나씩 익히고 실천하는 것만으로도 이 관계는 훨씬 단단해질 것이오.
                </p>
                {overcomeItems.map((item, i) => (
                  <OvercomeCard key={i} item={item} index={i} />
                ))}
              </section>
            )}
      ...
      {/* ═══════════ 제10장 · 결혼으로 이어지는가 ═══════════ */}
      {ch === "10" && (() => {
        type ConditionItem = { title: string; icon?: string; desc?: string; possibility?: string };
        const mpData   = (jc.marriagePossibility as Record<string, unknown> | undefined) ?? null;
        const mcData   = (jc.marriageConditions  as { items?: ConditionItem[] } | undefined) ?? null;
        const mtData   = (jc.marriageTiming      as Record<string, unknown> | undefined) ?? null;
        // 합충 점수를 50% 반영한 결혼 가능성 점수
        const hapChungRels = (report?.view && report?.partnerView) ? calcCrossRelations(report.view, report.partnerView) : [];
        const { score: hapChungScore } = calcKunghapScore(hapChungRels);
        const llmScore = (mpData?.score as number | undefined) ?? 78;
        const mpScore  = Math.round(llmScore * 0.3 + hapChungScore * 0.7);
        const mpLabel = mpScore >= 90 ? "천생연분에 가까운 인연이오"
          : mpScore >= 80 ? "결혼으로 이어질 강한 인연이오"
          : mpScore >= 70 ? "결혼을 충분히 그려볼 수 있는 인연이오"
          : mpScore >= 60 ? "마음을 모은다면 결혼까지 닿을 수 있는 인연이오"
          : mpScore >= 50 ? "결혼을 서두르기엔 아직 이른 인연이오"
          : mpScore >= 40 ? "이 인연, 결혼보다 연애에 더 어울리는 사이이오"
          : mpScore >= 30 ? "결혼으로 가기엔 넘어야 할 것이 많은 인연이오"
          : "인연은 있으나, 결혼은 쉽지 않을 사이이오";
        const mpBasis  = (mpData?.basis as string | undefined) ?? "";
        const mpParas  = (mpData?.paragraphs as string[] | undefined) ?? [];
        const condItems = mcData?.items ?? [];

        // 시기별 관계의 흐름(ch9)에서 최고/최저 시기 추출
        type FlowItem10 = { label: string; tone: string; score?: number; text?: string };
        const flowItems10 = ((jc.crisisFlow as { items?: FlowItem10[] } | undefined)?.items) ?? [];
        const toneScore10 = (tone: string, s?: number) => s !== undefined ? Math.min(100, Math.max(5, s)) : tone === "good" ? 80 : tone === "caution" ? 55 : 30;
        const bestFlowItem = flowItems10.reduce<FlowItem10 | null>((best, cur) =>
          !best || toneScore10(cur.tone, cur.score) > toneScore10(best.tone, best.score) ? cur : best
        , null);
        const worstFlowItem = flowItems10.reduce<FlowItem10 | null>((worst, cur) =>
          !worst || toneScore10(cur.tone, cur.score) < toneScore10(worst.tone, worst.score) ? cur : worst
        , null);
        const mtDataOverridden = mtData
          ? {
              ...mtData,
              ...(bestFlowItem ? { bestPeriod: bestFlowItem.label } : {}),
              ...(worstFlowItem ? { cautionPeriod: worstFlowItem.label, cautionDesc: worstFlowItem.text } : {}),
            }
          : mtData;
        return (
          <>
            {/* 헤더 */}
            <div className="text-center px-6 py-4" style={{ background: "#111" }}>
              <p className="text-[10px] tracking-[0.25em] mb-2" style={{ color: "rgba(255,255,255,0.5)", fontFamily: SERIF }}>제 10 장 · 결혼</p>
            ...
            <section className="px-5 pt-2 pb-4">
              <Heading>결혼 가능성 지수</Heading>
              <p className="text-[13px] leading-[1.85] mb-5" style={{ color: INK_SOFT, fontFamily: SERIF }}>
                결혼 가능성은 단순한 궁합 점수가 아니오. 두 사람의 일간·십성·오행이 서로 어떤 관계를 맺고 있는지, 관성·재성·인성의 흐름이 어떻게 맞물리는지, 글자들간의 합과 충을 종합한 결과이오. 숫자 하나로 인연의 무게를 다 담을 수는 없지만, 방향을 가리키는 나침반으로 활용하시기 바라오.
              </p>
              <MarriageGauge score={mpScore} label={mpLabel} />
              {/* 4단락 풀이 */}
              {mpParas.length > 0 && (
                <div className="mt-5">
                  {mpParas.map((p, i) => (
            ...
            <div className="relative w-full mt-8">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/media/report/kunghap_yeonae/kunghap_yeonae_10/kunghap_yeonae_10_1.jpg" alt="" className="w-full block" />
              <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, transparent 50%, rgba(253,248,244,1) 100%)" }} />
              <div className="absolute pointer-events-none" style={{ top: "13.5%", left: "50%", transform: "translate(-50%, -50%)" }}>
                <p className="text-[18px] font-bold leading-[1.5] text-center whitespace-nowrap" style={{ color: "#2a2320", fontFamily: "'Nanum Myeongjo', 'Apple SD Gothic Neo', serif" }}>하늘의 기운이 맞아야<br />결혼의 문이 열리오.<br />그 시기를 알려드리겠소.</p>
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", paddingTop: 40, paddingBottom: 32 }}>
              <div style={{ width: 1, height: 48, background: "linear-gradient(to bottom, rgba(180,120,100,0.3), rgba(180,120,100,0.6))" }} />
            ...
            {mtDataOverridden && (
              <section className="px-5 pt-2 pb-4">
                <Heading>결혼을 하게된다면, 좋은시기</Heading>
                <p className="text-[13px] leading-[1.85] mb-5" style={{ color: INK_SOFT, fontFamily: SERIF }}>
                  결혼은 마음만 맞는다고 이루어지지 않소. 하늘의 기운, 즉 대운과 세운이 두 사람 모두에게 좋은 방향으로 흐를 때 비로소 결혼의 문이 열리오. 서두르지 말고 기운이 무르익는 시기를 기다리며 준비하시오.
                </p>
                <MarriageTimingCard data={mtDataOverridden} />
              </section>
            )}
