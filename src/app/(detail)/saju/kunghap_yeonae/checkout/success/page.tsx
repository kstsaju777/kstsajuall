  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #ff4499aa,0 0 20px 4px #ff66bbaa} 50%{box-shadow:0 0 16px 4px #ff66bbcc,0 0 40px 10px #ff449988} }
      ...
      <div className="relative w-20 h-20 mb-6">
        <div className="absolute inset-0 rounded-full" style={{ background: "radial-gradient(circle, #ff449922 0%, transparent 70%)" }} />
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            top: "50%", left: "50%", marginTop: "-2px", marginLeft: "-2px",
            background: i % 2 === 0 ? "#ff4499" : "#ff66bb",
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#ff4499" : "#ff66bb"}`,
      ...
      <p className="text-[18px] font-bold mb-1" style={{ color: "#fff5f5", fontFamily: "'Noto Serif KR', serif", textShadow: "0 0 20px #ff449988" }}>
        결과지를 완성하고 있소…
      </p>
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#ff66bb", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#ff66bb" }}>
        ...
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#3d0a20" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #cc0077, #ff4499, #ff66bb)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(255,68,153,0.15)", border: "1px solid rgba(255,68,153,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#ff66bb" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#ff66bb" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있소.<br />오래 걸려도 정상이니 그대로 기다려 주시오.<br />새로고침하면 처음부터 다시 시작되어<br />시간이 더 걸리게 되오.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#cc7799" }}>
...
export default function KunghapYeonaeCheckoutSuccessPage() {
  return (
    <Suspense>
      <SuccessInner />
    </Suspense>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const confirmRes = await fetch("/api/kunghap_yeonae/payment-confirm", {
      ...
      const imageTask = fetch("/api/kunghap_yeonae-report", {
