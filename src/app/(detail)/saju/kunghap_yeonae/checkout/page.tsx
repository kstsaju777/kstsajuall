...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCT = { name: "연애궁합", original: 49800, discount: 50, price: 24900 };
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        router.push(`/saju/kunghap_yeonae/report-preview?${new URLSearchParams({ name, gender, partnerName, partnerGender }).toString()}`);
        return;
      }
