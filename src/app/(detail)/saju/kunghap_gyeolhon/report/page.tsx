...
//
// 미리보기: http://localhost:3000/saju/kunghap_gyeolhon/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/kunghap_gyeolhon-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/kunghap_gyeolhon/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/kunghap_gyeolhon/report?${id ? `id=${id}&` : ""}ch=${n}`);
