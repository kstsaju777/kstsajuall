...
const LABEL_CLR = "#f5a0c0";
const BORDER_CLR = "#3a4050";
const PH_CLR    = "#7a8590";
const TEXT_CLR  = "#f5f5f5";
const IMG_BG    = "/media/cards/kunghap_gyeolhon/gyeolhon-0.jpg";
...
function StepChildInfo({ onPrev, onNext, initial, isPartner }: {
  onPrev: () => void;
  ...
  return (
    <>
      ...
      <div className="px-6 pt-4 pb-3 overflow-y-auto" style={{ backgroundColor: CARD_BG, maxHeight: "70vh" }}>
        {/* 성별 */}
        <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>{who} 성별</p>
        ...
        <div className="flex gap-3 mb-4">
          {["남성", "여성"].map((label) => (
            <button key={label} onClick={() => { setGender(label); setShowDate(true); }}
              ...
              style={{
                backgroundColor: gender === label ? "rgba(225,51,125,0.15)" : "rgba(255,255,255,0.04)",
        ...
        {showDate && (
          <div style={{ animation: "slideUp 0.35s ease" }}>
            ...
            <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>{who} 생년월일</p>
            <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
            ...
            <div className="flex gap-2 mb-3">
              {["양력", "음력"].map((c) => (
                <button key={c} onClick={() => setCalendar(c)}
                  ...
                  style={{
                    backgroundColor: calendar === c ? "rgba(225,51,125,0.15)" : "rgba(255,255,255,0.04)",
            ...
            {dateEntered && (
              <div style={{ animation: "slideUp 0.35s ease" }}>
                ...
                <p className="text-[12px] font-medium mb-0.5" style={{ color: "#8a8a8a" }}>{who} 태어난 시간</p>
                <h2 className="text-[20px] mb-3" style={{ color: TEXT_CLR }}>
                  <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>{isPartner ? "상대방이 " : "그대가 "}</span>
                ...
                <div className="flex gap-3">
                  <div className="flex-1 relative">
                    <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold flex items-center justify-between"
                      ...
                      style={{
                        backgroundColor: (btime && btime !== "모름") ? "rgba(225,51,125,0.15)" : "rgba(255,255,255,0.04)",
                        ...
                        <div className="absolute z-50 w-full rounded-xl overflow-hidden"
                          ...
                          {BIRTH_TIMES.map((t) => (
                            <button key={t} className="w-full text-left px-4 py-2.5 text-[14px] transition-colors"
                              ...
                              style={{ backgroundColor: btime === t ? "rgba(225,51,125,0.15)" : "transparent", color: btime === t ? "#fff" : "rgba(255,255,255,0.7)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                              {t}
                            </button>
                          ))}
                        </div>
                  ...
                  <button className="px-4 py-3 rounded-xl text-[14px] font-bold"
                    ...
                    style={{
                      backgroundColor: btime === "시간 모름" ? "rgba(225,51,125,0.15)" : "rgba(255,255,255,0.04)",
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <h2 className="text-[20px] mb-4" style={{ color: TEXT_CLR }}>
          <span className="font-bold" style={{ color: TEXT_CLR }}>{name ?? (isPartner ? "상대방" : "그대")}님의 </span>
          ...
          <div className="grid grid-cols-4 gap-2">
            {(pillars ?? Array(4).fill(null)).map((p, i) => (
              <div key={i} className="flex flex-col items-center gap-1.5">
                ...
                <span style={{ fontSize: 13, color: "#e1337d", lineHeight: 1, fontWeight: 700 }}>{p?.stemSs || "-"}</span>
                ...
                <span style={{ fontSize: 13, color: "#e1337d", lineHeight: 1, fontWeight: 700 }}>{p?.branchSs || "-"}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="rounded-2xl px-4 py-3" style={{ backgroundColor: "rgba(225,51,125,0.07)", border: "1px solid rgba(225,51,125,0.25)" }}>
          <p className="text-[13px] leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>
            ...
            {isPartner ? " 두 사람의 사주를 함께 보아 인연의 깊이와 혼인의 흐름을 풀어드리겠소이다." : " 이 여덟 글자 안에 그대의 결혼 인연과 배우자 복, 앞으로의 혼인 흐름이 담겨 있소이다."}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-1 rounded-2xl p-2" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
        {(pillars ?? Array(4).fill(null)).map((p, i) => (
          <div key={i} className="flex flex-col items-center gap-1">
            ...
            <div className="w-full flex items-center justify-center" style={{ aspectRatio: "1" }}>
              {p ? <img src={ganCharImage(p.stem)} alt={p.stem} style={{ width: "100%", height: "100%", objectFit: "contain" }} /> : <div className="w-full h-full animate-pulse rounded" style={{ backgroundColor: "rgba(255,255,255,0.08)" }} />}
            </div>
            <div className="w-full flex items-center justify-center" style={{ aspectRatio: "1" }}>
              {p ? <img src={jiCharImage(p.branch)} alt={p.branch} style={{ width: "100%", height: "100%", objectFit: "contain" }} /> : <div className="w-full h-full animate-pulse rounded" style={{ backgroundColor: "rgba(255,255,255,0.08)" }} />}
            </div>
...
// ─── 고민 입력 ───────────────────────────────────────────────────────────────
function StepConcern({ onPrev, onNext, initial, name, myDate, myTime, myCalendar, partnerDate, partnerTime, partnerCalendar, partnerName }: {
  onPrev: () => void; onNext: (v: string) => void; initial?: string; name?: string;
  myDate?: string; myTime?: string; myCalendar?: string;
  partnerDate?: string; partnerTime?: string; partnerCalendar?: string; partnerName?: string;
}) {
  const [text, setText] = useState(initial ?? "");
  const filled = text.trim().length > 0;
  const MAX = 200;
  const PLACEHOLDER = `예) 지금 사귀는 사람과 결혼까지 갈 수 있을지, 내 사주에 결혼운이 있는 건지 너무 궁금해요. 결혼 적령기도 알고 싶어요.`;
    ...
    <>
      <div className="px-6 pt-4 pb-3" style={{ backgroundColor: CARD_BG }}>
        ...
        <div className="flex gap-2 mb-4">
          <MiniMyeongsik date={myDate} time={myTime} calendar={myCalendar} name={name} />
        ...
        <h2 className="text-[20px] mb-1" style={{ color: TEXT_CLR }}>
          <span className="font-normal" style={{ color: "rgba(245,245,245,0.45)" }}>결혼에 대해 </span>
          <span className="font-bold">궁금한 점이 있소?</span>
        </h2>
        <p className="text-[12px] mb-3" style={{ color: "rgba(255,255,255,0.5)" }}>
          소인이 작은 조언을 해주겠소. (안적어도 괜찮소.)
        </p>
        <div className="rounded-2xl px-4 pt-4 pb-3" style={{ backgroundColor: "rgba(255,255,255,0.05)", border: "1px solid rgba(225,51,125,0.25)" }}>
          <textarea
            value={text}
              ...
              <div className="absolute bottom-full left-0 right-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                ...
                {EMAIL_DOMAINS.map((d) => (
                  <div key={d} onClick={() => { setDomain(d); setOpen(false); }}
                    ...
                    style={{ backgroundColor: domain === d ? "rgba(225,51,125,0.2)" : "transparent", color: domain === d ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                    {d}
                  </div>
                ))}
              </div>
                ...
                <div className="absolute bottom-full left-0 z-20 rounded-2xl overflow-hidden shadow-xl mb-2"
                  ...
                  {PHONE_PREFIXES.map((p) => (
                    <div key={p} onClick={() => { setPhonePrefix(p); setPhoneOpen(false); }}
                      className="px-4 py-3 text-[14px] cursor-pointer"
                      style={{ backgroundColor: phonePrefix === p ? "rgba(225,51,125,0.2)" : "transparent", color: phonePrefix === p ? "#fff" : "#ddd", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {p}
                    </div>
                  ))}
                </div>
...
// ─── 로딩 ────────────────────────────────────────────────────────────────────
function StepLoading({ name, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, date, time, calendar, gender, email, concern, phone }: {
  name: string; partnerName: string; partnerDate?: string; partnerTime?: string; partnerCalendar?: string; partnerGender?: string;
  date: string; time: string; calendar: string; gender: string; email: string; concern: string; phone: string;
}) {
...
export default function GyeolhonFormPage() {
      ...
      {[1, 2, 3, 4, 5, 7].includes(step) && (
        <FormShell>
          ...
          {step === 3 && (
            <StepChildInfo
              onPrev={() => setStep(2)}
              onNext={(gender, date, time, calendar, name) => next({ partnerGender: gender, partnerDate: date, partnerTime: time, partnerCalendar: calendar, partnerName: name }, 4)}
              initial={{ gender: form.partnerGender, date: form.partnerDate, time: form.partnerTime, calendar: form.partnerCalendar, name: form.partnerName }}
              isPartner
            />
          )}
          {step === 4 && (
          ...
          {step === 5 && (
            <StepConcern
              onPrev={() => setStep(4)}
              onNext={(concern) => next({ concern }, 6)}
              initial={form.concern} name={form.name}
              myDate={form.date} myTime={form.time} myCalendar={form.calendar}
