...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCTS = [
  { id: "gyeolhon", name: "결혼궁합", original: 59800, discount: 50, price: 29900 },
];
  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4"
      ...
      <button onClick={onPay}
        ...
        <span>🔓</span>
        <span>{name}님 · {partnerName}님 결혼궁합 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async (_productId: string) => {
    ...
    try {
      ...
      const res = await fetch("/api/kunghap_gyeolhon-report", {
        method: "POST",
      ...
      if (!res.ok) {
        router.push(`/saju/kunghap_gyeolhon/report-preview?${new URLSearchParams({ name, gender, partnerName, partnerGender }).toString()}`);
        return;
      }
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        {/* ① 헤더 이미지 + 제목 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_gyeolhon/jaehwe-1.jpg"
          bottomImgSrc="/media/cards/kunghap_gyeolhon/jaehwe-2.jpg"
          label="결혼궁합 · 정밀 리포트"
          headline={`${name}님과\n${partnerName}님의 인연,\n낱낱이 봤어요`}
          accentWord="낱낱이"
          bgColor={WHITE}
        ...
        {/* ③ 분석 섹션 헤더 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_gyeolhon/jaehwe-3.jpg"
          bottomImgSrc="/media/cards/kunghap_gyeolhon/jaehwe-1.jpg"
        ...
        <ImageDivider src="/media/cards/kunghap_gyeolhon/jaehwe-2.jpg" bgColor={CREAM} />
