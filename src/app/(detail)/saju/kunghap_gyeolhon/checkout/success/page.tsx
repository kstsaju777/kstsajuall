  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #e1337daa,0 0 20px 4px #f055a0aa} 50%{box-shadow:0 0 16px 4px #f055a0cc,0 0 40px 10px #e1337d88} }
      ...
      <div className="relative w-20 h-20 mb-6">
        ...
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            ...
            background: i % 2 === 0 ? "#e1337d" : "#f055a0",
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#e1337d" : "#f055a0"}`,
      ...
      <p className="text-[18px] font-bold mb-1" style={{ color: "#fff5f5", fontFamily: "'Noto Serif KR', serif", textShadow: "0 0 20px #e1337d88" }}>
        결과지를 완성하고 있소…
      </p>
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#f055a0", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#f055a0" }}>
        ...
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#1a1000" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #aa1955, #e1337d, #f055a0)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(225,51,125,0.15)", border: "1px solid rgba(225,51,125,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#f055a0" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#f055a0" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있소.<br />오래 걸려도 정상이니 그대로 기다려 주시오.<br />새로고침하면 처음부터 다시 시작되어<br />시간이 더 걸리게 되오.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#887766" }}>
...
export default function KunghapGyeolhonCheckoutSuccessPage() {
  return (
    <Suspense>
      <SuccessInner />
    </Suspense>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const confirmRes = await fetch("/api/kunghap_gyeolhon/payment-confirm", {
      ...
      await Promise.all(chapters.map(async (ch) => {
        try {
          const r = await fetch("/api/kunghap_gyeolhon-report", {
            method: "POST", headers: { "Content-Type": "application/json" },
      ...
      await fetch("/api/kunghap_gyeolhon-report", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: resultId }) }).catch(() => {});
      // 이미지까지 완료된 시점에 완성 여부 재확인 → 알림톡 발송
      await fetch("/api/kunghap_gyeolhon-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: resultId, content: {} }) }).catch(() => {});
      navigatingRef.current = true;
      router.push(`/saju/kunghap_gyeolhon/report-preview?id=${resultId}&gender=${encodeURIComponent(gender)}&name=${encodeURIComponent(name)}&partnerName=${encodeURIComponent(partnerName ?? "")}&partnerGender=${encodeURIComponent(partnerGender ?? "")}`);
