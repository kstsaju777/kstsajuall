          ...
          <>
            ...
            <div className="rounded-2xl px-4 py-3 mt-3" style={{ backgroundColor: "rgba(224,168,176,0.07)", border: "1px solid rgba(224,168,176,0.25)" }}>
              <p className="text-[13px] leading-relaxed" style={{ color: "rgba(255,255,255,0.65)" }}>
                ...
                {isPartner ? " 두 사람의 사주를 함께 보아 재회의 흐름과 인연의 깊이를 풀어드리겠소이다." : " 이 여덟 글자 안에 그대의 재회 인연과 앞으로의 흐름이 담겨 있소이다."}
              </p>
            </div>
          </>
        )}
...
function StepLoading({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, concern, phone, breakupReason, whoEnded, breakupDate }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center" style={{ backgroundColor: "#080a0f" }}>
      ...
      <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(15,8,10,0.5), rgba(15,8,10,0.95))" }} />
...
// ─── 메인 ─────────────────────────────────────────────────────────────────────
type FormData = { breakupReason: string; whoEnded: string; breakupDate: string; gender: string; date: string; calendar: string; time: string; name: string; concern: string; partnerGender: string; partnerDate: string; partnerCalendar: string; partnerTime: string; partnerName: string; email: string; phone: string; };
    ...
    <>
      ...
      {(step <= 3 || (step >= 5 && step <= 9) || step === 11) && (
        <FormShell>
          ...
          {step === 11 && (
            <StepEmail initial={form.email} initialPhone={form.phone} onPrev={() => setStep(10)} onNext={(email, phone) => next({ email, phone }, 12)} />
          )}
        </FormShell>
      )}
