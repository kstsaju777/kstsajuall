  ...
  return (
    <div className="flex-shrink-0 px-5 pb-7 pt-4"
      ...
      <button onClick={onPay}
        ...
        <span>🔓</span>
        <span>{name}님 · {partnerName}님 재회궁합 확인하기</span>
      </button>
    </div>
  );
}
...
function CheckoutContent() {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const reportUrl = d?.resultId ? `https://www.hongyeondang.com/saju/kunghap_jaehwe/report?id=${d.resultId}` : `https://www.hongyeondang.com/saju/kunghap_jaehwe/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`;
      ...
      router.push(d?.resultId ? `/saju/kunghap_jaehwe/report?id=${d.resultId}` : `/saju/kunghap_jaehwe/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } catch {
      router.push(`/saju/kunghap_jaehwe/report?${new URLSearchParams({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, ch: "0" }).toString()}`);
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col" style={{ backgroundColor: CREAM }}>
      <div className="flex-1 min-h-0 overflow-y-auto" style={{ scrollbarWidth: "none" }}>
        ...
        {/* ① 헤더 이미지 + 제목 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_jaehwe/jaehwe-1.jpg"
          bottomImgSrc="/media/cards/kunghap_jaehwe/jaehwe-2.jpg"
          label="재회궁합 · 정밀 리포트"
          headline={`${name}님과\n${partnerName}님의 인연,\n낱낱이 봤어요`}
          accentWord="낱낱이"
          bgColor={WHITE}
        ...
        {/* ③ 분석 섹션 헤더 */}
        <ImageTextBlock
          topImgSrc="/media/cards/kunghap_jaehwe/jaehwe-3.jpg"
          bottomImgSrc="/media/cards/kunghap_jaehwe/jaehwe-1.jpg"
        ...
        <ImageDivider src="/media/cards/kunghap_jaehwe/jaehwe-2.jpg" bgColor={CREAM} />
