  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #ff6b9daa,0 0 20px 4px #ffaaccaa} 50%{box-shadow:0 0 16px 4px #ffaacccc,0 0 40px 10px #ff6b9d88} }
      ...
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#ff6b9d", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#ff6b9d" }}>
        ...
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#1a1000" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #c44b7a, #ff6b9d, #ffaac8)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(255,107,157,0.15)", border: "1px solid rgba(255,107,157,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#ff6b9d" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#ff6b9d" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있소.<br />오래 걸려도 정상이니 그대로 기다려 주시오.<br />새로고침하면 처음부터 다시 시작되어<br />시간이 더 걸리게 되오.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#e0a8b0" }}>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await fetch("/api/kunghap_jaehwe-report", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: resultId }) }).catch(() => {});
      // 이미지까지 완료된 시점에 완성 여부 재확인 → 알림톡 발송
      await fetch("/api/kunghap_jaehwe-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: resultId, content: {} }) }).catch(() => {});
