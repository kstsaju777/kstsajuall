...
import { isChapterReady, CHAPTER_SECTIONS, buildSajuImageMeaning } from "@/lib/saju/report-content";
...
function DaeunTableWithDetail({ view, yongsinEl, heusinEl, gisinEl }: {
  ...
  // ⚠️ 이 대운에 실제로 있는 신(용신/희신/기신)만 언급하도록 동적으로 구성 —
  // "길운"이라고 해서 무조건 "용신·희신 둘 다"라고 뭉뚱그리면, 실제론 희신만 있는데
  // 용신까지 있다고 잘못 말하는 오류가 생기므로 반드시 실제 존재하는 신만 나열한다.
  ...
  const para5 = applyTense(tier === "good"
    ? `이 모든 흐름을 종합해보면, ${goodRoleStr ? `${goodRoleStr}의 기운이` : "이로운 기운이"} 든든하게 뒷받침해주는 길운의 대운이오. 노력한 것들이 하나둘 결실로 돌아오는 황금 같은 시간이니, 주저하지 말고 이 흐름을 믿고 과감하게 나아가시오.`
    : tier === "caution"
    ? `이 모든 흐름을 종합해보면, 기신의 기운이 섞여 들어와 사주 균형이 흔들릴 수 있는 때요. 그러니 큰 결정보다는 내실을 다지는 데 집중하고, 소비와 감정 관리에 조금 더 세심하게 신경 쓰는 것이 현명하오.`
    : `이 모든 흐름을 종합해보면, 여러 기운이 서로 뒤섞인 평온한 흐름의 시기요. 극적인 변화보다는 꾸준한 걸음이 중심이 되는 때이니, 남과 비교하지 말고 자신만의 속도로 묵묵히 걸어가시오.`);
...
const SAJU_ORDER = ["정통명리 종합사주", "대박이야 재물사주", "나만솔로? 연애사주", "영재발굴 자녀사주", "오래살자 건강사주", "우리아가 유아사주"];
      ...
      {ch === "0" && (
        <>
          ...
          <div className="relative overflow-hidden mx-0" style={{ background: CREAM }}>
            <video src="/media/report/total/total-0/total-0-3.mp4" autoPlay loop muted playsInline className="w-full object-cover" />
            <div className="absolute inset-0" style={{ background: `linear-gradient(to bottom, ${CREAM} 0%, transparent 35%, transparent 65%, ${CREAM} 100%)` }} />
          </div>
