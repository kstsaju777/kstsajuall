...
function FormShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      {/* 배경 이미지 — 전체 화면 */}
      <img
        src="/media/cards/kunghap_business/business-0.jpg"
        aria-hidden
...
function BottomNav({
  ...
  return (
    <div className="flex items-center gap-3 px-5 pb-8 pt-4" style={{ backgroundColor: CARD_BG }}>
      ...
      <div className="flex-1" style={{ position: "relative", borderRadius: "16px", padding: nextDisabled ? "0" : "2px", overflow: "hidden", background: nextDisabled ? "transparent" : "#1d6fce", opacity: nextDisabled ? 0.35 : 1 }}>
        {!nextDisabled && (
          <div style={{
            position: "absolute", top: "50%", left: "50%",
            width: "200%", height: "200%",
            background: "conic-gradient(from 0deg, transparent 0%, transparent 65%, #4e9eff 78%, #a8d4ff 85%, #4e9eff 92%, transparent 100%)",
            animation: "bizNavRotate 2s linear infinite",
            pointerEvents: "none",
          }} />
        )}
        <button
          ...
          className="w-full py-3.5 rounded-2xl text-white text-[16px] font-bold relative"
          style={{
            backgroundColor: NAVY,
            letterSpacing: "-0.3px",
            animation: nextDisabled ? "none" : "bizNavPulse 2s ease-in-out infinite, bizNavBeat 2.5s ease-in-out infinite",
          }}
        >
          {nextLabel}
        </button>
          ...
          {(["내가", "상대방이", "둘 다"] as const).map((opt) => {
            ...
            return (
              <button
                ...
                style={{
                  backgroundColor: active ? "rgba(78,158,255,0.18)" : "rgba(255,255,255,0.04)",
                  border: `1.5px solid ${active ? NAVY : "rgba(255,255,255,0.18)"}`,
                  color: active ? "#ffffff" : "#dddddd",
                  opacity: active ? 1 : 0.55,
                }}
    ...
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      ...
      <div className="relative z-10 px-8 text-center">
        <p className="text-[26px] font-bold leading-relaxed whitespace-pre-line" style={{ color: "#fff", minHeight: "2.2em" }}>
          {displayed}
          <span className="inline-block w-[2px] h-[1.1em] ml-1 align-middle animate-pulse" style={{ backgroundColor: "#4e9eff", verticalAlign: "middle" }} />
        </p>
      </div>
    </div>
  );
        ...
        <div className="flex gap-3 mb-4">
          {["남성", "여성"].map((label) => (
            <button key={label} onClick={() => handleGender(label)}
              ...
              style={{
                backgroundColor: gender === label ? "rgba(78,158,255,0.15)" : "rgba(255,255,255,0.04)",
            ...
            <div className="flex gap-2 mb-3">
              {["양력", "음력", "윤달"].map((c) => (
                <button key={c} onClick={() => setCalendar(c)}
                  ...
                  style={{
                    backgroundColor: calendar === c ? "rgba(78,158,255,0.15)" : "rgba(255,255,255,0.04)",
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 relative">
                    <button className="w-full py-3 px-4 rounded-xl text-[15px] font-bold transition-all flex items-center justify-between"
                      ...
                      style={{
                        backgroundColor: (btime && btime !== "모름") ? "rgba(78,158,255,0.15)" : "rgba(255,255,255,0.04)",
                          ...
                          {[
                            ...
                            return (
                              <button key={t.label} className="w-full px-4 py-3 flex items-center justify-between transition-all"
                                ...
                                style={{ backgroundColor: selected ? "rgba(78,158,255,0.18)" : "transparent", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <button onClick={() => { setBtime("모름"); setTimeOpen(false); }}
                    ...
                    style={{
                      backgroundColor: btime === "모름" ? "rgba(78,158,255,0.15)" : "rgba(255,255,255,0.04)",
    ...
    <>
      <div className="px-6 pt-6 pb-2" style={{ backgroundColor: CARD_BG }}>
        ...
        <textarea
          ...
          placeholder="예) 함께 사업을 시작하려는데 우리 비즈니스 궁합이 잘 맞는지, 어떤 부분에서 충돌할 수 있는지 궁금해요"
          rows={5}
        ...
        />
        <p className="text-right text-[12px] mt-0.5 pr-0.5" style={{ color: text.length >= MAX ? "#e55" : "#6a9fc0" }}>
          {text.length}/{MAX}
        </p>
      </div>
              ...
              >
                {EMAIL_DOMAINS.map((d) => (
                  <div
                    ...
                    style={{
                      backgroundColor: domain === d ? "rgba(78,158,255,0.25)" : "transparent",
...
// ─── Step 7: 로딩 ─────────────────────────────────────────────────────────────
function BusinessEmailIntro({ onNext }: { onNext: () => void }) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center" style={{ minHeight: "100dvh", backgroundColor: "#0a0c10" }}>
      ...
      <div className="relative z-10 px-8 text-center">
        <p className="text-[26px] font-bold leading-relaxed whitespace-pre-line" style={{ color: "#fff", minHeight: "2.2em" }}>
          {displayed}
          <span className="inline-block w-[2px] h-[1.1em] ml-1 align-middle animate-pulse" style={{ backgroundColor: NAVY, verticalAlign: "middle" }} />
        </p>
      </div>
    </div>
  );
...
function StepLoading({ name, date, time, calendar, gender, email, partnerName, partnerDate, partnerTime, partnerCalendar, partnerGender, concern }: {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ backgroundColor: "#0a0a0a" }}>
      ...
      <img src="/media/cards/kunghap_business/business-0.jpg" className="absolute inset-0 w-full h-full object-cover" style={{ filter: "blur(8px)", transform: "scale(1.1)" }} />
      ...
      <div className="absolute bottom-0 left-0 right-0 px-6 pb-12">
        <p className="text-center text-[15px] font-bold mb-3" style={{ color: "#ffffff" }}>
          사주팔자 정밀분석중... <span style={{ color: "#4e9eff" }}>{pct}%</span>
        </p>
        <div className="w-full h-2.5 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(255,255,255,0.2)" }}>
          <div className="h-full rounded-full" style={{ width: `${pct}%`, backgroundColor: "#1d6fce", transition: "width 0.2s" }} />
        </div>
      </div>
    </div>
  );
...
export default function Business() {
  ...
  return (
    <>
      ...
      {step === 6 && <BusinessEmailIntro onNext={() => setStep(7)} />}
      {step === 8 && (
        <StepLoading
