...
//
// 미리보기: http://localhost:3000/saju/kunghap_business/report
//
...
function ReportPreviewInner() {
    ...
    if (id) {
      ...
      fetch("/api/kunghap_business-report", {
        ...
        .then((d) => {
          ...
          if (d.resultId) router.replace(`/saju/kunghap_business/report?id=${d.resultId}&gender=${encodeURIComponent(d.gender ?? gender)}`);
        })
        .catch(() => {})
        .finally(() => setLoading(false));
    }
  ...
  const next = (n: string) => router.push(`/saju/kunghap_business/report?${id ? `id=${id}&` : ""}ch=${n}`);
