...
function StickyCTA() {
  ...
  return (
    <div className="fixed bottom-0 z-40 px-5 pb-6" style={{ left:"max(0px,calc(50vw - 240px))", width:"min(100%,480px)", paddingTop:"180px", background:"linear-gradient(to top, #0a0a0a 65%, transparent)" }}>
      ...
      <style>{`@keyframes btnNeon { 0%,100% { background:#4e9eff; box-shadow:0 0 12px 3px #4e9eff88; } 50% { background:#1a6fcc; box-shadow:0 0 12px 3px #1a6fcc88; } }`}</style>
      <button onClick={()=>router.push("/saju/kunghap_business/form")} className="w-full py-2 rounded-2xl font-bold active:scale-95 transition-transform" style={{color:"#fff",fontSize:"22px",animation:"btnNeon 3s ease-in-out infinite"}}>
        비즈니스궁합 보러가기
      </button>
    </div>
  );
...
export default function BusinessPage() {
  ...
  return (
    <div className="w-full relative" style={{height:"100dvh",backgroundColor:BG,overflow:"hidden"}}>
      ...
      <img src="/media/cards/kunghap_business/typo-1.png" alt="" style={{position:"absolute",bottom:"130px",left:0,width:"100%",objectFit:"contain",pointerEvents:"none",zIndex:41}}/>
