  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <style>{`
        ...
        @keyframes glow-pulse { 0%,100%{box-shadow:0 0 8px 2px #4e9effaa,0 0 20px 4px #77bbff55} 50%{box-shadow:0 0 16px 4px #77bbffcc,0 0 40px 10px #4e9eff88} }
      ...
      <div className="relative w-20 h-20 mb-6">
        ...
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            ...
            background: i % 2 === 0 ? "#4e9eff" : "#77bbff",
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#4e9eff" : "#77bbff"}`,
      ...
      <p className="text-[18px] font-bold mb-1" style={{ color: "#e8f4ff", fontFamily: "'Noto Serif KR', serif", textShadow: "0 0 20px #4e9eff88" }}>
        결과지를 완성하고 있소…
      </p>
      ...
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#77bbff" }}>
        ...
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#001020" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #2266cc, #4e9eff, #77bbff)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <div className="w-full max-w-[280px] mt-5 px-4 py-3 rounded-xl text-center"
        style={{ background: "rgba(78,158,255,0.15)", border: "1px solid rgba(78,158,255,0.4)" }}>
        <p className="text-[15px] font-semibold mb-1" style={{ color: "#77bbff" }}>⚠️ 절대 새로고침 금지</p>
        <p className="text-[13px] leading-relaxed" style={{ color: "#77bbff" }}>
          결과지 생성에 최대 5분 정도 걸릴 수 있소.<br />오래 걸려도 정상이니 그대로 기다려 주시오.<br />새로고침하면 처음부터 다시 시작되어<br />시간이 더 걸리게 되오.
        </p>
      </div>
      <p className="text-[13px] text-center leading-relaxed mt-4" style={{ color: "#336688" }}>
...
function ErrorScreen({ message }: { message: string }) {
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center px-8 text-center" style={{ background: "#00050f" }}>
...
export default function KunghapBusinessCheckoutSuccessPage() {
  return (
    <Suspense>
      <SuccessInner />
    </Suspense>
...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const confirmRes = await fetch("/api/kunghap_business/payment-confirm", {
      ...
      const imageTask = fetch("/api/kunghap_business-report", {
      ...
      const poll = async () => {
        for (let i = 0; i < 150; i++) { // 최대 5분(2초 간격)
          if (cancelled) return;
          try {
            const r = await fetch(`/api/kunghap_business-report?id=${encodeURIComponent(resultId)}`);
