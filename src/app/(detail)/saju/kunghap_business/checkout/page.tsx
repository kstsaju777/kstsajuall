  ...
  return (
    <div ref={ref} style={{ ...style, flex: 1 }}>
      <div className="rounded-2xl overflow-hidden"
        ...
        <div className="py-2.5 text-center" style={{ backgroundColor: "#eef5ff", borderBottom: `1px solid #a8d0ff` }}>
  ...
  return (
    <div style={{ backgroundColor: WHITE }}>
      ...
      <div className="px-6 pt-10 pb-5 text-center">
        <p className="text-[11px] tracking-[0.2em] mb-3 font-medium" style={{ color: RED }}>비즈니스궁합 · 정밀 리포트</p>
        <h1 className="text-[26px] font-black leading-snug" style={{ color: GRAY1 }}>
          두 사람의 비즈니스,<br />
          사주가 <span style={{ color: "#4e9eff" }}>모두</span> 알고 있소
        </h1>
      ...
      <div className="relative overflow-hidden w-full" style={{ aspectRatio: "9/16" }}>
        ...
        <video
          src="/media/cards/kunghap_business/business-0.mp4"
...
const PREVIEW_SECTIONS = [
  {
    icon: "🤝", title: "두 사람의 비즈니스 궁합", free: true,
    content: "각자의 일간과 오행 구성을 바탕으로 두 사람의 비즈니스 궁합과 협업 스타일을 분석합니다. 서로의 차이가 갈등을 만드는지, 아니면 오히려 보완이 되는지 사주 속에 답이 있습니다.\n\n함께 일할 때 시너지가 나는 순간과 주의해야 할 역할 분배까지 사주는 조용히 기록하고 있습니다.",
    blurLines: [],
  },
  {
    icon: "💼", title: "비즈니스 파트너십 분석", free: false,
    content: "",
    blurLines: [
      "두 사람의 ████간 상생·상극 관계가 파트너십의 핵심입니다.",
      "████년 ████월, 운의 흐름이 두 사람의 사업을 더 강하게 합니다.",
      "이 시기의 결정은 사주상 ████한 영향을 받고 있습니다.",
    ],
  },
  {
    icon: "📊", title: "역할 분담과 리더십", free: false,
    content: "",
    blurLines: [
      "상대방의 일간 기준으로 당신은 ████에 해당합니다.",
      "두 사람의 사주에서 ████이 주도권을 쥐는 구조입니다.",
      "최적의 역할 분담은 ████한 방식으로 설계하는 것이 좋소.",
    ],
  },
  {
    icon: "📅", title: "사업 확장과 운의 흐름", free: false,
    content: "",
    blurLines: [
      "두 사람이 함께 도약할 수 있는 시기는 ████년 ████월입니다.",
      "현재 두 사람의 대운은 ████으로 ████한 기운이 작용합니다.",
      "이 시기를 놓치면 다음 기회는 ████년 이후로 밀립니다.",
    ],
  },
  {
    icon: "⚡", title: "협업을 방해하는 요인", free: false,
    content: "",
    blurLines: [
      "당신의 사주에서 ████살이 협업을 방해하고 있습니다.",
...
const REVIEWS = [
  ...
  { star: 5, text: "비즈니스 파트너와의 궁합을 확인했는데 역할 분담 조언이 정말 도움됐습니다.", name: "40대 대표 이○○", date: "2025.04.28" },
  { star: 5, text: "두 사람 모두 분석해주는 게 정말 좋았어요. 파트너 입장에서 이해할 수 있게 됐습니다.", name: "30대 자영업자 박○○", date: "2025.05.03" },
...
// ─── 결제 모달 ────────────────────────────────────────────────────────────────
const PRODUCT = { name: "비즈니스궁합", original: 49800, discount: 50, price: 24900 };
...
function CreatingScreen({ doneCount, currentChapter }: { doneCount: number; currentChapter: number }) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col items-center justify-center z-50 px-8"
      ...
      <div className="relative w-20 h-20 mb-6">
        ...
        {[0,1,2,3,4,5].map(i => (
          <div key={i} className="absolute w-1 h-1 rounded-full" style={{
            ...
            background: i % 2 === 0 ? "#77bbff" : "#aad4ff",
            boxShadow: `0 0 6px 2px ${i % 2 === 0 ? "#4e9eff" : "#88ccff"}`,
      ...
      <p key={currentChapter} className="text-[13px] mb-8" style={{ color: "#88ccff", animation: "title-fade 4s ease-in-out", minHeight: 20 }}>
        {doneCount < TOTAL ? CHAPTER_TITLES[currentChapter - 1] + " 풀이 중" : "마무리 중이오…"}
      </p>
      <div className="w-full max-w-[280px] mb-3">
        <div className="flex justify-between text-[11px] mb-2" style={{ color: "#4477bb" }}>
          <span>{doneCount} / {TOTAL} 장 완성</span>
          <span>{pct}%</span>
        </div>
        <div className="w-full h-3 rounded-full overflow-hidden relative" style={{ background: "#000d1a" }}>
          <div className="h-full rounded-full relative overflow-hidden transition-all duration-700"
            style={{ width: `${pct}%`, background: "linear-gradient(90deg, #1a3d7a, #4e9eff, #88ccff)", animation: pct > 0 ? "glow-pulse 1.8s ease-in-out infinite" : "none" }}>
      ...
      <p className="text-[11px] text-center leading-relaxed mt-4" style={{ color: "#335588" }}>
