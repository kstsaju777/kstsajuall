...
- intro: "[계절·기후] 속에서 태어난 {이름1}님은 [자연물 이미지]와 같은 기운을 타고났소." 형식으로 시작, 이어서 반드시 2~3문장을 더 추가하여 총 3~4문장으로 풍부하게 작성 (짧게 끝내지 말 것, 각 문장 40자 이상)
- callout: 천간(겉모습)과 지지(내면)의 핵심 대비 한 문장
- paragraphs 2개, 각각 반드시 4~5문장(각 문장 40자 이상)으로 풍부하게 작성: ①신강·신약 판단 + 지배적 십성의 장점과 그림자 + 용신·기신 언급 + 격국 이름과 특성 ②천간 십성 기반 겉모습 vs 지지 십성 기반 내면의 온도 차이와 대인관계 패턴·주의점
...
- growthPhases: 2025~2034년을 5개 구간으로 균등 분할. 각 phase:
  ...
  - score: 이 2년 구간의 양육 주의도 0~100 정수. ⚠️ 아래 [세운별 십성 — 미리 계산된 값]에 나온 천간·지지 십성을 반드시 그대로 가져다 쓰시오 — 절대 직접 계산하거나 다른 십성으로 추측하지 마시오. 관성·상관·겁재처럼 주의가 필요한 십성일수록 높은 값(관성=80↑, 상관=58, 겁재=55 기준). 순한 십성(인성·식신)은 낮은 값.
  - tone: "good"(순탄, score 40 미만) / "caution"(보통, 40~65) / "warn"(주의, 65 초과)
  - title: 이 구간의 핵심 키워드 (4~8글자, 해당 구간 십성 기운 반영)
  - desc: 반드시 다음 5문장 구조로 작성. 총 150~200자. 한자 사용 금지, 간지명은 한국어 독음으로(을사년·병오년·정미년 등). ⚠️ 모든 문장은 반드시 홍연 말투(~이오/~하오/~했소/~겠소)로만 끝맺으시오 — '~됩니다', '~봅니다', '~바랍니다', '~ㅂ니다' 같은 현대 존댓말체는 이 desc를 포함해 이 섹션 전체에서 절대 쓰면 안 되오.
...
const JANYEO_CH_SCHEMA: Record<number, string> = {
  1: `{
  "wonguk": {
    "intro": "【필수 규칙】월지 글자를 확인해 계절·기후 파악 후 '[계절·기후] 속에서 태어난 [이름]님은 [자연물 이미지]와 같은 기운을 타고났소.' 형식으로 시작, 이어서 반드시 2~3문장을 추가로 더 작성하시오(총 3~4문장). 각 문장은 40자 이상으로, 이 자연 이미지가 구체적으로 어떤 성향·기질로 이어지는지 풀어서 서술할 것 — 한 문장으로 짧게 끝내지 마시오.",
    "callout": "천간(겉모습)과 지지(내면)의 핵심 대비 한 문장. 홍연 말투.",
    "paragraphs": [
      "신강/신약 판단(정성적 표현만, 점수·단계 숫자 금지) → 타고난 생명력·추진력을 자연 이미지로 이어 묘사 → 지배적 십성(비겁·식상 등)의 장점과 과할 때 그림자 → 용신·기신 구체 언급 → 격국 이름과 특성까지 전부 담아서, 반드시 홍연 말투로 4~5문장(각 문장 40자 이상)으로 풍부하게 작성하시오. 짧게 요약하지 말 것.",
      "천간 십성 기반 겉모습(표현·행동 방식) → 지지 십성 기반 내면(감추어진 욕구·경계심) → 두 얼굴의 온도 차이와 대인관계 패턴·주의점까지 전부 담아서, 반드시 홍연 말투로 4~5문장(각 문장 40자 이상)으로 풍부하게 작성하시오. 짧게 요약하지 말 것."
    ]
  },
  "ohaeng": { "intro": "어떤 기운이 강한지 도입", "callout": "가장 강한 오행과 의미", "paragraphs": ["강한 오행 성향과 삶의 패턴", "약한 오행 영향", "오행 균형 조언"] },
  "chonyeongi": {
    "paragraphs": [
      "【초년기 풀이 — 년주(年柱) 기반, 필수 규칙】⚠️ 작성 전 STEP 1: 반드시 [기둥별 십성 확인표]의 '년주' 행을 먼저 찾아 천간 십성과 지지 십성을 확인하오. 이 두 값이 이 풀이 전체의 기준이오. STEP 2: 아래 [십성별 의미 가이드]에서 확인표에서 찾은 십성에 해당하는 항목만 읽고, 그 의미를 풀이에 반영하오. 확인표에 없는 십성(예: 비견·겁재·식신 등 다른 십성)은 절대 언급하지 마오. 가이드의 다른 십성 설명은 참고 자료일 뿐이며 이 사람의 풀이에 절대 사용 금지이오. ⓪ 위에 제공된 [기둥별 십성 확인표]에서 년주 항목의 천간 십성과 지지 십성 값을 그대로 가져오오. 절대 추론하거나 임의로 변경하지 마오. 확인표에 없는 십성은 언급 자체를 금지하오. ① 풀이 첫째 또는 둘째 문장 이내에 반드시 '년주 천간 [글자명]은 [십성]이고, 지지 [글자명]은 [십성]이오.' 형식으로 확인표에서 가져온 십성 값을 직접 명시하오. 이 명시 없이 풀이를 전개하는 것은 절대 금지이오. ② 드러난 십성(년주 천간 십성·지지 십성)만을 해석하며, 명식에 없는 십성은 언급하지 마오. ③ 육친으로 보면 년주는 조부모를 나타내오. 아래 [십성별 의미 가이드]를 반드시 참고하여, 년주 천간 십성과 지지 십성 각각의 의미를 초년기 풀이에 녹여내오.\n\n[년주 천간 십성별 의미 가이드 — 조부(조상)의 인연·태어난 환경]\n비견=조상으로부터 독립적 성향 물려받음·경쟁 많은 초년·형제자매 친구 중요 / 겁재=경쟁적 상황 많음·나눔·희생 강요받았을 가능성·자립성 강해짐 / 식신=조화롭고 안정적 가정환경·성실하고 차분한 성격 형성·조상 인연 긍정적 / 상관=창의적·자유로운 환경·규칙보다 자유 추구·부모와 갈등 가능 / 편재=초년 변화 많거나 물질적 불안정·부모가 대담하고 외향적 / 정재=안정적 가정환경·책임감과 성실함을 부모로부터 배움 / 편관=부모가 강인하고 독립적·도전과 변화 많은 환경 / 정관=규칙·질서 중시 환경·전통적이고 책임감 있는 부모 / 편인=부모가 독특하고 창의적·다양한 시도 경험·고립감 가능 / 정인=부모 도움 많이 받음·학문적이고 안정된 환경·부모와 조화로운 관계\n\n[년주 지지 십성별 의미 가이드 — 조모(조상)의 인연·성장 환경]\n비견=형제자매·친구로부터 큰 영향·독립적으로 각자의 길 추구 / 겁재=또래와 갈등·경쟁 많음·그 과정에서 성장 / 식신=안정적이고 평화로운 가정·부모 도움으로 안정적 초년 / 상관=독특하고 창의적 성향·자유로운 사고·반항적 성향 가능 / 편재=어릴때부터 재물 경험·부모 지원 가능·변동성 강함 / 정재=조상으로부터 물질적 지원 가능·실속있고 안정적 환경 / 편관=독립적이고 변동성 강한 환경·부모와 갈등·긴장 가능 / 정관=안정적이고 체계적 교육·부모에 대한 존경심 형성 / 편인=독립적이고 창의적 환경·부모와의 관계 복잡하거나 특이함 / 정인=안정적이고 현실적인 부모·초년기에 보호받는 느낌 ④ 년주 천간 오행과 지지 오행 사이의 관계를 아래 표에서 정확히 확인하오. [상극표: 목극토 / 토극수 / 수극화 / 화극금 / 금극목] [상생표: 목생화 / 화생토 / 토생금 / 금생수 / 수생목] [비화: 같은 오행끼리] — 이 표에 없는 조합은 상극이 아니오. 상극이 맞는 경우에만 그 긴장이 초년기에 어떤 갈등이나 부침으로 작용했는지 첨언하오. 상생이면 그 기운이 서로 도왔다고 서술하오. 비화이면 기운이 강하게 모였다고 서술하오. ⑤ 위 [섹션별 시제 지시]에서 chonyeongi에 지정된 시제로만 모든 문장을 작성하오. 다른 시제를 섞지 마오. ⑥ 첫 문장은 반드시 '초년기에는'으로 시작하오. ⑦ 좋은 면과 좋지 않은 면을 반드시 모두 담으오. 반드시 홍연 말투로 300자(공백 포함) 내외로 작성하오. 풀이 대상은 아이이므로 부모가 읽는 시점에서 따뜻하고 실용적으로 서술하오."
    ]
  },
  "cheongneongi": {
    "paragraphs": [
      "【청년기 풀이 — 월주(月柱) 기반, 필수 규칙】⓪ 위에 제공된 [기둥별 십성 확인표]에서 월주 항목의 천간 십성과 지지 십성 값을 그대로 가져오오. 절대 추론하거나 임의로 변경하지 마오. 확인표에 없는 십성은 언급 자체를 금지하오. ① 풀이 첫째 또는 둘째 문장 이내에 반드시 '월주 천간 [글자명]은 [십성]이고, 지지 [글자명]은 [십성]이오.' 형식으로 확인표에서 가져온 십성 값을 직접 명시하오. 이 명시 없이 풀이를 전개하는 것은 절대 금지이오. ② 드러난 십성(월주 천간 십성·지지 십성)만을 해석하며, 명식에 없는 십성은 언급하지 마오. ③ 육친으로 보면 월주는 부모를 나타내오. 아래 [십성별 의미 가이드]를 반드시 참고하여, 월주 천간 십성과 지지 십성 각각의 의미를 청년기 풀이에 녹여내오.\n\n[월주 천간 십성별 의미 가이드 — 부친·사회활동·직업]\n비견=동등한 입장과 경쟁 많음·직업적 독립성 강함·자기주장 강한 부모 관계 / 겁재=재물 나눔·소모 경험·직업적 과감한 도전·부모와 자율적·독립적 관계 / 식신=직업에서 안정적이고 조화로운 성과·타인과 협력·부모와 원만한 관계 / 상관=창의적·독립적·틀을 깨는 아이디어·직업에서 새로운 방식 추구 / 편재=재물 흐름 크고 변동성 있음·직업적으로 활발하고 대담·부모와 자유로운 관계 / 정재=재물관리 능력 뛰어남·성실하고 안정적 직업 기반·책임감 있는 부모 관계 / 편관=직업 변동 많고 도전적·새로운 시도 즐김·자신의 방식 고수 / 정관=안정적이고 책임감 있는 직업 태도·전통·규범 중시·부모와 조화로운 관계 / 편인=창의적·독립적 성향·직업 변동 많음·부모와 독특한 관계 방식 / 정인=학문적·지적 직업 태도·사회적 신뢰·부모에게 존경받음\n\n[월주 지지 십성별 의미 가이드 — 모친·사회활동·직업]\n비견=사회 교류 활발·협력보다 경쟁 강함·동료와 끊임없는 경쟁 / 겁재=과도한 사회 경쟁·강한 추진력과 대담한 도전·재물 관리 주의 / 식신=사회적 안정 기반·직업에서 지속적 성과·주변과 조화롭고 평화로운 관계 / 상관=독창적이고 도전적·예술적·창의적 직업 적합·지나친 자유로움으로 갈등 가능 / 편재=재물운 큰 기회·변동성 강해 관리 필요·사교적이고 외향적 활동 적합 / 정재=안정적 재물운·직업에서 성실·가족을 위한 책임감 강조 / 편관=강한 추진력과 리더십·도전적·지나친 독단 주의 / 정관=안정적 직업으로 사회 신뢰·책임감·상사·부모와 조화·규범 준수 / 편인=독립적·창의적·지적 직업에서 두각·지나친 고립 주의 / 정인=학문적·지적 활동 강함·부모와 안정적·조화로운 관계·직업적 신뢰 높음 ④ 월주 천간 오행과 지지 오행 사이의 관계를 아래 표에서 정확히 확인하오. [상극표: 목극토 / 토극수 / 수극화 / 화극금 / 금극목] [상생표: 목생화 / 화생토 / 토생금 / 금생수 / 수생목] [비화: 같은 오행끼리] — 이 표에 없는 조합은 상극이 아니오. 상극이 맞는 경우에만 그 긴장이 청년기에 어떤 갈등이나 부침으로 작용할지 첨언하오. 상생이면 그 기운이 서로 도울 것이라 서술하오. 비화이면 기운이 강하게 모인다고 서술하오. ⑤ 위 [섹션별 시제 지시]에서 cheongneongi에 지정된 시제로만 모든 문장을 작성하오. 다른 시제를 섞지 마오. 두 십성이 함께 만들어내는 청년기 전체 분위기를 통합하여 서술하오. ⑥ 월주는 사회생활에서의 기질을 나타내므로, 월주 십성과 오행을 근거로 조직 생활이 어울리는 아이인지 독립·창업이 어울리는 아이인지 명확히 언급하오. ⑦ 좋은 면과 좋지 않은 면을 반드시 모두 담으오. 반드시 홍연 말투(~이오/~하오/~했소/~겠소)로 300자(공백 포함) 내외로 작성하오."
    ]
  },
  "jungnyeongi": {
    "paragraphs": [
      "【중년기 풀이 — 일주(日柱) 기반, 필수 규칙】⓪ 위에 제공된 [기둥별 십성 확인표]에서 일주 항목의 천간 십성과 지지 십성 값을 그대로 가져오오. 절대 추론하거나 임의로 변경하지 마오. 확인표에 없는 십성은 언급 자체를 금지하오. ① 풀이 첫째 또는 둘째 문장 이내에 반드시 '일주 천간 [글자명]은 [오행]이고, 지지 [글자명]은 [십성]이오.' 형식으로 확인표에서 가져온 값을 직접 명시하오. 이 명시 없이 풀이를 전개하는 것은 절대 금지이오. ② 드러난 십성(일주 천간의 오행 본질·일지 십성)만을 해석하며, 명식에 없는 십성은 언급하지 마오. ③ 육친으로 보면 일주는 나 자신과 배우자를 나타내오. 일간의 오행·음양 본질이 이 아이의 중년기 삶의 방식을 결정하고, 아래 [십성별 의미 가이드]를 반드시 참고하여 일지 십성이 배우자와의 관계·결혼생활·내면 심리에 어떤 영향을 미칠지 구체적으로 풀이하오. 아울러 사회적 역할 완성, 직업 전성기 혹은 전환점, 내면의 성숙도 함께 서술하오.\n\n[일주 지지 십성별 의미 가이드 — 배우자궁·결혼생활·내면심리]\n비견=배우자와 대등하고 평등한 관계·서로 의견 존중·협력 기반·경쟁적 의견충돌 가능 / 겁재=경쟁적이고 도전적인 기운·배우자 활발하고 적극적·활기차고 역동적·지나친 갈등·간섭 가능 / 식신=안정적이고 평화로운 기운·배우자 온화하고 가정적·안정 중시하나 변화 두려워할 수 있음 / 상관=자유로운 분위기 선호·배우자 창의적이고 개성 강함·전통 결혼관 탈피·지나친 자유 추구 시 불안정 / 편재=활발하고 다채로운 관계·배우자 외향적이고 매력적·활기찬 결혼생활·지나친 외향성으로 갈등 가능 / 정재=안정적이고 현실적·배우자 성실하고 책임감 강함·신뢰 기반 결혼생활·낭만 부족할 수 있음 / 편관=강렬하고 도전적인 기운·배우자 강인하고 독립적·열정적이고 다이나믹·갈등·긴장 가능 / 정관=안정적이고 전통적 가치·배우자 신뢰할 수 있고 책임감·믿음직한 결혼생활·틀에 박힌 삶 가능 / 편인=독창적이고 독립적·배우자 창의적이고 독특·개성 넘치는 결혼생활·감정적 예민함·고립 가능 / 정인=안정적이고 배려심 강함·배우자 가정적이고 보호적·따뜻하고 조화로운 결혼생활·지나친 의존·희생 가능 ④ 일주 천간 오행과 지지 오행 사이의 관계를 아래 표에서 정확히 확인하오. [상극표: 목극토 / 토극수 / 수극화 / 화극금 / 금극목] [상생표: 목생화 / 화생토 / 토생금 / 금생수 / 수생목] [비화: 같은 오행끼리] — 이 표에 없는 조합은 상극이 아니오. 상극이 맞는 경우에만 그 긴장이 중년기에 어떤 갈등이나 삶의 전환으로 작용할지 첨언하오. 상생이면 그 기운이 서로 도울 것이라 서술하오. 비화이면 기운이 강하게 모인다고 서술하오. ⑤ 위 [섹션별 시제 지시]에서 jungnyeongi에 지정된 시제로만 모든 문장을 작성하오. 다른 시제를 섞지 마오. 두 기운이 함께 만들어내는 중년기 전체 분위기를 통합하여 서술하오. ⑥ 좋은 면과 좋지 않은 면을 반드시 모두 담으오. ⑦ 위에 [간여지동] 확정값이 주입되어 있으면, 반드시 '간여지동'이라는 명칭을 언급하며 그 의미(강한 자아·독립성, 배우자와의 갈등 소지)를 담으시오. 주입되어 있지 않으면 언급하지 마시오. 반드시 홍연 말투(~이오/~하오/~했소/~겠소)로 300자(공백 포함) 내외로 작성하오."
    ]
  },
  "nonyeongi": {
    "paragraphs": [
      "【말년기 풀이 — 시주(時柱) 기반, 필수 규칙】⓪ 위에 제공된 [기둥별 십성 확인표]에서 시주 항목의 천간 십성과 지지 십성 값을 그대로 가져오오. 절대 추론하거나 임의로 변경하지 마오. 확인표에 없는 십성은 언급 자체를 금지하오. ① 풀이 첫째 또는 둘째 문장 이내에 반드시 '시주 천간 [글자명]은 [십성]이고, 지지 [글자명]은 [십성]이오.' 형식으로 확인표에서 가져온 십성 값을 직접 명시하오. 이 명시 없이 풀이를 전개하는 것은 절대 금지이오. ② 드러난 십성(시주 천간 십성·지지 십성)만을 해석하며, 명식에 없는 십성은 언급하지 마오. ③ 육친으로 보면 시주는 자녀를 나타내오. 아래 [십성별 의미 가이드]를 반드시 참고하여, 시주 천간 십성과 지지 십성 각각의 의미를 말년기 풀이에 녹여내오. 아울러 노후 재물·건강, 인생의 결실과 마무리도 함께 서술하오.\n\n[시주 천간 십성별 의미 가이드 — 말년·자녀(아들)]\n비견=자식 도움보다 자기의지로 삶 꾸림·말년에도 독립적·자신만의 방식 고수 / 겁재=말년에 재물 나눔·희생 발생·책임감 과중·자녀와 재물·책임 갈등 가능·소통 중요 / 식신=안정적이고 평화로운 말년·자식과 원만하고 조화로운 관계·자녀에게 풍요 제공 / 상관=도전적이고 창의적인 말년·틀을 깨는 일 많음·자녀 독립적이고 반항적 가능 / 편재=말년에 큰 재물 기회·외부 재물 활용·자녀에게 대담하고 관대한 태도 / 정재=말년에도 꾸준하고 안정적 재물·자녀에게 경제적 도움 가능한 위치 / 편관=말년에 변동 많고 도전적 삶·자녀와의 관계도 독립적이고 강렬 / 정관=안정적 사회적 지위 유지·자녀와 조화로운 관계·규칙적이고 책임감 있는 모습 / 편인=창의적이고 독특한 말년·직관적·독립적·다소 고독해질 가능성 / 정인=안정감 있고 조화로운 말년·자녀·후손에게 존경받는 위치·학문적이고 지혜로운 태도\n\n[시주 지지 십성별 의미 가이드 — 말년·자녀(딸)]\n비견=말년에 동료·형제자매·친구와 교류 활발·인간관계에서 도움받음·자녀에게서 협력과 도움 가능 / 겁재=자녀에게서 도움 받으나 감정적 갈등 가능·말년에 도전적 행동으로 인생 전환점 만들기도 함 / 식신=자식이 안정적이고 평화롭게 자리잡음·평화롭고 성실한 말년·자녀가 창의적 / 상관=자식 개성 강하고 독립적·말년에 갈등 상황 가능·조화를 신경써야 함 / 편재=자식 활발하고 사교적·재물 변동 많아 안정보다 변화 추구 성향 / 정재=자식 성실하고 안정적·재물관리 신중하게 해야 하는 경향 / 편관=자식 독립적이고 자기주장 강함·갈등·변동 유발 가능성 / 정관=자식 전통적이고 안정적 성향·너무 틀에 박히지 않도록 균형 필요 / 편인=자식 독창적이고 자유로운 성향·감정적 갈등 발생 여지 있음 / 정인=자식 안정적이고 현실적·부모에게 도움이 되는 역할 가능성 ④ 시주 천간 오행과 지지 오행 사이의 관계를 아래 표에서 정확히 확인하오. [상극표: 목극토 / 토극수 / 수극화 / 화극금 / 금극목] [상생표: 목생화 / 화생토 / 토생금 / 금생수 / 수생목] [비화: 같은 오행끼리] — 이 표에 없는 조합은 상극이 아니오. 상극이 맞는 경우에만 그 긴장이 말년기에 어떤 갈등이나 마무리의 과제로 작용할지 첨언하오. 상생이면 그 기운이 서로 도울 것이라 서술하오. 비화이면 기운이 강하게 모인다고 서술하오. ⑤ 위 [섹션별 시제 지시]에서 nonyeongi에 지정된 시제로만 모든 문장을 작성하오. 다른 시제를 섞지 마오. 두 기운이 함께 만들어내는 말년기 전체 분위기를 통합하여 서술하오. ⑥ 좋은 면과 좋지 않은 면을 반드시 모두 담으오. 반드시 홍연 말투(~이오/~하오/~했소/~겠소)로 300자(공백 포함) 내외로 작성하오."
    ]
  },
  ...
  "yongsin": { "intro": "【필수】⚠️ 용신·희신·기신은 반드시 오행 이름(목/화/토/금/수)으로만 부르시오 — '용신인 비겁 기운'처럼 십성 이름(비견·겁재·비겁·식상·재성·관성·인성 등)을 용신·희신·기신이라고 부르면 절대 안 되오. 용신 오행이 일간에 대해 어떤 역할(제어·다듬기·균형·생조 등)을 하는지, 왜 이 기운이 가장 필요한지 체감 언어로 서술하는 도입", "callout": "용신 오행과 일간 오행의 관계(상생·상극·비화)를 한 문장으로 짚는 핵심 문장 — 오행 이름 명시 필수", "paragraphs": ["용신 오행이 일간에 어떤 역할을 하는지, 이 기운이 있을 때 삶이 어떻게 달라지는지 체감 언어로 2~3문장. 반드시 오행 이름(목/화/토/금/수)으로만 지칭하고 십성 이름을 용신이라 부르지 마시오.","희신 오행이 일간을 어떻게 보완·뒷받침하는지(뿌리·영양분·안정 등) 2문장 — 오행 이름 명시, 십성 이름으로 대체 금지","기신 오행이 왜 삶을 버겁게 만드는지(일간을 더 강하게 하거나 흐름을 막는 이유) 2문장으로 마무리 — 전체 합계 800자 내외, 오행 이름 명시, 십성 이름으로 대체 금지"], "yongsinEl": "목/화/토/금/수 중 하나(용신 오행 — 일간과의 상생·상극·비화 관계 근거로 도출)", "heusinEl": "목/화/토/금/수 중 하나(희신 오행)", "gisinEl": "목/화/토/금/수 중 하나(기신 오행)", "yongsinReason": "이 기운이 있을 때 삶에 어떤 변화가 생기는지 체감 언어로 15자 이내 (예: 있으면 중심이 잡히고 성과가 난다)", "heusinReason": "이 기운이 있으면 어떤 점이 좋아지는지 체감 언어로 15자 이내 (예: 있으면 관계가 부드러워진다)", "gisinReason": "이 기운이 강하면 삶이 어떻게 흔들리는지 체감 언어로 15자 이내 (예: 강하면 고집이 세져 막힌다)" },
...
export function buildJanyeoChapterPrompt(
  ...
  input: {
    ...
    heusinEl?: string;
    gisinEl?: string;
    deungResult?: { deungnyeong: boolean; deungji: boolean; deungsi: boolean; deungse: boolean; ilganEl: string; woljiEl: string; iljiEl: string; sijiEl: string; seCount: number };
  }
  ...
  if (input.pillars && input.pillars.length > 0) {
    ...
    const SIP_ORDER_JY = ["비견","겁재","식신","상관","편재","정재","편관","정관","편인","정인"];
    const sipLines = SIP_ORDER_JY.map(k => `${k}×${sipCnt[k] ?? 0}`).join(" / ");
  ...
  if (chapter === 1 && input.pillars && input.pillars.length > 0) {
    ...
    // 월지 계절 — LLM이 표를 보고 직접 찾다가 다른 계절로 착각하는 오류를 막기 위해 서버가 확정 계산해서 주입
...
nonyeongi(말년기) 풀이: 반드시 ${tenseOf.nonyeongi}으로만 작성\n`;
  ...
  // 2장: 득령·득지·득시·득세 판정표 생성 — 클라이언트 계산값 우선, 없으면 서버 재계산
  let deungTable = "";
  if (chapter === 2) {
    ...
    if (dr) {
      ...
      const byPosDe: Record<string, typeof input.pillars[0]> = {};
      for (const p of input.pillars) byPosDe[p.pos] = p;
      const siju = byPosDe["시주"]; const ilju = byPosDe["일주"]; const wolju = byPosDe["월주"]; const nyeonju = byPosDe["년주"];
  ...
  if ((chapter === 6 || chapter === 7) && input.seun && input.seun.length > 0) {
    ...
    seunDaeunNote += `\n【중요】 ${phaseField} 5개의 yearStart/yearEnd는 2025~2034를 균등하게 나누시오. score·tone·desc는 반드시 위 [세운별 십성] 표에 적힌 실제 십성값을 그대로 근거로 삼으시오 — 표에 없는 십성을 지어내거나 직접 재계산하면 절대 안 되오. desc 첫 문장은 반드시 "[yearStart]년 [간지독음]년과 [yearEnd]년 [간지독음]년은~" 형식으로 시작하시오. 한자 사용 금지.\n`;
  }
