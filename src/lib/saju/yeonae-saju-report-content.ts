...
- ilganDesc: 이 사람의 일간({일간명})을 자연물 하나에 빗대어 연애 기질을 풀이하오. 한자는 절대 쓰지 말고 한글 이름만 사용하시오. 아래 구조를 반드시 따르오.
...
- text: 이 시기 인연의 흐름과 기운을 4문장으로 서술. 각 문장은 반드시 40자 이상. ① 이 시기에 어떤 연애운 기운이 흐르고 인연에 어떤 영향을 주는지, 세운·십성의 흐름을 근거로 구체적으로 서술. ② 이 시기 인연의 실제 모습이 어떻게 나타날 가능성이 높은지(새로운 만남·감정의 깊이·관계 변화 등) 독자가 "내 얘기"처럼 느낄 수 있는 체감 언어로 풀이. ③ 이 시기에 반드시 해야 할 행동 또는 반드시 피해야 할 행동을 구체적으로 제시. ④ 격려 또는 경계의 마무리 한 문장. 홍연 화자(~했소/~이오/~하오, '~습니다/~입니다/~것이다' 절대 금지). 호칭 사용. 사주 용어는 쓰되 독자가 이해할 수 있는 쉬운 언어로 풀어 설명할 것. ⚠️ 아래 [연도별 연애운 추세]에 적힌 점수 숫자(예: '66점')는 절대 문장에 그대로 인용하지 마시오 — 점수는 방향(상승·하락·유지 등)을 판단하는 내부 참고용일 뿐이니, text에는 반드시 그 방향성만 말로 풀어 쓰고 숫자는 한 글자도 등장시키지 마시오.
※ 구간별 추세 요약의 구간을 【절대로 합치거나 나누지 말 것】. 구간 수, label, trend 모두 아래 목록 그대로 사용. 반드시 2033년까지 전부 커버할 것.
⚠️ text 안의 방향 표현(오른다·낮아진다·유지된다 등)은 반드시 같은 item의 trend 값과 정확히 일치해야 하오. trend가 "인연 상승"인데 text에서 "하강"·"저점" 등 반대 방향을 암시하면 절대 안 되며, trend가 "인연 하강"·"인연 저점"일 때만 조심·주의 표현을 쓰고, "인연 상승"·"인연 절정"일 때는 반드시 긍정적 표현만 쓰시오.
⚠️ "대운"이라는 단어 자체와 대운 간지 이름(예: "임신 대운", "정미 세운" 등 대운 명칭)은 loveFlow·lovePeak·loveNow 이 섹션 전체에서 절대 언급하지 마시오. "임신 대운의 중반부", "정인 대운으로의 과도기"처럼 대운을 근거로 드는 표현도 전부 금지이며, 오직 "○○년"(세운)만으로 시기를 설명하시오.
