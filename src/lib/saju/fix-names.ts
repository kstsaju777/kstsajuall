...
export function fixNamesInText(
  ...
  // 8. 잔여 이중 밑줄(__) 제거 — GLOBAL_GRAMMAR_RULES가 마크다운 굵게(**)는
  // 금지하지만 GFM 방식 굵게(__단어__)는 금지 목록에 없어서, 우리 이름 토큰
  // (__MY__/__PT__)과 같은 문법을 LLM이 강조용으로도 섞어 쓰다 "지희양는__"처럼
  // 잔재가 노출되는 사고가 있었다(고객 신고). 이 시점엔 위에서 정상 토큰을 전부
  // 치환했으므로, 남은 __는 전부 이 마크다운 잔재이니 안전하게 제거한다.
  r = r.replace(/__/g, "");
