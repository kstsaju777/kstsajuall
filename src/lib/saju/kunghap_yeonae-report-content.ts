...
- score: 합충 점수 {{HAP_CHUNG_SCORE_NUM}}의 70%와 사주 전반 판단(신강·신약·일간 관계·오행 균형 등) 30%를 합산한 정수를 입력하시오. 반드시 위 tone 구간에 맞는 값이어야 하오.
- label: 점수에 맞는 한 줄 라벨.
...
export function buildYeonaeKunghapChapterPrompt(
  ...
  const { name, gender, manseryeokText, partnerName, partnerGender, partnerManseryeokText, ohaengSummary, partnerOhaengSummary, ilgan, partnerIlgan, mySipseong, partnerSipseong, hapChungScore, bestTimingPeriod, worstTimingPeriod, worstTimingText, timingContext, prevChapterSummary, ilganFull, partnerIlganFull, myPillars, partnerPillars, daeunSeunContext } = input;
