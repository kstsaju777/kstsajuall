...
export function isYeonaeKunghapChapterReady(
  ...
  return keys.every((k) => {
    ...
    if ((k === "hapList" || k === "chungList") && "items" in v) return Array.isArray(v.items);
...
⚠️ 반드시 위 【본인】 만세력 텍스트에서 다음 항목을 직접 읽어 풀이에 반영하시오:
  - 일간: 사주팔자는 년주·월주·일주·시주 순서로 나열되오. 반드시 세 번째 기둥(일주)의 천간이 일간이오. 년주·월주·시주의 천간과 혼동하지 마시오.
...
- patternType: 연애 유형명 (예: "리드형 연인", "감성 수호자형", "자유로운 영혼형", "헌신적 동반자형").
...
- patternType: 상대의 연애 유형명 (예: "리드형 연인", "감성 수호자형", "자유로운 영혼형", "헌신적 동반자형").
- patternIcon: 유형에 맞는 이모지 1개.
...
⚠️ 점수 근거를 길게 풀어내는 섹션이라 보고서체(-다/-이다)로 새기 쉬우니, 마지막 문장까지 다시 확인해 홍연 말투로만 끝맺으시오.
...
- items 3~4개: 내가 이 사람에게 무의식적으로 기대하는 것.
  각 item: label(기대 제목, 5자 이내), icon(이모지 1개), desc(한 줄 핵심 설명), detailDesc(이 기대가 생기는 사주적 이유와 기대가 충족/미충족될 때 어떤 감정이 드는지 4~5문장, 150자 이상. 충분히 풀어서 서술하오).
...
- items 3~4개: 상대가 나에게 무의식적으로 기대하는 것.
  각 item: label(기대 제목, 5자 이내), icon(이모지 1개), desc(한 줄 핵심 설명), detailDesc(사주적 이유와 기대가 충족/미충족될 때 어떤 감정이 드는지 4~5문장, 150자 이상. 충분히 풀어서 서술하오).
...
- warningItems 2~3개: { trigger: "주의 상황", action: "처방 한 줄" }.`,
  ...
  6: `[hapList 섹션 — 합 목록]
items 2~4개: 두 사람 사주에서 발견되는 합. 실제 명식에서 존재하는 합만 기재.
각 item:
- type: 합의 구체적 종류 (예: "갑기합", "을경합", "자축합", "인오술삼합", "해묘미삼합"). 가능한 한 정확한 천간합·지지합·삼합 이름으로.
- hapEl: 합화 결과 오행 (예: "토", "금", "화"). 합하여 어떤 오행이 되는지. 삼합의 경우 해당 국의 오행.
...
[chungList 섹션 — 충 목록]
items 0~3개: 두 사람 사주에서 발견되는 충. 실제 명식에서 없으면 빈 배열 [].
각 item:
...
- hapSummary: 이 두 사람의 합 전체를 한 줄로 요약 (예: "세 곳의 합이 두 사람을 자연스럽게 묶어주고 있소").
- chungSummary: 이 두 사람의 충 전체를 한 줄로 요약. 없으면 충 없음을 긍정적으로 (예: "충이 없어 직접적인 마찰이 적은 편이오").
- desc: 합·충을 종합한 궁합 분석을 4~5문장으로. 합과 충의 비율, 전체 관계 에너지, 이 점수가 나온 사주적 근거, 관계의 전체적인 방향성. 홍연 말투.
⚠️ 개수·비율 근거를 짚는 섹션이라 보고서체(-다/-이다)로 새기 쉬우니, 홍연 말투로만 끝맺으시오.`,
...
⚠️ [최우선 일관성 규칙] bestPeriod와 desc는 반드시 서로 같은 연도를 가리켜야 하오. desc 본문 어디에도 "{{BEST_TIMING_PERIOD}}"가 아닌 다른 연도·연도범위를 "결혼하기 좋은 시기"로 언급하면 절대 안 되오 — 예를 들어 확정값이 "2030년"인데 desc 첫 문장에서 "2027~2028년이 결혼 시기로 적합한 이유는"처럼 다른 연도를 쓰는 것은 명백한 오류이오. desc의 첫 문장은 반드시 "{{BEST_TIMING_PERIOD}}"라는 글자 그대로를 그대로 인용하며 시작하시오.
- bestPeriod: 반드시 "{{BEST_TIMING_PERIOD}}"로 입력하시오 (확정값 — 절대 다른 시기 입력 금지).
- desc: 왜 {{BEST_TIMING_PERIOD}}가 결혼하기 좋은 시기인지를 아래 순서로 서술하시오. 500자 안팎(450~550자)으로 간결하게 압축하시오 — 아래 ①~④ 항목을 전부 다루되 각 항목은 한두 문장으로만 짧게 서술하고, 늘어지게 풀어 쓰지 마시오. 홍연 말투.
  ⚠️ 대운은 절대 언급하지 마시오 — "대운", 대운 간지, 대운 내 위치(초반/중반/후반) 등 대운 관련 표현은 전부 금지이오. 오직 세운(해당 연도)만 근거로 서술하시오.
  ① {{BEST_TIMING_PERIOD}} 해당 연도가 어떤 간지(예: 경술년)인지 명시하시오. 프롬프트 하단 "[서버 계산 확정값]" 블록의 세운 간지·십성 값을 그대로 인용하고, 절대 재계산하지 마시오.
  ...
  ④ 두 사람 모두의 세운 십성을 종합하여 왜 이 해가 결혼하기 좋은 해인지 결론짓고, 이 시기가 가져올 변화를 감성적으로 마무리하시오.
...
- cautionDesc: {{WORST_TIMING_PERIOD}}가 왜 결혼을 서두르면 안 되는 위험한 시기인지 서술하시오. 이 시기의 특성: {{WORST_TIMING_TEXT}} — 이 내용을 바탕으로 사주적 근거(세운·십성·합충)를 추가하여 4~5문장(200자 이상)으로 풀어 쓰시오. 대운은 절대 언급하지 마시오. 홍연 말투.`,
...
- 홍연 말투(~이오/~하오/~겠소) 유지.
- 각 단락 방향:
  ① 홍연이 처음 이 인연을 마주했을 때의 인상과 감상. 두 사람이 어떤 사람인지를 분석이 아닌 따뜻한 시선으로 소개하시오.
  ...
  ④ 앞으로의 시간에 대한 조언. 좋은 시기에 어떻게 움직이고, 힘든 시기에 어떻게 서로를 붙잡아야 하는지.
    ⚠️ 대운·세운의 시기를 언급할 때 반드시 "○○년"(연도) 단위까지만 쓰고, "○○년 9월부터"처럼 월·일 단위는 절대 지어내지 마시오 — 프롬프트에 주입된 데이터에 없는 정밀도이오. (예: "2024년 9월부터 2033년까지" ✗ → "2024년부터 2033년까지" ○)
  ⑤ 홍연의 마지막 인사. 이 인연이 오래도록 빛나기를 바라는 진심 어린 축원. 두 사람 각각에게 한 마디씩 건네며 마무리하시오.
⚠️ 5단락이나 되는 긴 편지라 쓰다 보면 자기도 모르게 "~습니다", "~입니다"체로 새기 쉬우니, 마지막 단락까지 다 쓴 뒤 모든 문장을 처음부터 다시 확인하여 홍연 말투로만 끝맺으시오.`,
};
...
⚠️ 계절 단어("봄" "여름" "가을" "겨울")는 고유 단어이오. 절대 변형 금지.`;
  ...
  const myDataBlock = [
    ilgan ? `⛔ 일간 확정값: ${ilganFull || ilgan} — 풀이에서 이 일간을 다른 글자로 쓰는 것 절대 금지. 예) "${ilganFull || ilgan}의 기운" "○○님은 ${ilganFull || ilgan}을 일간으로" 처럼만 쓰시오.` : "",
  ...
  const partnerDataBlock = [
    partnerIlgan ? `⛔ 일간 확정값: ${partnerIlganFull || partnerIlgan} — 풀이에서 이 일간을 다른 글자로 쓰는 것 절대 금지. 예) "${partnerIlganFull || partnerIlgan}의 기운" "○○님은 ${partnerIlganFull || partnerIlgan}을 일간으로" 처럼만 쓰시오.` : "",
