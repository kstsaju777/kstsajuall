...
- ilganDesc: 이 사람의 일간({일간명})을 자연물 하나에 빗대어 재물 기질을 풀이하오. 한자는 절대 쓰지 말고 한글 이름만 사용하시오. 아래 구조를 반드시 따르오.
...
- peakWhen: 황금기가 오는 시기 (예: "30대 후반~40대 초반", "2027년 무렵" — "대운"이라는 단어 자체와 대운 간지 이름을 절대 언급하지 말 것. 오직 나이대 또는 세운(연도)으로만 표현)
- peakDesc: 황금기의 본질과 그 시기를 어떻게 준비해야 하는지 3~4문장. 어떤 세운(연도)이 열리는지 구체적으로. ⚠️ "대운"이라는 단어 자체를 이 섹션에서 절대 쓰지 마시오 — "대운의 흐름과 만나는", "이 대운에서" 같은 일반적인 표현도 전부 금지이며, 오직 세운(연도)만으로 시기를 설명하시오. 홍연 화자. 호칭 사용.
...
- summary: 이 명식의 향후 연도별 재물 흐름 전반적인 방향과 특징. 3~4문장. 언제가 기회고 언제가 위기인지 큰 그림. 홍연 화자. 호칭 사용.
- items: 위 [구간별 추세 요약]의 구간을 【절대로 합치거나 나누지 말 것】. 구간 수, label, trend 모두 아래 목록 그대로 사용. 임의로 묶거나 재분류하면 안 됨. 반드시 2033년까지 전부 커버할 것.
  각 item: label(시기 예: "2024~2026"), trend(계산된 추세 그대로: "상승 중"|"고점"|"하락 중"|"저점"|"유지"), title(이 시기 재물 기운을 한 마디로. 추세와 일치하게. 예: 상승 중→"재물이 오르는 시기", 고점→"재물운의 정점", 하락 중→"숨고르기 시기", 저점→"조심해야 할 시기"), text(이 시기 재물 기운의 원인·흐름·해야 할 일을 3~4문장으로. 각 문장은 반드시 40자 이상. ① 이 시기에 어떤 기운이 흐르고 재물에 어떤 영향을 주는지 사주 원국과 세운(연도)의 흐름을 근거로 구체적으로 서술. ② 이 시기 재물의 실제 움직임(수입·지출·투자·사업 등)이 어떻게 나타날 가능성이 높은지 체감 언어로 풀이. ③ 이 시기에 반드시 해야 할 행동 또는 반드시 피해야 할 행동을 구체적으로 제시. ④ 격려 또는 경계의 마무리 한 문장. 홍연 화자(~했소/~이오/~하오). 호칭 사용. 사주 용어는 쓰되 독자가 이해할 수 있는 쉬운 언어로 풀어 설명할 것.).
  ⚠️ text의 방향(상승·하락·정체 등) 표현은 반드시 같은 item의 trend 값과 정확히 일치해야 하오. trend가 "상승 중"인데 text에서 "하락"·"조심"·"숨고르기" 등 반대 방향을 언급하면 절대 안 되오. trend가 "저점"·"하락 중"일 때만 주의·경계 표현을 쓰고, "상승 중"·"고점"일 때는 반드시 긍정적 표현만 쓰시오.
  ⚠️ "대운"이라는 단어 자체와 대운 간지 이름(예: "경진 대운", "갑진 대운" 등)은 이 섹션(wealthPeak·moneyTrap·warningFlow 전체) 어디에서도 절대 언급하지 마시오. "대운의 흐름과 만나는", "이 대운에서" 같은 일반적인 표현도 전부 금지이오. 시기를 가리킬 때는 오직 "○○년"(세운) 또는 나이대·구간(label)으로만 표현하시오.`,
...
- items: 핵심 정리 3개. 각 item:
  ...
  - desc: 핵심을 담은 풀이. 반드시 3문장, 각 문장 40자 이상. ⚠️ "이 명식의", "이 사주의"처럼 사물을 주어로 삼아 뭉뚱그리지 말고, 반드시 "{이름}님의 가장 큰 재물 무기는" 처럼 사람을 주어로 직관적으로 시작하시오. ① 이 행동이 왜 {이름}님에게 특히 중요한지 사주 원국 근거 포함. ② 이 행동을 잘했을 때와 못했을 때 재물에 어떤 차이가 생기는지 체감 언어로 서술. ③ 구체적인 실천 방법 또는 마무리 조언. 홍연 화자(~했소/~이오/~하오). 호칭 사용.
...
const JAEMUL_CH_SCHEMA: Record<number, string> = {
  ...
  "warningFlow": {
    "summary": "향후 연도별 재물 흐름 전반적 방향 3~4문장 (대운 간지 이름 언급 금지)",
    "items": [
  ...
  "wealthSummary": {
    ...
    "items": [
      { "icon": "💡", "title": "자주성을 무기로 삼으시오", "desc": "{이름}님의 가장 큰 재물 무기는 누구의 눈치도 보지 않고 스스로 판단하고 실행하는 독립적인 추진력으로, 이 힘이 잘 발휘될 때는 남들이 망설이는 기회에 먼저 움직여 앞서나가는 결과로 이어지오. 깊이 파고든 분야에서 재물이 따라오는 기질이므로, 이 자주성을 살렸을 때와 억눌렀을 때 수입의 차이가 눈에 띄게 벌어지오. 다만 자주성이 고집으로 굳어지지 않도록 실력은 혼자 쌓되 결정은 한 번 더 검토하는 습관을 철칙으로 삼으시오." },
      { "icon": "⚠️", "title": "새는 돈부터 먼저 막으시오", "desc": "{이름}님은 재물이 들어오는 통로는 넓으나 나가는 구멍도 함께 크니, 기분이나 감정 상태에 따라 지출이 크게 흔들리는 경향이 반복되면 결국 남는 것이 없게 되오. 좋을 때 쓰고 나쁠 때 또 쓰는 패턴이 굳어지기 전에 수입의 일정 비율을 자동이체로 강제 저축하는 구조를 먼저 만들어두어야 하오. 큰 지출은 반드시 하루 이상 보류하는 규칙 하나만 지켜도 재물 누수를 크게 줄일 수 있소." },
      { "icon": "🎯", "title": "주도하는 일에 집중하시오", "desc": "{이름}님의 재물은 남이 시키는 일보다 본인이 기획하고 주도하는 일에서 훨씬 크게 열리며, 창의성과 독립성이 허용되는 환경일수록 수입이 자연스럽게 따라오는 기질이오. 지금 당장 큰 수익보다 전문성을 쌓는 데 재물과 시간을 투자하면, 그것이 나중에 아무도 빼앗을 수 없는 가장 단단한 재물 기반이 될 것이오. 주도할 수 있는 일을 찾고, 그 안에서 깊어지는 것이 재물을 여는 가장 확실한 열쇠이오." }
    ],
    "closing": "마무리 격려 한 문장"
  }
}`,
