...
- peakWhen: 황금기가 오는 시기 (예: "30대 후반~40대 초반", "2027년 무렵" — 대운 간지 이름은 절대 언급하지 말 것)
- peakDesc: 황금기의 본질과 그 시기를 어떻게 준비해야 하는지 3~4문장. 어떤 세운(연도)이 열리는지 구체적으로. 대운 간지 이름은 절대 언급하지 마시오. 홍연 화자. 호칭 사용.
...
- items: 위 [구간별 추세 요약]의 구간을 【절대로 합치거나 나누지 말 것】. 구간 수, label, trend 모두 아래 목록 그대로 사용. 임의로 묶거나 재분류하면 안 됨. 반드시 2033년까지 전부 커버할 것.
  ...
  ⚠️ 대운 간지 이름(예: "경진 대운", "갑진 대운" 등)은 이 섹션 전체에서 절대 언급하지 마시오. 시기를 가리킬 때는 오직 "○○년"(세운) 또는 나이대·구간(label)으로만 표현하고, "OO 대운으로 접어들며" 같은 대운 전환 표현도 쓰지 마시오.`,
