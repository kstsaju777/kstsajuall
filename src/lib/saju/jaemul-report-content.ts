...
- paragraphs: 아래 2개 단락 배열.
  ① 이 격국의 재물 기질과 강점을 체감 언어로 서술. 실제 삶의 장면을 그려가며 풍부하게 서술하오. (5~6문장, 250자 이상)
  ② 이 격국이 과하거나 채워지지 않을 때 재물에서 나타나는 주의점과 극복 방향. (3~4문장, 150자 이상)
⚠️ geokguk 전체 합산 600자 미만이면 오류이오.
`,
...
③ 【필수】먼저 위 [기둥별 십성] 목록을 직접 확인하여 편관·정관이 실제로 있는지부터 판단하오. 실제로 있을 때만 그 기둥·종류(편관/정관)를 명시하며 재물이 안정적으로 지켜지는 구조를 서술하고, 목록에 편관·정관이 전혀 없으면 절대 "정관이~" 식으로 있는 것처럼 지어내지 말고 "관성이 없어 재물이 흩어지기 쉬우니" 식으로 부재를 짚어줌. (1~2문장)
...
- paragraphs: 3개 단락. 각 단락 3~4문장(단락당 100자 이상).
- 이 사람의 재물 기운, 강점, 주의할 점을 따뜻하게 갈무리하되 희망적인 메시지로 맺으시오.
- ⚠️ 절대 금지: '이번 재물사주 풀이를 마치며', '따뜻한 손편지를 드리오', '풀이를 마무리하며' 등 풀이 행위를 언급하는 도입 문장 금지. 인사말·안부 인사·편지 시작 멘트 일절 금지. 반드시 이 사람의 재물 기운 본론으로 첫 문장부터 바로 시작하오.

[concernAdvice 섹션 — 고민에 대한 명리학적 조언]
고민({고민})이 비어있거나 없으면 concernAdvice.paragraphs를 빈 배열([])로 출력하오. 고민이 있을 때만 아래 규칙으로 작성하오. 고민의 주제(재물·인연·직업 등)에 관계없이 반드시 작성하오.

- paragraphs[0] (3~4문장, 150자 이상): 고민의 핵심을 {이름1}님의 일간·오행·십성 구조와 연결해 풀이하오. 사주 속에서 이 고민이 왜 생기는지 명리학적 근거와 함께 설명하오.
- paragraphs[1] (3~4문장, 150자 이상): 현재 대운·세운 흐름이 이 고민에 어떤 영향을 주는지 분석하오. 상황이 바뀌거나 돌파구가 오는 시기와 방향을 짚어주오.
- paragraphs[2] (2~3문장, 100자 이상): 지금 당장 실천할 수 있는 명리학적 조언과 마음가짐을 전하오. 홍연의 따뜻하고 단호한 말투로 마무리하오.
총 글자수: 400자 이상. 홍연 말투(~이오/~하오/~겠소). 점수·수치 언급 금지.`,
};
...
const JAEMUL_CH_SCHEMA: Record<number, string> = {
  ...
  "geokguk": {
    ...
    "paragraphs": ["격국의 재물 기질·강점 (5~6문장, 250자 이상)", "과하거나 부족할 때 재물 주의점과 극복 방향 (3~4문장, 150자 이상)"]
  },
}`,
  2: `{
  "wealthStyle": {
