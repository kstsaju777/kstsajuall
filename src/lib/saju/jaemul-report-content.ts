...
- items: 위 [구간별 추세 요약]의 구간을 【절대로 합치거나 나누지 말 것】. 구간 수, label, trend 모두 아래 목록 그대로 사용. 임의로 묶거나 재분류하면 안 됨. 반드시 2033년까지 전부 커버할 것.
  ...
  ※ 세운·대운 구분 필수: 특정 연도(예: 2027년)를 가리킬 때는 반드시 "○○년" 또는 "○○년 정미년"처럼 세운으로만 부르고, 그 해의 간지에 "대운"을 붙여 "정미대운"이라고 부르면 절대 안 되오. "대운"이라는 말은 오직 위 [실제 대운 전환 시점] 표에 실제로 명시된 대운 간지·구간에만 사용하시오. 표에 없는 해를 "OO대운"이라고 부르면 탈락이오.`,
