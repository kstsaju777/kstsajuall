...
⚠️ 위에 "chonyeongi(초년기) 풀이에서 반드시 이 명칭을 언급"이라고 태그된 [간여지동] 확정값이 주입되어 있으면, 반드시 "간여지동"이라는 명칭을 언급하며 그 의미를 이 풀이에 담으시오. 그런 태그가 없으면 절대 언급하지 마시오.
...
⚠️ 위에 "cheongneongi(청년기) 풀이에서 반드시 이 명칭을 언급"이라고 태그된 [간여지동] 확정값이 주입되어 있으면, 반드시 "간여지동"이라는 명칭을 언급하며 그 의미를 이 풀이에 담으시오. 그런 태그가 없으면 절대 언급하지 마시오.
...
⚠️ 위에 "jungnyeongi(중년기) 풀이에서 반드시 이 명칭을 언급"이라고 태그된 [간여지동] 확정값이 주입되어 있으면, 반드시 "간여지동"이라는 명칭을 언급하며 그 의미(강한 자아·독립성, 배우자와의 갈등 소지)를 이 풀이에 담으시오. 그런 태그가 없으면 절대 언급하지 마시오.
...
⚠️ 위에 "nonyeongi(말년기) 풀이에서 반드시 이 명칭을 언급"이라고 태그된 [간여지동] 확정값이 주입되어 있으면, 반드시 "간여지동"이라는 명칭을 언급하며 그 의미를 이 풀이에 담으시오. 그런 태그가 없으면 절대 언급하지 마시오.
...
export function buildYouareChapterPrompt(
  ...
  if (chapter === 1 && input.pillars && input.pillars.length > 0) {
    ...
    // 간여지동(干與支同) — 천간·지지의 오행·음양이 완전히 같은 특수 조합. 4개 기둥 어디든 해당될 수 있고
    // 해당 기둥이 속한 생애주기 풀이에서 반드시 언급해야 하는데 누락되는 경우가 많아 서버가 확정 판정
    const GANYEOJIDONG_SET_YA = new Set(["甲寅","乙卯","丙午","丁巳","戊辰","戊戌","己丑","己未","庚申","辛酉","壬子","癸亥"]);
    const POS_SECTION_YA: Record<string, string> = { "년주": "chonyeongi(초년기)", "월주": "cheongneongi(청년기)", "일주": "jungnyeongi(중년기)", "시주": "nonyeongi(말년기)" };
    for (const pos of ["년주", "월주", "일주", "시주"]) {
      const p = input.pillars.find(pp => pp.pos === pos);
      if (p && GANYEOJIDONG_SET_YA.has(`${p.gan}${p.ji}`)) {
        const pKr = `${JI_KR_YA[p.ji] ?? p.ji}${p.jiEl}`;
        seasonNote += `\n[간여지동(干與支同) — 서버 확정값, ${POS_SECTION_YA[pos]} 풀이에서 반드시 이 명칭을 언급하며 의미를 반영할 것]\n이 ${pos}(${pKr})는 천간·지지의 오행·음양이 완전히 같은 간여지동 조합이오. 자아가 강하고 독립적이며, 해당 육친과의 관계에서 갈등·긴장 소지가 있는 특수한 구조이니 이 특성을 ${POS_SECTION_YA[pos]} 풀이에 반드시 담으시오.\n`;
      }
    }
  }
