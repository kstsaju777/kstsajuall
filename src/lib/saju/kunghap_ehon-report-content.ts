...
const CH_GUIDE: Record<number, string> = {
  1: `★ 필수 규칙 — 위반 시 전면 재작성:
1. 명식표에서 일간(일주 천간)을 직접 읽어 확인하오. 풀이 전체에서 이 일간 글자를 최소 2회 이상 직접 인용하오. 한자는 절대 쓰지 말고 한글로만 표기하오. (예: "을목 일간", "을목의 기운" 등)
...
- strengthDesc: 상대 기질의 긍정적인 면 — 배우자와의 관계에서 빛나는 강점. 3~4문장. 실제 일간·십성에서 도출한 구체적 근거 포함.
- shadowDesc: 갈등 상황에서 상대에게 드러나는 그림자 — 이혼 위기를 부를 수 있는 성향. 구체적인 상황과 함께 서술. 3~4문장.
...
- paragraphs 3개 (홍연 말투):
  ①배우자와의 관계에서 나타나는 상대의 감정 방식과 갈등 성향 — 어떤 상황에서 갈등이 폭발하는지, 배우자를 어떻게 대하는지 구체적 상황으로 서술. 실제 일간·십성·합충 근거 포함. 6~8문장, 250자 이상.
  ②결혼 생활에서 반복되는 상대의 패턴과 배우자에게 주는 영향 — 이 패턴이 부부 사이에 어떤 균열을 만드는지, 의뢰인은 어떻게 느끼는지 깊이 있게. 6~8문장, 250자 이상.
  ③이혼으로 이어질 수 있는 상대의 사주적 특징과 주의점 — 이 패턴이 왜 이혼 위기를 만드는지, 상대가 인식하면 어떻게 달라질 수 있는지. 5~7문장, 220자 이상.`,
...
- paragraphs 4개: 각 단락 최소 7~9문장, 280자 이상.
  ①이 점수가 나온 사주적 근거 — 두 사람의 사주에서 합의 수와 충·형·해 등 긴장 관계의 수를 명시하고(제3장 합충 분석 결과와 반드시 일치시키오), 이 비율이 두 사람의 결혼 구조에 어떤 의미인지 풀이하시오. 개수 외의 구체적인 관계명을 임의로 지어내는 것 절대 금지. 점수 수치(숫자)는 언급하지 마시오.
...
⚠️ [4장 전체 공통 — 홍연 말투 재확인] 이 장은 점수·통계적 분석 성격이 강해 자기도 모르게 "~높다", "~보인다", "~필요하다", "~구조이다"처럼 보고서·논문체(-다/-이다)로 흘러가기 쉽소. divorceScore·divorceReason·conflictStyle 전 섹션의 모든 문장을 쓴 뒤 반드시 마지막 문장까지 다시 확인하여, 예외 없이 '~이오/~하오/~했소/~겠소/~오'로 끝맺으시오. (예: '가능성이 높다' ✗ → '가능성이 높소' ○, '~로 보인다' ✗ → '~로 보이오' ○, '~구조이다' ✗ → '~구조이오' ○)
...
- paragraphs: 2개. 각 4~5문장.
    ①이 합들이 합쳐져 두 사람 사이에 형성하는 기운의 장 — 함께 있을 때와 떠나려 할 때 어떤 작용을 하는지.
    ②합의 기운이 이혼 결정을 미루게 만드는 구체적 상황과 홍연의 조언.

[chungList 섹션 — 충 목록]
두 사람 사주에서 발견되는 충 0~3개. 없으면 빈 배열.
...
⚠️ 자녀운은 반드시 시주 천간·지지를 기준으로 분석하오. 시주의 십성이 자녀성(식신·상관)과 어떻게 연결되는지, 일간과의 관계에서 자녀와의 인연이 두텁거나 얕은지를 구체적으로 서술하오. 막연한 성향 묘사 금지 — 반드시 시주 간지와 십성 근거를 포함하오.
...
- 단락 구성 흐름 (순서대로):
  ...
  ⑤새로운 출발을 응원하는 문장 — 이 이별이 끝이 아니라 각자의 새 장이 시작됨을 알리며. 희망적이고 힘 있게.
...
⚠️ 긴 편지라 쓰다 보면 자기도 모르게 "~습니다", "~입니다"체로 새기 쉬우니, 마지막 문장까지 다시 확인해 홍연 말투로만 끝맺으시오.`,
};
...
합 총 ${input.ch3Summary.hapCount}개: ${(input.ch3Summary.hapList as string[]).join(", ") || "없음"}
...
${input.reconcileEnablers && input.reconcileEnablers.length > 0
  ...
  : "· 두 사람 사이에 합 관계 없음. enablers는 사주 원국의 오행 상생·용신 관계를 근거로 작성하오."}
